...
class TestRunner:
    ...
    def __init__(self):
        ...
        # File paths
        self.test_file = self.test_dir / f"test_convert_html_to_pdf_minimal_{self.timestamp}.py"
        self.config_file = self.test_dir / f"pytest_convert_html_to_pdf_{self.timestamp}.ini"
        self.conftest_file = self.test_dir / f"conftest_convert_html_to_pdf_{self.timestamp}.py"
        
        # Output files
        self.html_report = self.test_dir / f"result_convert_html_to_pdf_{self.timestamp}.html"
        self.json_report = self.test_dir / f"result_convert_html_to_pdf_{self.timestamp}.json"
        self.junit_report = self.test_dir / f"result_convert_html_to_pdf_{self.timestamp}_junit.xml"
        self.coverage_html = self.test_dir / f"result_convert_html_to_pdf_coverage_{self.timestamp}"
        self.coverage_json = self.test_dir / f"result_convert_html_to_pdf_coverage_{self.timestamp}.json"
        self.execution_summary = self.test_dir / f"result_convert_html_to_pdf_execution_summary_{self.timestamp}.txt"
    ...
    def setup_environment(self):
        ...
        src_path = self.project_root / "src" / "utilities" / "pdf_tools" / "pdf_conversion"
    ...
    def check_dependencies(self):
        ...
        required_packages = [
            ...
            'pytest-cov', 'pytest-mock', 'PyQt5', 'pdfkit'
        ]
...
"""
    ...
    def generate_test_report_summary(self):
        ...
        # Save summary
        summary_file = self.test_dir / f"result_convert_html_to_pdf_project_completion_{self.timestamp}.json"
        try:
