...
try:
    ...
    print("Error: Could not import SizeAnalyzer from src.utilities.analysis.core.size_analyzer_logic")
    sys.exit(1)

try:
    from memory_optimized_size_analyzer_2025_09_09 import (
        MemoryOptimizedSizeAnalyzer, create_memory_optimized_analyzer)
except ImportError:
    print("Error: Could not import MemoryOptimizedSizeAnalyzer. Make sure memory_optimized_size_analyzer_2025_09_09.py is in the same directory.")
    sys.exit(1)
...
class MemoryLeakFixValidationSuite:
    ...
    def run_memory_comparison_test(self, operations_count: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            # Compare results
            comparison = self._compare_memory_results(original_results, optimized_results)
    ...
    def _test_analyzer_memory_usage(self, analyzer, test_dir: str, operations_count: int, analyzer_type: str) -> Dict[str, Any]:
        ...
        for i in range(operations_count):
            ...
            operation_result = {
                ...
                'memory_change_after_gc_mb': (post_gc_memory - pre_memory) / 1024 / 1024,
            ...
            print(f"    Memory: {operation_result['memory_change_after_gc_mb']:+.3f} MB, "
        ...
        if successful_ops:
            avg_memory_change = sum(float(op['memory_change_after_gc_mb']) for op in successful_ops) / len(successful_ops)
            avg_object_change = sum(int(op['object_change_after_gc']) for op in successful_ops) / len(successful_ops)
            max_memory_change = max(float(op['memory_change_after_gc_mb']) for op in successful_ops)
            max_object_change = max(int(op['object_change_after_gc']) for op in successful_ops)
        else:
            avg_memory_change = avg_object_change = max_memory_change = max_object_change = 0
    ...
    def _compare_memory_results(self, original: Dict, optimized: Dict) -> Dict[str, Any]:
        """Compare memory usage results between analyzers."""
        memory_improvement = original['total_memory_change_mb'] - optimized['total_memory_change_mb']
        memory_improvement_percent = (memory_improvement / max(original['total_memory_change_mb'], 0.001)) * 100
        
        avg_memory_improvement = original['avg_memory_change_per_op_mb'] - optimized['avg_memory_change_per_op_mb']
        avg_memory_improvement_percent = (avg_memory_improvement / max(original['avg_memory_change_per_op_mb'], 0.001)) * 100
        
        object_improvement = original['avg_object_change_per_op'] - optimized['avg_object_change_per_op']
        object_improvement_percent = (object_improvement / max(original['avg_object_change_per_op'], 1)) * 100
    ...
    def _validate_fix_success(self, comparison: Dict) -> Dict[str, Any]:
        ...
        if comparison['memory_improvement_percent'] > 50:
            validation['criteria_met'].append(f"Memory improvement: {comparison['memory_improvement_percent']:.1f}% > 50%")
        else:
            validation['criteria_failed'].append(f"Memory improvement: {comparison['memory_improvement_percent']:.1f}% <= 50%")
        
        # Criterion 2: Average memory per operation < 1 MB
        if comparison['optimized_avg_memory_per_op_mb'] < 1.0:
            validation['criteria_met'].append(f"Avg memory per op: {comparison['optimized_avg_memory_per_op_mb']:.3f} MB < 1.0 MB")
        else:
            validation['criteria_failed'].append(f"Avg memory per op: {comparison['optimized_avg_memory_per_op_mb']:.3f} MB >= 1.0 MB")
        
        # Criterion 3: Object improvement > 80%
        if comparison['object_improvement_percent'] > 80:
            validation['criteria_met'].append(f"Object improvement: {comparison['object_improvement_percent']:.1f}% > 80%")
        else:
            validation['criteria_failed'].append(f"Object improvement: {comparison['object_improvement_percent']:.1f}% <= 80%")
        
        # Criterion 4: Total memory increase < 10 MB for 20 operations
        if comparison['optimized_total_memory_mb'] < 10.0:
            validation['criteria_met'].append(f"Total memory increase: {comparison['optimized_total_memory_mb']:.1f} MB < 10.0 MB")
        else:
            validation['criteria_failed'].append(f"Total memory increase: {comparison['optimized_total_memory_mb']:.1f} MB >= 10.0 MB")
...
def main():
    ...
    results_file = os.path.join(os.path.dirname(__file__), "memory_leak_fix_validation_results_2025-09-09.json")
