...
try:
    from PyQt5.QtCore import (QApplication, QDir, QObject, QSettings,
                              QStandardPaths, Qt, QTimer, pyqtSignal)
    from PyQt5.QtGui import (QAction, QActionGroup, QFont, QIcon, QKeySequence,
                             QPixmap)
    from PyQt5.QtWidgets import (QApplication, QDesktopWidget, QDialog,
                                 QFileDialog, QFrame, QHBoxLayout,
                                 QInputDialog, QLabel, QMainWindow, QMenuBar,
...
try:
    ...
    from .ui.pane_manager import (LayoutType, PaneConfiguration, PaneManager,
                                  PaneType)
...
class SessionManager(QObject if QT_AVAILABLE else object):
    ...
    def __init__(self, parent=None):
        ...
        # Session storage
        self.settings = QSettings('RFU', 'FileExplorer') if QT_AVAILABLE else None
...
class MenuManager(QObject if QT_AVAILABLE else object):
    ...
    def _create_file_menu(self):
        ...
        new_pane_action.triggered.connect(lambda: self._emit_action("new_pane"))
        ...
        open_folder_action.triggered.connect(lambda: self._emit_action("open_folder"))
    ...
    def _create_view_menu(self):
        ...
        for name, mode in view_modes:
            ...
            action.triggered.connect(lambda checked, m=mode: self._emit_action(f"view_mode_{m}"))
        ...
        for name, layout in layouts:
            ...
            action.triggered.connect(lambda checked, l=layout: self._emit_action(f"layout_{l}"))
        ...
        show_hidden_action.triggered.connect(lambda: self._emit_action("toggle_hidden"))
    ...
    def _create_tools_menu(self):
        ...
        add_bookmark_action.triggered.connect(lambda: self._emit_action("add_bookmark"))
        bookmarks_menu.addAction(add_bookmark_action)
        self.actions["add_bookmark"] = add_bookmark_action
        
        manage_bookmarks_action = QAction("&Manage Bookmarks...", self.main_window)
        manage_bookmarks_action.triggered.connect(lambda: self._emit_action("manage_bookmarks"))
        ...
        preferences_action.triggered.connect(lambda: self._emit_action("preferences"))
    ...
    def _setup_shortcuts(self):
        ...
        for shortcut, action_name in shortcuts.items():
            ...
            action.triggered.connect(lambda checked, name=action_name: self._emit_action(name))
    ...
    def update_recent_folders(self, folders: List[str]):
        ...
        for i, folder in enumerate(folders[:10]):  # Limit to 10 recent folders
            ...
            action.triggered.connect(lambda checked, path=folder: self._emit_action(f"open_recent|{path}"))
...
class PaneCoordinator(QObject if QT_AVAILABLE else object):
    ...
    def register_pane(self, pane_id: str, pane_widget):
        ...
        if hasattr(pane_widget, 'fileSelected'):
            pane_widget.fileSelected.connect(
                lambda file_path, pid=pane_id: self._on_pane_file_selected(pid, file_path)
            )
    ...
    def paste_files(self, destination_path: str):
        ...
        self.logger.debug(f"Pasted {len(operation_data['source_files'])} files to {destination_path}")
...
class MainApplication(QMainWindow if QT_AVAILABLE else object):
    ...
    def _setup_ui(self):
        ...
        self.pane_coordinator.crossPaneOperation.connect(self._handle_cross_pane_operation)
    ...
    def _create_initial_panes(self):
        ...
        try:
            ...
            # Apply layout
            self.pane_manager.apply_layout(layout_type, [left_config, right_config])
    ...
    def _create_new_pane(self):
        ...
        try:
            ...
            if pane_widget:
                ...
                self.status_bar.showMessage(f"Created new pane: {pane_id}", 2000)
    ...
    def _open_folder_dialog(self):
        ...
        try:
            ...
            current_path = getattr(active_pane, 'current_path', str(Path.home()))
        ...
        try:
            ...
            if active_pane and hasattr(active_pane, 'set_path'):
                ...
                if folder_path not in self.recent_folders:
                    ...
                    self.recent_folders = self.recent_folders[:10]  # Limit to 10
                    
                    # Update menu
                    if self.menu_manager:
                        self.menu_manager.update_recent_folders(self.recent_folders)
                
                self.status_bar.showMessage(f"Opened folder: {folder_path}", 2000)
    ...
    def _manage_bookmarks(self):
        ...
        self.status_bar.showMessage("Bookmark management not implemented yet", 2000)
        ...
        try:
            ...
            if active_pane and hasattr(active_pane, 'file_list'):
                ...
                if selected_files:
                    ...
                    self.status_bar.showMessage(f"Copied {len(selected_files)} files", 2000)
        ...
        try:
            ...
            if active_pane and hasattr(active_pane, 'file_list'):
                ...
                if selected_files:
                    ...
                    self.status_bar.showMessage(f"Cut {len(selected_files)} files", 2000)
    ...
    def _handle_cross_pane_operation(self, operation: str, data: Dict[str, Any]):
        """
    ...
    def _save_session(self):
        ...
        try:
            session_data = {
                ...
                'pane_layout': self.pane_manager.get_layout_state() if self.pane_manager else {},
...
def main():
    ...
    if not QT_AVAILABLE:
        logger.error("PyQt5 is not available. Please install PyQt5 to run the application.")
        return 1
