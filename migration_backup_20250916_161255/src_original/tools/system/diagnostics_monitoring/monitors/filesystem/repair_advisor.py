...
class RepairAdvisor:
    ...
    def _generate_corruption_recommendation(
        ...
        # Get repair strategy
        strategy = self.repair_strategies.get(corruption_type, {
        ...
        recommendation = {
            ...
            'recommended_actions': [action.value for action in strategy['actions']],
    ...
    def _generate_detailed_steps(
        ...
        for action in actions:
            if action == RepairAction.BACKUP_FILE:
                ...
                if self.platform_detector.is_linux() or self.platform_detector.is_macos():
                    steps.extend([
    ...
    def _estimate_repair_time(self, actions: List[RepairAction]) -> str:
        ...
        total_minutes = sum(time_estimates.get(action, 5) for action in actions)
    ...
    def _generate_repair_summary(self, recommendations: Dict[str, Any]) -> None:
    ...
    def _generate_backup_recommendations(
        ...
        if critical_issues > 0:
            ...
            backup_recs.append(
                "Create backup of affected files before repair"
            )
    ...
    def _generate_prevention_tips(self, recommendations: Dict[str, Any]) -> None:
    ...
    def get_repair_script(self, recommendation: Dict[str, Any]) -> str:
        ...
        script_lines = [
            "#!/bin/bash" if not self.platform_detector.is_windows() else "@echo off",
        ...
        for step in steps:
            if step.strip().startswith(('cp ', 'mv ', 'rm ', 'chmod ', 'touch ')):
