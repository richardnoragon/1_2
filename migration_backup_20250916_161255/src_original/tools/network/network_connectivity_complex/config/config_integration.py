...
class NetworkConfigManager:
    ...
    def _ensure_network_config(self):
        ...
        try:
            ...
            if 'network_connectivity' not in current_config:
                self.logger.info("Adding network connectivity configuration to main config")
                ...
                self.logger.info("Network connectivity configuration added successfully")
    ...
    def _create_default_network_config(self):
        ...
        try:
            ...
            self.logger.info("Created default network connectivity configuration")
        except Exception as e:
    ...
    def get_tool_config(self, tool_name: str, key: str = None, default: Any = None) -> Any:
        ...
        try:
            ...
            self.logger.error(f"Error getting tool config for {tool_name}: {e}")
            return default
    ...
    def set_tool_config(self, tool_name: str, key: str, value: Any) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error setting tool config for {tool_name}: {e}")
            return False
    ...
    def get_performance_config(self, key: str = None, default: Any = None) -> Any:
    ...
    def reset_tool_config(self, tool_name: str) -> bool:
        ...
        try:
            default_config = get_default_config()
            tool_defaults = default_config['network_connectivity'].get(tool_name, {})
            
            if tool_defaults:
                self.config_manager.set_setting(
                    'network_connectivity', tool_name, tool_defaults
                )
                self.logger.info(f"Reset {tool_name} configuration to defaults")
                return True
            else:
                self.logger.warning(f"No default configuration found for {tool_name}")
                return False
        
        except Exception as e:
            self.logger.error(f"Error resetting tool config for {tool_name}: {e}")
            return False
    ...
    def export_network_config(self, file_path: str) -> bool:
        ...
        try:
            ...
            network_config = self.config_manager.get_setting('network_connectivity', {})
    ...
    def import_network_config(self, file_path: str, validate: bool = True) -> bool:
        ...
        try:
            ...
            self.config_manager.set_setting('network_connectivity', '', imported_config)
            
            self.logger.info(f"Imported network configuration from {file_path}")
            return True
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                'security_level': network_config.get('security', {}).get('security_level', 'unknown'),
                'performance_monitoring': network_config.get('performance', {}).get('enable_performance_monitoring', False),
                'logging_enabled': network_config.get('general', {}).get('enable_logging', False),
...
def get_tool_config(tool_name: str, key: str = None, default: Any = None) -> Any:
    ...
    Returns:
        Configuration value
    """
    return get_network_config_manager().get_tool_config(tool_name, key, default)
