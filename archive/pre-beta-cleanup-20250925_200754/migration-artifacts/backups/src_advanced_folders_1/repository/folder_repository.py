...
class FolderRepository(BaseRepository):
    ...
    def create(self, folder_config: FolderConfiguration) -> FolderConfiguration:
        """
        ...
        try:
            with self.connection_manager.get_transaction() as conn:
                ...
                main_params = (
                    ...
                    folder_config.last_scan_time.isoformat() if folder_config.last_scan_time else None,
    ...
    def update(self, folder_config: FolderConfiguration) -> FolderConfiguration:
        """
        ...
        try:
            with self.connection_manager.get_transaction() as conn:
                ...
                rows_affected = conn.execute(main_update_sql, main_params).rowcount
                ...
                delete_targets_sql = f"DELETE FROM {self.TARGET_DIRS_TABLE} WHERE folder_id = ?"
    ...
    def delete(self, folder_id: str) -> bool:
        ...
        try:
            with self.connection_manager.get_transaction() as conn:
                ...
                delete_main_sql = f"DELETE FROM {self.TABLE_NAME} WHERE folder_id = ?"
    ...
    def name_exists(self, name: str, exclude_id: Optional[str] = None) -> bool:
        ...
        if exclude_id:
            ...
            result = self.execute_query(query, (name, exclude_id), fetch_one=True)
        else:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        targets_query = f"SELECT COUNT(*) as total_targets FROM {self.TARGET_DIRS_TABLE}"
    ...
    def _row_to_folder_config(self, row: sqlite3.Row) -> FolderConfiguration:
        ...
        try:
            ...
            targets_query = f"SELECT * FROM {self.TARGET_DIRS_TABLE} WHERE folder_id = ?"
            target_rows = self.execute_query(targets_query, (row["folder_id"],))
            ...
            config_data = {
                ...
                "performance_settings": json.loads(row["performance_settings"]),
            ...
            for target_row in target_rows:
                target_data = {
                    ...
                    "include_subdirectories": bool(target_row["include_subdirectories"]),
                    "follow_symlinks": bool(target_row["follow_symlinks"]),
                    "max_depth": target_row["max_depth"],
                    "exclude_patterns": json.loads(target_row["exclude_patterns"])
                }
