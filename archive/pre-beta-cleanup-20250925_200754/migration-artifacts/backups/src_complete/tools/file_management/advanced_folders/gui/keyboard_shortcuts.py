...
class ShortcutAction:
    ...
    def __init__(self, action_id: str, description: str, 
                 default_shortcut: str, callback: Callable,
...
class KeyboardShortcutsManager(QObject):
    ...
    shortcutConflict = pyqtSignal(str, str, str)  # action_id, shortcut, conflicting_action
    ...
    def _initialize_default_shortcuts(self):
        ...
        self.register_shortcut(
            "new_folder", "Create New Folder", "Ctrl+N",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "open_folder", "Open Folder Configuration", "Ctrl+O",
            self._placeholder_callback, "global"
        )
        ...
        self.register_shortcut(
            "save_as", "Save Configuration As...", "Ctrl+Shift+S",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "close_folder", "Close Current Folder", "Ctrl+W",
            self._placeholder_callback, "global"
        )
        ...
        self.register_shortcut(
            ...
            self._placeholder_callback, "search_results"
        )
        ...
        # Search operations
        self.register_shortcut(
            "quick_search", "Quick Search", "Ctrl+F",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "advanced_search", "Advanced Search", "Ctrl+Shift+F",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "clear_search", "Clear Search", "Ctrl+Shift+C",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "next_result", "Next Search Result", "F3",
            self._placeholder_callback, "search_results"
        )
        
        self.register_shortcut(
            "prev_result", "Previous Search Result", "Shift+F3",
            self._placeholder_callback, "search_results"
        )
        
        # Navigation shortcuts
        self.register_shortcut(
            "focus_tree", "Focus Folder Tree", "Ctrl+1",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "focus_search", "Focus Search Results", "Ctrl+2",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "focus_preview", "Focus Preview Pane", "Ctrl+3",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "toggle_tree", "Toggle Folder Tree", "F9",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "toggle_preview", "Toggle Preview Pane", "F10",
            self._placeholder_callback, "global"
        )
        
        # View operations
        self.register_shortcut(
            "view_details", "Details View", "Ctrl+D",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "view_list", "List View", "Ctrl+L",
            self._placeholder_callback, "global"
        )
        
        self.register_shortcut(
            "view_icons", "Icons View", "Ctrl+I",
            self._placeholder_callback, "global"
        )
        ...
        self.register_shortcut(
            ...
            self._placeholder_callback, "preview"
        )
        ...
        self.register_shortcut(
            "shortcuts_help", "Show Keyboard Shortcuts", "Ctrl+/",
            self._show_shortcuts_help, "global"
        )
        
        self.register_shortcut(
            "accessibility_mode", "Toggle Accessibility Mode", "Ctrl+Alt+A",
            self._placeholder_callback, "global"
        )
        ...
        self.register_shortcut(
            "expand_all", "Expand All", "Ctrl+Shift+E",
            self._placeholder_callback, "folder_tree"
        )
        
        self.register_shortcut(
            "collapse_all", "Collapse All", "Ctrl+Shift+W",
            self._placeholder_callback, "folder_tree"
        )
        
        self.register_shortcut(
            "sort_name", "Sort by Name", "Ctrl+Shift+N",
            self._placeholder_callback, "search_results"
        )
        
        self.register_shortcut(
            "sort_size", "Sort by Size", "Ctrl+Shift+S",
            self._placeholder_callback, "search_results"
        )
        
        self.register_shortcut(
            "sort_date", "Sort by Date", "Ctrl+Shift+D",
            self._placeholder_callback, "search_results"
        )
        
        self.register_shortcut(
            "sort_type", "Sort by Type", "Ctrl+Shift+T",
            self._placeholder_callback, "search_results"
        )
    
    def register_shortcut(self, action_id: str, description: str, 
                         default_shortcut: str, callback: Callable,
                         context: str = "global", enabled: bool = True) -> bool:
        ...
        try:
            ...
            shortcut_action = ShortcutAction(
                action_id, description, default_shortcut, 
    ...
    def set_shortcut(self, action_id: str, new_shortcut: str) -> bool:
        ...
        try:
            ...
            # Check for conflicts
            conflicts = self._check_shortcut_conflicts(action_id, new_shortcut, exclude_action=action_id)
            if conflicts:
            ...
            self.logger.debug(f"Updated shortcut {action_id}: {old_shortcut} -> {new_shortcut}")
    ...
    def set_active_context(self, context: str):
        ...
        if context != self.active_context:
            self.logger.debug(f"Changing shortcut context: {self.active_context} -> {context}")
    ...
    def get_shortcuts_for_context(self, context: str) -> List[ShortcutAction]:
        ...
        return [self.shortcuts[action_id] for action_id in self.context_shortcuts[context]
                if action_id in self.shortcuts]
    ...
    def _create_qaction(self, shortcut_action: ShortcutAction):
        ...
        qaction.triggered.connect(
            lambda checked, aid=shortcut_action.action_id: self._handle_shortcut_triggered(aid)
        )
    ...
    def _update_context_shortcuts(self):
        ...
        for shortcut_action in self.shortcuts.values():
            if shortcut_action.qaction:
                ...
                shortcut_action.qaction.setEnabled(is_active and shortcut_action.enabled)
    ...
    def _check_shortcut_conflicts(self, action_id: str, shortcut: str, 
                                exclude_action: Optional[str] = None) -> List[str]:
        ...
        for other_action_id, other_shortcut_action in self.shortcuts.items():
            ...
            if (other_shortcut_action.current_shortcut == shortcut and
                other_shortcut_action.context == self.shortcuts.get(action_id, ShortcutAction("", "", "", lambda: None)).context):
    ...
    def get_shortcut_conflicts(self) -> Dict[str, List[str]]:
        ...
        for action_id, shortcut_action in self.shortcuts.items():
            conflicting_actions = self._check_shortcut_conflicts(
                action_id, shortcut_action.current_shortcut, exclude_action=action_id
            )
...
class KeyboardShortcutsHelpDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.shortcuts_table.setHorizontalHeaderLabels(["Action", "Shortcut", "Context", "Description"])
