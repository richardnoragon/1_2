...
class CatalogWindow(StandardWindow):
    ...
    def _setup_menu_callbacks(self):
        ...
        if hasattr(self, 'menu_manager'):
            ...
            self.menu_manager.register_callback('new_catalog',
                                                self.clear_directory)
            self.menu_manager.register_callback('save_catalog',
                                                self.generate_catalog)
            self.menu_manager.register_callback('open_catalog',
                                                self.open_catalog)
            self.menu_manager.register_callback('export_catalog',
    ...
    def _should_skip_hidden_file(self, filename):
        ...
        return (not self.show_hidden_check.isChecked() and
                filename.startswith('.'))
...
<body>
    ...
    def _create_html_file_entries(self):
        ...
        try:
            ...
            file_entries = (f"<tr><td colspan='3'>"
    ...
    def _create_html_footer(self):
        ...
        try:
            if self.recursive_check.isChecked():
                ...
                file_count = len([f for f in os.listdir(self.current_directory)
                                  if (not self._should_skip_hidden_file(f) and
