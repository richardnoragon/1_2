...
class MemoryTracker:
    ...
    def _get_virtual_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_info = {
                ...
                'memory_available_gb': round(vmem.available / self.BYTES_TO_GB, 2),
        ...
        try:
            ...
            if hasattr(swap, 'sin') and hasattr(swap, 'sout'):
                ...
                if self._last_swap_stats:
                    ...
                    if time_delta > 0:
                        sin_rate = (swap.sin - self._last_swap_stats['sin']) / time_delta
                        sout_rate = (swap.sout - self._last_swap_stats['sout']) / time_delta
                        ...
                        if len(self._swap_rate_history) > 100:
                            self._swap_rate_history = self._swap_rate_history[-100:]
    ...
    def _calculate_memory_pressure(self, memory_info: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if swap_sin_rate > 0 or swap_sout_rate > 0:
                ...
                swap_io_pressure = min(20, (swap_sin_rate + swap_sout_rate) / 1024)
            ...
            error_handler.handle_error(e, "MemoryTracker._calculate_memory_pressure")
            return 0.0
        ...
        try:
            ...
            if 'memory_free' in metrics and 'memory_available' in metrics:
                ...
                if available_memory > 0:
                    fragmentation = ((available_memory - free_memory) / 
                                   available_memory) * 100
                    metrics['memory_fragmentation'] = round(max(0, fragmentation), 2)
            
            # Calculate memory trends
            if len(self._memory_pressure_history) >= 2:
                recent_pressure = self._memory_pressure_history[-10:]
                metrics['pressure_trend'] = self._calculate_trend(recent_pressure)
            
        except Exception as e:
            error_handler.handle_error(e, "MemoryTracker._calculate_derived_metrics")
    ...
    def _get_fallback_memory_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(e, "MemoryTracker._get_fallback_memory_info")
    ...
    def get_memory_pressure_history(self, limit: Optional[int] = None) -> List[float]:
    ...
    def get_swap_rate_history(self, limit: Optional[int] = None) -> List[Dict[str, float]]:
