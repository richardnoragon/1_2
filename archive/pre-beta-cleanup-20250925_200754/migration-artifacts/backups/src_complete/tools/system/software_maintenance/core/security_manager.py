...
class SecurityManager(MaintenanceToolBase):
    ...
    def _load_backup_registry(self):
        ...
        registry_file = Path("software_maintenance/config/backup_registry.json")
        ...
        if registry_file.exists():
            try:
                ...
                self.log_info(f"Loaded {len(self.backup_registry)} backup entries")
    ...
    def _load_restore_points(self):
        ...
        if restore_file.exists():
            try:
                ...
                self.log_info(f"Loaded {len(self.restore_points)} restore points")
    ...
    def request_admin_elevation(self) -> bool:
        ...
        try:
            if sys.platform == "win32":
                ...
                self.log_warning("Administrative privileges required. "
    ...
    def create_file_backup(self, file_path: str, 
                          backup_name: str = None) -> Optional[str]:
        """
    ...
    def create_directory_backup(self, dir_path: str, 
                               backup_name: str = None) -> Optional[str]:
        """
        ...
        try:
            ...
            if not source.exists() or not source.is_dir():
                self.log_warning(f"Source directory does not exist: {dir_path}")
                return None
            ...
            # Calculate size
            size_mb = sum(f.stat().st_size for f in backup_path.rglob('*') 
    ...
    def create_system_restore_point(self, name: str, 
        ...
        try:
            ...
            if result.returncode != 0:
                self.log_warning(f"System restore point creation failed: "
                               f"{result.stderr}")
    ...
    def cleanup_old_backups(self) -> int:
        ...
        total_size_gb = sum(backup.size_mb for backup in 
                           self.backup_registry.values()) / 1024
        
        # Sort backups by creation time (oldest first)
        sorted_backups = sorted(self.backup_registry.items(),
        ...
        for backup_id, backup in sorted_backups:
            ...
            # Check size limit
                ...
                should_delete = True
                self.log_info(f"Total backup size ({total_size_gb:.2f} GB) "
    ...
    def get_backup_statistics(self) -> Dict[str, any]:
        ...
        total_size_mb = sum(backup.size_mb for backup in 
        ...
        return {
            ...
            'oldest_backup': min((b.creation_time for b in 
                                self.backup_registry.values()), 
                               default=None),
            'newest_backup': max((b.creation_time for b in 
                                self.backup_registry.values()), 
                               default=None)
        }
