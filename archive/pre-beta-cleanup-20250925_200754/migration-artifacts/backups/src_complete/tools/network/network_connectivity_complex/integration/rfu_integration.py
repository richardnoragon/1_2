...
class RFUIntegrationManager:
    ...
    def _integrate_configuration(self):
        ...
        try:
            ...
            if 'network_connectivity' not in self.rfu_config.config:
                self.logger.info("Adding network connectivity to RFU configuration")
                
                # Get default network configuration
                default_config = self.nc_config.get_configuration()
                
                # Add to RFU configuration
                self.rfu_config.config['network_connectivity'] = default_config.get(
        ...
        if gui_handler:
            ...
            def forward_to_rfu_gui(notification):
                try:
                    ...
                    self.logger.error(f"Failed to forward notification to RFU: {e}")
    ...
    def _setup_integration_metrics(self):
        ...
        metrics = [
            ("rfu.integration.config_syncs", MetricType.COUNTER, MetricUnit.COUNT),
            ("rfu.integration.log_forwards", MetricType.COUNTER, MetricUnit.COUNT),
            ("rfu.integration.notifications_sent", MetricType.COUNTER, MetricUnit.COUNT),
            ("rfu.integration.errors", MetricType.COUNTER, MetricUnit.COUNT),
            ("rfu.integration.health_score", MetricType.GAUGE, MetricUnit.PERCENT)
        ]
        ...
        try:
            ...
            def export_key_metrics():
                try:
                    ...
                    self.logger.info(f"NC Metrics Summary: {len(summary)} metrics active")
        ...
        try:
            ...
            def nc_error_handler(error, context):
                try:
                    ...
                    self.error_handler.handle_error(error, f"NetworkConnectivity.{context}")
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'nc_config_valid': len(self.nc_config.validate_configuration()) == 0
        }
    ...
    def validate_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'network_connectivity' in self.rfu_config.config:
                ...
                validation_results['issues'].append("Network connectivity config missing from RFU")
            
            # Test logging bridge
            if self.nc_logging._handlers.get('rfu_bridge'):
                validation_results['logging_bridge'] = True
            else:
                validation_results['issues'].append("RFU logging bridge not active")
            ...
            if gui_handler and gui_handler._gui_callbacks:
                ...
                validation_results['issues'].append("Notification forwarding not configured")
            ...
            if metrics_stats['total_metrics'] > 0:
                ...
                validation_results['issues'].append("No metrics being collected")
            
            # Test error handling
            validation_results['error_handling'] = True  # Assume working if no errors
    ...
    def repair_integration(self) -> bool:
        ...
        try:
            ...
            if validation['overall_valid']:
                ...
                self.logger.warning(f"Integration repair incomplete: {validation['issues']}")
                return False
    ...
    def shutdown_integration(self):
        ...
        try:
            ...
            self._integration_status = {k: False for k in self._integration_status}
