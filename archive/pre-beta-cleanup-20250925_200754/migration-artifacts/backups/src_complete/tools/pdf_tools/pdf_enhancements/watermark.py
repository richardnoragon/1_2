...
def add_watermark(input_file: str, watermark_text: str, pages: tuple = None, opacity: float = 0.5) -> bool:
    """Add watermark to PDF file"""
    try:
        logger.info("Starting watermark application")
        logger.debug("Parameters - Input: %s, Text: %s, Pages: %s, Opacity: %f",
...
class WatermarkUI(QtWidgets.QMainWindow):
    ...
    def load_settings(self):
        ...
        try:
            # Set opacity
            self.opacitySpinBox.setValue(float(self.config.get('opacity', 0.5)))
    ...
    def save_settings(self):
        ...
        try:
            ...
            settings = {
                ...
                'last_directory': self.last_directory if hasattr(self, 'last_directory') else ''
            }
