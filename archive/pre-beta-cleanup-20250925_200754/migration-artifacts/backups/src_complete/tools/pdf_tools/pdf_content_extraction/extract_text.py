...
def extract_text_from_pdf(input_file: str, output_file: str = None, pages: str = None) -> bool:
    ...
    try:
        ...
        try:
            with pdfplumber.open(input_file) as pdf:
                logger.debug("PDF opened successfully, pages: %d", len(pdf.pages))
                ...
                for i in page_list:
                    try:
                        ...
                        logger.error("Error extracting text from page %d: %s", i+1, str(e))
        ...
        logger.error("Unexpected error during text extraction: %s", str(e), exc_info=True)
        ...
        try:
            ...
            try:
                ...
                if isinstance(result, str):
                    ...
                    self.statusBar().showMessage("Text extraction complete", 3000)
                else:
                    logger.warning("Text extraction failed")
                    QMessageBox.warning(self, "Warning", "Failed to extract text!")
                    self.statusBar().showMessage("Text extraction failed", 3000)
                    
            except Exception as e:
                logger.error("Error during text extraction: %s", str(e), exc_info=True)
            ...
            logger.error("Error in extract operation: %s", str(e), exc_info=True)
        ...
        try:
            ...
            if filename:
                ...
                try:
                    ...
                    QMessageBox.critical(self, "Error", f"Error saving text: {str(e)}")
                    self.statusBar().showMessage("Error saving text", 3000)
                    
        except Exception as e:
            logger.error("Error in save operation: %s", str(e))
            QMessageBox.critical(self, "Error", f"Error in save operation: {str(e)}")
            self.statusBar().showMessage("Error occurred", 3000)
