...
class LogManager:
    ...
    def _setup_logging(self):
        ...
        logger.setLevel(logging.DEBUG)  # Default to DEBUG to capture all levels
        
        # Also configure the main RFU logger for test compatibility
        main_logger = logging.getLogger('RFU')
        main_logger.setLevel(logging.INFO)  # As expected by test_log_initialization
    ...
    def _log_with_context(self, level: int, message: str, exc_info=None) -> None:
    ...
    def configure(self, max_size: int = None, backup_count: int = None) -> None:
    ...
    def set_level(self, level):
        ...
        for handler in self.logger.handlers:
            if isinstance(handler, logging.StreamHandler) and not isinstance(handler, logging.handlers.RotatingFileHandler):
                handler.setLevel(level)  # Console handler follows the set level
