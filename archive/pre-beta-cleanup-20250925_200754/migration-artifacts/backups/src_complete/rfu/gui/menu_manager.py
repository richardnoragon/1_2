...
from PyQt5.QtWidgets import (
    QMainWindow, QMenuBar, QMenu, QAction, QActionGroup, 
...
class MenuManager:
    ...
    def _create_help_menu(self):
        ...
        report_bug_action = self._create_action(
            ...
            callback=lambda: webbrowser.open('https://github.com/richardnoragon/issues')
        )
    ...
    def _create_action(self, text: str, shortcut: str, tooltip: str, 
    ...
    def _get_callback(self, action_name: str) -> Callable:
        ...
        return self.callbacks.get(action_name, lambda: self._default_action(action_name))
    ...
    def _populate_recent_files_menu(self, menu: QMenu):
        ...
        if recent_files:
            for i, file_path in enumerate(recent_files[:10]):  # Limit to 10 recent files
                if os.path.exists(file_path):
                    action = QAction(f"&{i+1} {os.path.basename(file_path)}", self.parent_window)
