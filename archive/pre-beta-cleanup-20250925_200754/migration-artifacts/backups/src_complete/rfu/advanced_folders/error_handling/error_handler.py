...
from ..exceptions import (AdvancedFoldersException, ConfigurationException,
                          FileSystemException, PerformanceException,
...
class ErrorHandler:
    ...
    def _wrap_exception(
        ...
        if isinstance(exception, (OSError, FileNotFoundError, PermissionError)):
    ...
    def _log_error(
        ...
        if isinstance(exception, ValidationException):
            ...
            log_level = "warning"
        elif isinstance(exception, (RepositoryException, ConfigurationException)):
            log_level = "error"
        else:
            log_level = "error"
    ...
    def _update_error_stats(self, exception: AdvancedFoldersException) -> None:
        ...
        if exception.error_code:
            self.error_counts[exception.error_code] = self.error_counts.get(
            ...
            try:
                ...
                if reraise:
                    error_handler.handle_exception(
                        e, error_context, operation or func.__name__, reraise=True
                    )
                else:
                    error_handler.handle_exception(
    ...
    def execute_with_fallback(
        ...
        **kwargs
        ...
        try:
            ...
            if not any(isinstance(e, exc_type) for exc_type in fallback_config["conditions"]):
                raise
