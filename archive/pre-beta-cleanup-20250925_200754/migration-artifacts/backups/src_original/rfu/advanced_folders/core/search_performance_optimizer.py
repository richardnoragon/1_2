...
class PerformanceMetrics:
    ...
    def efficiency_score(self) -> float:
        ...
        if self.memory_peak_mb > 0 and self.duration_ms > 0:
            return (self.items_processed * 1000) / (self.memory_peak_mb * self.duration_ms)
        return 0.0
...
class PerformanceProfiler:
    ...
    def stop_profiling(self) -> List[Dict[str, Any]]:
        ...
        self.logger.debug(f"Performance profiling stopped, collected {len(data)} samples")
        return data
...
class AdaptiveCache:
    ...
    def put(self, key: str, value: Any) -> None:
        ...
        with self._lock:
            ...
            while (len(self._cache) >= self.max_size or
                   self._memory_usage + value_size > self.max_memory_mb * 1024 * 1024):
    ...
    def _evict_one(self) -> bool:
        ...
        if self.strategy == CacheStrategy.LRU:
            # Least recently used
            victim = min(self._access_times.keys(), 
                        key=lambda k: self._access_times[k])
        
        elif self.strategy == CacheStrategy.LFU:
            # Least frequently used
            victim = min(self._access_counts.keys(),
...
class SearchResultRanker:
    ...
    def _calculate_relevance_score(
        ...
        if content and query_lower in content:
            ...
            score += self.weights['content_match'] * min(occurrences * 0.1, 1.0)
...
class PerformanceOptimizer:
    ...
    def _get_cache_memory_limit(self) -> float:
        ...
        try:
            ...
            if self.performance_level == PerformanceLevel.BASIC:
                ...
                return min(200.0, total_memory_gb * 0.05)  # 5% of RAM, max 200MB
            elif self.performance_level == PerformanceLevel.AGGRESSIVE:
                return min(500.0, total_memory_gb * 0.1)   # 10% of RAM, max 500MB
            else:  # MAXIMUM
                return min(1000.0, total_memory_gb * 0.15) # 15% of RAM, max 1GB
    ...
    def _init_performance_db(self) -> None:
        ...
        try:
            ...
            self.logger.warning(f"Failed to initialize performance database: {str(e)}")
        ...
        if cache_key:
            ...
            if cached_result is not None:
                ...
                metrics = PerformanceMetrics(
                    ...
                    cache_hits=1,
                    cache_misses=0,
                    items_processed=len(cached_result) if isinstance(cached_result, list) else 1,
                    items_per_second=0
                )
        ...
        try:
            ...
            if isinstance(result, list) and len(result) > 1:
                ...
                if isinstance(query, str) and query:
                    ...
                    result = self.ranker.rank_results(result, query, preferences)
            ...
            memory_peak_mb = max(
                ...
                max((sample['memory_mb'] for sample in profile_data), default=memory_after)
            )
            
            avg_cpu = (
                sum(sample['cpu_percent'] for sample in profile_data) / len(profile_data)
                if profile_data else 0.0
            )
            ...
            items_per_second = (
                (items_processed * 1000) / duration_ms if duration_ms > 0 else 0
            )
    ...
    def _execute_search_task(self, task: SearchTask) -> Tuple[Any, PerformanceMetrics]:
        ...
        metrics = PerformanceMetrics(
            ...
            items_per_second=(len(result) * 1000) / duration_ms if duration_ms > 0 else 0
        )
    ...
    def _store_metrics(self, metrics: PerformanceMetrics) -> None:
        ...
        try:
            ...
            self.logger.warning(f"Failed to store metrics to database: {str(e)}")
    ...
    def get_performance_report(
        ...
        if operation_type:
            metrics = [
                m for m in self.operation_stats.get(operation_type, [])
        ...
        items_per_second = [m.items_per_second for m in metrics if m.items_per_second > 0]
        efficiency_scores = [m.efficiency_score for m in metrics if m.efficiency_score > 0]
        ...
        report = {
            ...
            'performance': {
                ...
                'avg_items_per_second': round(sum(items_per_second) / len(items_per_second), 2) if items_per_second else 0,
                'avg_efficiency_score': round(sum(efficiency_scores) / len(efficiency_scores), 4) if efficiency_scores else 0
            },
            'cache': cache_stats,
            'system': {
                'max_workers': self.max_workers,
