...
class DriveInfo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'mount_time': self.mount_time.isoformat() if self.mount_time else None,
...
class DriveDetector(ABC):
    ...
    @abstractmethod
    def start_monitoring(self, callback: Callable[[str, DriveInfo], None]) -> bool:
...
class WindowsDriveDetector(DriveDetector):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.warning("Windows WMI/Win32 modules not available, using basic detection")
    ...
    def _get_drives_wmi(self) -> List[DriveInfo]:
        ...
        try:
            ...
            for disk in c.Win32_LogicalDisk():
                ...
                drive_info.drive_type = drive_type_map.get(disk.DriveType, DriveType.UNKNOWN)
                ...
                if disk.FreeSpace:
                    ...
                    drive_info.used_space = drive_info.total_size - drive_info.free_space
                
                # Set status
                drive_info.is_ready = True
                drive_info.is_mounted = True
                drive_info.status = DriveStatus.READY
                drive_info.is_removable = drive_info.drive_type in {DriveType.REMOVABLE, DriveType.CDROM}
    ...
    def _enhance_with_physical_info(self, drives: List[DriveInfo], wmi_conn) -> None:
                    ...
                    for logical_disk in partition.associators("Win32_LogicalDiskToPartition"):
                        ...
                        for drive in drives:
                            if drive.device_id == logical_disk.DeviceID:
                                drive.serial_number = physical_disk.SerialNumber or ""
                                drive.model = physical_disk.Model or ""
                                drive.interface = physical_disk.InterfaceType or ""
            ...
            for letter in string.ascii_uppercase:
                ...
                if os.path.exists(drive_path):
                    try:
                        ...
                        self.logger.debug(f"Failed to get info for drive {letter}: {e}")
    ...
    def start_monitoring(self, callback: Callable[[str, DriveInfo], None]) -> bool:
        ...
        self._monitor_thread = threading.Thread(target=self._monitor_drives, daemon=True)
        ...
        while self._monitoring:
            try:
                ...
                for drive_path in new_drives:
                    drive_info = next((d for d in current_drives if d.path == drive_path), None)
                ...
                for drive_path in removed_drives:
                    if self._callback:
                        ...
                        drive_info = DriveInfo(path=drive_path, status=DriveStatus.UNAVAILABLE)
            ...
            for volume in volumes_path.iterdir():
                if volume.is_dir() and not volume.name.startswith('.'):
                    try:
                        ...
                        drive_info.is_ready = True
                        drive_info.status = DriveStatus.READY
                        drive_info.is_removable = drive_info.drive_type == DriveType.REMOVABLE
                        
                        # Get filesystem type
                        drive_info.filesystem = self._get_filesystem_type(volume)
                        
                        drives.append(drive_info)
                        
                    except Exception as e:
                        self.logger.debug(f"Failed to get info for volume {volume}: {e}")
...
class LinuxDriveDetector(DriveDetector):
            ...
            with open('/proc/mounts', 'r') as f:
                for line in f:
                    ...
                    if len(parts) >= 6:
                        ...
                        if self._should_include_mount(device, mount_point, filesystem):
                            drive_info = DriveInfo(
                                path=mount_point,
                                mount_point=mount_point,
                                device_id=device,
                                filesystem=filesystem,
                                drive_type=self._detect_drive_type(device, mount_point)
                            )
                            
                            # Get label from filesystem
                            drive_info.label = self._get_filesystem_label(device)
                            
                            drive_info.update_space_info()
                            drive_info.is_mounted = True
                            drive_info.is_ready = True
                            drive_info.status = DriveStatus.READY
                            drive_info.is_system = mount_point in ['/', '/boot', '/usr', '/var']
                            drive_info.is_removable = self._is_removable_device(device)
    ...
    def _should_include_mount(self, device: str, mount_point: str, filesystem: str) -> bool:
        """Determine if mount point should be included."""
        # Exclude virtual filesystems
        virtual_fs = {
            'proc', 'sysfs', 'devfs', 'devpts', 'tmpfs', 'debugfs',
        ...
        return (device.startswith('/dev/') or 
                '://' in device or  # Network mounts
                mount_point in ['/', '/home', '/boot'])
...
class DriveManager(QObject if QT_AVAILABLE else object):
    ...
    def refresh_drives(self) -> List[DriveInfo]:
        ...
        try:
            ...
            self.logger.debug(f"Refreshed drives: found {len(current_drives)} drives")
    ...
    def get_drives(self, include_hidden: bool = False, 
                   drive_types: Optional[List[DriveType]] = None) -> List[DriveInfo]:
        """
        ...
        try:
            ...
            if success:
                ...
                if self._space_monitor_timer:
                    self._space_monitor_timer.start(30000)  # Check every 30 seconds
    ...
    def _on_drive_event(self, event_type: str, drive_info: DriveInfo) -> None:
        ...
        try:
            if event_type == "drive_added":
                ...
                self.logger.info(f"Drive added: {drive_info.get_display_name()}")
                ...
                if drive_info.path in self.drives:
                    self.drives[drive_info.path] = drive_info
                self.logger.debug(f"Drive changed: {drive_info.get_display_name()}")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_capacity_gb': total_capacity / (1024**3) if total_capacity > 0 else 0,
            'total_free_gb': total_free / (1024**3) if total_free > 0 else 0,
            'total_used_gb': total_used / (1024**3) if total_used > 0 else 0,
            'average_usage_percent': (total_used / total_capacity * 100) if total_capacity > 0 else 0,
