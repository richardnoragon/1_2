...
try:
    from PyQt5.QtCore import (QByteArray, QPoint, QRect, QSettings, QSize,
                              QVariant)
...
class EnhancedJSONEncoder(json.JSONEncoder):
    ...
    def default(self, obj: Any) -> Any:
        ...
        try:
            ...
            # Handle QRect
            if QT_AVAILABLE and hasattr(obj, 'x') and hasattr(obj, 'y') and hasattr(obj, 'width') and hasattr(obj, 'height'):
                return {
                    '__qt_type__': 'QRect',
                    'x': obj.x(),
                    'y': obj.y(),
            ...
            # Handle QSize
            if QT_AVAILABLE and hasattr(obj, 'width') and hasattr(obj, 'height') and not hasattr(obj, 'x'):
                return {
                    '__qt_type__': 'QSize',
                    'width': obj.width(),
                    'height': obj.height()
            ...
            # Handle QPoint
            if QT_AVAILABLE and hasattr(obj, 'x') and hasattr(obj, 'y') and not hasattr(obj, 'width'):
                return {
                    '__qt_type__': 'QPoint',
                    'x': obj.x(),
                    'y': obj.y()
            ...
            # Handle QColor
            if QT_AVAILABLE and hasattr(obj, 'red') and hasattr(obj, 'green') and hasattr(obj, 'blue'):
                return {
                    '__qt_type__': 'QColor',
                    'red': obj.red(),
                    'green': obj.green(),
            ...
            # Handle QFont
            if QT_AVAILABLE and hasattr(obj, 'family') and hasattr(obj, 'pointSize'):
                return {
                    '__qt_type__': 'QFont',
                    'family': obj.family(),
                    'pointSize': obj.pointSize(),
                    'bold': obj.bold() if hasattr(obj, 'bold') else False,
                    'italic': obj.italic() if hasattr(obj, 'italic') else False
                }
            ...
            if QT_AVAILABLE and isinstance(obj, QVariant):
                return {
                    ...
                    'value': self.default(obj.value()) if obj.value() is not None else None
                }
    ...
    try:
        if '__qt_type__' in dct:
            ...
            if qt_type == 'QByteArray' and QT_AVAILABLE:
                ...
                return QColor(dct['red'], dct['green'], dct['blue'], dct.get('alpha', 255))
...
class EnhancedConfigurationManager:
    ...
    def __init__(self, config_file: Union[str, Path] = None):
        ...
        self.config_file = Path(config_file) if config_file else Path("config/enhanced_config.json")
        ...
        try:
            if self.config_file.exists():
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    self.config_data = json.load(f, object_hook=enhanced_json_decoder)
                ...
                self.config_data = {}
                self.logger.info(f"No existing configuration found, using defaults")
    ...
    def _handle_corrupt_config(self):
        ...
        try:
            ...
            if self.config_file.exists():
                ...
                self.logger.warning(f"Backed up corrupt config to {backup_path}")
    ...
    def _simplify_config_data(self, data: Any) -> Any:
        ...
        try:
            if isinstance(data, dict):
                return {k: self._simplify_config_data(v) for k, v in data.items()}
    ...
    def validate_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results['errors'].append(f"Serialization test failed: {e}")
            
            # Attempt to fix by simplifying
            try:
                simplified = self._simplify_config_data(self.config_data)
                json.dumps(simplified)
                validation_results['warnings'].append("Configuration can be saved with simplification")
                validation_results['corrected_items'] = 1
            except Exception as fix_error:
                validation_results['errors'].append(f"Cannot fix configuration: {fix_error}")
...
def get_enhanced_config_manager(config_file: Union[str, Path] = None) -> EnhancedConfigurationManager:
...
if __name__ == '__main__':
    ...
    if QT_AVAILABLE:
        ...
        config_manager.set_setting("qt_objects", "byte_array", QByteArray(b"test data"))
        config_manager.set_setting("qt_objects", "rect", QRect(10, 20, 100, 200))
