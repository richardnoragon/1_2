...
class AddressBarCompleter(BaseCompleter):
    ...
    def _get_directory_completions(self, base_dir: Path, prefix: str) -> List[str]:
        """
        ...
        try:
            ...
            for item in base_dir.iterdir():
                try:
                    if item.is_dir():
                        item_name = item.name
                        if not prefix or item_name.lower().startswith(prefix.lower()):
...
class AddressBar(QWidget):
    ...
    def setup_actions(self) -> None:
        ...
        # Keyboard shortcuts
        self.path_edit.addAction(QAction("Go Up", self, shortcut=QKeySequence("Alt+Up"),
    ...
    def set_read_only(self, read_only: bool) -> None:
        ...
        self.bookmark_button.setEnabled(not read_only and bool(self.current_path))
        
        if read_only:
            self.path_edit.setStyleSheet("QLineEdit { background-color: #f5f5f5; }")
        else:
            self.update_appearance()
