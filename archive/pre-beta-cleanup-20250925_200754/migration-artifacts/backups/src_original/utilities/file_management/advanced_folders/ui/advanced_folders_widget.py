...
try:
    ...
    from utilities.file_management.advanced_folders.core.folder_configuration import (
        FolderConfiguration, FolderConfigurationManager, SearchParameters)
    from utilities.file_management.advanced_folders.core.search_engine import (
        FileResult, SearchEngine, SearchProgress)
...
class FolderConfigurationDialog(QDialog):
    ...
    def __init__(self, parent=None, config: Optional[FolderConfiguration] = None):
        ...
        self.setWindowTitle(
            "Edit Folder Configuration" if self.is_editing 
    ...
    def create_search_tab(self):
        ...
        self.filename_pattern_edit.setPlaceholderText("Enter pattern (e.g., *.txt)")
        ...
        self.content_search_edit.setPlaceholderText("Search within file content")
        ...
        self.date_criteria_combo.addItems(["Modified Date", "Created Date", "Accessed Date"])
    ...
    def load_configuration(self):
        ...
        if params.date_from:
            self.date_from_edit.setDate(QDate.fromString(
                params.date_from.date().isoformat(), Qt.ISODate
            ))
        if params.date_to:
            self.date_to_edit.setDate(QDate.fromString(
                params.date_to.date().isoformat(), Qt.ISODate
            ))
    ...
    def get_configuration(self) -> FolderConfiguration:
        ...
        params.date_criteria = self.date_criteria_combo.currentText().lower().replace(' date', '')
        
        params.size_min = self.size_min_spin.value() * 1024 if self.size_min_spin.value() > 0 else None
        params.size_max = self.size_max_spin.value() * 1024 * 1024 if self.size_max_spin.value() > 0 else None
...
class AdvancedFoldersWidget(StandardWindow if StandardWindow != QWidget else QWidget):
    ...
    def export_to_csv(self, filename: str):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as f:
            ...
            # Write header
            writer.writerow([
                'Name', 'Path', 'Size', 'Type', 'Created', 'Modified', 'Accessed'
            ])
