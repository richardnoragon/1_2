...
def test_core_functionality():
    ...
    try:
        ...
        try:
            ...
            if not encryption_logic.encrypt_file(test_file, key, encrypted_file):
                raise ValueError("File encryption failed")
            print("✓ File encryption working")
            
            decrypted_file = test_file + ".decrypted"
            if not encryption_logic.decrypt_file(encrypted_file, key, decrypted_file):
            ...
            # Test configuration
            buffer_size = config.get_setting('performance_settings.buffer_size')
            if buffer_size != 8192:
                raise ValueError("Configuration reading failed")
            
            if not config.set_setting('performance_settings.buffer_size', 16384):
