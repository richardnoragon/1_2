...
class MainTestRunner:
    ...
    def __init__(self):
        ...
        # Output file paths
        self.html_report = self.test_dir / f"result_main_{self.date_suffix}.html"
        self.json_report = self.test_dir / f"result_main_{self.date_suffix}.json"
        self.xml_report = self.test_dir / f"result_main_{self.date_suffix}.xml"
        self.coverage_dir = self.test_dir / f"result_main_{self.date_suffix}_coverage"
        self.coverage_xml = self.test_dir / f"result_main_{self.date_suffix}_coverage.xml"
        self.execution_log = self.test_dir / f"result_main_{self.date_suffix}_execution.log"
        
        # Summary files
        self.summary_json = self.test_dir / f"result_main_{self.date_suffix}_summary.json"
        self.summary_txt = self.test_dir / f"result_main_{self.date_suffix}_summary.txt"
        self.summary_md = self.test_dir / f"result_main_{self.date_suffix}_completion_summary.md"
    ...
    def run_tests(self) -> Dict[str, Any]:
        ...
        # Parse results
        test_results = self._parse_test_results(result, start_time, end_time, execution_time)
    ...
    def _parse_test_results(self, result: subprocess.CompletedProcess, 
        ...
        if self.json_report.exists():
            try:
                ...
                test_results['errors'].append(f"Failed to parse JSON report: {e}")
        ...
        if self.coverage_xml.exists():
            try:
                ...
                test_results['errors'].append(f"Failed to parse coverage XML: {e}")
    ...
    def _create_timeout_result(self, start_time: datetime.datetime) -> Dict[str, Any]:
    ...
    def _create_error_result(self, start_time: datetime.datetime, error: str) -> Dict[str, Any]:
    ...
    def _generate_text_summary(self, test_results: Dict[str, Any]):
        ...
        with open(self.summary_txt, 'w', encoding='utf-8') as f:
            ...
            f.write("-" * 40 + "\n")
            f.write(f"Status: {'SUCCESS' if execution_info['success'] else 'FAILED'}\n")
            f.write(f"Exit Code: {execution_info['exit_code']}\n")
            f.write(f"Duration: {execution_info['execution_time_seconds']:.2f} seconds\n")
            ...
            if coverage_info:
                ...
                f.write(f"Lines Covered: {coverage_info.get('lines_covered', 0)}\n")
                f.write(f"Total Lines: {coverage_info.get('lines_valid', 0)}\n\n")
    ...
    def _generate_markdown_summary(self, test_results: Dict[str, Any]):
        ...
        with open(self.summary_md, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**Test Framework:** pytest with comprehensive plugins  \n")
            f.write(f"**Status:** {'✅ SUCCESS' if execution_info['success'] else '❌ FAILED'}  \n\n")
            
            f.write("## Executive Summary\n\n")
            f.write("Comprehensive unit testing has been completed for the main.py module of Richard's File Utilities. ")
            f.write("This testing suite provides thorough coverage of all functions and methods including:\n\n")
            ...
            f.write("|--------|-------|\n")
            f.write(f"| Execution Status | {'SUCCESS' if execution_info['success'] else 'FAILED'} |\n")
            f.write(f"| Duration | {execution_info['execution_time_seconds']:.2f} seconds |\n")
            f.write(f"| Exit Code | {execution_info['exit_code']} |\n")
            
            if test_summary:
                if 'total' in test_summary:
                    f.write(f"| Total Tests | {test_summary.get('total', 'N/A')} |\n")
                if 'passed' in test_summary:
                    f.write(f"| Tests Passed | {test_summary.get('passed', 'N/A')} |\n")
                if 'failed' in test_summary:
                    f.write(f"| Tests Failed | {test_summary.get('failed', 'N/A')} |\n")
            ...
            if coverage_info:
                ...
                f.write(f"**Line Coverage:** {line_rate * 100:.1f}%  \n")
                f.write(f"**Lines Covered:** {lines_covered} out of {lines_valid}  \n\n")
                
                if line_rate >= 0.8:
                    f.write("✅ **Excellent coverage** - Above 80% threshold\n\n")
                elif line_rate >= 0.7:
                    f.write("✅ **Good coverage** - Above 70% threshold\n\n")
                elif line_rate >= 0.6:
                    f.write("⚠️ **Moderate coverage** - Consider improving test coverage\n\n")
                else:
                    f.write("❌ **Low coverage** - Significant improvements needed\n\n")
            else:
            ...
            if execution_info['success']:
                f.write("✅ **Testing Successful:** All test categories have been executed successfully. ")
                f.write("The main.py module demonstrates robust error handling and comprehensive functionality.\n\n")
                
                if coverage_info and coverage_info.get('line_rate', 0.0) >= 0.7:
                    f.write("✅ **Coverage Acceptable:** Code coverage meets quality standards.\n\n")
                else:
                    f.write("⚠️ **Coverage Improvement:** Consider adding more test cases to improve coverage.\n\n")
                
                f.write("**Next Steps:**\n")
                f.write("1. Review generated HTML and JSON reports for detailed analysis\n")
                f.write("2. Address any specific issues identified in the test output\n")
                f.write("3. Consider adding integration tests for end-to-end scenarios\n")
                f.write("4. Implement continuous testing in development workflow\n\n")
            else:
                f.write("❌ **Testing Issues:** Some test execution problems were encountered. ")
                f.write("Review the execution log and error details for resolution.\n\n")
                
                f.write("**Immediate Actions Required:**\n")
                f.write("1. Review execution log for specific error details\n")
                f.write("2. Address any missing dependencies or configuration issues\n")
                f.write("3. Verify test environment setup\n")
                f.write("4. Re-run tests after resolving identified issues\n\n")
            
            f.write("---\n")
            f.write(f"*Report generated automatically on {datetime.datetime.now().strftime('%Y-%m-%d at %H:%M:%S')}*\n")
    ...
    def display_results(self, test_results: Dict[str, Any]):
        ...
        print(f"⏰ Duration: {execution_info['execution_time_seconds']:.2f} seconds")
        print(f"📋 Status: {'✅ SUCCESS' if execution_info['success'] else '❌ FAILED'}")
