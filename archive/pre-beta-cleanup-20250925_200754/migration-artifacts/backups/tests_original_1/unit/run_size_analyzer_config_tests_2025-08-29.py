...
def setup_environment():
    ...
    requirements_file = test_dir / "test_requirements_size_analyzer_config_2025-08-29.txt"
...
def generate_summary_report(test_dir):
    ...
    coverage_json_file = test_dir / "result_size_analyzer_config_coverage_2025-08-29.json"
    ...
    reports = [
        ...
        ("Coverage HTML", "result_size_analyzer_config_coverage_2025-08-29/index.html"),
        ("Coverage JSON", "result_size_analyzer_config_coverage_2025-08-29.json")
    ]
    ...
    if json_report_file.exists():
        try:
            ...
            print(f"   Total Tests: {summary_data['test_results']['total_tests']}")
            ...
            print(f"   Duration: {summary_data['test_results']['duration']:.2f}s")
    ...
    if coverage_json_file.exists():
        try:
            ...
            summary_data["coverage_results"] = {
                ...
                "branch_coverage_percent": totals.get("percent_covered_display", "0%")
            }
            
            print(f"\\n📈 Coverage Results:")
            print(f"   Line Coverage: {summary_data['coverage_results']['coverage_percent']:.1f}%")
            print(f"   Statements: {summary_data['coverage_results']['statements']}")
            print(f"   Missing Lines: {summary_data['coverage_results']['missing']}")
            print(f"   Branches: {summary_data['coverage_results']['branches']}")
            
        except Exception as e:
            print(f"⚠ Could not parse coverage report: {e}")
    
    # Save summary report
    summary_file = test_dir / "result_size_analyzer_config_summary_2025-08-29.json"
    try:
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("⚠ Tests completed with issues - check the reports for details.")
        
        print(f"\\n📁 Test Directory: {test_dir}")
        print("\\n📊 Generated Reports:")
        for report_name in summary_data.get("reports_generated", []):
            file_path = summary_data["file_locations"].get(report_name, "Unknown")
