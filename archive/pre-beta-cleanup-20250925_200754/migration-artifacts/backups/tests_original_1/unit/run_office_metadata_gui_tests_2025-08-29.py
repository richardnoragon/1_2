...
class OfficeMetadataGUITestRunner:
    ...
    def __init__(self):
        ...
        self.coverage_html = f"result_office_metadata_gui_coverage_{self.timestamp}"
        self.coverage_json = f"result_office_metadata_gui_coverage_{self.timestamp}.json"
        self.coverage_xml = f"result_office_metadata_gui_coverage_{self.timestamp}.xml"
        self.execution_log = f"result_office_metadata_gui_execution_{self.timestamp}.log"
        self.summary_report = f"result_office_metadata_gui_summary_{self.timestamp}.json"
    ...
    def process_results(self, return_code, execution_time, stdout, stderr):
        ...
        if coverage_json_path.exists():
            try:
                ...
                print(f"   📈 Coverage: {summary['coverage']['percent_covered']:.1f}%")
...
"""
        ...
        report_file = self.test_dir / f"result_office_metadata_gui_report_{self.timestamp}.md"
    ...
    def run(self):
        ...
        # Process results
        summary = self.process_results(return_code, execution_time, stdout, stderr)
