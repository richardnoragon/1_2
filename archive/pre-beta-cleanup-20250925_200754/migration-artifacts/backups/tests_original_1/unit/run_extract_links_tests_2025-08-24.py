...
class TestRunner:
        ...
        for dep in dependencies:
            try:
                ...
                if result.returncode != 0:
                    ...
                    self.results["errors"].append(f"Dependency installation failed: {dep}")
                    
            except subprocess.TimeoutExpired:
                print(f"Warning: Timeout installing {dep}")
                self.results["errors"].append(f"Dependency installation timeout: {dep}")
    ...
    def parse_results(self):
        ...
        if json_report_path.exists():
            try:
                ...
                self.results["errors"].append(f"Failed to parse JSON report: {str(e)}")
                
        # Parse coverage JSON if available
        coverage_json_path = self.test_dir / f"{RESULTS_PREFIX}_coverage_{TEST_DATE}.json"
        if coverage_json_path.exists():
            try:
                ...
                if 'percent_covered' in totals:
                    self.results["coverage_percentage"] = totals['percent_covered']
                    
                self.results["reports_generated"].append("Coverage JSON")
                
            except Exception as e:
                self.results["errors"].append(f"Failed to parse coverage JSON: {str(e)}")
        ...
        print(f"✓ Found {len(self.results['reports_generated'])} generated reports")
    ...
    def generate_summary(self):
        ...
        summary_file = self.test_dir / f"{RESULTS_PREFIX}_execution_summary_{TEST_DATE}.txt"
        with open(summary_file, 'w') as f:
            f.write(summary_text)
            
        # Write detailed results to JSON
        results_file = self.test_dir / f"{RESULTS_PREFIX}_detailed_results_{TEST_DATE}.json"
    ...
    def run_complete_test_suite(self):
        ...
        try:
            ...
            return self.results["status"] in ["passed", "failed"]  # True if tests ran
