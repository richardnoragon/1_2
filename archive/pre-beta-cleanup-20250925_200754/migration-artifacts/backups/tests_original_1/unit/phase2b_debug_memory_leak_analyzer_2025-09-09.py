...
class AdvancedMemoryProfiler:
    ...
    def start_advanced_monitoring(self, sample_interval: float = 0.1,
                                  track_objects: bool = True) -> None:
            ...
            while self.monitoring:
                try:
                    ...
                    sample = {
                        ...
                        'gc_uncollectable': [stats['uncollectable'] for stats in gc_stats]
                    }
    ...
    def _detect_real_time_leak(self) -> Tuple[bool, float]:
        ...
        sample_duration = (timestamps[-1] - timestamps[0]) / (n - 1)  # seconds per sample
    ...
    def _generate_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'memory_increase_mb': (memory_values[-1] - self.baseline_memory) / 1024 / 1024,
    ...
    def _analyze_memory_leak_patterns(self, samples: List[Dict]) -> Dict[str, Any]:
        ...
        if total_duration_minutes > 0:
            leak_rate_mb_per_min = (slope * n * 60.0) / (total_duration_minutes * 1024 * 1024)
        else:
            leak_rate_mb_per_min = 0
        
        # Analyze patterns
        leak_patterns = {
            'overall_trend': 'increasing' if slope > 0 else 'stable' if slope == 0 else 'decreasing',
    ...
    def _analyze_object_patterns(self, samples: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'top_growing_objects': dict(sorted(
    ...
    def _analyze_performance_impact(self, samples: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'cpu_memory_correlation': self._calculate_correlation(cpu_values, memory_values),
            'performance_degradation': max(cpu_values) > 50.0 if cpu_values else False
        }
    ...
    def _generate_recommendations(self, samples: List[Dict]) -> List[str]:
        ...
        if memory_increase > 50:  # >50MB increase
            recommendations.append("CRITICAL: Investigate major memory leak - >50MB increase detected")
        elif memory_increase > 10:  # >10MB increase
            recommendations.append("HIGH: Significant memory growth detected - implement resource cleanup")
        
        # Check for object growth
        object_samples = [s for s in samples if 'object_counts' in s]
        if object_samples:
            recommendations.append("Implement object lifecycle monitoring in production")
            recommendations.append("Add garbage collection checkpoints in long-running operations")
...
class SizeAnalyzerDebugWrapper:
    ...
    def debug_operation(self, operation_name: str, operation_data: Dict = None):
        ...
        try:
            ...
            # Calculate changes
            memory_change = post_state['memory_info']['rss'] - pre_state['memory_info']['rss']
    ...
    def analyze_directory_debug(self, directory_path: str, **kwargs) -> Dict[str, Any]:
        ...
        with self.debug_operation('analyze_directory', {'path': directory_path, 'kwargs': kwargs}) as op_id:
    ...
    def get_debug_summary(self) -> Dict[str, Any]:
        ...
        # Memory analysis
        memory_changes = [op.get('memory_change_mb', 0) for op in operations if op.get('success', False)]
        total_memory_change = sum(memory_changes)
        avg_memory_change = total_memory_change / len(memory_changes) if memory_changes else 0
        
        # Object analysis
        object_changes = [op.get('object_count_change', 0) for op in operations if op.get('success', False)]
        total_object_change = sum(object_changes)
        avg_object_change = total_object_change / len(object_changes) if object_changes else 0
        ...
        return {
            ...
            'memory_analysis': {
                ...
                'memory_leak_detected': avg_memory_change > 1.0  # >1MB average increase
            },
            'object_analysis': {
                ...
                'object_leak_detected': avg_object_change > 100  # >100 objects average increase
            },
            ...
            'error_analysis': {
                ...
                'error_types': list(set(err.get('error_type', 'unknown') for err in self.debug_data['error_events']))
            }
        }
...
class MemoryLeakDetectionTestSuite:
    ...
    def setup_debug_environment(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "phase2b_perf_test", 
            os.path.join(os.path.dirname(__file__), "phase2b_performance_load_testing_no_compromise_2025-09-09.py")
        )
        ...
        self.test_datasets = {
            'debug_simple': generator.create_production_scale_dataset('simple_uniform', scale_factor=1),
            'debug_mixed': generator.create_production_scale_dataset('mixed_sizes', scale_factor=1),
            'debug_nested': generator.create_production_scale_dataset('deep_nesting', scale_factor=1)
        }
    ...
    def run_memory_leak_debug_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.profiler.start_advanced_monitoring(sample_interval=0.05, track_objects=True)
            ...
            comprehensive_results = {
                ...
                'leak_diagnosis': self._diagnose_memory_leak(memory_analysis, debug_results),
                'recommendations': self._generate_fix_recommendations(memory_analysis, debug_results)
            }
    ...
    def _execute_debug_scenario(self, debug_wrapper: SizeAnalyzerDebugWrapper) -> Dict[str, Any]:
    ...
    def _diagnose_memory_leak(self, memory_analysis: Dict, debug_results: Dict) -> Dict[str, Any]:
        ...
        if growing_objects:
            ...
            max_growth_type = max(growing_objects.keys(),
        ...
        if memory_info.get('memory_leak_detected', False):
            diagnosis['sizeanalyzer_specific'] = {
                'avg_memory_increase_per_operation': memory_info.get('avg_memory_change_mb', 0),
    ...
    def _generate_fix_recommendations(self, memory_analysis: Dict, debug_results: Dict) -> List[str]:
