...
class SelectiveTestExecutor:
    ...
    def __init__(self, base_path: str = None, repo_path: str = None):
        ...
        self.repo_path = Path(repo_path) if repo_path else self.base_path.parent.parent
    ...
    def _initialize_test_mappings(self):
        ...
        self.test_mappings.update({
            'test_database_integration.py': {
                'source_patterns': ['src/rfu/core/', 'src/utilities/*/core/'],
                'dependencies': ['sqlite3', 'database'],
                'categories': ['database', 'core']
            },
            'test_file_operations_integration.py': {
                'source_patterns': ['src/utilities/file_operations/', 'src/rfu/core/file_ops/'],
            ...
            'test_external_service_integration.py': {
                'source_patterns': ['src/utilities/network/', 'src/utilities/*/network/'],
        ...
        self.test_mappings.update({
            ...
            'test_security_validation.py': {
                'source_patterns': ['src/utilities/security/', 'src/rfu/core/'],
                'dependencies': ['security', 'encryption'],
                'categories': ['security', 'validation']
            }
        })
    
    def detect_code_changes(self, base_commit: str = 'HEAD~1', 
                           target_commit: str = 'HEAD') -> List[CodeChange]:
        ...
        try:
            ...
            for diff_item in diff:
                ...
                lines_deleted = diff_item.diff.count('\n-') if diff_item.diff else 0
    ...
    def _calculate_change_impact_score(self, file_path: str, change_type: str,
                                      lines_added: int, lines_deleted: int) -> float:
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        if current_time - mod_time < 86400:  # Modified in last 24 hours
    ...
    def analyze_test_impact(self, changes: List[CodeChange]) -> List[TestImpactAnalysis]:
    ...
    def _calculate_test_impact(self, test_file: str, 
                              changes: List[CodeChange]) -> TestImpactAnalysis:
        ...
        for change in changes:
            ...
            for pattern in source_patterns:
                if pattern in change.file_path:
                    change_impact += (change.impact_score * 
    ...
    def select_tests_for_execution(self, base_commit: str = 'HEAD~1',
                                  target_commit: str = 'HEAD',
                                  force_critical: bool = True) -> Dict:
        ...
        # Filter tests to run
        tests_to_run = [
            analysis.test_file for analysis in impact_analyses 
        ...
        selection_result = {
            ...
            'selection_ratio': len(tests_to_run) / max(len(impact_analyses), 1),
            ...
            'optimization_metrics': {
                ...
                'selection_efficiency': len(tests_to_run) / max(len(impact_analyses), 1)
            }
        }
    ...
    def create_test_dependency_mapping(self) -> Dict:
        ...
        for test_file, config in self.test_mappings.items():
            ...
            if test_path and test_path.exists():
                actual_dependencies = self._analyze_test_dependencies(test_path)
                ...
                combined_mapping = {
                    ...
                    'actual_file_references': actual_dependencies.get('file_refs', []),
        ...
        high_confidence = sum(
            1 for mapping in dependency_mapping['mappings'].values()
    ...
    def _analyze_test_dependencies(self, test_path: Path) -> Dict:
        ...
        try:
            ...
            return {'imports': [], 'file_refs': [], 'total_lines': 0, 'import_count': 0}
    
    def _calculate_dependency_confidence(self, config: Dict, actual_deps: Dict) -> float:
        ...
        pattern_matches = sum(
            1 for pattern in source_patterns
    ...
    def generate_selective_execution_plan(self, 
        ...
        if optimization_target == 'aggressive':
            ...
            filtered_tests = [
                analysis['test_file'] for analysis in selection_result['test_impact_analysis']
        ...
        execution_plan = {
            ...
            'estimated_execution_time': len(filtered_tests) * 30,  # 30s per test
            'optimization_summary': {
                'total_available_tests': selection_result['total_available_tests'],
                'tests_after_selection': len(filtered_tests),
                'reduction_percentage': (
                    (selection_result['total_available_tests'] - len(filtered_tests)) /
                    max(selection_result['total_available_tests'], 1)
                ) * 100,
                'estimated_time_savings': selection_result['optimization_metrics']['estimated_time_savings']
            }
        }
    ...
    def execute_selective_tests(self, execution_plan: Dict) -> Dict:
        ...
        results['effectiveness_metrics'] = {
            ...
            'success_rate': (results['execution_summary']['passed_tests'] / 
                           max(len(test_files), 1)),
            'optimization_effective': execution_time < execution_plan['estimated_execution_time']
        }
...
def run_selective_test_execution(base_commit: str = 'HEAD~1',
    ...
    print(f"  - Total available tests: {execution_plan['optimization_summary']['total_available_tests']}")
    print(f"  - Tests selected: {execution_plan['optimization_summary']['tests_after_selection']}")
    print(f"  - Reduction: {execution_plan['optimization_summary']['reduction_percentage']:.1f}%")
    print(f"  - Estimated time savings: {execution_plan['optimization_summary']['estimated_time_savings']:.0f}s")
    ...
    return {
        ...
        'summary': {
            'optimization_effective': execution_result['effectiveness_metrics']['optimization_effective'],
            'time_savings_achieved': execution_plan['optimization_summary']['estimated_time_savings'],
            'success_rate': execution_result['effectiveness_metrics']['success_rate']
        }
    }
...
if __name__ == "__main__":
    ...
    print(f"- Optimization Effective: {result['summary']['optimization_effective']}")
