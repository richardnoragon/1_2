...
def demonstrate_core_features():
    ...
    try:
        ...
        print(f"   ✅ String Setting: {string_setting} (type: {type(string_setting).__name__})")
        print(f"   ✅ Integer Setting: {integer_setting} (type: {type(integer_setting).__name__})")
        print(f"   ✅ Boolean Setting: {boolean_setting} (type: {type(boolean_setting).__name__})")
        print(f"   ✅ JSON Setting: {json_setting} (type: {type(json_setting).__name__})")
        ...
        for entry in history_results:
            ...
            print(f"   📄 {entry['file_name']} - {entry['tool_name']} ({entry['access_count']} accesses)")
        ...
        session_id = info.get('session_id')
        test_logs = [
            ('INFO', 'Demo.Integration', 'SQLite integration test started', 'IntegrationDemo', session_id, '{}'),
            ('WARNING', 'Demo.Validation', 'Testing warning message storage', 'IntegrationDemo', session_id, '{"test": true}'),
            ('ERROR', 'Demo.Test', 'Testing error message storage', 'IntegrationDemo', session_id, '{"error_code": 404}'),
            ('INFO', 'Demo.Success', 'SQLite integration validation successful', 'IntegrationDemo', session_id, '{"status": "complete"}')
        ]
        ...
        for stat in usage_results:
            ...
            print(f"   🛠️ {stat['tool_name']}.{stat['operation_type']}: {stat['usage_count']} uses ({success_rate:.1f}% success)")
