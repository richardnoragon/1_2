...
class PerformanceMonitor:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    snapshot.memory_percent, snapshot.io_read_bytes, 
                    snapshot.io_write_bytes, snapshot.thread_count,
                    snapshot.test_context
                ))
        except Exception:
            pass
    
    def monitor_test_execution(self, test_name: str, 
                              test_execution_func: Callable) -> TestPerformanceMetrics:
    ...
    def _calculate_test_metrics(self, test_name: str, start_time: float, end_time: float,
                               snapshots: List[PerformanceSnapshot],
                               initial_io: Dict, final_io: Dict) -> TestPerformanceMetrics:
        ...
        memory_usage_avg = statistics.mean(memory_values) if memory_values else 0
        ...
        efficiency_score = self._calculate_efficiency_score(
            cpu_usage_avg, memory_usage_avg, duration, io_read_total + io_write_total
        )
    ...
    def _calculate_efficiency_score(self, cpu_avg: float, memory_avg: float,
                                   duration: float, io_total: int) -> float:
    ...
    def _determine_performance_grade(self, duration: float, cpu_avg: float,
                                   memory_avg: float, efficiency_score: float) -> str:
    ...
    def _store_test_metrics(self, metrics: TestPerformanceMetrics):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                metrics.test_name, metrics.start_time, metrics.duration,
                metrics.cpu_usage_avg, metrics.cpu_usage_peak,
                metrics.memory_usage_avg, metrics.memory_usage_peak,
                metrics.io_read_total, metrics.io_write_total,
                metrics.resource_efficiency_score, metrics.performance_grade
            ))
    ...
    def generate_performance_report(self, time_window_hours: int = 24) -> Dict:
        ...
        report = {
            ...
            'performance_summary': self._analyze_performance_summary(recent_tests),
            'optimization_recommendations': self._generate_optimization_recommendations(recent_tests)
        }
    ...
    def _generate_optimization_recommendations(self, test_data: List) -> List[str]:
...
def monitor_test_suite_performance(test_suite_func: Callable, 
                                  test_suite_name: str = "test_suite") -> Dict:
...
if __name__ == "__main__":
    ...
    print(f"- Performance Grade: {result['monitoring_summary']['performance_grade']}")
    print(f"- Efficiency Score: {result['monitoring_summary']['efficiency_score']:.1f}")
