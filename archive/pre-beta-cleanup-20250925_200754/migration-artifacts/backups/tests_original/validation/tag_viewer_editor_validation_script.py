...
class TagViewerEditorValidator:
    ...
    def validate_file_structure(self) -> Dict[str, Any]:
        ...
        files_to_check = {
            ...
            'backup_py': Path('backup/tag_viewer_editor_migration/2025-01-27_14-16-46/tag_viewer_editor.py'),
            'backup_ui': Path('backup/tag_viewer_editor_migration/2025-01-27_14-16-46/tag_viewer_editor.ui'),
            'backup_manifest': Path('backup/tag_viewer_editor_migration/2025-01-27_14-16-46/backup_manifest.txt'),
        ...
        for file1_key, file2_key, description in comparisons:
            ...
            if file1_info.get('exists') and file2_info.get('exists'):
                ...
                results[description] = {
                    ...
                    'size_match': file1_info.get('size') == file2_info.get('size')
                }
            else:
                results[description] = {
    ...
    def test_pyqt5_compatibility(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results['pyqt5_version'] = getattr(PyQt5.QtCore, 'PYQT_VERSION_STR', 'Unknown')
        ...
        try:
            ...
            results['import_tests']['class_methods'] = [
                method for method in dir(TagViewerEditor) 
                if not method.startswith('_') or method in ['__init__']
            ]
            
            # Test UI file loading
            ui_file = self.base_dir / 'file_utilities_2/gui/tag_viewer_editor.ui'
            if ui_file.exists():
                try:
                    ...
                    results['ui_loading_test'] = {
                        ...
                        'widgets_found': [
                            attr for attr in dir(temp_widget) 
                            if not attr.startswith('_') and hasattr(getattr(temp_widget, attr), 'objectName')
                        ]
                    }
                except Exception as e:
                    results['ui_loading_test'] = {
    ...
    def validate_import_statements(self) -> Dict[str, Any]:
        ...
        if rfuhub_file.exists():
            try:
                ...
                new_import_found = 'from file_utilities_2.gui.tag_viewer_editor import TagViewerEditor' in content
                old_import_found = 'from tag_viewer_editor import TagViewerEditor' in content
                ...
                results['rfuhub_integration'] = {
                    ...
                    'integration_method': 'open_tag_metadata_editor' in content
                }
        ...
        for py_file in python_files:
            if py_file.name in ['tag_viewer_editor_validation_script.py', 'tag_viewer_editor.py']:
                continue
    ...
    def verify_backup_system(self) -> Dict[str, Any]:
        ...
        backup_dir = self.base_dir / 'backup/tag_viewer_editor_migration/2025-01-27_14-16-46'
        
        # Check backup structure
        expected_files = ['tag_viewer_editor.py', 'tag_viewer_editor.ui', 'backup_manifest.txt']
        ...
        if manifest_file.exists():
            try:
                ...
                results['manifest_validation'] = {
                    'contains_timestamp': '2025-01-27_14-16-46' in manifest_content,
                    'contains_file_info': 'tag_viewer_editor.py' in manifest_content,
                    'contains_checksums': 'SHA256' in manifest_content,
                    'contains_rollback_instructions': 'ROLLBACK INSTRUCTIONS' in manifest_content
                }
            except Exception as e:
        ...
        results['restoration_capability'] = {
            ...
            'restore_script_readable': restore_script.exists() and os.access(restore_script, os.R_OK)
        }
    ...
    def analyze_legacy_files(self) -> Dict[str, Any]:
        ...
        for file_name in legacy_files:
            file_key = f"legacy_{file_name.split('.')[1]}"  # legacy_py or legacy_ui
            ...
            if legacy_info.get('exists'):
                ...
                if migrated_info.get('exists'):
                    analysis['identical_to_migrated'] = (
                        legacy_info.get('checksum') == migrated_info.get('checksum')
                    )
                
                backup_key = f"backup_{file_name.split('.')[1]}"
                backup_info = file_info.get(backup_key, {})
                if backup_info.get('exists'):
                    analysis['identical_to_backup'] = (
                        legacy_info.get('checksum') == backup_info.get('checksum')
                    )
        ...
        migration_successful = all(
            self.validation_results['file_structure'].get(f"migrated_{ext}", {}).get('exists', False)
            for ext in ['py', 'ui']
        )
        ...
        results['safety_assessment'] = {
            ...
            'safe_to_remove_legacy': all_legacy_identical_to_backup and migration_successful
        }
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        print("🚀 Starting comprehensive tag_viewer_editor migration validation...\n")
        
        # Run all validation steps
        self.validation_results['file_structure'] = self.validate_file_structure()
        self.validation_results['checksums'] = self.validate_migration_integrity()
        self.validation_results['pyqt5_compatibility'] = self.test_pyqt5_compatibility()
        self.validation_results['import_validation'] = self.validate_import_statements()
        self.validation_results['integration_tests'] = self.test_integration_with_rfuhub()
        self.validation_results['backup_verification'] = self.verify_backup_system()
        self.validation_results['legacy_analysis'] = self.analyze_legacy_files()
    ...
    def _compile_issues_and_recommendations(self):
        ...
        elif not pyqt5_compat.get('import_tests', {}).get('direct_import', {}).get('success'):
            issues.append("❌ CRITICAL: Cannot import migrated TagViewerEditor class")
        ...
        if legacy_analysis.get('safety_assessment', {}).get('safe_to_remove_legacy'):
            recommendations.append("✅ Legacy files can be safely removed")
        else:
            recommendations.append("⚠️ Retain legacy files until issues are resolved")
        
        if not issues:
            recommendations.append("🎉 Migration validation successful - all tests passed!")
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"Validation Script: tag_viewer_editor_validation_script.py")
        report.append("")
        ...
        if not issues:
            ...
            report.append("✅ All migration validation tests completed successfully")
        else:
        ...
        for desc, result in checksums.items():
            if 'error' in result:
                ...
                report.append(f"   {status} {desc}: Checksums {'match' if result.get('match') else 'differ'}")
        
        # PyQt5 Compatibility
        report.append("\n3. PYQT5 COMPATIBILITY")
        pyqt5 = self.validation_results.get('pyqt5_compatibility', {})
        pyqt5_status = "✅" if pyqt5.get('pyqt5_available') else "❌"
        report.append(f"   {pyqt5_status} PyQt5 Available: {pyqt5.get('pyqt5_available')}")
        
        import_test = pyqt5.get('import_tests', {}).get('direct_import', {})
        import_status = "✅" if import_test.get('success') else "❌"
        report.append(f"   {import_status} Import Test: {import_test.get('success', False)}")
        
        ui_test = pyqt5.get('ui_loading_test', {})
        ui_status = "✅" if ui_test.get('success') else "❌"
        report.append(f"   {ui_status} UI Loading: {ui_test.get('success', False)}")
        
        # Integration Tests
        report.append("\n4. INTEGRATION TESTS")
        integration = self.validation_results.get('integration_tests', {})
        int_status = "✅" if integration.get('import_test', {}).get('success') else "❌"
        report.append(f"   {int_status} RFU Hub Integration: {integration.get('import_test', {}).get('success', False)}")
        
        # Backup Verification
        report.append("\n5. BACKUP VERIFICATION")
        backup = self.validation_results.get('backup_verification', {})
        backup_status = "✅" if backup.get('backup_structure', {}).get('backup_dir_exists') else "❌"
        report.append(f"   {backup_status} Backup Directory: {backup.get('backup_structure', {}).get('backup_dir_exists', False)}")
        
        # Legacy File Analysis
        report.append("\n6. LEGACY FILE ANALYSIS")
        legacy = self.validation_results.get('legacy_analysis', {})
        safe_remove = legacy.get('safety_assessment', {}).get('safe_to_remove_legacy', False)
        legacy_status = "✅" if safe_remove else "⚠️"
        report.append(f"   {legacy_status} Safe to Remove Legacy: {safe_remove}")
