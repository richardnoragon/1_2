...
def test_legacy_files_removed():
    ...
    for file_name in legacy_files:
        ...
        results[file_name] = {
            'exists': exists,
            'status': 'FAIL - File still exists' if exists else 'PASS - File removed'
        }
...
def test_migrated_files_exist():
    ...
    for file_path in migrated_files:
        ...
        results[file_path] = {
            'exists': exists,
            'status': 'PASS - File exists' if exists else 'FAIL - File missing'
        }
...
def test_backup_integrity():
    ...
    for file_path in backup_files:
        ...
        results[file_path] = {
            'exists': exists,
            'status': 'PASS - Backup exists' if exists else 'FAIL - Backup missing'
        }
        print(f"  {file_path}: {results[file_path]['status']}")
...
def test_import_functionality():
    ...
    try:
        ...
        try:
            ...
            print("  Class availability: PASS - TagViewerEditor class is accessible")
...
def test_no_legacy_imports():
    ...
    for py_file in python_files:
        ...
        if any(skip in str(py_file) for skip in ['test_', 'backup/', 'validation_script']):
            continue
            
        try:
            content = py_file.read_text(encoding='utf-8')
            if 'from tag_viewer_editor import' in content or 'import tag_viewer_editor' in content:
                # Check if it's just a comment
                lines = content.split('\n')
                for i, line in enumerate(lines):
                    if ('from tag_viewer_editor import' in line or 'import tag_viewer_editor' in line) and not line.strip().startswith('#'):
                        legacy_imports_found.append(f"{py_file}:{i+1} - {line.strip()}")
        except Exception:
            continue
