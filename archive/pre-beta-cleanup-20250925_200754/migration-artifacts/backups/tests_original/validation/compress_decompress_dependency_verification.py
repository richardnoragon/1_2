...
class DependencyVerifier:
    ...
    def test_pyqt5_dependencies(self) -> Dict[str, Any]:
        ...
        pyqt5_modules = [
            ('PyQt5.QtWidgets', ['QApplication', 'QMainWindow', 'QWidget', 'QLineEdit', 'QPushButton', 'QComboBox', 'QSlider']),
            ...
            try:
                ...
                for class_name in required_classes:
                    if hasattr(module, class_name):
                        ...
                        self.critical_failures.append(f'{module_name}.{class_name} missing')
                ...
                self.critical_failures.append(f'{module_name} not available: {e}')
    ...
    def test_gui_common_framework(self) -> Dict[str, Any]:
        ...
        framework_modules = [
            ...
            ('gui.common.styles', ['get_base_styles', 'get_custom_styles', 'Theme']),
            ('gui.common.settings', ['AppearanceSettings']),
            ('gui.common.dialogs', ['show_error_dialog', 'show_info_dialog', 'get_existing_directory', 'get_open_file_name', 'get_save_file_name']),
            ...
            try:
                ...
                for item_name in required_items:
                    if hasattr(module, item_name):
                        ...
                        self.critical_failures.append(f'{module_name}.{item_name} missing')
        ...
        if ui_file.exists():
            ...
            try:
                with open(ui_file, 'r', encoding='utf-8') as f:
                    content = f.read(100)
                results['compress_decompress.ui']['sample_content'] = content[:50] + '...'
            ...
            self.critical_failures.append('compress_decompress.ui file not found')
    ...
    def test_drag_drop_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                results['drag_drop_functionality'] = f'INSTANTIATION_ERROR: {e}'
            ...
            self.critical_failures.append(f'Drag/drop functionality not available: {e}')
    ...
    def generate_report(self) -> str:
                ...
                if isinstance(dep_info, dict) and 'status' in dep_info:
                    ...
                    if status == 'AVAILABLE':
                        ...
                        if 'version' in dep_info:
                            report.append(f"     Version: {dep_info['version']}")
                    else:
                        report.append(f"  ❌ {dep_name}: {status}")
        ...
        if not self.critical_failures:
            ...
            report.append(f"     {len(self.critical_failures)} critical issues must be resolved")
            report.append("     Install missing dependencies before proceeding")
        ...
        if file_utils_1_path.exists():
            ...
            if gui_common_path.exists():
                ...
                report.append("     GUI common framework needs to be migrated first")
        else:
        ...
        if not self.critical_failures:
            report.append("  1. ✅ All dependencies satisfied - migration can proceed")
            report.append("  2. 🔄 Ensure gui.common framework is available in file_utilities_1")
            report.append("  3. 📋 Test UI file loading after migration")
            report.append("  4. 🧪 Verify drag-and-drop functionality post-migration")
        else:
            report.append("  1. 📥 Install missing dependencies (see requirements above)")
            report.append("  2. 🔧 Resolve critical framework issues")
            report.append("  3. 🧪 Re-run verification after fixes")
            report.append("  4. ⏸️  Postpone migration until all issues resolved")
