...
def execute_comprehensive_tests():
    ...
    print("╠" + "═" * 78 + "╣")
    print(f"║ Execution Time: {execution_start.strftime('%Y-%m-%d %H:%M:%S')}".ljust(79) + "║")
    ...
    output_files = {
        'html_report': test_directory / f"result_miner_{date_stamp}_comprehensive_report.html",
        'json_results': test_directory / f"result_miner_{date_stamp}_detailed_results.json", 
        'junit_xml': test_directory / f"result_miner_{date_stamp}_junit_output.xml",
        'coverage_html_dir': test_directory / f"result_miner_{date_stamp}_coverage_html",
        'coverage_json': test_directory / f"result_miner_{date_stamp}_coverage_data.json",
        'coverage_xml': test_directory / f"result_miner_{date_stamp}_coverage_report.xml",
        'execution_log': test_directory / f"result_miner_{date_stamp}_execution_detailed.log",
        'performance_metrics': test_directory / f"result_miner_{date_stamp}_performance.json",
        'final_summary': test_directory / f"result_miner_{date_stamp}_final_summary.json"
    }
    ...
    with open(output_files['execution_log'], 'w', encoding='utf-8') as log_file:
        ...
        log_file.write(f"Test Execution Duration: {execution_duration:.2f} seconds\\n")
    ...
    print("╠" + "═" * 78 + "╣")
    print(f"║ Status: {'✅ SUCCESS' if return_code == 0 else '❌ FAILURE'}".ljust(79) + "║")
    ...
    if coverage_summary:
        ...
        print(f"║ Coverage Percentage: {coverage_percent:.1f}%".ljust(79) + "║")
        print(f"║ Lines Covered: {covered_lines}/{total_lines}".ljust(79) + "║")
