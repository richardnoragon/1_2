...
class MemoryOptimizedSizeAnalyzer(QObject):
    ...
    def __init__(self, hub_connector=None):
        ...
        self._collect_file_details = True  # Can be disabled for memory efficiency
    ...
    def analyze_directory(self, directory_path: str,
                          ...
                          collect_file_details: Optional[bool] = None) -> Dict[str, Any]:
        """
        ...
        collect_details = collect_file_details if collect_file_details is not None else self._collect_file_details
        ...
        try:
            ...
            for file_info in self._scan_directory_optimized(directory_path, include_extensions):
                ...
                if collect_details and len(analysis.get('files', [])) < self._max_file_details:
                    analysis['files'].append(file_info)
                
                # Update file type statistics
                self._update_file_type_stats_optimized(file_info, analysis['file_types'])
                ...
                if total_items > 0 and items_processed % 100 == 0:  # Less frequent updates
                    percentage = min(int((items_processed / total_items) * 70) + 10, 80)
                ...
                if items_processed % 1000 == 0:
                    self.progress_message.emit(f"Processed {items_processed} items...")
            ...
            analysis['directory_count'] = self._count_directories_optimized(directory_path)
            ...
            if collect_details and 'files' in analysis:
                analysis['largest_files'] = self._find_largest_files_optimized(
                    analysis['files'], min(top_files_count, 100)  # Limit to 100 max
                )
            
            # Generate directory tree (optional for memory efficiency)
            if collect_details:
                analysis['directory_tree'] = self._generate_directory_tree_optimized(directory_path)
            ...
            duration = (self._performance_metrics['end_time'] - self._performance_metrics['start_time']).total_seconds()
            
            if duration > 0:
                self._performance_metrics['files_per_second'] = analysis.get('file_count', 0) / duration
                self._performance_metrics['bytes_per_second'] = analysis.get('total_size', 0) / duration
            
            # Add performance metrics to analysis (lightweight)
            analysis['performance_metrics'] = {
                'duration_seconds': duration,
                'files_per_second': self._performance_metrics.get('files_per_second', 0),
            ...
            if self._hub_connector:
                self._hub_connector.report_status_to_hub("completed", {
            ...
            if self._hub_connector:
                self._hub_connector.report_error_to_hub(error_msg, {
    ...
    def _scan_directory_optimized(self, directory_path: str, include_extensions: Optional[List[str]]):
    ...
    def _count_items_optimized(self, directory_path: str) -> int:
        ...
        try:
            for root, dirs, files in os.walk(directory_path):
                total += len(files)
                if self._should_cancel or total > 100000:  # Limit for memory protection
                    break
        except (OSError, PermissionError):
            pass
    ...
    def _update_file_type_stats_optimized(self, file_info: Dict[str, Any], file_types: Dict[str, Dict[str, Any]]) -> None:
        ...
        file_types[ext]['average_size'] = file_types[ext]['total_size'] / file_types[ext]['count']
    
    def _find_largest_files_optimized(self, files: List[Dict[str, Any]], count: int) -> List[Dict[str, Any]]:
    ...
    def _generate_directory_tree_optimized(self, directory_path: str, max_depth: int = 10) -> Dict[str, Any]:
        ...
        def build_tree(path: str, current_depth: int = 0) -> Dict[str, Any]:
            if current_depth > max_depth or self._should_cancel:
                return {'name': os.path.basename(path), 'path': path, 'size': 0, 'truncated': True}
    ...
    def cancel_operation(self) -> None:
        ...
        if self._hub_connector:
            self._hub_connector.report_status_to_hub("cancelled", {
...
class SizeAnalyzerMemoryOptimizedContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.initial_memory:
            ...
            memory_change = final_memory['rss_mb'] - self.initial_memory['rss_mb']
            if memory_change > 10:  # Report significant changes
                print(f"Memory change: +{memory_change:.2f} MB")


# Convenience function for easy usage
def create_memory_optimized_analyzer(hub_connector=None, 
                                     collect_file_details: bool = False,
                                     max_file_details: int = 1000) -> MemoryOptimizedSizeAnalyzer:
