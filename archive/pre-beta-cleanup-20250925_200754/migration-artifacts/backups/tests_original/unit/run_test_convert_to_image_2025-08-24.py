...
def setup_environment():
    ...
    src_path = (current_dir / ".." / ".." / "src" / "utilities" /
...
def generate_summary_report():
    ...
    with open(summary_text_file, 'w') as f:
        ...
        f.write(f"Execution Timestamp: {summary['test_execution_timestamp']}\n")
        ...
        for report in summary["reports_generated"]:
            f.write(f"  - {report['file']} ({report['size']} bytes, created {report['created']})\n")
...
def main():
    ...
    print("  - result_convert_to_image_coverage_2025-08-24.json (Coverage JSON)")
