...
def complex_pdf_file(temp_directory):
    ...
    page1 = doc.new_page()
    page1.insert_text((72, 72), "Title Text", fontsize=18, fontname="helv-bold")
    page1.insert_text((72, 120), "Regular paragraph text with normal font size.", fontsize=12)
    page1.insert_text((72, 150), "Small footnote text", fontsize=8)
    
    # Page 2: Text with tables/structured content
    page2 = doc.new_page()
    page2.insert_text((72, 72), "Table Data", fontsize=14, fontname="helv-bold")
    page2.insert_text((72, 100), "Column 1\tColumn 2\tColumn 3", fontsize=10)
    page2.insert_text((72, 120), "Data 1\tData 2\tData 3", fontsize=10)
    page2.insert_text((72, 140), "Row 2 Data 1\tRow 2 Data 2\tRow 2 Data 3", fontsize=10)
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "pdf_tools: Tests for PDF processing tools")
    config.addinivalue_line("markers", "unit_test: Unit test classification")
    config.addinivalue_line("markers", f"date_{TEST_DATE.replace('-', '_')}: Tests created on {TEST_DATE}")
...
def assert_valid_pdf_text(text, expected_content=None):
    ...
    if expected_content:
        assert expected_content in text, f"Expected '{expected_content}' in text"
