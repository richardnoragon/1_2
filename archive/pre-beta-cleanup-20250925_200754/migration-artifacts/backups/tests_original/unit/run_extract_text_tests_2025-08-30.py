...
class TestExecutionManager:
    ...
    def __init__(self):
        ...
        # Setup paths
        self.test_file = self.test_dir / f"test_extract_text_{self.date_suffix}.py"
        self.results_dir = self.test_dir
        
        # Report file paths
        self.html_report = self.results_dir / f"result_extract_text_report_{self.date_suffix}.html"
        self.json_report = self.results_dir / f"result_extract_text_json_{self.date_suffix}.json"
        self.coverage_html = self.results_dir / f"result_extract_text_coverage_{self.date_suffix}"
        self.coverage_json = self.results_dir / f"result_extract_text_coverage_{self.date_suffix}.json"
        self.execution_log = self.results_dir / f"result_extract_text_execution_log_{self.date_suffix}.txt"
        
        print(f"Test Execution Manager initialized at {self.execution_timestamp}")
    ...
    def execute_tests(self):
        ...
        try:
            with open(self.execution_log, 'w', encoding='utf-8') as log_file:
                log_file.write(f"Test Execution Log - {self.execution_timestamp.isoformat()}\n")
                log_file.write("="*80 + "\n")
                ...
                log_file.write(f"End Time: {datetime.datetime.now().isoformat()}\n")
    ...
    def generate_summary_report(self, test_success, test_result):
        ...
        summary_file = self.results_dir / f"result_extract_text_summary_{self.date_suffix}.json"
        ...
        for file_type, file_path in summary["generated_files"].items():
            ...
            summary["file_status"][file_type] = {
                ...
                "modified": path_obj.stat().st_mtime if path_obj.exists() else None
            }
        ...
        if self.json_report.exists():
            try:
                with open(self.json_report, 'r', encoding='utf-8') as f:
                    ...
                    summary["execution_results"]["execution_duration"] = test_data.get("duration", 0)
            except Exception as e:
                summary["test_statistics"] = {"error": f"Failed to parse JSON report: {str(e)}"}
        ...
        if self.coverage_json.exists():
            try:
                ...
                summary["coverage_statistics"] = {"error": f"Failed to parse coverage report: {str(e)}"}
        ...
        if "coverage_statistics" in summary:
            ...
            if "error" not in cov:
                ...
                print(f"  Total statements: {cov.get('total_statements', 'N/A')}")
                print(f"  Covered statements: {cov.get('covered_statements', 'N/A')}")
