...
def setup_test_environment():
    ...
    directories = [
        'logs',
        'output',
        'result_highlight_coverage_2025-08-24'
    ]
...
def run_tests(test_dir):
    ...
    pytest_cmd = [
        ...
        '-c', 'pytest_highlight_2025-08-24.ini',
...
def generate_execution_summary(test_dir, test_result):
    ...
    if test_result:
        summary_lines.extend([
            ...
            "STDERR OUTPUT:",
            "-" * 40,
            test_result.stderr if test_result.stderr else "No errors reported",
            ""
        ])
    else:
        summary_lines.extend([
            ...
            if test_data.get('summary', {}).get('failed', 0) > 0:
                ...
                for test in test_data.get('tests', []):
                    if test.get('outcome') == 'failed':
                        summary_lines.append(f"- {test.get('nodeid', 'Unknown test')}")
                        if test.get('call', {}).get('longrepr'):
                            summary_lines.append(f"  Error: {test['call']['longrepr'][:100]}...")
    ...
    summary_path = test_dir / "result_highlight_execution_summary_2025-08-24.txt"
    ...
    print("-" * 40)
    if test_result:
        print(f"Overall Status: {'PASSED' if test_result.returncode == 0 else 'FAILED'}")
    else:
    ...
    if coverage_json_path.exists():
        try:
            with open(coverage_json_path, 'r') as f:
                coverage_data = json.load(f)
            print(f"Coverage: {coverage_data.get('totals', {}).get('percent_covered', 'N/A')}%")
        except:
