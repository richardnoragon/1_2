...
class BlockerDependencyMapper:
    ...
    def build_dependency_graph(self):
        ...
        for blocker_id, blocker in self.blockers.items():
            priority_weight = self.get_priority_weight(blocker.get('priority', '🟡 HIGH'))
    ...
    def calculate_earliest_start_times(self) -> Dict[str, datetime]:
        ...
        for blocker_id in sorted_blockers:
            ...
            if not dependencies:
                ...
                for dep in dependencies:
                    dep_completion = self.blockers[dep].get('estimated_completion', '2025-09-02')
    ...
    def generate_gantt_data(self) -> pd.DataFrame:
        ...
        for blocker_id, blocker in self.blockers.items():
            ...
            end_time = datetime.fromisoformat(blocker.get('estimated_completion', '2025-09-02'))
    ...
    def visualize_dependency_graph(self, output_file: str = "blocker_dependencies.png"):
        ...
        for node in self.dependency_graph.nodes():
            priority = self.dependency_graph.nodes[node].get('priority', '🟡 HIGH')
        ...
        plt.title("Blocker Resolution Dependencies", fontsize=16, fontweight='bold')
        plt.axis('off')
        plt.tight_layout()
    ...
    def create_gantt_chart(self, output_file: str = "blocker_gantt_chart.png"):
        ...
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            bar = ax.barh(
                ...
                hatch=pattern, edgecolor='black'
            )
            
            # Add task label
            ax.text(
                start_offset + duration/2, i,
                f"{row['Task']}\n{row['Title'][:15]}...",
                ha='center', va='center',
        ...
        ax.set_title('Blocker Resolution Gantt Chart', fontsize=16, fontweight='bold')
...
"""
        ...
        report += f"**Total Critical Path Duration:** {total_critical_days} days\n\n"

        # Circular dependencies
        if circular_deps:
            report += "## ⚠️ CIRCULAR DEPENDENCIES DETECTED\n\n"
            for i, cycle in enumerate(circular_deps):
                report += f"**Cycle {i+1}:** {' → '.join(cycle + [cycle[0]])}\n"
        ...
        for blocker_id in self.dependency_graph.nodes():
            ...
            if in_degree > 1:
                predecessors = list(self.dependency_graph.predecessors(blocker_id))
...
def main():
    ...
    print("📄 Dependency analysis report saved to: blocker_dependency_analysis.md")
    print("\n" + "="*60)
    print(report)
