...
class ContentExtractorConfig:
    ...
    supported_mime_types: Set[str] = field(default_factory=lambda: {
        ...
        "text/markdown", "text/x-python", "text/x-c",
...
class ContentExtractionResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "text_content": (self.text_content[:10000] + "..." 
...
class ContentExtractor(ABC):
    ...
    @abstractmethod
    def extract_content(self, file_path: str, config: ContentExtractorConfig) -> ContentExtractionResult:
...
class TextContentExtractor(ContentExtractor):
    ...
    def extract_content(self, file_path: str, config: ContentExtractorConfig) -> ContentExtractionResult:
        ...
        try:
            ...
            # Extract metadata
            metadata = (self._extract_text_metadata(content, file_path_obj) 
                        if config.extract_metadata else {})
    ...
    def _detect_encoding(self, file_path: Path, config: ContentExtractorConfig) -> str:
        ...
        try:
            ...
            detected_encoding = detection.get('encoding', config.fallback_encoding)
    ...
    def _read_text_content(self, file_path: Path, encoding: str, config: ContentExtractorConfig) -> str:
        ...
        try:
            ...
            try:
                ...
                return raw_content.decode(config.fallback_encoding, errors='replace')
    
    def _extract_text_metadata(self, content: str, file_path: Path) -> Dict[str, Any]:
...
class BinaryContentExtractor(ContentExtractor):
    ...
    def extract_content(self, file_path: str, config: ContentExtractorConfig) -> ContentExtractionResult:
        ...
        try:
            ...
            extraction_time = (time.time() - start_time) * 1000
            self.logger.error(f"Binary metadata extraction failed for {file_path}: {str(e)}")
...
class ContentIndexManager:
    ...
    def __init__(
        ...
        self.logger.info(f"Content index manager initialized with strategy: {indexing_strategy.value}")
    ...
    def _init_whoosh_backend(self) -> None:
        ...
        try:
            ...
            schema = fields.Schema(
                ...
                text_content=fields.TEXT(analyzer=StandardAnalyzer(), stored=True),
            ...
            self.logger.warning(f"Failed to initialize Whoosh backend: {str(e)}")
            self.whoosh_index = None
    ...
    def index_file(self, file_info: FileInfo) -> ContentExtractionResult:
        ...
        with self._index_lock:
            ...
            extractor = self._find_extractor(file_info.path, file_info.mime_type)
            
            # Extract content
            result = extractor.extract_content(file_info.path, self.extractor_config)
    ...
    def index_files_batch(self, file_infos: List[FileInfo]) -> List[ContentExtractionResult]:
        """
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_file):
                try:
                    result = future.result(timeout=self.extractor_config.timeout_seconds)
                    results.append(result)
                except Exception as e:
                    file_info = future_to_file[future]
                    self.logger.error(f"Failed to index {file_info.path}: {str(e)}")
    ...
    def search_content(
        ...
        if self.whoosh_index:
            try:
                ...
                self.logger.warning(f"Whoosh search failed, falling back to SQLite: {str(e)}")
    ...
    def _should_index_file(self, file_info: FileInfo) -> bool:
        ...
        if self.indexing_strategy == ContentIndexingStrategy.METADATA_ONLY:
            ...
            return (
                file_info.mime_type in self.extractor_config.supported_mime_types and
                file_info.size_bytes < self.extractor_config.max_file_size_mb * 1024 * 1024
            )
            ...
            if file_info.size_bytes > self.extractor_config.max_file_size_mb * 1024 * 1024:
                return False  # Skip very large files
            
            # Always index text files
            if file_info.mime_type in self.extractor_config.supported_mime_types:
                return True
            
            # Index metadata for other files
            return True
    
    def _find_extractor(self, file_path: str, mime_type: str) -> ContentExtractor:
    ...
    def _search_whoosh(
        ...
        try:
            ...
            if content_types:
                type_queries = [Term("content_type", ct) for ct in content_types]
    ...
    def _update_extraction_stats(self, result: ContentExtractionResult) -> None:
        ...
        self.extraction_stats['total_extraction_time_ms'] += result.extraction_time_ms
