...
from ..exceptions import (FileSystemException, PermissionException,
                          ValidationException)
    ...
    def from_path(
        ...
        try:
            ...
            encoding = (
                ...
                if is_file and include_metadata else None
            )
...
class FileSystemScanner:
    ...
    def scan_directory(
        ...
        try:
            ...
            self.logger.info(f"Scan completed. Statistics: {self.statistics.to_dict()}")
        ...
        try:
            for file_info in self._walk_directory(
                ...
                if self.memory_limit_mb > 0:
                    ...
                    if self.statistics.current_memory_mb > self.memory_limit_mb:
                        raise FileSystemException(
                ...
                if self.progress_callback and self._files_processed % 100 == 0:
                    self.progress_callback(self._files_processed, -1)  # Unknown total
    ...
    def _scan_multi_threaded(
        ...
        def process_directory_batch(directories: List[Tuple[Path, int]]) -> List[FileInfo]:
            ...
            for directory, depth in directories:
                ...
                try:
                    for file_info in self._scan_directory_contents(
                        ...
                        if (file_info.is_directory and 
                            target.include_subdirectories and
                            (target.max_depth is None or depth < target.max_depth) and
                            self._should_process_path(Path(file_info.path), target, security)):
                            
                            directory_queue.put((Path(file_info.path), depth + 1))
                
                except Exception as e:
                    self.logger.error(f"Error processing directory {directory}: {str(e)}")
                    self.statistics.add_error("directory_processing", str(directory), str(e))
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            while not directory_queue.empty() or active_futures:
                ...
                while (len(active_futures) < self.max_workers and 
                    ...
                    if batch:
                        future = executor.submit(process_directory_batch, batch)
                ...
                for future in as_completed(active_futures, timeout=0.1):
                    ...
                    try:
                        ...
                        for file_info in batch_results:
                            ...
                            self._files_processed += 1
                            if self.progress_callback and self._files_processed % 100 == 0:
                                self.progress_callback(self._files_processed, -1)
                        ...
                        self.statistics.add_error("future_processing", "batch", str(e))
                        if self.error_callback:
            ...
            if target.include_subdirectories:
                try:
                    for item in directory.iterdir():
                        ...
                        if item.is_dir() and self._should_process_path(item, target, security):
                            yield from self._walk_directory(
                                item, base_path, target, security, include_metadata, depth + 1
                            )
                
                except PermissionError as e:
                    self.statistics.permission_errors += 1
                    self.statistics.add_error("permission", str(directory), str(e))
                    if self.error_callback:
                        self.error_callback(e, str(directory))
                except OSError as e:
    ...
    def _scan_directory_contents(
        ...
        try:
                ...
                try:
                    ...
                    if item.is_symlink():
                        ...
                        if not target.follow_symlinks:
                            ...
                            file_info = FileInfo.from_path(item, base_path, include_metadata)
                            yield file_info
                            continue
                    ...
                    if file_info.is_file:
                        ...
                        self.statistics.total_size_bytes += file_info.size_bytes
                        self.statistics.largest_file_size = max(
                            self.statistics.largest_file_size, file_info.size_bytes
                        )
            ...
            self.statistics.permission_errors += 1
            self.statistics.add_error("directory_permission", str(directory), str(e))
            if self.error_callback:
                self.error_callback(e, str(directory))
        except OSError as e:
            self.statistics.access_errors += 1
            self.statistics.add_error("directory_access", str(directory), str(e))
            if self.error_callback:
                self.error_callback(e, str(directory))
    ...
    def _should_process_path(
        ...
        try:
            ...
            if not security.include_system_files and self._is_system_file(path):
                return False
...
class BatchFileSystemScanner:
        ...
        try:
            ...
            for target_id, target in targets:
                scanner = FileSystemScanner(
                    ...
                    progress_callback=lambda c, t, tid=target_id: self._target_progress(tid, c, t),
                    error_callback=lambda e, p, tid=target_id: self._target_error(tid, e, p)
                )
            ...
            with ThreadPoolExecutor(max_workers=len(targets)) as executor:
                ...
                for target_id, target in targets:
                    ...
                    future = executor.submit(
                        ...
                        scanner, target, security_settings, include_metadata
                    )
                    ...
                    try:
                        ...
                        for file_info in file_infos:
                            ...
                            if file_info.is_file:
                                ...
                                self.combined_statistics.total_size_bytes += file_info.size_bytes
                            elif file_info.is_directory:
                                self.combined_statistics.directories_scanned += 1
                        ...
                        if self.error_callback:
                            self.error_callback(e, target_id, "target_processing")
    ...
    def _scan_target_to_list(
        ...
        results = []
        for file_info in scanner.scan_directory(target, security_settings, include_metadata):
            if self._cancel_requested.is_set():
                break
    ...
    def get_target_statistics(self, target_id: str) -> Optional[ScanStatistics]:
