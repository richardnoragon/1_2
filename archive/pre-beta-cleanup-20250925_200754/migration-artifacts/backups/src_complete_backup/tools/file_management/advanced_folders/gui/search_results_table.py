...
from PyQt5.QtCore import (QAbstractTableModel, QItemSelectionModel,
                          QModelIndex, QRect, QSize, QSortFilterProxyModel, Qt,
                          QTimer, QVariant, pyqtSignal)
from PyQt5.QtGui import (QBrush, QClipboard, QColor, QFont, QIcon,
                         QKeySequence, QPalette, QPixmap)
from PyQt5.QtWidgets import (QAbstractItemView, QAction, QApplication,
                             QCheckBox, QComboBox, QFrame, QGroupBox,
                             QHBoxLayout, QHeaderView, QLabel, QLineEdit,
                             QMenu, QMessageBox, QProgressBar, QScrollArea,
                             QSplitter, QTableView, QTextEdit, QToolButton,
                             QVBoxLayout, QWidget)
...
class SearchResultItem:
    ...
    def _format_column_value(self, column: str) -> str:
        ...
        if column == "name":
            ...
            return metadata.file_extension.upper() if metadata.file_extension else "Unknown"
        elif column == "modified":
            return metadata.modified_date.strftime("%Y-%m-%d %H:%M") if metadata.modified_date else "Unknown"
        elif column == "created":
            return metadata.created_date.strftime("%Y-%m-%d %H:%M") if metadata.created_date else "Unknown"
        elif column == "folder":
    ...
    def _create_file_icon(self) -> QIcon:
        ...
        Returns:
            File type icon
        """
        ext = self.file_metadata.file_extension.lower() if self.file_metadata.file_extension else ""
...
class SearchResultsModel(QAbstractTableModel):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.columns = [
            ...
            {'name': 'modified', 'title': 'Modified', 'width': 120, 'sortable': True},
            {'name': 'path', 'title': 'Path', 'width': 300, 'sortable': True},
            {'name': 'folder', 'title': 'Folder', 'width': 100, 'sortable': True},
        ]
    ...
    def sort(self, column: int, order: Qt.SortOrder):
        ...
        if column_name == 'size':
            self.results.sort(
                key=lambda x: x.file_metadata.file_size or 0,
                reverse=reverse
            )
    ...
    def set_item_selection(self, items: List[SearchResultItem], selected: bool):
        ...
        if items:
            start_row = min(self.results.index(item) for item in items if item in self.results)
            end_row = max(self.results.index(item) for item in items if item in self.results)
