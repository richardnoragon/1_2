...
class MigrationManager:
    ...
    def create_migration(self, migration_name: str, migration_type: str,
    ...
    def execute_migration(self, migration_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['success']:
                ...
                self.logger.error(f"Migration {migration_id} failed: {result.get('error')}")
        ...
        for migration_file in self.migration_dir.glob("*.json"):
            try:
                if migration_file.stat().st_mtime < cutoff_date:
                    ...
                    if migration_data.get('status') in ['completed', 'failed', 'rolled_back']:
    ...
    def _create_backup(self, migration_id: str, migration_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if migration_type in ['cache', 'full_system']:
                # Backup cache data
                cache_dirs = [Path("__pycache__"), Path("build"), Path(".pytest_cache")]
                for cache_dir in cache_dirs:
                    if cache_dir.exists():
                        shutil.copytree(cache_dir, backup_path / cache_dir.name, dirs_exist_ok=True)
            ...
            manifest = {
                ...
                'backup_contents': [str(p.relative_to(backup_path)) for p in backup_path.rglob("*") if p.is_file()]
            }
    ...
    def _migrate_configuration(self, migration_metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _migrate_preferences(self, migration_metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _migrate_tool_integration(self, migration_metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _migrate_cache_data(self, migration_metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_dirs = [Path("__pycache__"), Path("build/temp"), Path(".pytest_cache")]
            for cache_dir in cache_dirs:
                if cache_dir.exists():
    ...
    def _migrate_full_system(self, migration_metadata: Dict[str, Any]) -> Dict[str, Any]:
        """Perform full system migration."""
        try:
            results = []
            
            # Migrate each component
            for migration_type in ['config', 'database', 'preferences', 'tools']:
                component_result = self.migration_types[migration_type](migration_metadata)
                results.append({
    ...
    def _perform_rollback(self, migration_type: str, backup_location: str) -> Dict[str, Any]:
            ...
            if migration_type in ['config', 'full_system']:
                ...
                if config_path.exists():
                    try:
                        with open(config_path, 'r') as f:
                            config_data = json.load(f)
                        validation_results['checks_performed'].append('config_file_valid_json')
                    except json.JSONDecodeError as e:
                        validation_results['valid'] = False
                        validation_results['errors'].append(f"Configuration JSON invalid: {e}")
                else:
                    validation_results['valid'] = False
                    validation_results['errors'].append("Configuration file missing after migration")
            ...
            if migration_type in ['database', 'full_system']:
                ...
                for db_file in db_files:
                    try:
                        ...
                        conn.close()
                        validation_results['checks_performed'].append(f'database_{db_file.name}_accessible')
                    except sqlite3.Error as e:
                        validation_results['valid'] = False
                        validation_results['errors'].append(f"Database {db_file} corrupted: {e}")
