...
class DatabaseLogService:
    ...
    def get_log_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Total logs
            result = self.db_manager.execute_query("SELECT COUNT(*) as count FROM app_logs")
            ...
            stats['by_tool'] = {row['tool_name']: row['count'] for row in result}
    ...
    def cleanup_old_logs(self, retention_days: int = 90) -> int:
        ...
        try:
            ...
            query = ("DELETE FROM app_logs WHERE timestamp < "
                     "datetime(?, ? || ' days')")
            affected = self.db_manager.execute_update(
                query,
                (cutoff_date, f'-{retention_days}')
            )
            return affected
        except Exception:
            return 0
    ...
    def export_logs(self, export_path: str, **filters) -> bool:
        ...
        try:
            logs = self.get_logs(**filters, limit=10000)  # Large limit for export
