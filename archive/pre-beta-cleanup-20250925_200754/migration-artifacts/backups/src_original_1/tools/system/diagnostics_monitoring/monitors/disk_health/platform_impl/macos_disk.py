...
class MacOSDiskMonitor(MonitorBase):
    ...
    def _parse_system_profiler_disk(self, item: Dict[str, Any]
        ...
        try:
            ...
            disk_info = {
                ...
                'size_bytes': 0,
                'interface_type': item.get('spstorage_connection_type',
                                           'Unknown'),
    ...
    def _parse_volume_info(self, volume: Dict[str, Any]
                           ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if volume_info['size_bytes'] > 0:
                ...
                volume_info['free_percent'] = (
                    (volume_info['available_bytes'] /
                     volume_info['size_bytes']) * 100
                )
    ...
    def _get_diskutil_info(self, disk_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Calculate usage
            if (disk_info['size_bytes'] > 0 and
                    disk_info['available_bytes'] > 0):
                disk_info['used_bytes'] = (
                    disk_info['size_bytes'] - disk_info['available_bytes']
                )
                disk_info['used_percent'] = (
                    (disk_info['used_bytes'] / disk_info['size_bytes']) * 100
                )
    ...
    def _enhance_with_ioreg_data(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(
                [self._available_tools['ioreg'], '-r', '-c',
                 'IOBlockStorageDriver', '-a'],
                capture_output=True,
                text=True,
                timeout=30
            )
    ...
    def _get_macos_smart_data(self, device_id: str
                              ) -> Optional[Dict[str, Any]]:
    ...
    def _get_smartctl_data_macos(self, device_id: str
                                 ) -> Optional[Dict[str, Any]]:
    ...
    def _get_diskutil_smart_data(self, device_id: str
                                 ) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = subprocess.run(
                [self._available_tools['diskutil'], 'info', '-plist',
                 device_id],
                capture_output=True,
                text=True,
                timeout=15
            ...
            if smart_status != 'Unknown':
                return {
                    'overall_health': ('passed' if smart_status == 'Verified'
                                       else 'failed'),
    ...
    def _update_volume_usage(self, device_id: str,
                ...
                if len(lines) >= 2:
                    ...
                    if len(fields) >= 4:
                        ...
                        disk_info.update({
                            ...
                            'used_percent': ((used_blocks / total_blocks) * 100
                                             if total_blocks > 0 else 0),
                            'free_percent': ((available_blocks / total_blocks)
    ...
    def _collect_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for device_id, disk_info in self._logical_disks.items():
                try:
                    ...
                    if logical_data:
                        ...
                        # Count health status
                        health_status = logical_data.get('health_status',
                                                         'unknown')
            ...
            data['summary']['total_logical_disks'] = len(
                data['logical_disks']
            )
    ...
    def _collect_physical_disk_data(self, device_id: str,
        ...
        try:
            ...
            if smart_data:
                ...
                data['health_status'] = smart_data.get('overall_health',
                                                       'unknown')
    ...
    def _collect_logical_disk_data(self, device_id: str,
                                   disk_info: Dict[str, Any]
                                   ) -> Optional[Dict[str, Any]]:
    ...
    def _determine_physical_disk_health(self,
                                        disk_data: Dict[str, Any]) -> str:
    ...
    def _determine_logical_disk_health(self,
                                       disk_data: Dict[str, Any]) -> str:
    ...
    def _validate_data(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            required_fields = ['timestamp', 'platform', 'physical_disks',
                               'logical_disks', 'summary']
            ...
            if (not isinstance(data['physical_disks'], list) or
                    not isinstance(data['logical_disks'], list)):
                return False
    ...
    def get_macos_disk_details(self, device_id: str,
                               disk_type: str = 'logical'
                               ) -> Optional[Dict[str, Any]]:
    ...
    def get_disk_performance_history(self, device_id: str,
                                     hours: int = 24
                                     ) -> List[Dict[str, Any]]:
