...
class MacOSFilesystemImpl:
    ...
    def __init__(self):
        ...
        self.supported_filesystems = {
            'apfs': {
                ...
                'features': ['snapshots', 'clones', 'encryption', 'compression'],
                'fsck_tool': 'fsck_apfs'
            },
            'hfs': {
    ...
    def _check_tool_availability(self) -> None:
        ...
        for tool_name, tool_path in self.tools.items():
            if not os.path.exists(tool_path):
                self.logger.warning(f"Tool not found: {tool_name} at {tool_path}")
    ...
    def get_filesystem_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(e, "MacOSFilesystemImpl.get_filesystem_info")
    ...
    def _process_disk_partitions(self, disk: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _get_single_volume_info(self, volume_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if volume_info['is_mounted'] and volume_info['mount_point']:
                space_info = self._get_volume_space_usage(volume_info['mount_point'])
            ...
            if filesystem in self.supported_filesystems:
                fs_info = self._get_filesystem_specific_info(device_id, filesystem)
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                detailed_info.update({
                    ...
                    'is_case_sensitive': plist_data.get('CaseSensitive', False),
                    'is_journaled': plist_data.get('Journaled', False),
                    'is_encrypted': plist_data.get('Encrypted', False),
                    'encryption_status': self._get_encryption_status(plist_data),
                    'owners_enabled': plist_data.get('OwnersEnabled', False),
                    'permissions_enabled': plist_data.get('PermissionsEnabled', False)
                })
            
            return detailed_info
            
        except Exception as e:
            self.logger.error(f"Error getting detailed volume info for {device_id}: {e}")
            return {}
                    ...
                    if len(lines) >= 2:
                        ...
                        if len(fields) >= 4:
                            ...
                            space_info.update({
                                ...
                                'free_percent': (free_kb / total_kb * 100) if total_kb > 0 else 0
                            })
    ...
    def _get_filesystem_specific_info(self, device_id: str, filesystem: str) -> Dict[str, Any]:
        """Get filesystem-specific information.
        ...
        try:
            ...
            self.logger.error(f"Error getting {filesystem} info for {device_id}: {e}")
            return {}
                        ...
                        for volume in container.get('Volumes', []):
                            if volume.get('DeviceIdentifier') == device_id:
                                apfs_info.update({
                                    ...
                                    'volume_group': volume.get('VolumeGroupUUID', ''),
                                    'case_sensitive': volume.get('CaseSensitive', False),
                                    'encrypted': volume.get('Encrypted', False),
                                    'space_sharing': container.get('CapacityInUse', 0) > 0
                                })
                                break
    ...
    def _get_apfs_snapshots(self, device_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            if self.tools['diskutil']:
                result = subprocess.run(
                    [self.tools['diskutil'], 'apfs', 'listSnapshots', device_id, '-plist'],
                    capture_output=True,
                    text=True,
                    timeout=30
                )
            ...
            self.logger.error(f"Error getting APFS snapshots for {device_id}: {e}")
            return []
            ...
            if self.tools['diskutil']:
                ...
                if result.returncode == 0:
                    ...
                    hfs_info.update({
                        ...
                        'case_sensitive': plist_data.get('CaseSensitive', False),
                        'block_size': plist_data.get('DeviceBlockSize', 0),
                        'total_blocks': plist_data.get('TotalSize', 0) // plist_data.get('DeviceBlockSize', 1),
                        'free_blocks': plist_data.get('FreeSpace', 0) // plist_data.get('DeviceBlockSize', 1)
                    })
                ...
                for container in plist_data.get('Containers', []):
                    ...
                    for volume in container.get('Volumes', []):
                        container_info['volumes'].append({
                            'device_id': volume.get('DeviceIdentifier', ''),
    ...
    def _get_system_filesystem_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            system_info['time_machine_status'] = self._get_time_machine_status()
    ...
    def _get_macos_version(self) -> str:
        ...
        try:
            if self.tools['system_profiler']:
                result = subprocess.run(
                    [self.tools['system_profiler'], 'SPSoftwareDataType', '-xml'],
                    capture_output=True,
                    text=True,
                    timeout=30
                )
                ...
                if result.returncode == 0:
                    ...
                    for item in plist_data:
                        ...
                        if items:
                            ...
                            version = software_info.get('system_version', 'Unknown')
    ...
    def run_fsck(
        ...
        try:
            ...
            elif filesystem.lower() in ['hfs', 'hfs+'] and self.tools['fsck_hfs']:
    ...
    def is_admin_required(self, operation: str) -> bool:
        ...
        """
        admin_operations = {
            'fsck_repair', 'disk_utility_repair', 'system_repair',
            'snapshot_management', 'time_machine_config'
        }
