...
class SoftwareUpdater(MaintenanceToolBase):
    ...
    def _load_configuration(self):
        ...
        if config_file.exists():
            try:
                ...
                self.auto_install_security = config.get('auto_install_security', False)
    ...
    def _load_update_history(self):
        ...
        if history_file.exists():
            try:
                ...
                self.log_info(f"Loaded {len(self.update_history)} update sessions")
    ...
    def _save_update_history(self):
        ...
        try:
            history_data = [session.to_dict() for session in self.update_history]
    ...
    def _load_schedules(self):
        ...
        schedules_file = Path("software_maintenance/config/update_schedules.json")
            ...
            try:
                ...
                for schedule_id, schedule_data in schedules_data.items():
                    schedule = UpdateSchedule(
                        ...
                        include_security_only=schedule_data['include_security_only'],
    ...
    def check_for_updates(self, software_list: List[str] = None) -> Dict[str, UpdateInfo]:
        """
        ...
        try:
            ...
            if success:
                ...
                self.log_info(f"Found {len(self.available_updates)} available updates")
    ...
    def update_software(self, software_name: str, 
        ...
        try:
            ...
            backup_id = None
            if create_backup or (create_backup is None and self.backup_before_update):
            ...
            if success:
                ...
                if backup_id:
                    self.log_info(f"Backup available for rollback: {backup_id}")
    ...
    def update_multiple_software(self, software_list: List[str], 
                                create_backups: bool = None) -> Dict[str, bool]:
        """
        ...
        try:
            ...
            self.log_info(f"Batch update completed: {session.successful_updates}/{total_updates} successful")
    ...
    def create_update_schedule(self, name: str, frequency: str, time: str,
                              include_security_only: bool = False,
                              auto_install: bool = False,
                              software_filter: List[str] = None) -> str:
        """
    ...
    def _update_scheduler(self):
        ...
        self.log_info(f"Updated scheduler with {len(self.update_schedules)} schedules")
    
    def _scheduled_update_check(self, schedule_obj: UpdateSchedule):
        """Perform a scheduled update check."""
        try:
            self.log_info(f"Running scheduled update check: {schedule_obj.name}")
            
            # Scan for software
            self.scan_installed_software()
            
            # Check for updates
            software_list = schedule_obj.software_filter if schedule_obj.software_filter else None
            ...
            if schedule_obj.include_security_only:
                updates_to_process = {
                    name: update for name, update in self.available_updates.items()
    ...
    def get_update_statistics(self) -> Dict[str, any]:
        ...
        total_updates = sum(session.successful_updates for session in self.update_history)
        total_failures = sum(session.failed_updates for session in self.update_history)
        
        # Recent activity (last 30 days)
        recent_cutoff = datetime.now() - timedelta(days=30)
        recent_sessions = [s for s in self.update_history if s.start_time >= recent_cutoff]
        recent_updates = sum(session.successful_updates for session in recent_sessions)
        ...
        return {
            ...
            'active_schedules': len([s for s in self.update_schedules.values() if s.enabled]),
            'security_updates_available': len([u for u in self.available_updates.values() 
                                             if u.is_security_update])
        }
    
    def execute(self, scan_software: bool = True, check_updates: bool = True,
                include_system: bool = False, save_reports: bool = True) -> bool:
        """
