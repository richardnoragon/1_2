...
class PerformanceLogCollector(LogCollector):
    ...
    def _trigger_performance_alert(self, tool_name: str, metric: str,
...
class ErrorLogCollector(LogCollector):
    ...
    def _correlate_errors(self, log_entry: LogEntry):
        ...
        for errors in self._error_patterns.values():
            for error in errors:
                if (error.correlation_id == log_entry.correlation_id and
    ...
    def _create_error_correlation(self, related_errors: List[LogEntry]):
        ...
        correlation_entry = LogEntry(
            ...
            metadata={
                ...
                'tools_affected': list(set(e.tool_name for e in related_errors))
            }
        )
...
class LogHandler:
    ...
    def __init__(self, name: str, formatter: LogFormatter, log_filter: LogFilter):
        self.name = name
    ...
    def should_handle(self, log_entry: LogEntry) -> bool:
        ...
        # Tool filtering
        if (self.log_filter.tools and 
            log_entry.tool_name not in self.log_filter.tools):
            return False
        
        # Operation filtering
        if (self.log_filter.operations and log_entry.operation and
            log_entry.operation not in self.log_filter.operations):
            return False
        
        # Time filtering
        if (self.log_filter.start_time and 
            log_entry.timestamp < self.log_filter.start_time):
            return False
        
        if (self.log_filter.end_time and 
            log_entry.timestamp > self.log_filter.end_time):
            return False
        
        # Keyword filtering
        if self.log_filter.keywords:
            message_lower = log_entry.message.lower()
            if not any(keyword.lower() in message_lower 
                      for keyword in self.log_filter.keywords):
                return False
        
        # Correlation ID filtering
        if (self.log_filter.correlation_ids and log_entry.correlation_id and
            log_entry.correlation_id not in self.log_filter.correlation_ids):
            return False
...
class FileLogHandler(LogHandler):
    ...
    def __init__(self, name: str, file_path: str, formatter: LogFormatter,
                 log_filter: LogFilter, rotation_policy: LogRotationPolicy,
    ...
    def _should_rotate(self) -> bool:
        ...
        if self.rotation_policy in [LogRotationPolicy.SIZE_BASED, 
...
class DatabaseLogHandler(LogHandler):
    ...
    def __init__(self, name: str, connection_string: str, 
                 formatter: LogFormatter, log_filter: LogFilter):
        super().__init__(name, formatter, log_filter)
...
class LoggingService:
    ...
    def _create_default_handlers(self):
        ...
        security_filter = LogFilter(min_level=LogLevel.SECURITY, 
                                   max_level=LogLevel.SECURITY)
    ...
    def log_structured(self, level: LogLevel, tool_name: str, message: str,
                      operation: str = None, context: Dict[str, Any] = None,
                      metadata: Dict[str, Any] = None, 
                      correlation_id: str = None, session_id: str = None,
                      user_id: str = None, performance_data: Dict[str, Any] = None,
                      error_data: Dict[str, Any] = None):
        ...
        with self._lock:
            ...
            for collector in self._collectors.values():
                try:
                    ...
                    self.logger.error(f"Error in collector {collector.name}: {e}")
    ...
    def _store_log_entry(self, log_entry: LogEntry):
        ...
        if log_entry.correlation_id:
            self._log_index[f"corr:{log_entry.correlation_id}"].append(log_entry)
    ...
    def search_logs(self, log_filter: LogFilter, 
    ...
    def _matches_filter(self, log_entry: LogEntry, 
                       log_filter: LogFilter) -> bool:
        ...
        # Tool filtering
        if (log_filter.tools and 
            log_entry.tool_name not in log_filter.tools):
            return False
        
        # Operation filtering
        if (log_filter.operations and log_entry.operation and
            log_entry.operation not in log_filter.operations):
            return False
        
        # Time filtering
        if (log_filter.start_time and 
            log_entry.timestamp < log_filter.start_time):
            return False
        
        if (log_filter.end_time and 
            log_entry.timestamp > log_filter.end_time):
            return False
        ...
        # Correlation ID filtering
        if (log_filter.correlation_ids and log_entry.correlation_id and
            log_entry.correlation_id not in log_filter.correlation_ids):
            return False
    ...
    def export_logs(self, file_path: str, log_filter: LogFilter,
                   format_type: LogFormat = LogFormat.JSON) -> bool:
        """Export logs to file.
        ...
        try:
            ...
            with open(file_path, 'w', encoding='utf-8') as f:
                if format_type == LogFormat.JSON:
                    ...
                    # Write CSV header
                    header = [
                        'timestamp', 'level', 'tool_name', 'operation',
                        'message', 'correlation_id', 'metadata'
                    ]
        ...
        with self._lock:
            ...
            for entry in self._log_entries:
                ...
                if entry.correlation_id:
                    self._log_index[f"corr:{entry.correlation_id}"].append(entry)
...
def log_performance(tool_name: str, message: str,
                   performance_data: Dict[str, Any], **kwargs):
    """Log performance message."""
    get_logging_service().log_structured(
        LogLevel.PERFORMANCE, tool_name, message,
