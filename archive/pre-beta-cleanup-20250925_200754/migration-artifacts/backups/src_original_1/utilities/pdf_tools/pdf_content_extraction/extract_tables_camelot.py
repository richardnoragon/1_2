...
def extract_tables(input_file: str, output_folder: str = None, **kwargs) -> bool:
    ...
    try:
        ...
        logger.debug("Parameters - Output folder: %s, Settings: %s", output_folder, kwargs)
        
        if not output_folder:
            output_folder = os.path.join(os.path.dirname(input_file), 'extracted_tables')
        ...
        for i, table in enumerate(tables, 1):
            try:
                output_file = os.path.join(output_folder, f"{base_name}_table_{i}.csv")
                logger.debug("Saving table %d to: %s", i, output_file)
                table.to_csv(output_file)
            except Exception as e:
                logger.error("Failed to save table %d: %s", i, str(e))
                QMessageBox.warning(None, "Warning", f"Failed to save table {i}: {str(e)}")
                continue
...
class TableExtractorWindow(QMainWindow):
    ...
    def initUI(self):
        try:
            ...
            if self.config:
                ...
                self.flavorCombo.setCurrentText(settings.get('flavor', 'lattice'))
                self.lineScaleSpinBox.setValue(settings.get('line_scale', 15))
                self.backgroundCheckBox.setChecked(settings.get('process_background', False))
                self.tableBordersCombo.setCurrentText(settings.get('table_borders', 'normal'))
                self.edgeToleranceSpinBox.setValue(settings.get('edge_tol', 50))
                self.rowToleranceSpinBox.setValue(settings.get('row_tol', 2))
                self.columnToleranceSpinBox.setValue(settings.get('column_tol', 2))
            ...
            QMessageBox.critical(self, "Error", f"Failed to initialize UI components: %s", str(e))
            self.close()
    ...
    def save_settings(self):
        try:
            ...
            QMessageBox.critical(self, "Error", f"Error saving settings: %s", str(e))
    ...
    def extract_tables(self):
        try:
            ...
            params = {
                ...
                'row_tol': self.rowToleranceSpinBox.value(),
                'column_tol': self.columnToleranceSpinBox.value(),
                'pages': self.pagesEdit.text() if self.pagesEdit.text() else None
            }
            ...
            try:
                ...
                def progress_callback(current_page, total_pages):
                    ...
                    self.statusBar().showMessage(f"Processing page {current_page} of {total_pages}...")
                    QtWidgets.QApplication.processEvents()
                ...
                if success:
                    ...
                    QMessageBox.warning(self, "Warning", "Failed to extract tables!")
                    self.progressBar.setValue(0)
                ...
                self.outputText.append(f"\nError: {str(e)}")
                QMessageBox.critical(self, "Error", f"Error extracting tables: {str(e)}")
                self.progressBar.setValue(0)
                self.statusBar().showMessage("Extraction failed", 3000)
