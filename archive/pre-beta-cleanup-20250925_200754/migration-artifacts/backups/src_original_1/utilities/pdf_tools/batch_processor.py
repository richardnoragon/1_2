...
class BatchProcessor(QObject):
    ...
    def create_batch_job(self, tool_name: str, operation_type: str,
                        input_files: List[str], output_dir: Optional[str] = None,
    ...
    def get_active_jobs(self) -> List[BatchJob]:
        ...
        with self.jobs_lock:
            return [job for job in self.jobs.values() 
                   if job.status in [BatchJobStatus.RUNNING, BatchJobStatus.QUEUED]]
    
    def get_job_statistics(self) -> Dict[str, Any]:
        """Get processing statistics."""
        with self.jobs_lock:
            total_jobs = len(self.jobs)
            running_jobs = len([j for j in self.jobs.values() 
                              if j.status == BatchJobStatus.RUNNING])
            queued_jobs = len([j for j in self.jobs.values() 
    ...
    def _process_queue(self):
        ...
        while True:
            try:
                ...
                logger.error(f"Error in batch processing queue: {e}", exc_info=True)
                time.sleep(1.0)
    ...
    def _process_job(self, job_id: str):
        ...
        try:
            ...
            operation_id = self.progress_manager.create_batch_operation(
                job.tool_name, job.operation_type, 
                [item.input_file for item in job.items]
            )
            self.progress_manager.start_operation(operation_id, "Starting batch job...")
            ...
            for i, item in enumerate(job.items):
                ...
                try:
                    ...
                    self.progress_manager.update_progress(
                        operation_id, int(progress_percent),
                        f"Processing {current_item}", item.input_file
                    )
                    
                    if job.progress_callback:
                        job.progress_callback(job_id, progress_percent, current_item)
                    
                    self.job_progress.emit(job_id, progress_percent, current_item)
                    
                    # Process the item (this would call the actual tool)
                    success = self._process_item(job, item)
                    
                    item.end_time = time.time()
                    item.status = BatchJobStatus.COMPLETED if success else BatchJobStatus.FAILED
                    
                    self.item_completed.emit(job_id, item.input_file, success)
                    
                    logger.debug(f"Processed item {i+1}/{len(job.items)}: {item.input_file}")
                    ...
                    logger.error(f"Failed to process item {item.input_file}: {e}")
            ...
            if job.status == BatchJobStatus.CANCELLED:
                ...
                job.status = BatchJobStatus.COMPLETED
                message = (f"Job completed with {job.failed_items} failures "
...
def create_batch_job(tool_name: str, operation_type: str, input_files: List[str],
                    output_dir: Optional[str] = None, 
                    parameters: Dict[str, Any] = None,
                    priority: BatchJobPriority = BatchJobPriority.NORMAL) -> str:
    """Create a batch job."""
    return get_batch_processor().create_batch_job(
        tool_name, operation_type, input_files, output_dir, parameters, priority
    )


def start_batch_job(job_id: str, progress_callback: Optional[Callable] = None,
                   completion_callback: Optional[Callable] = None):
    """Start a batch job."""
    get_batch_processor().start_job(job_id, progress_callback, completion_callback)
