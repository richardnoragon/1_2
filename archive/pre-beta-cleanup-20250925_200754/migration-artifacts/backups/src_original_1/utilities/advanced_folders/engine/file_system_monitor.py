...
class MonitoringStats:
    ...
    events_by_type: Dict[str, int] = field(default_factory=lambda: {
        "created": 0, "modified": 0, "deleted": 0, "moved": 0
    })
...
class FileSystemEventProcessor(FileSystemEventHandler):
    ...
    def _create_change_event(self, fs_event, 
                           change_type: FileChangeType) -> FileChangeEvent:
...
class BatchProcessor(QThread):
    ...
    def _collect_batch(self) -> List[FileChangeEvent]:
        ...
        while (len(batch) < self.config.max_events_per_batch and
...
class FileSystemMonitor(QObject):
    ...
    def __init__(self, config: Optional[FileSystemMonitorConfig] = None,
                 parent=None):
        ...
        if not WATCHDOG_AVAILABLE:
            self.logger.error("Watchdog library not available for file monitoring")
            raise ImportError("Watchdog library required for file system monitoring")
        ...
        self.debouncer = EventDebouncer(
            self.config.debounce_delay_seconds,
        ...
        # Connect signals
        self.batch_processor.events_processed.connect(self._handle_batch_processed)
        self.batch_processor.processing_error.connect(self._handle_processing_error)
    ...
    def start_monitoring(self, paths: List[str]) -> bool:
        ...
        if len(paths) > self.config.max_monitored_paths:
            self.logger.error(f"Too many paths to monitor: {len(paths)} > "
    ...
    def _handle_batch_processed(self, events: List[FileChangeEvent]):
        ...
        self.logger.debug(f"Processed batch of {len(events)} file system events")
