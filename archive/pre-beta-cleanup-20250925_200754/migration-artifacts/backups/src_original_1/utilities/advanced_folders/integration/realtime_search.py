...
class RealtimeSearchManager(QObject):
    ...
    searchResultsChanged = pyqtSignal(str, list, dict)  # query, results, metadata
    ...
    def set_search_parameters(self, folder_id: str, parameters: Dict[str, Any]):
    ...
    def perform_incremental_search(self, query: str):
        ...
        # Check cache first
        cache_key = self._generate_cache_key(query, self.current_folder_id, 
        ...
        if self._is_cache_valid(cache_key):
            ...
            self.searchStatusChanged.emit("cached", 
    ...
    def _execute_search(self):
        ...
        self.logger.debug(f"Started search operation: {self.active_search_operation}")
    
    def _handle_search_results_updated(self, query_id: str, results: List[Any]):
        ...
        """
        if query_id.startswith(f"{self.current_folder_id}_{self.current_query}"):
            ...
            # Emit results
            self.searchResultsChanged.emit(self.current_query, results, metadata)
            ...
            if result_count == 0:
                ...
                status_msg = f"Found {result_count:,} results in {search_time:.2f}s"
    ...
    def _handle_operation_completed(self, operation_type: str, operation_id: str, 
        ...
        """
        if operation_type == 'search' and operation_id == self.active_search_operation:
            # Search operation completed successfully
            self.logger.debug(f"Search operation {operation_id} completed successfully")
    
    def _handle_operation_failed(self, operation_type: str, operation_id: str, 
                               error: str):
        ...
        """
        if operation_type == 'search' and operation_id == self.active_search_operation:
            self.logger.error(f"Search operation {operation_id} failed: {error}")
            self.searchStatusChanged.emit("error", f"Search failed: {error}")
            self.performance_metrics['active_searches'] -= 1
            self.active_search_operation = None
    
    def _generate_cache_key(self, query: str, folder_id: str, 
    ...
    def _cache_search_results(self, cache_key: str, results: List[Any], 
    ...
    def _generate_basic_suggestions(self, query: str) -> List[str]:
        ...
        if '.' not in query:
            file_extensions = ['.pdf', '.docx', '.xlsx', '.png', '.jpg', '.txt']
    ...
    def set_search_delay(self, delay_ms: int):
        ...
        """
        self.search_delay_ms = max(100, min(1000, delay_ms))  # Clamp between 100-1000ms
