...
class FileOperationWorker(QRunnable, ABC):
    ...
    def __init__(self, task: WorkerTask, progress_callback=None):
        ...
        self.logger = logging.getLogger(f'RFU.Worker.{self.__class__.__name__}')
    ...
    def run(self) -> None:
        ...
        try:
            self.logger.info(f"Starting worker: {self.task.operation.operation_id}")
            ...
            self.logger.info(f"Worker completed: {self.task.operation.operation_id}")
    ...
    def _handle_error(self, error: FileOperationError) -> None:
        ...
        self.logger.error(f"Worker error: {self.task.operation.operation_id} - {error}")
        
        # Determine if retry is appropriate
        if (error.recoverable and 
            self.task.retry_count < self.task.max_retries):
            self.task.retry_count += 1
            self.logger.info(f"Scheduling retry {self.task.retry_count}/{self.task.max_retries}")
    ...
    def cancel(self) -> None:
        ...
        self.cancelled.set()
        self.logger.info(f"Worker cancelled: {self.task.operation.operation_id}")
    
    def is_cancelled(self) -> bool:
        """Check if worker has been cancelled."""
        return self.cancelled.is_set()
    
    def update_progress(self, current_file: str = "", additional_data: Dict = None) -> None:
        """
...
class OperationQueue(QObject):
    ...
    def __init__(self, max_workers: int = 3):
        ...
        self.logger.info(f"OperationQueue initialized with {max_workers} workers")
            ...
            while not self.task_queue.empty():
                try:
                    ...
                    if task.operation.operation_id == operation_id:
                        cancelled = True
                        self.logger.info(f"Queued task cancelled: {operation_id}")
                    else:
    ...
    def _process_queue(self) -> None:
        ...
        with self.queue_lock:
            ...
            if (len(self.active_workers) >= self.max_workers or 
            ...
            try:
                ...
                self.logger.info(f"Worker started: {task.operation.operation_id}")
