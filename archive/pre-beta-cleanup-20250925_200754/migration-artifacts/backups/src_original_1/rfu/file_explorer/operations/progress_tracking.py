...
class OperationProgress:
    ...
    def remaining_time(self) -> Optional[float]:
        ...
        if (self.progress_percentage <= 0 or 
...
class ProgressTracker(QObject):
    ...
    def start_tracking(self, operation_id: str, operation_type: str,
                      total_files: int = 0, total_bytes: int = 0,
                      **kwargs) -> OperationProgress:
        """
        ...
        with self.progress_lock:
            ...
            self.logger.info(f"Started tracking: {operation_id} "
    ...
    def _calculate_performance_metrics(self, progress: OperationProgress) -> None:
        """Calculate real-time performance metrics."""
        elapsed_time = progress.elapsed_time
        
        if elapsed_time > 0:
            # Transfer rate (bytes per second)
            progress.transfer_rate_bps = progress.bytes_processed / elapsed_time
    ...
    def _estimate_completion_time(self, progress: OperationProgress) -> None:
        ...
        if (progress.transfer_rate_bps <= 0 or 
            progress.total_bytes <= progress.bytes_processed):
    ...
    def pause_operation(self, operation_id: str) -> bool:
        ...
        """
        with self.progress_lock:
            if (operation_id not in self.active_operations or
                not self.active_operations[operation_id].can_pause):
                return False
    ...
    def resume_operation(self, operation_id: str) -> bool:
        ...
        with self.progress_lock:
            if (operation_id not in self.active_operations or
                not self.active_operations[operation_id].is_paused):
                return False
    ...
    def cancel_operation(self, operation_id: str) -> bool:
        ...
        with self.progress_lock:
            if (operation_id not in self.active_operations or
                not self.active_operations[operation_id].can_cancel):
                return False
    ...
    def complete_operation(self, operation_id: str, success: bool = True,
        ...
        with self.progress_lock:
            ...
            self.logger.info(f"Operation completed: {operation_id} "
                           f"(success={success})")
    ...
    def add_callback(self, operation_id: str, callback: ProgressCallback) -> None:
        """
    ...
    def remove_callback(self, operation_id: str, 
            ...
            if operation_id in self.callbacks:
                for callback in self.callbacks[operation_id]:
                    try:
                        ...
                        self.logger.error(f"Callback error for {operation_id}: {e}")
            
            # Emit progress signal
            self.progress_updated.emit(operation_id, {
                'files_processed': progress.files_processed,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.progress_lock:
            ...
            total_files = sum(op.files_processed for op in self.operation_history)
            total_bytes = sum(op.bytes_processed for op in self.operation_history)
            ...
            return {
                ...
                'average_transfer_rate': (total_bytes / total_time) if total_time > 0 else 0
            }