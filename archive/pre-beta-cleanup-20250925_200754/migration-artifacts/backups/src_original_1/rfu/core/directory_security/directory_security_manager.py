...
from .directory_encryption import DirectoryPathEncryption, DirectoryEncryptionResult, DirectoryDecryptionResult
from .directory_permissions import DirectoryPermissionManager, PermissionCheckResult
...
class DirectorySecurityManager:
    ...
    def store_directory_preference(self, user_id: str, tool_name: str, 
                                  directory_type: str, directory_path: str,
                                  metadata: Optional[Dict[str, Any]] = None) -> DirectoryStorageResult:
        """
        ...
        """
        try:
            self.logger.info(f"Storing directory preference for user {user_id[:8]}... tool {tool_name}")
            ...
            if not validation_result.valid:
                self.audit_logger.log_security_violation(
                    user_id, 'invalid_path', {
                        'reason': validation_result.reason,
                        'tool_name': tool_name,
                        'original_path_hash': self._create_path_hash(directory_path)
                    }
                )
            ...
            if not permission_result.authorized:
                self.audit_logger.log_access_denied(
                    user_id, self._create_path_hash(sanitized_path), 'write', 
            ...
            if (pii_result.contains_pii or 
                pii_result.sensitivity_level >= self.security_level_threshold or
                self.enable_path_encryption):
                ...
                if not encryption_result.success:
                    self.logger.error("Failed to encrypt sensitive directory path")
                    return DirectoryStorageResult(
                        success=False,
            ...
            self.logger.info(f"Directory preference stored successfully with ID {storage_id}")
    ...
    def retrieve_directory_preference(self, user_id: str, path_hash: str) -> DirectoryRetrievalResult:
        ...
        """
        try:
            self.logger.info(f"Retrieving directory preference for user {user_id[:8]}...")
    ...
    def list_user_directories(self, user_id: str, tool_name: Optional[str] = None,
                             directory_type: Optional[str] = None) -> List[DirectoryRetrievalResult]:
        """
        ...
        try:
            ...
            directories = self._get_user_directories(user_id, tool_name, directory_type)
            ...
            self.audit_logger.log_directory_operation(
                user_id, 'list_operation', 'list', True, {
                    'count': len(results),
    ...
    def delete_directory_preference(self, user_id: str, path_hash: str) -> bool:
        """
    ...
    def _store_encrypted_directory(self, user_id: str, tool_name: str, directory_type: str,
                                  encrypted_path: bytes, path_hash: str, is_pii_sensitive: bool,
                                  sensitivity_level: int, encryption_metadata: Optional[Dict],
                                  pii_indicators: List[str], metadata: Dict[str, Any]) -> Optional[str]:
        """Store encrypted directory in database"""
        try:
            storage_id = secrets.token_urlsafe(32)
            
            # Convert metadata to JSON
            encryption_json = json.dumps(encryption_metadata) if encryption_metadata else None
            ...
            self.db_manager.execute_query(query, (
                storage_id, user_id, tool_name, directory_type, encrypted_path, path_hash,
                is_pii_sensitive, sensitivity_level, encryption_json, pii_json,
    ...
    def _get_encrypted_directory(self, user_id: str, path_hash: str) -> Optional[Dict]:
        ...
        try:
            ...
            if result:
                ...
                return {
                    ...
                    'encryption_metadata': json.loads(result[6]) if result[6] else None,
    ...
    def _get_user_directories(self, user_id: str, tool_name: Optional[str] = None,
                             directory_type: Optional[str] = None) -> List[Dict]:
    ...
    def _delete_encrypted_directory(self, user_id: str, path_hash: str) -> bool:
