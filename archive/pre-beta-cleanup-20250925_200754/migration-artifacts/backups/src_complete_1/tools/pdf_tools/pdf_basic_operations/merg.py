...
def merge_pdfs(input_files: list, output_file: str) -> bool:
    ...
    try:
        ...
        logger.info("Starting merge of %d files into: %s", len(input_files), output_file)
        pdf = pikepdf.Pdf.new()
        ...
        for input_file in input_files:
            try:
                ...
                QMessageBox.critical(None, "Error", f"Error processing PDF {input_file}: {str(e)}")
                return False
            except Exception as e:
                logger.error("Error processing file %s: %s", input_file, str(e))
                QMessageBox.critical(None, "Error", f"Error processing file %s: %s", input_file, str(e))
                return False
        ...
        try:
            ...
            logger.error("Permission denied when saving output file: %s", output_file)
            QMessageBox.critical(None, "Error", f"Permission denied when saving output file: %s", output_file)
        except Exception as e:
            logger.error("Error saving merged file: %s", str(e))
            QMessageBox.critical(None, "Error", f"Error saving merged file: %s", str(e))
        return False
            
    except Exception as e:
        logger.error("Unexpected error during merge operation: %s", str(e), exc_info=True)
        QMessageBox.critical(None, "Error", f"Unexpected error during merge operation: %s", str(e))
        return False
...
class MergeUI(QMainWindow):
    ...
    def update_button_states(self):
        ...
        self.upButton.setEnabled(has_selection and self.fileList.currentRow() > 0)
        self.downButton.setEnabled(has_selection and self.fileList.currentRow() < len(self.files) - 1)
        
        logger.debug("Updated button states: has_files=%s, has_selection=%s", has_files, has_selection)
    ...
    def add_files(self):
        try:
            ...
            if files:
                ...
                self.fileList.clear()
                self.fileList.addItems([os.path.basename(f) for f in self.files])
                self.update_button_states()
                logger.debug("Files added successfully")
                
        except Exception as e:
            logger.error("Error adding files: %s", str(e))
            QMessageBox.critical(self, "Error", f"Error adding files: %s", str(e))
    ...
    def remove_file(self):
        try:
            ...
            QMessageBox.critical(self, "Error", f"Error removing file: %s", str(e))
    ...
    def clear_files(self):
        try:
            ...
            QMessageBox.critical(self, "Error", f"Error clearing files: %s", str(e))
    ...
    def move_up(self):
        try:
            ...
            if current > 0:
                ...
                self.files[current], self.files[current-1] = self.files[current-1], self.files[current]
            ...
            QMessageBox.critical(self, "Error", f"Error moving file up: %s", str(e))
    ...
    def move_down(self):
        try:
            ...
            QMessageBox.critical(self, "Error", f"Error moving file down: %s", str(e))
    
    def merge_files(self):
        try:
            if not self.files:
                logger.warning("No files to merge")
                QMessageBox.warning(self, "Warning", "Please add files to merge first!")
                return
            ...
            if output_file:
                ...
                for i, file in enumerate(self.files, 1):
                    progress = int((i / total_files) * 90)  # Leave 10% for final operations
                    self.progressBar.setValue(progress)
                    self.statusBar().showMessage(f"Processing file {i} of {total_files}...")
                    QtWidgets.QApplication.processEvents()
                ...
                if merge_pdfs(self.files, output_file):
                    ...
                    QMessageBox.warning(self, "Warning", "Failed to merge PDFs!")
                    self.statusBar().showMessage("Merge failed", 3000)
                    
                self.progressBar.hide()
                    
        except Exception as e:
            logger.error("Error during merge operation: %s", str(e), exc_info=True)
            QMessageBox.critical(self, "Error", f"Error during merge operation: %s", str(e))
