    ...
    try:
        ...
        for page_num in page_list:
            try:
                ...
                logger.debug("Converted page %d to %s", page_num + 1, output_file)
                
            except Exception as e:
                logger.error("Error converting page %d: %s", page_num + 1, str(e))
                continue

        pdf_file.close()
        logger.info("Conversion completed. Created %d images", len(output_files))
        return output_files
...
class ConvertToImageUI(QtWidgets.QMainWindow):
    def __init__(self):
        try:
            ...
            logger.error("Failed to initialize PDF to Image converter: %s", str(e))
            raise
    ...
    def convert_file(self):
        try:
            ...
            if pages_text:
                try:
                    pages = tuple(int(p.strip())-1 for p in pages_text.split(','))
            ...
            QMessageBox.critical(self, "Error", f"Error in convert operation: {str(e)}")
