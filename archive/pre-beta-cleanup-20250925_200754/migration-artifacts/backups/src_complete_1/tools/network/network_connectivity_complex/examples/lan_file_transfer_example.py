...
from network_connectivity.tools.lan_file_transfer import (
    LANFileTransfer, TransferStatus, DeviceStatus
)
...
def main():
    ...
    max_concurrent = lan_transfer.get_tool_config('max_concurrent_transfers', 3)
    encryption_enabled = lan_transfer.get_tool_config('encryption_enabled', True)
...
def demonstrate_callbacks():
    ...
    def on_progress_update(progress):
        print(f"📊 Transfer progress: {progress.percentage_complete:.1f}% "
