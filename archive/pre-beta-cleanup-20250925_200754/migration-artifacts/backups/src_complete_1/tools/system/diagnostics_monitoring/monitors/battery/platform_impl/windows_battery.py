...
class WindowsBatteryMonitor(MonitorBase):
    ...
    def _initialize_platform_specific(self) -> None:
        ...
        try:
            ...
            if self.win32_enabled:
                ...
                self.logger.warning("Win32 API not available - limited functionality")
            ...
            self.logger.error(f"Failed to initialize Windows battery monitoring: {e}")
    ...
    def _initialize_wmi(self) -> None:
        ...
        try:
            ...
            self.logger.info("WMI connection established for battery monitoring")
            ...
            self.logger.error(f"Failed to initialize WMI for battery monitoring: {e}")
                ...
                for line in lines:
                    if 'GUID:' in line:
                        ...
                        if len(parts) > 1:
                            ...
                            name = guid_part.split('(')[1].rstrip(')') if '(' in guid_part else 'Unknown'
    ...
    def _discover_windows_batteries(self) -> None:
        ...
        try:
            ...
            error_handler.handle_error(e, "WindowsBatteryMonitor.discover_batteries")
    ...
    def _discover_batteries_wmi(self) -> None:
        ...
        try:
            ...
            for battery in self.wmi_connection.Win32_Battery():
                try:
                    battery_id = battery.DeviceID or f"battery_{len(self._batteries)}"
            ...
            for battery in self.wmi_connection.Win32_PortableBattery():
                try:
                    battery_id = battery.DeviceID or f"portable_{len(self._batteries)}"
                    ...
                    if battery_id not in self._batteries:
                        battery_info = {
                            ...
                            'chemistry': self._get_chemistry_name(battery.Chemistry),
                            'is_portable': True
                        }
                    ...
                    self.logger.warning(f"Error processing portable battery: {e}")
                    continue
    ...
    def _get_thermal_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.wmi_enabled and self.wmi_connection:
                ...
                try:
                    for thermal_zone in self.wmi_connection.Win32_PerfRawData_Counters_ThermalZoneInformation():
                        if thermal_zone.Temperature:
                            # Convert from tenths of Kelvin to Celsius
                            temp_celsius = (thermal_zone.Temperature / 10.0) - 273.15
    ...
    def _collect_battery_data(
        ...
        try:
            ...
            if data.get('design_capacity') and data.get('full_charge_capacity'):
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error getting Windows battery health status: {e}")
            return {
                'status': 'error',
