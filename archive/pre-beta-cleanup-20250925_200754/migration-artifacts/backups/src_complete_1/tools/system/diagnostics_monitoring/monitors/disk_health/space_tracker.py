...
class SpaceTracker:
    ...
    def get_io_stats(self, device: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for cached_device in self._io_stats_cache:
                if (device in cached_device or
                        cached_device in device or
                        self._devices_match(device, cached_device)):
    ...
    def _update_io_stats(self) -> None:
        ...
        try:
            ...
            for device, counters in disk_io.items():
                ...
                # Calculate rates
                if previous_data:
                    time_delta = (current_time - datetime.fromisoformat(
                    ...
                    if time_delta > 0:
                        io_data.update({
                            'read_rate_bytes_per_sec': (
                                (counters.read_bytes -
                                 previous_data['read_bytes']) / time_delta
                            ),
                            'write_rate_bytes_per_sec': (
                                (counters.write_bytes -
                                 previous_data['write_bytes']) / time_delta
                            ),
                            'read_rate_ops_per_sec': (
                                (counters.read_count -
                                 previous_data['read_count']) / time_delta
                            ),
                            'write_rate_ops_per_sec': (
                                (counters.write_count -
                                 previous_data['write_count']) / time_delta
                            )
                        })
                else:
    ...
    def _store_space_history(
        ...
        try:
            ...
            if history_len > self._max_history_points:
                self._space_history[mountpoint] = (
                    self._space_history[mountpoint][-self._max_history_points:]
                )
    ...
    def predict_full_time(
        ...
        try:
            ...
            if (trend['trend'] != 'increasing' or
                    trend['rate_percent_per_hour'] <= 0):
                return None
