...
class ChargeCycleTracker:
        ...
        try:
            ...
            if is_charging and not last_charging:
                ...
                self._handle_charging_start(
                    battery_id, charge_percent, timestamp, 
        ...
        try:
            ...
            if battery_id in self._current_sessions:
                ...
                if session.start_percent is not None:
                    ...
                    session_duration = (timestamp - session.start_time).total_seconds() / 60.0
                    
                    if session_duration > 0:
                        session.charging_rate = charge_gained / session_duration
            ...
            if charge_percent >= 95:
                ...
                self._battery_states[battery_id]['phase'] = ChargingPhase.DISCHARGING
        ...
        try:
            if battery_id in self._current_sessions:
                ...
                if voltage is not None and current is not None:
                    power_w = abs(voltage * current / 1000.0)  # Convert mA to A
    ...
    def _check_cycle_completion(
        ...
        try:
            ...
            if (state['phase'] == ChargingPhase.CHARGING and 
                charge_percent >= self.cycle_completion_threshold):
                ...
                if min_percent is not None:
                    ...
                    if depth_of_discharge >= self.min_discharge_depth:
                        ...
                        if len(self._cycles[battery_id]) > 1000:
                            self._cycles[battery_id] = self._cycles[battery_id][-1000:]
        ...
        try:
            ...
            if battery_id in self._charging_sessions:
                ...
                for session in self._charging_sessions[battery_id]:
                    if (session.start_time >= cycle.start_time and 
                        session.end_time and session.end_time <= cycle.end_time):
                ...
                if relevant_sessions:
                    ...
                    if cycle.charging_time_minutes and cycle.charging_time_minutes > 0:
                        ...
                        if charge_gained > 0:
                            cycle.cycle_efficiency = min(
                                100.0, 
                                (charge_gained / cycle.charging_time_minutes) * 60.0
                            )
    ...
    def get_cycle_data(self, battery_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if cycles:
                avg_depth = sum(cycle.depth_of_discharge for cycle in cycles) / total_cycles
                ...
                if charging_times:
                    avg_charging_time = sum(charging_times) / len(charging_times)
                
                # Recent cycle analysis (last 10 cycles)
                recent_cycles = cycles[-10:] if len(cycles) >= 10 else cycles
                recent_avg_depth = (
                    sum(cycle.depth_of_discharge for cycle in recent_cycles) / 
                    len(recent_cycles)
                )
                
                # Calculate cycle degradation trend
                degradation_trend = self._calculate_cycle_degradation_trend(cycles)
            else:
                avg_depth = 0.0
                avg_charging_time = None
            ...
            return {
                ...
                'last_cycle_date': (
                    ...
                    if cycles and cycles[-1].end_time else None
                ),
                'cycle_efficiency_avg': self._calculate_average_efficiency(cycles)
            }
            
        except Exception as e:
            self.logger.error(f"Error getting cycle data for {battery_id}: {e}")
            return {}
    
    def _calculate_cycle_degradation_trend(
        self, 
        cycles: List[ChargeCycle]
    ) -> float:
        ...
        try:
            ...
            analysis = {
                ...
                'average_charging_rate': (
                    ...
                    if charging_rates else None
                ),
                'average_peak_temperature': (
                    sum(peak_temperatures) / len(peak_temperatures) 
                    if peak_temperatures else None
                ),
                'max_peak_temperature': max(peak_temperatures) if peak_temperatures else None
            }
    ...
    def _calculate_average_efficiency(self, cycles: List[ChargeCycle]) -> Optional[float]:
    ...
    def get_cycle_history(
        ...
        try:
            ...
            self.logger.error(f"Error getting cycle history for {battery_id}: {e}")
            return []
    ...
    def get_charging_pattern_analysis(self, battery_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for session in recent_sessions:
                if session.end_time:
                    duration = (session.end_time - session.start_time).total_seconds() / 3600.0
                    charging_times.append(duration)
            
            # Charging level analysis
            start_levels = [session.start_percent for session in recent_sessions]
            end_levels = [
                session.end_percent for session in recent_sessions 
                if session.end_percent is not None
            ]
            
            # Temperature analysis
            peak_temps = [
                session.peak_temperature for session in recent_sessions 
                if session.peak_temperature is not None
            ]
            
            # Generate recommendations
            recommendations = self._generate_charging_recommendations(
                recent_sessions, charging_times, start_levels, end_levels, peak_temps
            )
            ...
            analysis = {
                ...
                'average_charging_time_hours': (
                    ...
                    if charging_times else None
                ),
                'average_start_level': (
                    sum(start_levels) / len(start_levels) 
                    if start_levels else None
                ),
                'average_end_level': (
                    sum(end_levels) / len(end_levels) 
                    if end_levels else None
                ),
                'average_peak_temperature': (
                    sum(peak_temps) / len(peak_temps) 
                    if peak_temps else None
                ),
                'session_type_distribution': self._analyze_charging_sessions(recent_sessions).get(
            ...
            self.logger.error(f"Error analyzing charging patterns for {battery_id}: {e}")
            return {}
    ...
    def _generate_charging_recommendations(
        ...
        try:
            ...
            if not recommendations:
                recommendations.append("Charging patterns look optimal - continue current habits")
            
        except Exception as e:
            self.logger.error(f"Error generating charging recommendations: {e}")
            ...
            if os.path.exists(self._cycle_data_file):
                with open(self._cycle_data_file, 'r') as f:
                        ...
                        for cycle_data in cycle_list:
                            cycle = ChargeCycle(
                                ...
                                end_time=(
                                    ...
                                    if cycle_data.get('end_time') else None
                                ),
                                start_percent=cycle_data['start_percent'],
                                end_percent=cycle_data['end_percent'],
                                depth_of_discharge=cycle_data['depth_of_discharge'],
                                charging_time_minutes=cycle_data.get('charging_time_minutes'),
                                temperature_avg=cycle_data.get('temperature_avg'),
                                voltage_avg=cycle_data.get('voltage_avg'),
                                cycle_efficiency=cycle_data.get('cycle_efficiency'),
                    ...
                    for battery_id, session_list in sessions_data.items():
                        ...
                        for session_data in session_list:
                            session = ChargingSession(
                                ...
                                peak_temperature=session_data.get('peak_temperature'),
                                charging_rate=session_data.get('charging_rate'),
                                power_consumption=session_data.get('power_consumption'),
                                session_type=session_data.get('session_type', 'partial')
                            )
                            self._charging_sessions[battery_id].append(session)
                    ...
                    for battery_id, state in self._battery_states.items():
                        ...
                        if 'cycle_start_time' in state and isinstance(state['cycle_start_time'], str):
                            state['cycle_start_time'] = datetime.fromisoformat(state['cycle_start_time'])
                        if 'phase' in state and isinstance(state['phase'], str):
    ...
    def _save_cycle_data(self) -> None:
        ...
        try:
            ...
            for battery_id, state in self._battery_states.items():
                ...
                if 'last_timestamp' in state_copy and isinstance(state_copy['last_timestamp'], datetime):
                    state_copy['last_timestamp'] = state_copy['last_timestamp'].isoformat()
                if 'cycle_start_time' in state_copy and isinstance(state_copy['cycle_start_time'], datetime):
                    state_copy['cycle_start_time'] = state_copy['cycle_start_time'].isoformat()
                if 'phase' in state_copy and isinstance(state_copy['phase'], ChargingPhase):
