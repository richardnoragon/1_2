...
class LinuxDiskMonitor(MonitorBase):
    ...
    def _initialize_platform_specific(self) -> None:
        ...
        try:
            ...
            if not available_tools and not (self.proc_available or
                                            self.sys_available):
                self.logger.warning(
    ...
    def _discover_disks_lsblk(self) -> None:
        ...
        try:
            result = subprocess.run(
                [self._available_tools['lsblk'], '-J', '-o',
            ...
            try:
                ...
                for device in block_devices:
                    try:
                        ...
                        self.logger.warning(
                            f"Error parsing lsblk device: {e}"
                        )
                        continue
            ...
            for line in lines:
                ...
                if len(fields) >= 14:
                    ...
                    if device_name in self._physical_disks:
                        self._physical_disks[device_name]['io_stats'] = io_stats
                ...
                if len(fields) >= 6:
                    ...
                    if device.startswith('/dev/'):
                        ...
                        if device_name in self._logical_disks:
                            ...
                            self._physical_disks[device_name].update(mount_info)
            ...
            for device_name in os.listdir(block_path):
                try:
                    ...
                    if os.path.isdir(device_path):
                        ...
                        # Add to existing device info
                        if device_name in self._physical_disks:
                            self._physical_disks[device_name].update(sysfs_info)
                        elif device_name in self._logical_disks:
    ...
    def _get_sysfs_device_info(self, device_name: str,
            ...
            for attr_path, attr_name in sysfs_attributes.items():
                try:
                    ...
                    if os.path.exists(full_path):
                        ...
                        if attr_name in ['size_sectors', 'alignment_offset',
                                         'discard_alignment', 'queue_depth',
                                         'read_ahead_kb', 'max_sectors_kb',
                                         'physical_block_size',
                                         'logical_block_size',
                                         'minimum_io_size',
                                         'optimal_io_size']:
                        ...
                        elif attr_name in ['removable', 'read_only',
                                           'rotational']:
    ...
    def _get_linux_smart_data(self, device_id: str
    ...
    def _get_smartctl_data_linux(self, device_id: str
                                 ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for line in lines:
                ...
                if 'Model Number:' in line:
                    ...
                    hdparm_data['serial_number'] = line.split(':', 1)[1].strip()
                elif 'Firmware Revision:' in line:
                    hdparm_data['firmware_revision'] = (
    ...
    def _update_filesystem_usage(self, device_id: str,
                        ...
                        if len(fields) >= 4:
                            ...
                            disk_info.update({
                                ...
                                'used_percent': ((used_bytes / total_bytes)
                                                 * 100 if total_bytes > 0
                                                 else 0),
                                'free_percent': ((available_bytes / total_bytes)
                                                 * 100 if total_bytes > 0
                                                 else 0)
                            })
    ...
    def get_linux_disk_details(self, device_id: str,
