...
class BatteryHealthWidget(BaseWindow):
    ...
    def setup_control_panel(self, parent: ttk.Frame) -> None:
        ...
        try:
            ...
            ttk.Label(settings_frame, text="Update Interval (seconds):").pack(anchor=tk.W)
        ...
        while self.monitoring_active:
            try:
                ...
                if current_data:
                    ...
                    # Limit history size
                    if len(self.battery_data_history) > self.max_history_points:
                        self.battery_data_history = (
                            self.battery_data_history[-self.max_history_points:]
                        )
    ...
    def create_battery_widget(
        ...
        try:
            ...
            self.add_battery_info_row(
                left_column, "Status:", 
                battery.get('health_status', 'Unknown')
            )
            
            self.add_battery_info_row(
                left_column, "Charging:", 
            ...
            if secsleft and secsleft > 0:
                ...
                time_text = "Unknown"
            self.add_battery_info_row(right_column, "Time Remaining:", time_text)
            ...
            # Health progress bar
            ttk.Label(progress_frame, text="Battery Health:").pack(anchor=tk.W, pady=(10, 0))
    ...
    def add_battery_info_row(
        ...
        try:
            ...
            ttk.Label(row_frame, text=value, font=('TkDefaultFont', 9, 'bold')).pack(side=tk.LEFT)
        ...
        try:
            ...
            for battery_id, levels in charge_levels.items():
                if len(levels) == len(timestamps):
                    self.charge_ax.plot(timestamps, levels, label=battery_id, marker='o', markersize=3)
    ...
    def update_health_chart(self) -> None:
        ...
        try:
            ...
            for battery_id, levels in health_levels.items():
                if len(levels) == len(timestamps):
                    self.health_ax.plot(timestamps, levels, label=battery_id, marker='s', markersize=3)
    ...
    def update_power_chart(self) -> None:
        ...
        try:
            ...
            if timestamps and power_data:
                self.power_ax.plot(timestamps, power_data, 'r-', label='CPU Power', marker='d', markersize=3)
        ...
        try:
            ...
            if hasattr(self.battery_monitor, '_monitored_batteries'):
                for battery_id in self.battery_monitor._monitored_batteries:
                    cycle_history = self.battery_monitor.cycle_tracker.get_cycle_history(battery_id, days=30)
                    
                    if cycle_history:
                        cycle_numbers = list(range(1, len(cycle_history) + 1))
                        depths = [cycle.get('depth_of_discharge', 0) for cycle in cycle_history]
                        
                        self.cycle_ax.plot(cycle_numbers, depths, label=battery_id, marker='o', markersize=4)
