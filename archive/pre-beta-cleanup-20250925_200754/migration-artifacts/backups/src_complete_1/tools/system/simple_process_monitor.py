...
try:
    from PyQt5.QtWidgets import (
        ...
        QApplication, QMessageBox, QHeaderView, QLineEdit
    )
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 
                ...
                try:
                    ...
                    proc_info['create_time'] = datetime.fromtimestamp(
                        proc_info['create_time']).strftime('%H:%M:%S')
...
class SimpleProcessMonitorGUI(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.process_table.setHorizontalHeaderLabels([
            'PID', 'Process Name', 'CPU %', 'Memory %', 'Status', 'Start Time'
        ])
    ...
    def _filter_processes(self):
        ...
        if filter_text:
            filtered_processes = [
                proc for proc in self.all_processes 
        ...
        if filter_text:
            self.status_bar.showMessage(f"Showing {process_count} of {total_count} processes (filtered)")
        else:
    ...
    def _set_sort(self, sort_by):
        ...
        buttons = [self.sort_cpu_btn, self.sort_memory_btn, 
                  self.sort_name_btn, self.sort_pid_btn]
        for btn in buttons:
