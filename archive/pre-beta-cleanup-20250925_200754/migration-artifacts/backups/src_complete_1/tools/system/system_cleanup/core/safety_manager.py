...
class SafetyManager:
    ...
    def backup_registry_key(self, key_path: str, 
                           hive: int = winreg.HKEY_LOCAL_MACHINE) -> Optional[Path]:
    ...
    def restore_file(self, backup_path: Path, 
                    original_path: Optional[Path] = None) -> bool:
    ...
    def validate_file_safety(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._is_critical_extension(file_path):
                safety_info["warnings"].append("File has a potentially critical extension")
    ...
    def _is_system_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            critical_files = {
                ...
                'user32.dll', 'gdi32.dll', 'advapi32.dll', 'ole32.dll',
                'shell32.dll', 'shlwapi.dll', 'comctl32.dll', 'comdlg32.dll',
                'wininet.dll', 'urlmon.dll', 'msvcrt.dll', 'msvcp*.dll',
                'vcruntime*.dll', 'api-ms-*.dll'
            }
    ...
    def _is_critical_extension(self, file_path: Path) -> bool:
        ...
        critical_extensions = {
            '.sys', '.dll', '.exe', '.ocx', '.cpl', '.scr',
            '.drv', '.vxd', '.com', '.bat', '.cmd', '.reg'
        }
        
        return file_path.suffix.lower() in critical_extensions
    
    def validate_registry_key_safety(self, key_path: str, 
                                   hive: int = winreg.HKEY_LOCAL_MACHINE) -> Dict[str, Any]:
    ...
    def restore_all_backups(self) -> bool:
        ...
        for backup_info in self.file_backups:
            if not self.restore_file(backup_info["backup_path"], 
