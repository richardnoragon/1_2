...
class WindowsBrowserDetector(BrowserDetector):
    ...
    def detect_browsers(self) -> List[BrowserInfo]:
        ...
        for browser_name, reg_paths in self.registry_paths.items():
            browser_info = self._detect_browser_from_registry(browser_name, reg_paths)
            if browser_info:
    ...
    def _detect_browser_from_registry(self, browser_name: str, 
                                    registry_paths: List[str]) -> Optional[BrowserInfo]:
        ...
        try:
            ...
            self.logger.warning("winreg module not available, skipping registry detection")
            return None
    ...
    def _remove_duplicate_browsers(self, browsers: List[BrowserInfo]) -> List[BrowserInfo]:
    ...
    def _get_browser_version(self, browser: BrowserInfo) -> Optional[str]:
        ...
        try:
            if not browser.executable_path or not Path(browser.executable_path).exists():
                return None
    ...
    def get_browser_profiles(self, browser_name: str) -> List[BrowserProfile]:
        ...
        try:
            ...
            self.logger.error(f"Error getting profiles for {browser_name}: {e}")
        
        return profiles
    
    def _parse_firefox_profiles(self, profiles_ini: Path) -> List[BrowserProfile]:
...
class LinuxBrowserDetector(BrowserDetector):
    ...
    def _detect_with_which(self) -> List[BrowserInfo]:
        ...
        browser_commands = [
            ...
            'firefox', 'firefox-esr', 'opera', 'opera-stable', 'brave-browser'
        ]
    ...
    def _parse_desktop_file(self, desktop_file: Path) -> Optional[BrowserInfo]:
        ...
        try:
            ...
            browser_keywords = ['chrome', 'firefox', 'chromium', 'opera', 'brave']
                ...
                for keyword in browser_keywords:
                    if keyword in name or keyword in executable.lower():
                        ...
                        if Path(executable).exists():
                            browser_name = self._command_to_browser_name(executable)
                            return BrowserInfo(
                                name=browser_name,
                                executable_path=executable
                            )
    ...
    def _get_browser_version(self, browser: BrowserInfo) -> Optional[str]:
        ...
        try:
            ...
            # Try version flag
            result = subprocess.run([browser.executable_path, '--version'],
                                  capture_output=True, text=True, timeout=10)
...
class MacOSBrowserDetector(BrowserDetector):
    ...
    def detect_browsers(self) -> List[BrowserInfo]:
        ...
        try:
            ...
            if self._request_user_consent():
                ...
                self.logger.info("User consent granted for enhanced browser detection")
            else:
                self.logger.warning("User consent not granted, using sandbox mode")
    ...
    def _detect_with_mdfind(self) -> List[BrowserInfo]:
        ...
        for query in browser_queries:
            try:
                result = subprocess.run(['mdfind', query],
                                      capture_output=True, text=True, timeout=10)
                ...
                try:
                    ...
                    for item in root.iter():
                        if item.tag == 'string' and item.text:
                            ...
                            if any(browser in text for browser in ['chrome', 'firefox', 'safari', 'opera', 'brave', 'edge']):
                                # Extract browser information
                                self._extract_browser_info_from_xml(item, browsers)
                except ET.ParseError:
                    self.logger.debug("Failed to parse system_profiler XML output")
            ...
            if result.returncode == 0:
                try:
                    ...
                    for app in applications:
                        ...
                        browser_name = self._identify_browser_from_name(app_name)
                    ...
                    self.logger.debug("Failed to parse system_profiler JSON output")
    ...
    def _extract_browser_info_from_xml(self, item, browsers: List[BrowserInfo]) -> None:
                ...
                if result.returncode == 0:
                    ...
                    for line in result.stdout.split('\n'):
                        if 'CFBundleExecutable' in line:
                            ...
                            return str(Path(app_path) / 'Contents' / 'MacOS' / executable_name)
...
class CrossPlatformBrowserDetector:
    ...
    def __init__(self, verbose: bool = False):
        ...
        self.logger.info(f"CrossPlatformBrowserDetector initialized for {self.platform}")
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler(f'browser_detection_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
            ]
        )
    ...
    def _create_platform_detector(self) -> BrowserDetector:
        ...
        if self.platform == "windows":
            ...
            self.logger.warning(f"Unknown platform {self.platform}, using Linux detector")
            return LinuxBrowserDetector()
    ...
    def detect_all_browsers(self) -> List[BrowserInfo]:
        ...
        try:
            ...
            for browser in browsers:
                self.logger.info(f"  - {browser.name} v{browser.version or 'unknown'}")
    ...
    def generate_detection_report(self) -> str:
        ...
        for browser in browsers:
            status = " [DEFAULT]" if (default_browser and browser.name == default_browser.name) else ""
            report_lines.append(f"  • {browser.name} v{browser.version or 'unknown'}{status}")
            if browser.executable_path:
            ...
            if profiles:
                ...
                for profile in profiles[:3]:  # Show first 3 profiles
                    ...
                    report_lines.append(f"      - {profile.profile_name}{default_indicator}")
                if len(profiles) > 3:
                    report_lines.append(f"      ... and {len(profiles) - 3} more")
        ...
        report_lines.extend([
            "",
            f"Total Browsers Found: {len(browsers)}",
            "",
            "=" * 60
        ])
    ...
    def export_detection_results(self, export_format: str = "json") -> str:
        ...
        data = {
            ...
            "default_browser": asdict(default_browser) if default_browser else None,
            "browsers": []
        }
        ...
        for browser in browsers:
            ...
            browser_data['profiles'] = [
                asdict(profile) for profile in self.get_browser_profiles(browser.name)
            ]
