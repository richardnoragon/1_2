...
class SecurityStatusWidget(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        # Main frame
        main_frame = ttk.LabelFrame(self, text="Security Status",
                                    padding="10")
        ...
        ttk.Label(self.status_frame,
                  text="Encryption:").grid(row=0, column=0, sticky=tk.W)
        self.encryption_label = ttk.Label(self.status_frame,
                                          text="Unknown",
                                          foreground="gray")
        ...
        # Backup status
        ttk.Label(self.status_frame,
                  text="Backup:").grid(row=1, column=0, sticky=tk.W)
        self.backup_label = ttk.Label(self.status_frame,
                                      text="Unknown",
                                      foreground="gray")
        ...
        ttk.Label(self.status_frame,
                  text="Access Control:").grid(row=2, column=0, sticky=tk.W)
        self.access_label = ttk.Label(self.status_frame,
                                      text="Unknown",
                                      foreground="gray")
        ...
        # Security profile
        ttk.Label(self.status_frame,
                  text="Security Profile:").grid(row=3, column=0, sticky=tk.W)
        self.profile_label = ttk.Label(self.status_frame,
                                       text="Unknown",
                                       foreground="gray")
        self.profile_label.grid(row=3, column=1, sticky=tk.W, padx=(10, 0))
        
        # Refresh button
        ttk.Button(main_frame, text="Refresh Status",
                   command=self._update_status).pack(pady=(10, 0))
    ...
    def _update_status(self):
        ...
        try:
            ...
            if status.get('encryption_enabled', False):
                self.encryption_label.config(text="Enabled", 
                                           foreground="green")
            else:
                self.encryption_label.config(text="Disabled", 
                                           foreground="red")
            
            # Update backup status
            backup_count = status.get('backup_count', 0)
            if backup_count > 0:
                self.backup_label.config(text=f"Active ({backup_count})", 
                                       foreground="green")
            else:
                self.backup_label.config(text="No Backups", 
                                       foreground="orange")
            
            # Update access control status
            if status.get('access_control_enabled', False):
                self.access_label.config(text="Enabled", 
                                       foreground="green")
            else:
                self.access_label.config(text="Disabled", 
                                       foreground="red")
            ...
            self.profile_label.config(text=profile.title(), 
                                    foreground=color)
...
class EncryptionControlsWidget(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        # Main frame
        main_frame = ttk.LabelFrame(self, text="Encryption Controls", 
                                  padding="10")
        main_frame.pack(fill=tk.BOTH, expand=True)
        ...
        ttk.Button(key_frame, text="Rotate Encryption Key", 
                  command=self._rotate_key).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(key_frame, text="Backup Key", 
                  command=self._backup_key).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(key_frame, text="Test Encryption", 
                  command=self._test_encryption).pack(side=tk.LEFT)
    ...
    def _backup_key(self):
        ...
        try:
            ...
            if file_path:
                success = self.security_manager.backup_encryption_key(file_path)
                if success:
                    messagebox.showinfo("Success", 
                                      f"Key backed up to {file_path}")
                else:
                    messagebox.showerror("Error", 
                                       "Failed to backup key")
    ...
    def _test_encryption(self):
        ...
        try:
            ...
            test_data = {"test": "theme_data", "timestamp": str(datetime.datetime.now())}
            ...
            if encrypted:
                ...
                messagebox.showerror("Error", 
                                   "Encryption test failed: encryption failed")
...
class BackupManagementWidget(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        # Main frame
        main_frame = ttk.LabelFrame(self, text="Backup Management", 
                                  padding="10")
        main_frame.pack(fill=tk.BOTH, expand=True)
        ...
        ttk.Button(button_frame, text="Create Backup", 
                  command=self._create_backup).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="Restore Backup", 
                  command=self._restore_backup).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="Delete Backup", 
                  command=self._delete_backup).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="Refresh", 
                  command=self._refresh_backups).pack(side=tk.RIGHT)
        ...
        self.backup_tree = ttk.Treeview(list_frame, columns=columns, 
                                      show='headings')
        ...
        try:
            ...
            if backup_info:
                ...
                if result:
                    ...
                    success = self.backup_manager.restore_backup(backup_info[0])
                    if success:
                        messagebox.showinfo("Success", 
...
class ThemeSecurityGUI(tk.Toplevel):
    ...
    def _create_widgets(self):
        ...
        self.status_widget = SecurityStatusWidget(status_frame, 
        ...
        self.encryption_widget = EncryptionControlsWidget(encryption_frame,
                                                         self.security_manager)
        ...
        self.backup_widget = BackupManagementWidget(backup_frame,
                                                   self.backup_manager)
    ...
    def _create_config_tab(self, parent):
        ...
        self._create_config_section(scrollable_frame, "Encryption", 
                                  'encryption')
        self._create_config_section(scrollable_frame, "Access Control", 
                                  'access_control')
        self._create_config_section(scrollable_frame, "Backup", 
                                  'backup')
        self._create_config_section(scrollable_frame, "Recovery", 
                                  'recovery')
    ...
    def _create_config_section(self, parent, title, section_key):
        ...
        for key, value in section_config.items():
            ...
            if isinstance(value, bool):
                ...
                ttk.Entry(row_frame, textvariable=var, width=20).pack(side=tk.RIGHT)
            else:
                var = tk.StringVar(value=str(value))
                ttk.Entry(row_frame, textvariable=var, width=30).pack(side=tk.RIGHT)
...
def show_theme_security_gui(parent=None):
    ...
    try:
        ...
        if parent:
            messagebox.showerror("Error", 
                               f"Failed to open security GUI: {e}")
        else:
