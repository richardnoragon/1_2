...
from ..repositories import (FileMetadataRepository,
                            FolderConfigurationRepository, RepositoryManager)
...
class BackendIntegrationManager(QObject):
    ...
    folderConfigurationChanged = pyqtSignal(str, dict)  # folder_id, config_data
    searchResultsUpdated = pyqtSignal(str, list)  # query_id, results
    operationStarted = pyqtSignal(str, str)  # operation_type, operation_id
    operationCompleted = pyqtSignal(str, str, dict)  # operation_type, operation_id, result
    operationFailed = pyqtSignal(str, str, str)  # operation_type, operation_id, error
    ...
    def __init__(self, parent: Optional[QObject] = None):
        ...
        self.logger.info("Backend Integration Manager initialized successfully")
    
    def _initialize_repositories(self):
        """Initialize repository connections and validate schemas."""
        try:
            # Initialize folder configuration repository
            self.folder_repository = self.repository_manager.get_folder_repository()
            
            # Initialize file metadata repository
            self.metadata_repository = self.repository_manager.get_metadata_repository()
            
            # Validate repository connections
            if not self.folder_repository.test_connection():
                raise RuntimeError("Failed to connect to folder configuration repository")
    ...
    def create_folder_configuration(self, config_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            validation_result = self.config_manager.validate_configuration(config_data)
            ...
            self.folderConfigurationChanged.emit(saved_config.id, result['configuration'])
    ...
    def update_folder_configuration(self, folder_id: str, config_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if not existing_config:
                raise ValueError(f"Folder configuration not found: {folder_id}")
    ...
    def perform_search(self, folder_id: str, query: str, parameters: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.searchResultsUpdated.emit(f"{folder_id}_{query}", search_results)
    ...
    def cancel_search(self, operation_id: str) -> bool:
        ...
        if operation_id in self.active_operations:
            operation = self.active_operations[operation_id]
            if operation['type'] == 'search' and operation['status'] == 'active':
                # Cancel the search operation
                self.metadata_repository.cancel_search(operation_id)
                self._fail_operation(operation_id, "Operation cancelled by user")
                return True
    ...
    def _update_performance_metrics(self):
        ...
        try:
            ...
            self.performance_metrics['active_connections'] = self.db_manager.get_connection_count()
            
            # Update cache hit ratio
            self.performance_metrics['cache_hit_ratio'] = self.metadata_repository.get_cache_hit_ratio()
            ...
            self.performanceMetricsUpdated.emit(self.performance_metrics.copy())
