...
class FileTouchGUI(QMainWindow):
    ...
    def select_file(self):
        ...
        if file_path:
            ...
            self.status_label.setText(
                "File selected - Use 'Refresh' to load current timestamps"
            )
    ...
    def refresh_timestamps(self):
        ...
        if not self.current_file or not os.path.exists(self.current_file):
            QMessageBox.warning(
                self, "Warning", "Please select a valid file first."
            )
            return
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Error reading file timestamps: {str(e)}"
            )
    ...
    def apply_timestamps(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Error applying timestamps: {str(e)}"
            )
