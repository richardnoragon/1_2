...
class EmptyFoldersGUI(QMainWindow):
    ...
    def select_directory(self):
        ...
        if directory:
            ...
            self.status_label.setText(
                "Directory selected - Click 'Scan' to find empty folders"
            )
    ...
    def scan_folders(self):
        ...
        try:
            ...
            if self.empty_folders:
                for folder in self.empty_folders:
                    ...
                    try:
                        ...
                        if rel_path == ".":
                            display_path = (
                                os.path.basename(self.selected_directory)
                                + " (root)"
                            )
                        else:
                ...
                self.status_label.setText(
                    f"Found {len(self.empty_folders)} empty folders"
                )
            else:
            ...
            QMessageBox.critical(
                self, "Error", f"Error scanning directory: {str(e)}"
            )
    ...
    def delete_selected(self):
        ...
        if not selected_items:
            QMessageBox.warning(
                self, "Warning", "Please select folders to delete."
            )
            return
        ...
        if failed_count == 0:
            self.status_label.setText(
                f"Successfully deleted {deleted_count} empty folders"
            )
        else:
            self.status_label.setText(
                f"Deleted {deleted_count} folders, {failed_count} failed"
            )
