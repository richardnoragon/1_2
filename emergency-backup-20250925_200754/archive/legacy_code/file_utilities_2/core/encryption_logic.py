...
class EncryptionLogic(QObject):
    ...
    def save_key(self, key: bytes, file_path: str) -> bool:
        ...
        try:
            ...
            if self.hub_instance:
                self.hub_error_report.emit(
                    error_msg, {"operation": "key_save", "file_path": file_path})
            raise
    ...
    def load_key(self, file_path: str) -> bytes:
        ...
        try:
            ...
            if self.hub_instance:
                self.hub_error_report.emit(
                    error_msg, {"operation": "key_load", "file_path": file_path})
            raise
    ...
    def encrypt_file(self, file_path: str, key: bytes,
        ...
        try:
            ...
            # Emit milestone
            self.milestone_reached.emit("encryption_started", {
                "file_path": file_path,
                "file_size": file_size
            })
                ...
                with open(output_path, 'wb') as outfile:
                    while True:
                        ...
                        self._update_progress(
                            processed_bytes, file_size, f"Encrypting {filename}")
            
            # Complete operation
            self._complete_operation("encrypt_file", {
                "file_path": file_path,
                "output_path": output_path,
                "bytes_processed": processed_bytes
            })
            
            self.milestone_reached.emit("encryption_completed", {
                "file_path": file_path,
                "output_path": output_path,
                "bytes_processed": processed_bytes
            })
            ...
            self.error_occurred.emit(error_msg)
            if self.hub_instance:
                self.hub_error_report.emit(error_msg, {
    ...
    def decrypt_file(self, file_path: str, key: bytes, output_path: str = None) -> bool:
        ...
        try:
            ...
            # Emit milestone
            self.milestone_reached.emit("decryption_started", {
                "file_path": file_path,
                "file_size": file_size
            })
            ...
            with open(file_path, 'rb') as infile:
                with open(output_path, 'wb') as outfile:
                    ...
                    if self._should_stop:
                        self.operation_cancelled.emit("User requested cancellation")
                        return False
                    
                    # Update progress
                    self._update_progress(file_size // 2, file_size, f"Decrypting {os.path.basename(file_path)}")
                    ...
                    # Update progress
                    self._update_progress(file_size, file_size, f"Decryption complete")
            
            # Complete operation
            self._complete_operation("decrypt_file", {
                "file_path": file_path,
                "output_path": output_path,
                "bytes_processed": file_size
            })
            
            self.milestone_reached.emit("decryption_completed", {
                "file_path": file_path,
                "output_path": output_path,
                "bytes_processed": file_size
            })
    ...
    def encrypt_directory(self, directory_path: str, key: bytes, recursive: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i, file_path in enumerate(files_to_process):
                ...
                try:
                    ...
                    self._update_progress(i, len(files_to_process), 
                    ...
                    if self.encrypt_file(file_path, key):
                        ...
                        results['failed'] += 1
                        results['errors'].append(f"Failed to encrypt {file_path}")
    ...
    def decrypt_directory(self, directory_path: str, key: bytes, recursive: bool = True) -> Dict[str, Any]:
        """
            ...
            for i, file_path in enumerate(files_to_process):
                ...
                try:
                    ...
                    if self.decrypt_file(file_path, key):
                        ...
                        results['failed'] += 1
                        results['errors'].append(f"Failed to decrypt {file_path}")
    ...
    def stop(self):
        ...
        if self._current_operation:
            self.operation_cancelled.emit(f"Stopping {self._current_operation}")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_speed': self._total_bytes_processed / uptime if uptime > 0 else 0,
    ...
    def _start_operation(self, operation_type: str, parameters: Dict[str, Any]):
        ...
        if self.hub_instance:
            self.hub_status_change.emit("operation_started", {
                "operation_type": operation_type,
                "parameters": parameters
            })
    
    def _complete_operation(self, operation_type: str, results: Dict[str, Any]):
        ...
        duration = time.time() - self._operation_start_time if self._operation_start_time else 0
        
        completion_msg = f"{operation_type} completed in {duration:.2f} seconds"
        self.operation_complete.emit(completion_msg)
        
        if self.hub_instance:
            self.hub_status_change.emit("operation_completed", {
                "operation_type": operation_type,
                "results": results,
                "duration": duration
            })
