...
class SizeAnalyzerGUI(StandardWindow):
    ...
    def __init__(self, hub_instance=None):
        ...
        super().__init__(title="Size Analyzer",
    ...
    def _get_icon_path(self) -> str:
        ...
        possible_paths = [
            ...
            os.path.join(os.path.dirname(__file__), '..', '..',
                        'file_utilities_1', 'icons', 'size_analyzer.png'),
            os.path.join(os.path.dirname(__file__), 'icons',
                        'size_analyzer.png')
        ]
    ...
    def _setup_hub_integration(self):
        ...
        self.hub_connector.hub_broadcast_received.connect(
            self.hub_event_received.emit
        )
    ...
    def report_status_to_hub(self, status: str, details: Dict[str, Any] = None):
        ...
        try:
            ...
            self.show_error_dialog("Hub Communication Error",
    ...
    def request_hub_resources(self, resource_type: str,
        ...
        try:
            ...
            self.show_error_dialog("Hub Resource Error",
    ...
    def broadcast_hub_event(self, event_type: str, event_data: Dict[str, Any]):
        ...
        try:
            ...
            self.show_error_dialog("Hub Broadcast Error",
    ...
    def _load_ui_file(self, ui_file: str):
        ...
        try:
            ...
            self.show_error_dialog("UI Loading Error",
    ...
    def _apply_responsive_styling(self):
        ...
        if hasattr(self, 'output_list_view'):
            self.output_list_view.setStyleSheet(f"""
                QListView {{
                    background-color: {Colors.WINDOW_BACKGROUND};
                    border: 1px solid {Colors.TEXT_DISABLED};
                    border-radius: 4px;
    ...
    def _create_ui_programmatically(self):
        ...
        self.analyze_button = self.create_button("Analyze Directory Size", 
                                                 self._start_analysis)
        self.export_button = self.create_button("Export Results", 
                                                self._export_results, 
                                                primary=False)
        self.cancel_button = self.create_button("Cancel", 
    ...
    def _start_analysis(self):
        ...
        if not resource_granted:
            self.show_warning_dialog("Resource Unavailable",
                                     "System resources are currently busy. "
                                     "Please try again later.")
            return
        
        # Report analysis start to hub
        self.report_status_to_hub("analyzing", {
            "directory": self.selected_directory,
            "start_time": self.hub_connector.tool_state['last_activity'].isoformat()
        })
        self.tool_started.emit("Size Analyzer")
        ...
        self.broadcast_hub_event("analysis_started", {
            "tool": "Size Analyzer",
            "directory": self.selected_directory
        })
    ...
    def _set_analysis_state(self, analyzing: bool):
        """Set UI state during analysis with enhanced progress visualization."""
        ...
        if hasattr(self, 'export_button'):
            self.export_button.setEnabled(not analyzing and
        ...
        if hasattr(self, 'progress_group'):
            self.progress_group.setVisible(analyzing or
                                          self.current_analysis is not None)
    ...
    def _update_progress(self, percentage: int):
        ...
        self.tool_progress.emit("Size Analyzer", percentage,
    ...
    def _milestone_reached(self, milestone: str, percentage: int):
        ...
        self.broadcast_hub_event("milestone_reached", {
            "tool": "Size Analyzer",
            "milestone": milestone,
    ...
    def _analysis_complete(self, analysis: Dict[str, Any]):
        ...
        self.broadcast_hub_event("analysis_completed", {
            "tool": "Size Analyzer",
    ...
    def _handle_error(self, error_message: str):
        ...
        self.broadcast_hub_event("analysis_error", {
            "tool": "Size Analyzer",
            "error": error_message
        })
    ...
    def _operation_cancelled(self):
        ...
        self.broadcast_hub_event("analysis_cancelled", {
            "tool": "Size Analyzer",
    ...
    def _display_results(self, analysis: Dict[str, Any]):
        ...
        if file_types:
            ...
            type_header.setFont(QFont(Fonts.DEFAULT_FAMILY,
                                      Fonts.BODY_SIZE, QFont.Bold))
    ...
    def _display_detailed_results(self, analysis: Dict[str, Any]):
        ...
        if file_types:
            ...
            for ext, stats in sorted(file_types.items(),
                                     ...
                                     reverse=True):
                count = stats.get('count', 0)
                total_size = self.analyzer.format_size(
                    stats.get('total_size', 0))
                avg_size = self.analyzer.format_size(
                    stats.get('average_size', 0))
    ...
    def closeEvent(self, event):
        ...
        self.report_status_to_hub("shutting_down", {
            "shutdown_time": self.hub_connector.tool_state['last_activity'].isoformat()
        })
        
        # Broadcast shutdown event
        self.broadcast_hub_event("tool_shutdown", {
            "tool": "Size Analyzer",
            "reason": "user_closed"
        })
...
def main():
    ...
    try:
        ...
        QMessageBox.critical(None, "Fatal Error",
                             f"Failed to start Size Analyzer:\n{str(e)}")
        sys.exit(1)
