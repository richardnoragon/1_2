...
class DirectoryPermissionManager:
    ...
    def check_directory_permission(
        self, user_id: str, resource_path: str, action: str
                    ...
                    if self._is_permission_valid(permission):
                        if (
                            ...
                            return PermissionCheckResult(
                                ...
                                restrictions=self._get_permission_restrictions(
                                    permission
                                ),
                            )
    ...
    def revoke_directory_permission(
        self, user_id: str, resource_path: str, revoked_by: str
    ) -> bool:
        """
    ...
    def set_user_role(
        self, user_id: str, role: DirectoryRole, assigned_by: str
    ) -> bool:
        """
    ...
    def _get_user_permissions(
        self, user_id: str, resource_path: str
        ...
        try:
            ...
            for result in results:
                permission = DirectoryPermission(
                    ...
                    expires_at=(
                        datetime.fromisoformat(result[6])
                        if result[6]
                        else None
                    ),
    ...
    def _check_role_permission(
        self, role: DirectoryRole, required_permission: PermissionLevel
    ) -> PermissionCheckResult:
    ...
    def _remove_permission(self, user_id: str, resource_path: str) -> bool:
        ...
        try:
            ...
            self.db_manager.execute_query(
                query, (datetime.utcnow().isoformat(), user_id, resource_path)
            )
            return True
    ...
    def _get_permission_restrictions(
        self, permission: DirectoryPermission
    ) -> List[str]:
