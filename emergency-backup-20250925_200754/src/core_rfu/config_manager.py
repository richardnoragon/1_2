...
class ConfigManager:
    ...
    def _initialize(self, config_file=None):
        ...
        if config_file:
            ...
            self.config_path = (
                Path(__file__).parent.parent / "configuration.json"
            )
    ...
    def set_setting(self, section: str, key: str, value: Any) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error setting {section}.{key}: {str(e)}", exc_info=True
            )
    ...
    def add_recent_directory(self, directory: str) -> None:
        ...
        max_entries = self.config.get("general", {}).get(
    ...
    def get_recent_directories(self) -> list:
        ...
        recent_directories = self.config.get("general", {}).get(
            "recent_directories", []
        )
        self.logger.debug(
    ...
    def reset_section(self, section: str) -> None:
        ...
        defaults = {
            ...
            "secure_delete": {"default_passes": 3, "max_passes": 35},
            "compression": {
                "default_format": "zip",
    ...
    def save_profile(
        ...
        try:
            ...
            self.logger.error(f"Error saving profile: {str(e)}", exc_info=True)
    ...
    def load_profile(
        ...
        try:
            ...
            self.logger.error(
                f"Error loading profile: {str(e)}", exc_info=True
            )
            return None
    ...
    def get_profiles(self, module_name: str) -> list:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting profiles: {str(e)}", exc_info=True
            )
            return []
    ...
    def delete_profile(self, profile_name: str, module_name: str) -> bool:
        ...
        try:
            profiles = self.config.get("profiles", {})
            if (
                module_name in profiles
                and profile_name in profiles[module_name]
            ):
            ...
            self.logger.error(
                f"Error deleting profile: {str(e)}", exc_info=True
            )
            return False
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        if not self.config:
            return {"theme": "light", "language": "en", "show_hidden": False}
        ...
        if "general" in self.config:
            ...
            # Add defaults
            flat_config.update(
                {"theme": "light", "language": "en", "show_hidden": False}
            )
