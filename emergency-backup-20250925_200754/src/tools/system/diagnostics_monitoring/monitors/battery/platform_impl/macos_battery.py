...
class MacOSBatteryMonitor(MonitorBase):
    ...
    def _initialize_platform_specific(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to initialize macOS battery monitoring: {e}"
            )
    ...
    def _check_tool_availability(self) -> None:
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["pmset", "-g"], capture_output=True, timeout=5
                )
            ...
            try:
                result = subprocess.run(
                    ["ioreg", "-l"], capture_output=True, timeout=5
                )
    ...
    def _discover_macos_batteries(self) -> None:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "MacOSBatteryMonitor.discover_batteries"
            )
    ...
    def _discover_batteries_system_profiler(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error discovering batteries via system_profiler: {e}"
            )
            ...
            for item in power_data:
                if "sppower_battery_health_info" in item:
                    ...
                    if isinstance(battery_info, dict):
                        battery_data.update(
                            {
                                "cycle_count": battery_info.get(
                                    "sppower_battery_cycle_count"
                                ),
                                "condition": battery_info.get(
                                    "sppower_battery_health"
                                ),
                                "max_capacity": battery_info.get(
                                    "sppower_battery_max_capacity"
                                ),
                                "charge_remaining": battery_info.get(
                                    "sppower_battery_charge_remaining"
                                ),
                            }
                        )
                    ...
                    self.logger.debug(
                        f"Added battery via system_profiler: {battery_id}"
                    )
            ...
            for line in lines:
                ...
                if "AppleSmartBattery" in line:
                    if current_battery and battery_data:
                        ...
                        self._store_ioreg_battery(
                            current_battery, battery_data
                        )
    ...
    def _store_ioreg_battery(
        self, battery_id: str, battery_data: Dict[str, Any]
    ) -> None:
            ...
            for line in lines:
                ...
                if "InternalBattery" in line or "Battery" in line:
                    ...
                    if len(parts) >= 2:
                        ...
                        battery_id = (
                            f"pmset_{battery_name.lower().replace('-', '_')}"
                        )
                        ...
                        if ";" in status_part:
                            ...
                            if status_parts[0].strip().endswith("%"):
                                charge_str = (
                                    status_parts[0].strip().rstrip("%")
                                )
                                try:
                                    battery_data["charge_percent"] = float(
                                        charge_str
                                    )
                                except ValueError:
                                    pass
                            ...
                            if len(status_parts) > 1:
                                ...
                                battery_data["is_charging"] = (
                                    "charging" in charge_status.lower()
                                )

                            # Parse time remaining
                            if len(status_parts) > 2:
                                time_remaining = status_parts[2].strip()
                                battery_data["time_remaining_str"] = (
                                    time_remaining
                                )
                                ...
                                if (
                                    ":" in time_remaining
                                    and "remaining" in time_remaining
                                ):
                                    time_part = time_remaining.split(
                                        "remaining"
                                    )[0].strip()
                                    if ":" in time_part:
                                        try:
                        ...
                        self.logger.debug(
                            f"Updated battery via pmset: {battery_id}"
                        )
            ...
            if self.pmset_available:
                ...
                if result.returncode == 0:
                    ...
                    if battery_id in self._batteries:
                        ...
                        status.update(
                            {
                                "current_charge_percent": battery_info.get(
                                    "charge_percent"
                                ),
                                "is_charging": battery_info.get(
                                    "is_charging", False
                                ),
                                "charging_status": battery_info.get(
                                    "charging_status"
                                ),
                                "time_remaining_minutes": battery_info.get(
                                    "time_remaining_minutes"
                                ),
                            }
                        )
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                if battery_id in self._batteries:
                    ...
                    return {
                        "current_capacity": battery_data.get(
                            "current_capacity"
                        ),
                        ...
                        "external_connected": battery_data.get(
                            "external_connected"
                        ),
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting macOS battery health status: {e}"
            )
            return {
                "status": "error",
