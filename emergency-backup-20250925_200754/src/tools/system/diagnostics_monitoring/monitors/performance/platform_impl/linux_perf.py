...
class LinuxPerformanceImpl:
    ...
    def _get_frequency_sysfs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(cpufreq_path):
                ...
                current_freq_path = os.path.join(
                    cpufreq_path, "scaling_cur_freq"
                )
                if os.path.exists(current_freq_path):
                    with open(current_freq_path, "r") as f:
                        freq_khz = int(f.read().strip())
                        freq_info["scaling_cur_freq_mhz"] = round(
                            freq_khz / 1000, 2
                        )
                ...
                if os.path.exists(min_freq_path):
                    with open(min_freq_path, "r") as f:
                        freq_khz = int(f.read().strip())
                        freq_info["scaling_min_freq_mhz"] = round(
                            freq_khz / 1000, 2
                        )
                ...
                if os.path.exists(max_freq_path):
                    with open(max_freq_path, "r") as f:
                        freq_khz = int(f.read().strip())
                        freq_info["scaling_max_freq_mhz"] = round(
                            freq_khz / 1000, 2
                        )
    ...
    def get_cpu_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "LinuxPerformanceImpl.get_cpu_metrics"
            )
                            ...
                            metrics.update(
                                {
                                    ...
                                    "cpu_system_proc": round(
                                        (system / total) * 100, 2
                                    ),
                                    "cpu_idle_proc": round(
                                        (idle / total) * 100, 2
                                    ),
                                    "cpu_iowait_proc": round(
                                        (iowait / total) * 100, 2
                                    ),
                                    "cpu_irq_proc": round(
                                        (irq / total) * 100, 2
                                    ),
                                    "cpu_softirq_proc": round(
                                        (softirq / total) * 100, 2
                                    ),
                                    "cpu_usage_proc": round(
                                        ((total - idle) / total) * 100, 2
                                    ),
                                }
                            )
                    break
    ...
    def _get_temperature_sensors(self) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                ["sensors", "-A"], capture_output=True, text=True, timeout=10
            )
            ...
            if result.returncode == 0:
                ...
                if temperatures:
                    temp_info.update(
                        {
                            "cpu_temperature_sensors": round(
                                sum(temperatures) / len(temperatures), 1
                            ),
                            "cpu_temperature_cores": [
                                round(temp, 1) for temp in temperatures
                            ],
                            "cpu_temperature_max_sensors": round(
                                max(temperatures), 1
                            ),
                            "cpu_temperature_min_sensors": round(
                                min(temperatures), 1
                            ),
                        }
                    )
    ...
    def get_memory_details(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "LinuxPerformanceImpl.get_memory_details"
            )
            ...
            for line in lines:
                if ":" in line:
                    ...
                    if match:
                        ...
                        memory_info[f"meminfo_{key}_mb"] = round(
                            value_kb / 1024, 2
                        )
                        memory_info[f"meminfo_{key}_gb"] = round(
                            value_kb / (1024**2), 2
                        )
                            ...
                            memory_info.update(
                                {
                                    ...
                                    "free_available_gb": round(
                                        int(parts[6]) / (1024**3), 2
                                    ),
                                }
                            )
                            ...
                            memory_info.update(
                                {
                                    ...
                                    "free_swap_total_gb": round(
                                        int(parts[1]) / (1024**3), 2
                                    ),
                                    "free_swap_used_gb": round(
                                        int(parts[2]) / (1024**3), 2
                                    ),
                                    "free_swap_free_gb": round(
                                        int(parts[3]) / (1024**3), 2
                                    ),
                                }
                            )
                    ...
                    if len(parts) >= 6:
                        ...
                        for key in [
                            ...
                            if key in memory_info:
                                ...
                                memory_info[mb_key] = round(
                                    memory_info[key] / 1024, 2
                                )
    ...
    def get_process_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "LinuxPerformanceImpl.get_process_metrics"
            )
    ...
    def get_additional_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "LinuxPerformanceImpl.get_additional_metrics"
            )
                    ...
                    if line.startswith("MemTotal:"):
                        ...
                        if match:
                            ...
                            if "memory_total" in fallback_info:
                                ...
                                fallback_info["memory_percent"] = round(
                                    (used / total) * 100, 2
                                )
                            break
