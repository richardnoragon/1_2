...
class IntegrityMonitor(MonitorBase):
        ...
        return {
            ...
            "details": {
                ...
                "last_scan": (
                    ...
                    if current_data.get("recent_scans")
                    else None
                ),
            },
        }
    ...
    def _run_scan(self, scan_id: str, config: Dict[str, Any]) -> None:
        ...
        try:
            with self._scan_lock:
                if (
                    self._current_scan
                    and self._current_scan["scan_id"] == scan_id
                ):
            ...
            corruption_analysis = (
                self.corruption_detector.analyze_scan_results(scan_result)
            )
            ...
            with self._scan_lock:
                ...
                if len(self._scan_history) > self._max_scan_history:
                    self._scan_history = self._scan_history[
                        -self._max_scan_history :
                    ]
                ...
                recommendations = repair_recommendations.get(
                    "recommendations", []
                )
            ...
            self.data_collector.add_data_point(
                DataType.FILESYSTEM, self.monitor_name, final_result
            )
        ...
        try:
            ...
            for scheduled_scan in self._scheduled_scans:
                if scheduled_scan["schedule_time"] <= now:
                    ...
                    if (
                        scheduled_scan["recurring"]
                        and scheduled_scan["interval"]
                    ):
                        scheduled_scan["schedule_time"] = (
                            now + scheduled_scan["interval"]
                        )
                    else:
            ...
            error_handler.handle_error(
                e, "IntegrityMonitor._run_scheduled_scan"
            )
    ...
    def _get_current_integrity_status(self) -> str:
        ...
        critical_count = sum(
            1 for c in corruptions if c.get("severity") == "critical"
        )
        ...
        with self._scan_lock:
            if self._current_scan:
                return {
                    ...
                    "files_scanned": self._current_scan.get(
                        "files_scanned", 0
                    ),
    ...
    def _check_scan_alerts(self, scan_result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            critical_corruptions = [
                c for c in corruptions if c.get("severity") == "critical"
            ]
            if (
                len(critical_corruptions)
                >= self.alert_thresholds["critical_errors"]
            ):
                self._notify_alert_callbacks(
                    "critical_corruption",
                    AlertLevel.CRITICAL,
            ...
            if (
                duration_minutes
                > self.alert_thresholds["scan_duration_minutes"]
            ):
                self._notify_alert_callbacks(
                    "long_scan_duration",
                    AlertLevel.INFO,
            ...
            error_handler.handle_error(
                e, "IntegrityMonitor._check_scan_alerts"
            )
    ...
    def update_alert_thresholds(
        self, threshold_updates: Dict[str, Any]
    ) -> None:
        ...
        return [
            {
                ...
                "interval": (
                    str(scan["interval"]) if scan["interval"] else None
                ),
                "run_count": scan["run_count"],
                "last_run": (
                    scan["last_run"].isoformat() if scan["last_run"] else None
                ),
            }
