...
class SortingEngine:
    ...
    @staticmethod
    def sort_entries(
        entries: List[FileEntry], criteria: SortCriteria, reverse: bool = False
    ) -> List[FileEntry]:
    ...
    def sort_alphabetical(
        entries: List[FileEntry], reverse: bool = False
    ) -> List[FileEntry]:
        """Sort by name with A-Z color ranges."""
        sorted_entries = sorted(
            entries, key=lambda e: e.name.lower(), reverse=reverse
        )
    ...
    def sort_by_type(
        ...
        sorted_entries = sorted(
            entries,
            key=lambda e: (type_order.get(e.file_type, 99), e.name.lower()),
            reverse=reverse,
        )
    ...
    def sort_by_date(
        ...
        sorted_entries = sorted(entries, key=date_key, reverse=reverse)
    ...
    def sort_multi_criteria(
        entries: List[FileEntry], criteria_list: List[tuple]
    ) -> List[FileEntry]:
    ...
    def group_by_category(
        entries: List[FileEntry], criteria: SortCriteria
    ) -> dict:
    ...
    def get_category_stats(
        ...
        for category, group_entries in groups.items():
            ...
            stats[category] = {
                ...
                "avg_size": (
                    total_size / len(group_entries) if group_entries else 0
                ),
                "largest_file": (
                    max(group_entries, key=lambda e: e.size)
                    if group_entries
                    else None
                ),
                "smallest_file": (
                    min(group_entries, key=lambda e: e.size)
                    if group_entries
                    else None
                ),
            }
    ...
    @staticmethod
    def filter_entries(
        entries: List[FileEntry], filter_func: Callable[[FileEntry], bool]
    ) -> List[FileEntry]:
    ...
    @staticmethod
    def search_entries(
        entries: List[FileEntry], search_term: str, search_in_path: bool = True
    ) -> List[FileEntry]:
    ...
    @staticmethod
    def get_sort_key_for_criteria(
        entry: FileEntry, criteria: SortCriteria
    ) -> Any:
