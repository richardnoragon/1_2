...
class JSONExporter(BaseExporter):
    ...
    def _create_catalog_json(self, catalog_data) -> Dict:
        ...
        catalog_json = {
            ...
            "statistics": self._create_statistics_json(
                catalog_data.statistics
            ),
            "color_legend": self._create_legend_json(
                catalog_data.color_legend
            ),
            "files": [
                self._create_file_json(entry) for entry in catalog_data.entries
            ],
        }

        if self.include_schema:
            catalog_json["$schema"] = (
                "https://schemas.example.com/file-catalog/v1.0.json"
            )
    ...
    def _create_statistics_json(self, statistics) -> Dict:
        ...
        return {
            ...
            "file_type_counts": {
                ft.value: count
                for ft, count in statistics.file_type_counts.items()
            },
            "size_distribution": {
                sc.value: count
                for sc, count in statistics.size_distribution.items()
            },
            "date_distribution": {
                dc.value: count
                for dc, count in statistics.date_distribution.items()
            },
            "largest_file": (
                {
    ...
    def _create_file_json(self, entry) -> Dict:
        ...
        file_json = {
            ...
            "color_coding": (
                self._create_color_json(entry.color_category)
