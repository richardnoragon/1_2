...
class CatalogData:
    ...
    def get_filtered_entries(
        ...
        if size_range:
            ...
            filtered = [e for e in filtered if min_size <= e.size <= max_size]
    ...
    def _update_statistics_for_entry(self, entry: FileEntry) -> None:
        ...
        if (
            ...
            or entry.modified_date > stats.newest_file.modified_date
        ):
            stats.newest_file = entry

        if (
            not stats.oldest_file
            or entry.modified_date < stats.oldest_file.modified_date
        ):
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            "source_directory": (
                str(self.source_directory) if self.source_directory else None
            ),
            "total_files": self.statistics.total_files,
            "total_size": self.statistics.total_size,
            "total_size_formatted": self._format_size(
                self.statistics.total_size
            ),
            "file_types": {
                ft.value: count
                for ft, count in self.statistics.file_type_counts.items()
            },
            "size_distribution": {
                sc.value: count
                for sc, count in self.statistics.size_distribution.items()
            },
            "date_distribution": {
                dc.value: count
                for dc, count in self.statistics.date_distribution.items()
            },
