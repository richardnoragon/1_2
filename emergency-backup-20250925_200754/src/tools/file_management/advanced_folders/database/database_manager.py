...
class AdvancedFoldersDBManager:
    ...
    def _initialize_advanced_folders_schema(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "Advanced Folders schema initialized successfully"
            )

        except Exception as e:
            self.logger.error(
                f"Failed to initialize Advanced Folders schema: {e}"
            )
            raise
        ...
        try:
            with self.get_connection() as conn:
                ...
                for param in config.search_parameters:
                    self._insert_search_parameter(
                        conn, config.folder_id, param
                    )

                conn.commit()

            self.logger.info(
                f"Created folder configuration: {config.folder_name}"
            )
            return True
    ...
    def _insert_search_parameter(
        self, conn: sqlite3.Connection, folder_id: str, param: SearchParameter
    ) -> None:
    ...
    def update_folder_configuration(self, config: FolderConfiguration) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Updated folder configuration: {config.folder_name}"
            )
            return True
        ...
        try:
            with self.get_connection() as conn:
                ...
                if cursor.rowcount > 0:
                    self.logger.info(
                        f"Deleted folder configuration: {folder_id}"
                    )
                    return True
    ...
    def list_folder_configurations(
    ...
    def insert_file_metadata(
        self, folder_id: str, metadata: FileMetadata
    ) -> bool:
        """
    ...
    def search_files(
        self, folder_id: str, search_criteria: Dict[str, Any] = None
    ) -> List[FileMetadata]:
        """
            ...
            with self.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    metadata_data = {
                        ...
                        "metadata_extracted_at": datetime.fromisoformat(
                            row[15]
                        ),
    ...
    def get_performance_metrics(
        self, folder_id: str = None, operation_type: str = None
    ) -> List[Dict[str, Any]]:
        """
