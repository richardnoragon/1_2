...
class RenameWindow(StandardWindow):
    ...
    def _setup_menu_callbacks(self):
        ...
        if hasattr(self, "menu_manager"):
            ...
            self.menu_manager.register_callback(
                "new_rename", self.clear_selected_files
            )
            self.menu_manager.register_callback(
    ...
    def export_rename_results(self):
        ...
        if not self.selected_files:
            QMessageBox.information(
                self, "No Files", "No files selected for renaming."
            )
            return
        ...
        if file_path:
            try:
                with open(file_path, "w", encoding="utf-8") as f:
                    ...
                    f.write(
                        f"Rename Mode: {self._get_current_rename_mode()}\n"
                    )
        ...
        try:
            for filename in sorted(os.listdir(self.current_directory)):
                ...
                if os.path.isfile(file_path):
                    self.available_files_list.addItem(
    ...
    def preview_changes(self):
        ...
        if not self.selected_files:
            QMessageBox.warning(
                self, "Warning", "No files selected for renaming."
            )
            return
            ...
            try:
                ...
                if old_path != new_path:
                    ...
                    if os.path.exists(new_path):
                        errors.append(
                            f"{filename}: Target file already exists"
                        )
                        error_count += 1
                        continue
