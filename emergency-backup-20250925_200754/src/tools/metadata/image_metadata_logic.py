...
try:
    ...
    print(
        "Warning: piexif module not found. "
...
def parse_exif_value(
    value_str: str, original_type: type, tag_code: int
) -> Any:
...
class ImageMetadataLogic(QObject):
    ...
    def load_image_metadata(
        ...
        try:
            ...
            if not self.original_exif_dict or not any(
                self.original_exif_dict.values()
            ):
        ...
        try:
            ...
            for ifd_name, tags in modified_data.items():
                ...
                for tag_code, tag_info in tags.items():
                    ...
                    if (
                        current_value_str == original_display_val
                        ...
                        try:
                            ...
                            parsed_value = parse_exif_value(
                                current_value_str, original_type(), tag_code
                            )

                            # Handle specific helper cases (like UserComment) after parsing
                            if (
                                tag_code == piexif.ExifIFD.UserComment
                                and isinstance(parsed_value, str)
                            ):
                                new_exif_dict[ifd_name][tag_code] = (
                            ...
                            tag_name_for_error = get_tag_name(
                                ifd_name, tag_code
                            )
                            raise ValueError(
                                f"Error parsing value for tag '{tag_name_for_error}' ({ifd_name}/{tag_code}): {e}"
                            )
            ...
            if (
                ...
                new_exif_dict["thumbnail"] = self.original_exif_dict[
                    "thumbnail"
                ]
            else:
            ...
            if (
                ...
                try:
                    ...
                    raise ValueError(
                        f"Error converting data to EXIF format: {dump_e}"
                    )
            ...
            try:
                ...
                if not exif_bytes:
                    self.metadata_saved.emit(
                        True,
                        f"All EXIF data removed from {os.path.basename(file_path)}.",
                    )
                else:
                    self.metadata_saved.emit(
                        True,
                        f"EXIF data successfully saved to {os.path.basename(file_path)}.",
                    )
                return True
    ...
    def _process_metadata_for_display(
        self, exif_dict: Dict[str, Any], include_thumbnails: bool = False
    ) -> Dict[str, Any]:
