...
class PDFSecurityValidator:
    ...
    def validate_signature_config(
        ...
        if signature.private_key_path and not os.path.exists(
            signature.private_key_path
        ):
            return (
                False,
...
class PDFPasswordProtection:
    ...
    def encrypt_pdf(
        self, input_path: str, output_path: str, settings: SecuritySettings
    ) -> SecurityResult:
        ...
        try:
            ...
            valid, msg = PDFSecurityValidator.validate_security_settings(
                settings
            )
            if not valid:
                return SecurityResult(False, SecurityOperation.ENCRYPT, msg)
            ...
            if HAS_PIKEPDF:
                return self._encrypt_with_pikepdf(
                    input_path, output_path, settings
                )
            elif HAS_PYMUPDF:
                return self._encrypt_with_pymupdf(
                    input_path, output_path, settings
                )
            else:
    ...
    def _encrypt_with_pikepdf(
        ...
        try:
            with pikepdf.open(input_path) as pdf:
                ...
                pdf.save(
                    output_path,
                    encryption=pikepdf.Encryption(**encryption_dict),
                )
            ...
            return SecurityResult(
                False,
                SecurityOperation.ENCRYPT,
                f"pikepdf encryption failed: {str(e)}",
            )

    def _encrypt_with_pymupdf(
        self, input_path: str, output_path: str, settings: SecuritySettings
    ) -> SecurityResult:
        ...
        try:
            ...
            return SecurityResult(
                False,
                SecurityOperation.ENCRYPT,
                f"PyMuPDF encryption failed: {str(e)}",
            )
    ...
    def decrypt_pdf(
        ...
        try:
            ...
            if HAS_PIKEPDF:
                return self._decrypt_with_pikepdf(
                    input_path, output_path, password
                )
            elif HAS_PYMUPDF:
                return self._decrypt_with_pymupdf(
                    input_path, output_path, password
                )
            else:
                return SecurityResult(
    ...
    def _decrypt_with_pikepdf(
        ...
        try:
            ...
            return SecurityResult(
                False, SecurityOperation.DECRYPT, "Incorrect password"
            )
        except Exception as e:
            return SecurityResult(
                False,
    ...
    def get_security_info(
        ...
        try:
            ...
            if not valid:
                return SecurityResult(
                    False, SecurityOperation.GET_SECURITY_INFO, msg
                )
            ...
            if HAS_PIKEPDF:
                ...
                return SecurityResult(
                    False,
                    SecurityOperation.GET_SECURITY_INFO,
            ...
            return SecurityResult(
                False,
                SecurityOperation.GET_SECURITY_INFO,
                f"Failed to get security info: {str(e)}",
            )

    def _get_security_info_pikepdf(
        self, file_path: str, password: str
    ) -> SecurityResult:
        ...
        try:
            ...
            try:
                with pikepdf.open(file_path, password=password) as pdf:
                    ...
                    security_info["permissions"] = (
                        self._get_pikepdf_permissions(pdf)
                    )
            except pikepdf.PasswordError:
            ...
            return SecurityResult(
                False, SecurityOperation.GET_SECURITY_INFO, f"Failed: {str(e)}"
            )

    def _get_security_info_pymupdf(
        self, file_path: str, password: str
    ) -> SecurityResult:
            ...
            if doc.needs_pass:
                if password:
                    ...
                    if rc:
                        ...
                        security_info["permissions"] = (
                            self._get_pymupdf_permissions(doc)
                        )
                    else:
                ...
                security_info["permissions"] = self._get_pymupdf_permissions(
                    doc
                )
...
class PDFDigitalSignature:
    ...
    def sign_pdf(
        self, input_path: str, output_path: str, signature: DigitalSignature
    ) -> SecurityResult:
        ...
        try:
            ...
            if not valid:
                return SecurityResult(
                    False, SecurityOperation.DIGITAL_SIGN, msg
                )

            valid, msg = PDFSecurityValidator.validate_signature_config(
                signature
            )
            if not valid:
                return SecurityResult(
            ...
            if not HAS_CRYPTOGRAPHY:
                return SecurityResult(
                    False,
                    SecurityOperation.DIGITAL_SIGN,
            ...
            return self._create_signature_placeholder(
                input_path, output_path, signature
            )
    ...
    def _create_signature_placeholder(
        self, input_path: str, output_path: str, signature: DigitalSignature
    ) -> SecurityResult:
        ...
        try:
            ...
            return SecurityResult(
                False,
                SecurityOperation.DIGITAL_SIGN,
                f"Signature creation failed: {str(e)}",
            )
    ...
    def verify_signatures(self, file_path: str) -> SecurityResult:
        ...
        try:
            ...
            if not valid:
                return SecurityResult(
                    False, SecurityOperation.VERIFY_SIGNATURE, msg
                )
            ...
            return SecurityResult(
                True,
                SecurityOperation.VERIFY_SIGNATURE,
...
class PDFSecurityEngine:
    ...
    def encrypt_pdf(
        ...
        """Encrypt PDF with password and permissions"""
        return self.password_protection.encrypt_pdf(
            input_path, output_path, settings
        )
    ...
    def decrypt_pdf(
        ...
        """Decrypt password-protected PDF"""
        return self.password_protection.decrypt_pdf(
            input_path, output_path, password
        )
    ...
    def sign_pdf(
        ...
        """Add digital signature to PDF"""
        return self.digital_signature.sign_pdf(
            input_path, output_path, signature
        )
    ...
    def remove_security(
        self, input_path: str, output_path: str, owner_password: str
    ) -> SecurityResult:
        ...
        try:
            ...
            if result.success:
                ...
                result.message = (
                    f"Security restrictions removed and saved to {output_path}"
                )
            ...
            return SecurityResult(
                False,
                SecurityOperation.REMOVE_PASSWORD,
                f"Failed to remove security: {str(e)}",
            )

    def batch_security_operation(
        self, file_paths: List[str], operation: SecurityOperation, **kwargs
    ) -> List[SecurityResult]:
            ...
            try:
                if operation == SecurityOperation.ENCRYPT:
                    output_path = kwargs.get(
                        "output_path",
                        file_path.replace(".pdf", "_encrypted.pdf"),
                    )
                    ...
                    output_path = kwargs.get(
                        "output_path",
                        file_path.replace(".pdf", "_decrypted.pdf"),
                    )
