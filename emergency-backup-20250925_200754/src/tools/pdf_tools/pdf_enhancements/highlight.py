...
class HighlightUI(QMainWindow):
    ...
    def process(self):
        try:
            ...
            color = (
                self.colorCombo.currentText().lower()
                if action not in ["Redact", "Remove"]
                else None
            )
            opacity = (
            ...
            if pages_text:
                try:
                    ...
                    QMessageBox.warning(
                        self, "Error", "Invalid page numbers format"
                    )
                    return

            # Create output filename
            output_file = (
                os.path.splitext(self.input_file)[0] + "_processed.pdf"
            )

            try:
                logger.info("Starting PDF processing")
                logger.debug(
                    "Parameters - Input: %s, Output: %s, Search: %s, Action: %s, Color: %s, Opacity: %f, Pages: %s",
                    self.input_file,
                    output_file,
                    search_str,
                    action,
                ...
                QMessageBox.information(
                    self,
                    "Success",
                    f"File processed successfully and saved as {output_file}",
                )
...
def highlight_matching_data(
    page, matched_values, type, color=None, opacity=1.0
):
    """
    ...
    try:
        logger.debug(
            "Highlighting %d matches with type '%s' and color '%s'",
            len(matched_values),
            type,
            color,
        )
...
def process_data(
    ...
    try:
        ...
        logger.debug(
            "Parameters - Output: %s, Search: %s, Pages: %s, Action: %s, Color: %s, Opacity: %f",
            output_file,
            search_str,
            pages,
            action,
...
def process_file(**kwargs):
    ...
    try:
        ...
        if action == "Remove":
            ...
            remove_highlght(
                input_file=input_file, output_file=output_file, pages=pages
            )
        else:
            process_data(
