...
class DeleteCookiesTool(PrivacyToolBase):
    ...
    def get_description(self) -> str:
        ...
        return (
            "Deletes browser cookies from all supported browsers. "
    ...
    def preview_operation(self, **kwargs) -> Dict[str, Any]:
        ...
        preview = {
            "browsers": browsers
            or self.browser_detector.detect_installed_browsers(),
        ...
        for browser in preview["browsers"]:
            if not self.browser_detector.is_browser_running(browser):
                cookie_count = self._count_cookies(
    ...
    def execute_operation(self, **kwargs) -> PrivacyOperationResult:
        ...
        try:
            ...
            self._emit_progress(
                len(browsers), len(browsers), f"Cookie deletion complete"
            )
            ...
            return PrivacyOperationResult(
                success, message, total_deleted, errors
            )
    ...
    def _count_cookies(
        ...
        try:
            ...
            for cookie_path in cookie_paths:
                ...
                if (
                    browser == DataLocations.CHROME
                    or browser == DataLocations.EDGE
                ):
    ...
    def _delete_chromium_cookies(
        ...
        try:
            ...
            count_before = self._count_chromium_cookies(
                cookie_path, domain_filter, days_old
            )
            ...
            if days_old is not None:
                cutoff_time = datetime.now() - timedelta(days=days_old)
                cutoff_chromium = (
                    int(cutoff_time.timestamp() * 1000000) + 11644473600000000
                )
                query += " AND creation_utc < ?"
                params.append(cutoff_chromium)
    ...
    def _delete_firefox_cookies(
        ...
        try:
            # Count before deletion
            count_before = self._count_firefox_cookies(
                cookie_path, domain_filter, days_old
            )
    ...
    def _delete_safari_cookies(
        ...
        try:
            ...
            if cookie_path.exists():
                estimated_count = self._count_safari_cookies(
                    cookie_path, domain_filter, days_old
                )
