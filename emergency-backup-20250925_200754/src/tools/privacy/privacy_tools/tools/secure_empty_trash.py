...
class SecureEmptyTrashTool(PrivacyToolBase):
    ...
    def get_description(self) -> str:
        ...
        return (
            "Securely empties the system trash/recycle bin, "
    ...
    def _preview_macos_trash(self) -> Dict[str, Any]:
        ...
        return {"trash_items": items, "estimated_size": total_size}
    ...
    def execute_operation(
        self, secure_delete: bool = False, create_backup: bool = False
    ...
    def _empty_windows_trash(
        self, secure_delete: bool, create_backup: bool
    ) -> PrivacyOperationResult:
        ...
        try:
            if secure_delete:
                ...
                self._emit_status(
                    "Secure deletion on Windows requires "
            ...
            if success:
                ...
                return PrivacyOperationResult(
                    True, "Windows Recycle Bin emptied successfully", 1
                )
            else:
                return PrivacyOperationResult(
                    False,
            ...
            return PrivacyOperationResult(
                False, f"Windows trash operation failed: {str(e)}", 0, [str(e)]
            )

    def _empty_macos_trash(
        self, secure_delete: bool, create_backup: bool
    ) -> PrivacyOperationResult:
        """Empty macOS trash."""
        trash_dir = PlatformUtils.get_trash_directory()
        ...
        if not trash_dir or not trash_dir.exists():
            return PrivacyOperationResult(True, "Trash is already empty", 0)
        ...
        try:
            ...
            for i, item in enumerate(items):
                ...
                self._emit_progress(i, total_items, f"Processing: {item.name}")
            ...
            return PrivacyOperationResult(
                False, f"macOS trash operation failed: {str(e)}", 0, [str(e)]
            )

    def _empty_linux_trash(
        self, secure_delete: bool, create_backup: bool
    ) -> PrivacyOperationResult:
        """Empty Linux trash."""
        trash_dir = PlatformUtils.get_trash_directory()
        ...
        try:
            ...
            if files_dir.exists():
                ...
                for i, item in enumerate(items):
                    ...
                    self._emit_progress(
                        i, total_items, f"Processing: {item.name}"
                    )
                    ...
                    try:
                        ...
                        if item.is_file():
                            if secure_delete:
                                success = PlatformUtils.secure_delete_file(
                                    item
                                )
                            else:
                                item.unlink()
                            ...
                            if secure_delete:
                                success = self._safe_delete_directory(
                                    item, True
                                )
                            else:
                                shutil.rmtree(item)
            ...
            return PrivacyOperationResult(
                False, f"Linux trash operation failed: {str(e)}", 0, [str(e)]
            )
