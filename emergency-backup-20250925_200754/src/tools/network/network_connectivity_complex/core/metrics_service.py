...
class MetricAggregator:
    ...
    def aggregate_values(
        self, values: List[MetricValue], aggregation_type: str = "avg"
    ) -> Optional[float]:
    ...
    def get_time_window_values(
        self, metric: Metric, window_seconds: int = None
    ) -> List[MetricValue]:
        ...
        return [
            value for value in metric.values if value.timestamp >= cutoff_time
        ]
    ...
    def _collect_tool_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self._tool_instance, "is_running"):
                metrics[f"{self.tool_name}.is_running"] = int(
                    self._tool_instance.is_running
                )

            if hasattr(self._tool_instance, "is_healthy"):
                metrics[f"{self.tool_name}.is_healthy"] = int(
                    self._tool_instance.is_healthy
                )

            if hasattr(self._tool_instance, "_error_count"):
                metrics[f"{self.tool_name}.error_count"] = (
                    self._tool_instance._error_count
                )
...
class MetricsService:
    ...
    def _start_background_threads(self):
        ...
        self._cleanup_thread = threading.Thread(
            target=self._cleanup_loop, name="MetricsCleanup", daemon=True
        )
        self._cleanup_thread.start()

        # Alert checking thread
        self._alert_thread = threading.Thread(
            target=self._alert_loop, name="MetricsAlerts", daemon=True
        )
        ...
        while not self._stop_event.is_set():
            try:
                ...
                self._stop_event.wait(
                    self._config["alert_check_interval_seconds"]
                )
            except Exception as e:
    ...
    def record_value(
        ...
        with self._lock:
            try:
                ...
                self.logger.error(
                    f"Failed to record value for {metric_name}: {e}"
                )
                return False
    ...
    def get_metric_value(
        self, name: str, aggregation: str = "latest"
    ) -> Optional[float]:
    ...
    def get_metric_values(
        self, name: str, window_seconds: int = 3600
    ) -> List[MetricValue]:
        """Get metric values within a time window.
        ...
        with self._lock:
            ...
            return self._aggregator.get_time_window_values(
        ...
        with self._lock:
            ...
            for name, metric in self._metrics.items():
                if metric.values:
                    ...
                    values_1h = self._aggregator.get_time_window_values(
                        metric, 3600
                    )
                    ...
                    summary[name] = {
                        ...
                        "avg_last_hour": self._aggregator.aggregate_values(
                            values_1h, "avg"
                        ),
                        "min_last_hour": self._aggregator.aggregate_values(
                            values_1h, "min"
                        ),
                        "max_last_hour": self._aggregator.aggregate_values(
                            values_1h, "max"
                        ),
                    }
    ...
    def add_tool_collector(self, tool_name: str, tool_instance) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to add tool collector for {tool_name}: {e}"
            )
            return False
        ...
        with self._lock:
            for alert_name, alert in self._alerts.items():
                ...
                try:
                    ...
                    if condition_met:
                        self._trigger_alert(
                            alert_name, alert, values[-1].value
                        )

                except Exception as e:
                    self.logger.error(
                        f"Error checking alert {alert_name}: {e}"
                    )
    ...
    def _trigger_alert(
        ...
        try:
            ...
            self.logger.warning(
                "Notification service not available for metric alerts"
            )
        except Exception as e:
    ...
    def _cleanup_old_values(self):
        ...
        with self._lock:
            ...
            for metric in self._metrics.values():
                ...
                while (
                    metric.values and metric.values[0].timestamp < cutoff_time
                ):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "memory_usage_values": sum(
                    len(m.values) for m in self._metrics.values()
                ),
            }
...
def record_rate(name: str, rate: float, **kwargs):
    ...
    if name not in service._metrics:
        service.create_metric(
            name, MetricType.RATE, MetricUnit.RATE_PER_SECOND
        )
