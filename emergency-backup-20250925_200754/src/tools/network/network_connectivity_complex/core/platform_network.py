...
class WindowsNetworkDetector(NetworkDetectorBase):
            ...
            for adapter in self._wmi.Win32_NetworkAdapter():
                if adapter.NetEnabled:
                    ...
                    interface = NetworkInterface(
                        ...
                        is_wireless=(
                            "wireless" in adapter.Name.lower()
                            or "wifi" in adapter.Name.lower()
                        ),
                        mac_address=adapter.MACAddress,
                        ip_addresses=ip_addresses,
                        interface_type=adapter.AdapterType or "Unknown",
                        speed_mbps=(
                            int(adapter.Speed / 1000000)
                            if adapter.Speed
                            else None
                        ),
    ...
    def _get_interface_type(self, interface_name: str) -> str:
        ...
        if any(
            keyword in name_lower for keyword in ["ethernet", "eth", "lan"]
        ):
            return "Ethernet"
        elif any(
            keyword in name_lower for keyword in ["wireless", "wifi", "wlan"]
        ):
            return "Wireless"
