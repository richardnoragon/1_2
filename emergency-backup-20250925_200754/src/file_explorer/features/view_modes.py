...
class ViewConfiguration:
    ...
    visible_columns: List[str] = field(
        default_factory=lambda: ["name", "size", "type", "date_modified"]
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ViewConfiguration":
        ...
        config.visible_columns = data.get(
            "visible_columns", ["name", "size", "type", "date_modified"]
        )
...
class BaseViewRenderer(ABC):
    ...
    def __init__(self, view_widget: QWidget, config: ViewConfiguration):
        ...
        self.logger = logging.getLogger(
            f"RFU.ViewModes.{self.__class__.__name__}"
        )
...
class IconViewRenderer(BaseViewRenderer):
    ...
    def setup_view(self) -> bool:
        ...
        try:
            ...
            grid_size = (
                icon_size + self.config.item_spacing * 2 + 20
            )  # Text space
        ...
        try:
            ...
            if model:
                for column in range(model.columnCount()):
                    ...
                    if column_name:
                        is_visible = (
                            column_name.lower() in config.visible_columns
                        )
                        self.view_widget.setColumnHidden(
                            column, not is_visible
                        )
            ...
            if header and state.column_widths:
                ...
                if model:
                    for column in range(model.columnCount()):
                        ...
                        if (
                            column_name
                            and str(column_name) in state.column_widths
                        ):
...
class ViewModeManager(QObject):
    ...
    def __init__(self, database: Optional[FileExplorerDatabase] = None):
        ...
        self.view_configs: Dict[ViewMode, ViewConfiguration] = {
            ...
            ViewMode.THUMBNAIL: ViewConfiguration(
                view_mode=ViewMode.THUMBNAIL
            ),
        }

        # Per-directory configurations
        self.directory_configs: Dict[
            str, Dict[ViewMode, ViewConfiguration]
        ] = {}
    ...
    def set_view_mode(self, view_mode: ViewMode, directory: str = "") -> bool:
        ...
        try:
            ...
            if (
                self.current_directory
                and self.view_renderers[self.current_view_mode]
            ):

                current_state = self.view_renderers[
    ...
    def get_view_configuration(
        self, view_mode: ViewMode, directory: str = ""
    ) -> ViewConfiguration:
        """
    ...
    def set_view_configuration(
        ...
        try:
            ...
            if (
                view_mode == self.current_view_mode
                and self.view_renderers[view_mode]
            ):
    ...
    def register_view_renderer(
        self, view_mode: ViewMode, renderer: BaseViewRenderer
    ) -> bool:
        """
        ...
        try:
            ...
            config = self.get_view_configuration(
                view_mode, self.current_directory
            )
    ...
    def switch_to_view_mode(self, view_mode: ViewMode) -> bool:
        ...
        try:
            ...
            if not self.view_renderers[view_mode]:
                self.logger.warning(
                    f"No renderer available for {view_mode.name}"
                )
                return False
            ...
            if (
                self.current_directory in self.directory_states
                and view_mode in self.directory_states[self.current_directory]
            ):

                saved_state = self.directory_states[self.current_directory][
                    view_mode
                ]
            ...
            self.logger.info(
                f"Switched from {old_mode.name} to {view_mode.name}"
            )
            return True
    ...
    def get_view_state(
        ...
        if (
            directory in self.directory_states
            and view_mode in self.directory_states[directory]
        ):
    ...
    def get_available_view_modes(self) -> List[ViewMode]:
        ...
        return [
            mode
            for mode, renderer in self.view_renderers.items()
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "available_modes": [
                    mode.name for mode in self.get_available_view_modes()
                ],
...
if __name__ == "__main__":
    ...
    if saved_state:
        print(
            f"Saved state has {len(saved_state.selected_items)} selected items"
        )
