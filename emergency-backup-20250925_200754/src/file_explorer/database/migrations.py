...
class CreateInitialSchemaMigration(Migration):
    ...
    def _insert_initial_data(self, conn: sqlite3.Connection):
        ...
        # Get the config ID
        cursor = conn.execute(
            "SELECT id FROM pane_configurations WHERE config_name = 'Default Dual Pane'"
        )
...
class DatabaseMigrator(QObject):
    ...
    migration_completed = pyqtSignal(
        int, bool, str
    ...
    def __init__(self, db_path: Union[str, Path]):
        ...
        self.target_version = (
            max(self.migrations.keys()) if self.migrations else 0
        )
    ...
    def create_backup(self) -> Optional[Path]:
        ...
        try:
            ...
            backup_path = (
                self.backup_dir / f"file_explorer_backup_{timestamp}.db"
            )
    ...
    def migrate(self, target_version: Optional[int] = None) -> bool:
        ...
        for i, migration in enumerate(migrations_to_apply):
            ...
            self.migration_completed.emit(
                migration.version, True, "Migration applied successfully"
            )
        ...
        if success:
            ...
            self.logger.error(
                "Migration failed, database may need manual recovery"
            )
            if backup_path:
    ...
    def _validate_migration_dependencies(
        ...
        for migration in migrations:
            for dep_version in migration.get_dependencies():
                if (
                    dep_version > current_version
                    and dep_version not in applied_versions
                ):
                    self.logger.error(
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                try:
                    ...
                    if not migration.validate_preconditions(conn):
                        raise MigrationError(
                            f"Preconditions not met for migration {migration.version}"
                        )

                    # Apply migration
                    if not migration.upgrade(conn):
                        raise MigrationError(
                            f"Migration {migration.version} upgrade failed"
                        )

                    # Validate postconditions
                    if not migration.validate_postconditions(conn):
                        raise MigrationError(
                            f"Postconditions not met for migration {migration.version}"
                        )
