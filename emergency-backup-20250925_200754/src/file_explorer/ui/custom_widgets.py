            ...
            if path_obj.is_dir():
                ...
                try:
                    ...
                    self.file_size_label.setText(
                        self._format_file_size(folder_size)
                    )
                except Exception:
                ...
                file_type = (
                    path_obj.suffix[1:].upper() if path_obj.suffix else "File"
                )
                self.file_type_label.setText(f"{file_type} File")

                # File size
                self.file_size_label.setText(
                    self._format_file_size(stat_info.st_size)
                )
...
class QuickPreviewWidget(QFrame if QT_AVAILABLE else object):
    ...
    def set_file_preview(self, file_path: str):
        ...
        try:
            ...
            self.logger.error(
                f"Error setting file preview for {file_path}: {e}"
            )
            self._show_no_preview()
...
class SearchWidget(QFrame if QT_AVAILABLE else object):
    ...
    def _perform_search(self):
        ...
        self.logger.debug(
            f"Search requested: '{search_text}' with options {search_options}"
        )
