...
class DriveInfo:
    ...
    def update_capacity(self):
        ...
        try:
            if (
                self.status == DriveStatus.AVAILABLE
                and Path(self.mount_point).exists()
            ):
...
class WindowsDriveDetector:
    ...
    def __init__(self):
        ...
        self.logger = logging.getLogger(
            "RFU.FileExplorer.WindowsDriveDetector"
        )
        ...
        try:
            ...
            for drive_letter in drive_list:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to get info for drive {drive_letter}: {e}"
                    )
    ...
    def _get_drive_info(self, drive_path: str) -> Optional[DriveInfo]:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to get Windows drive info for {drive_path}: {e}"
            )
            return None
            ...
            for disk in self.wmi_connection.Win32_LogicalDisk(
                ...
                if hasattr(disk, "FreeSpace") and disk.FreeSpace:
                    ...
                    drive_info.used_size = (
                        drive_info.total_size - drive_info.free_size
                    )
    ...
    def _get_network_drives(self) -> List[DriveInfo]:
        ...
        try:
            for disk in self.wmi_connection.Win32_LogicalDisk(
                ...
                drive_info.label = (
                    disk.VolumeName or f"Network Drive {disk.DeviceID}"
                )
...
class MacOSDriveDetector:
    ...
    def _get_volume_info(self, volume_path: str) -> Optional[DriveInfo]:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to get macOS volume info for {volume_path}: {e}"
            )
            return None
...
class LinuxDriveDetector:
    ...
    def _create_drive_info(
        self, mount_info: Dict[str, str]
    ) -> Optional[DriveInfo]:
        """
    ...
    def _determine_drive_type(
        self, drive_info: DriveInfo, mount_info: Dict[str, str]
    ):
        """
...
class DriveMonitor(QThread):
    ...
    def _update_drives(self):
        ...
        try:
            ...
            for mount_point, drive_info in current_drives.items():
                ...
                elif self._drive_changed(
                    self.previous_drives[mount_point], drive_info
                ):
    ...
    def _drive_changed(
        self, old_drive: DriveInfo, new_drive: DriveInfo
    ) -> bool:
        """
        ...
        if abs(old_drive.free_size - new_drive.free_size) > (
            1024 * 1024 * 100
        ):  # 100MB
            return True
...
class CrossPlatformDriveDetector(QObject):
    ...
    def __init__(self, parent: Optional[QObject] = None):
        ...
        # Setup logging
        self.logger = logging.getLogger(
            "RFU.FileExplorer.CrossPlatformDriveDetector"
        )
    ...
    def refresh_drives(self) -> List[DriveInfo]:
        ...
        try:
            ...
            sorted_drives = sorted(
                detected_drives, key=lambda d: d.mount_point
            )
            ...
            self.logger.debug(
                f"Refreshed drives: {len(detected_drives)} found"
            )
    ...
    def is_path_on_removable_drive(self, path: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to check removable drive for {path}: {e}"
            )
            return False
    ...
    def is_path_on_network_drive(self, path: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to check network drive for {path}: {e}"
            )
            return False
    ...
    def test_drive_detection():
        ...
        for drive in drives:
            ...
            if drive.is_ready:
                ...
                print(
                    f"  Used: {capacity['used']} ({capacity['usage_percent']:.1f}%)"
                )

    # Connect signals
    detector.driveAdded.connect(
        lambda drive: print(f"ADDED: {drive.mount_point}")
    )
    detector.driveRemoved.connect(lambda mount: print(f"REMOVED: {mount}"))
    detector.driveChanged.connect(
        lambda drive: print(f"CHANGED: {drive.mount_point}")
    )
