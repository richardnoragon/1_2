...
class NetworkConnectivityValidator:
    ...
    def _detect_install_dir(self) -> Path:
        ...
        possible_dirs = [
            ...
            Path("C:/Program Files/NetworkConnectivity") if platform.system() == "Windows" else None,
            Path.home() / "Applications/NetworkConnectivity" if platform.system() == "Darwin" else None
        ]
        
        for dir_path in possible_dirs:
            if dir_path and dir_path.exists() and (dir_path / "__init__.py").exists():
                return dir_path
        
        # Default to current directory
        return Path.cwd() / "network_connectivity"
    
    def _run_test(self, test_name: str, test_func, category: str = "general") -> ValidationResult:
    ...
    def validate_configuration(self) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if not config_dir.exists():
            return False, "Configuration directory not found", {"config_dir": str(config_dir)}
    ...
    def validate_tools_initialization(self) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        tools_to_test = [
            ("BandwidthMonitor", "network_connectivity.tools.bandwidth_monitor"),
            ("PortScanner", "network_connectivity.tools.port_scanner"),
            ("WiFiAnalyzer", "network_connectivity.tools.wifi_analyzer"),
            ("LANFileTransfer", "network_connectivity.tools.lan_file_transfer")
        ]
        ...
        if success:
            message = f"All {len(tools_to_test)} tools initialized successfully"
        else:
    ...
    def validate_gui_components(self) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if platform.system() == "Linux" and not os.environ.get("DISPLAY"):
                return True, "GUI validation skipped (no display available)", {"skipped": True}
            ...
            gui_components = [
                ...
                ("BandwidthMonitorWidget", "network_connectivity.gui.widgets.bandwidth_monitor_widget"),
            ]
            ...
            details = {
                ...
                "qt_version": getattr(__import__("PyQt6.QtCore"), "QT_VERSION_STR", "unknown")
            }
            
            return success, message, details
            
        except ImportError as e:
            return False, f"GUI validation failed: PyQt6 not available - {e}", {"error": str(e)}
        except Exception as e:
    ...
    def validate_network_operations(self) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            network_tests["interface_detection"] = {
                ...
                "interfaces": [iface.name for iface in interfaces[:5]]  # First 5 only
            }
    ...
    def validate_performance(self) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            performance_tests["import_time"] = {
                "success": import_time < 2.0,  # Should import in under 2 seconds
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        validation_tests = [
            ...
            ("Tool Initialization", self.validate_tools_initialization, "tools"),
            ("GUI Components", self.validate_gui_components, "gui"),
            ("Network Operations", self.validate_network_operations, "network"),
        ...
        if summary["overall_success"]:
            self.logger.info(f"✓ Validation completed successfully ({successful_tests}/{total_tests} tests passed)")
        else:
            self.logger.error(f"✗ Validation failed ({failed_tests}/{total_tests} tests failed)")
