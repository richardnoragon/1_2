...
class CleanupOperationResult:
    ...
    def __init__(self, success: bool, message: str, 
                 items_processed: int = 0, space_freed: int = 0, 
    ...
    def __str__(self):
        return f"CleanupOperationResult(success={self.success}, " \
               f"message='{self.message}', items={self.items_processed}, " \
               f"space_freed={self.space_freed})"


class CleanupToolBase(QObject, ABC):
    """Base class for all system cleanup tools."""
    ...
    def _filter_files_by_age(self, files: List[Path], 
                           max_age_days: int) -> List[Path]:
    ...
    def _filter_files_by_size(self, files: List[Path], 
                            min_size_bytes: int) -> List[Path]:
    ...
    def _filter_files_by_extension(self, files: List[Path], 
        ...
        try:
            ...
            if not self._validate_windows_system():
                return CleanupOperationResult(
                    False, "Windows system required", 0, 0, 
                    ["Not running on Windows"]
                )
            
            if not self._validate_admin_privileges():
                return CleanupOperationResult(
                    False, "Administrator privileges required", 0, 0,
                    ["Insufficient privileges"]
                )
            
            if not self._validate_operation_parameters(**kwargs):
                return CleanupOperationResult(
                    False, "Invalid operation parameters", 0, 0,
