...
class SystemLocations:
    ...
    def get_windows_cache_directories(cls) -> List[Path]:
        ...
        # Font Cache
        font_cache = windows_dir / "ServiceProfiles" / "LocalService" / "AppData" / "Local" / "FontCache"
        if font_cache.exists():
            cache_dirs.append(font_cache)
        
        # Thumbnail Cache
        for user_dir in WindowsUtils.get_user_profile_directories():
            thumb_cache = user_dir / "AppData" / "Local" / "Microsoft" / "Windows" / "Explorer"
    ...
    def get_backup_directories(cls) -> List[Path]:
        ...
        for user_dir in WindowsUtils.get_user_profile_directories():
            user_backups = [
                user_dir / "AppData" / "Local" / "Microsoft" / "Windows" / "FileHistory",
    ...
    def get_history_directories(cls) -> List[Path]:
        ...
        for user_dir in WindowsUtils.get_user_profile_directories():
            history_locations = [
                ...
                user_dir / "AppData" / "Roaming" / "Microsoft" / "Windows" / "Recent" / "CustomDestinations",
    ...
    def get_shortcut_directories(cls) -> List[Path]:
        ...
        system_start_menu = Path(system_drive) / "ProgramData" / "Microsoft" / "Windows" / "Start Menu"
        if system_start_menu.exists():
            shortcut_dirs.append(system_start_menu)
        
        # User-specific Start Menus
        for user_dir in WindowsUtils.get_user_profile_directories():
            user_start_menu = user_dir / "AppData" / "Roaming" / "Microsoft" / "Windows" / "Start Menu"
            ...
            # Quick Launch
            quick_launch = user_dir / "AppData" / "Roaming" / "Microsoft" / "Internet Explorer" / "Quick Launch"
    ...
    def get_common_file_patterns(cls) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "cache_files": [
                "*.cache", "thumbs.db", "desktop.ini", "*.db-wal",
                "*.db-shm", "iconcache*.db"
            ],
            "backup_files": [
