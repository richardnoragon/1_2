...
class LinuxFilesystemImpl:
    ...
    def _process_filesystem_entry(self, fs_entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _get_block_devices(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = subprocess.run(
                [self.tools['lsblk'], '-J', '-o', 'NAME,SIZE,TYPE,MOUNTPOINT,FSTYPE,UUID'],
                capture_output=True,
                text=True,
                timeout=30
            )
                        ...
                        if ' on ' in line and ' type ' in line:
                            ...
                            if len(parts) >= 2:
                                ...
                                if len(rest) >= 2:
                                    ...
                                    options = fs_and_options[1].rstrip(')') if len(fs_and_options) > 1 else ''
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    lvm_info['volume_groups'] = vgs_data.get('report', [{}])[0].get('vg', [])
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    lvm_info['logical_volumes'] = lvs_data.get('report', [{}])[0].get('lv', [])
                except json.JSONDecodeError:
                    pass
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    lvm_info['physical_volumes'] = pvs_data.get('report', [{}])[0].get('pv', [])
                except json.JSONDecodeError:
                    pass
    ...
    def is_admin_required(self, operation: str) -> bool:
        ...
        """
        admin_operations = {
            'fsck_repair', 'filesystem_repair', 'lvm_operations',
            'tune2fs_operations', 'mount_operations'
        }
