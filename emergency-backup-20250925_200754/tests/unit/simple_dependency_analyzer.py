...
class SimpleDependencyAnalyzer:
    ...
    def find_critical_path(self) -> List[str]:
        ...
        # Topological sort
        queue = [
            blocker for blocker, degree in in_degree.items() if degree == 0
        ]
        result = []
    ...
    def calculate_earliest_completion(self) -> Dict[str, datetime]:
        ...
        for blocker_id in critical_path:
            ...
            if not dependencies:
                ...
                completion_times[blocker_id] = max(
                    latest_dep_completion, estimated_completion
                )
...
"""
        ...
        report += (
            f"**Total Critical Path Duration:** {total_duration} days\n\n"
        )
        ...
        if independent_blockers:
            ...
            for blocker_id in independent_blockers:
                ...
                report += (
                    f"- {blocker_id} {icon}: {blocker.get('title', '')}\n"
                )
        ...
        for blocker_id, blocker in self.blockers.items():
            if blocker.get("status") == "NOT_STARTED":
                ...
                can_start = True
                for dep in dependencies:
                    dep_status = self.blockers.get(dep, {}).get(
                        "status", "NOT_STARTED"
                    )
...
def main():
    ...
    # Save report
    with open(
        "blocker_dependency_analysis_simple.md", "w", encoding="utf-8"
    ) as f:
        f.write(report)

    print(
        "📄 Dependency analysis report saved to: blocker_dependency_analysis_simple.md"
    )
    print("\n" + "=" * 80)
    print(report)
