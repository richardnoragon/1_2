...
if analyzer_available:
    ...
    try:
        ...
        test_files = [
            ...
            {
                "name": "duplicate_1.txt",
                "size": 512,
                "content": "Duplicate" * 64,
            },
            {
                "name": "duplicate_2.txt",
                "size": 512,
                "content": "Duplicate" * 64,
            },
        ]
        ...
        print(
            f"  Created test dataset: {expected_total_files} files, {expected_total_size} bytes"
        )
        ...
        # Calculate accuracy
        file_accuracy = min(actual_files, expected_total_files) / max(
            actual_files, expected_total_files, 1
        )
        size_accuracy = min(actual_size, expected_total_size) / max(
            actual_size, expected_total_size, 1
        )
        overall_accuracy = (file_accuracy + size_accuracy) / 2

        accuracy_passed = (
            overall_accuracy
            >= mandatory_criteria["algorithm_accuracy_threshold"]
        )

        print(
            f"  Expected: {expected_total_files} files, {expected_total_size} bytes"
        )
    ...
    try:
        ...
        throughput = (
            result.get("file_count", 0) / execution_time
            if execution_time > 0
            else 0
        )
        ...
        compliance_score = (
            sum([time_passed, memory_passed, throughput_passed]) / 3
        )
        performance_passed = (
            compliance_score
            >= mandatory_criteria["performance_compliance_rate"]
        )

        print(
            f'  Execution Time: {execution_time:.2f}s (limit: {max_time}s) - {"[PASS]" if time_passed else "[FAIL]"}'
        )
        print(
            f'  Memory Usage: {memory_usage_mb:.2f}MB (limit: {max_memory}MB) - {"[PASS]" if memory_passed else "[FAIL]"}'
        )
        print(
            f'  Throughput: {throughput:.1f} files/sec (min: {min_throughput}) - {"[PASS]" if throughput_passed else "[FAIL]"}'
        )
    ...
    error_handling_passed = (
        robustness_score >= mandatory_criteria["error_handling_robustness"]
    )
