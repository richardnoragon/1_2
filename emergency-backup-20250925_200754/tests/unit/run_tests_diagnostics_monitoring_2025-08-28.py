...
def generate_summary_report(pytest_success, pytest_result, individual_results):
    ...
    passed_tests = sum(
        1 for r in individual_results.values() if r.get("status") == "passed"
    )
    failed_tests = sum(
        1 for r in individual_results.values() if r.get("status") == "failed"
    )
    skipped_tests = sum(
        1 for r in individual_results.values() if r.get("status") == "skipped"
    )
    error_tests = sum(
        1 for r in individual_results.values() if r.get("status") == "error"
    )
    timeout_tests = sum(
        1 for r in individual_results.values() if r.get("status") == "timeout"
    )
    ...
    summary_file = (
        RESULTS_DIR / f"result_diagnostics_monitoring_summary_{TIMESTAMP}.json"
    )
    try:
...
def main():
    ...
    try:
        ...
        summary = generate_summary_report(
            pytest_success, pytest_result, individual_results
        )
        ...
        if pytest_success or any(
            r.get("status") == "passed" for r in individual_results.values()
        ):
