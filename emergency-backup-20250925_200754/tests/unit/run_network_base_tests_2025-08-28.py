...
class NetworkBaseTestRunner:
    ...
    def __init__(self):
        ...
        self.execution_log = (
            self.logs_dir / "test_execution_network_base_2025-08-28.log"
        )
            ...
            try:
                subprocess.run(
                    [
                        ...
                        "install",
                        "-r",
                        "requirements_test_network_base_2025-08-28.txt",
                    ],
                    check=True,
                    cwd=self.base_dir,
                )
    ...
    def generate_summary(self, test_success):
        ...
        with open(summary_path, "w") as f:
            ...
            for file_type, filename in summary_data["output_files"].items():
                f.write(
                    f"  {file_type.replace('_', ' ').title()}: {filename}\n"
                )

        # Save JSON summary
        json_summary_path = (
            self.results_dir / f"result_network_base_summary_2025-08-28.json"
        )
    ...
    def print_final_report(self, summary_data):
        ...
        for file_type, filename in summary_data["output_files"].items():
            if Path(self.results_dir / filename).exists():
                ...
                print(
                    f"  ❌ {file_type.replace('_', ' ').title()}: {filename} (not found)"
                )
