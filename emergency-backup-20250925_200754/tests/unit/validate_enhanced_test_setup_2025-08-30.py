...
class EnhancedTestSetupValidator:
    ...
    def log_result(
        self, check_name: str, passed: bool, message: str, details: str = ""
    ):
    ...
    def generate_validation_report(self) -> Dict:
        ...
        if self.errors:
            report["recommendations"].append(
                "Fix all failed checks before running tests"
            )
        ...
        if passed_checks == total_checks:
            report["recommendations"].append(
                "All checks passed - ready to run enhanced tests"
            )
    ...
    def run_validation(self) -> bool:
        ...
        for check in checks:
            try:
                ...
                self.log_result(
                    check.__name__, False, f"Unexpected error: {e}"
                )
                all_passed = False
            print()
        ...
        report_file = (
            self.test_dir
            / "result_security_validator_validation_2025-08-30.json"
        )
        ...
        print(
            f"Success Rate: {report['validation_summary']['success_rate']:.1f}%"
        )
        print()
        ...
        if all_passed:
            ...
            print(
                "❌ SOME CHECKS FAILED - Please fix issues before running tests"
            )
