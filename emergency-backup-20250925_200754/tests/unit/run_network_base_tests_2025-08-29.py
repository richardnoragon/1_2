...
def run_network_base_tests():
    ...
    pytest_cmd = [
        ...
        "--json-report",
        "--json-report-file="
        + str(test_dir / f"result_network_base_2025-08-29.json"),
        "--cov=src.utilities.network.network_connectivity_complex.core.network_base",
        "--cov-report=html:"
        + str(test_dir / f"result_network_base_coverage_2025-08-29"),
        "--cov-report=json:"
        + str(test_dir / f"result_network_base_coverage_2025-08-29.json"),
...
def generate_summary_report(test_dir, start_time, exit_code):
    ...
    if pytest_json_file.exists():
        try:
            ...
            summary_data["pytest_summary"] = {
                "error": f"Could not parse pytest JSON: {e}"
            }
