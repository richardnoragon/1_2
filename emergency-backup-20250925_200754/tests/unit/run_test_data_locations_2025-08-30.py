...
class TestRunner:
    ...
    def generate_summary_report(self):
        ...
        summary = {
            "test_execution": {
                ...
                "duration_seconds": (
                    self.results.get("execution_time", {}).total_seconds()
        ...
        if "stdout" in self.results:
            summary["test_output"] = {
                "stdout_preview": (
                    self.results["stdout"][:1000] + "..."
                    if len(self.results["stdout"]) > 1000
                    else self.results["stdout"]
                ),
                "stderr_preview": (
                    self.results["stderr"][:500] + "..."
                    if len(self.results["stderr"]) > 500
                    else self.results["stderr"]
                ),
            }
    ...
    def verify_generated_files(self):
        ...
        expected_dirs = ["result_data_locations_2025-08-30_coverage"]
    ...
    def run(self):
        ...
        try:
            ...
            print(
                f"Summary generated: {'SUCCESS' if summary_success else 'FAILED'}"
            )
            print(
                f"Output files: {'ALL PRESENT' if files_present else 'SOME MISSING'}"
            )

            overall_success = (
                test_success and summary_success and files_present
            )
            print(
            ...
            if overall_success:
                ...
                print(
                    "- result_data_locations_2025-08-30_coverage/ (Coverage HTML)"
                )
                print(
                    "- result_data_locations_2025-08-30_coverage.json (Coverage JSON)"
                )
                print(
