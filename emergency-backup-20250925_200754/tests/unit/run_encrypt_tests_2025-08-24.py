...
def run_tests():
    ...
    html_report = (
        test_dir / f"result_encrypt_test_report_2025-08-24_{timestamp}.html"
    )
    json_report = (
        test_dir / f"result_encrypt_test_results_2025-08-24_{timestamp}.json"
    )
    coverage_html = (
        test_dir / f"result_encrypt_coverage_2025-08-24_{timestamp}"
    )
    coverage_json = (
        test_dir / f"result_encrypt_coverage_2025-08-24_{timestamp}.json"
    )
    ...
    try:
        result = subprocess.run(
            cmd, cwd=test_dir.parent.parent, capture_output=False
        )
        ...
        generate_summary_report(
            html_report, json_report, coverage_json, timestamp
        )
...
def generate_summary_report(
    ...
    summary_file = (
        ...
        / f"result_encrypt_test_summary_2025-08-24_{timestamp}.txt"
    )
    ...
    try:
        with open(summary_file, "w") as f:
            ...
            f.write(
                f"Test Date: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            )
                ...
                try:
                    ...
                    for file_path, file_data in files.items():
                        if "encrypt.py" in file_path:
                            f.write(
                                f"encrypt.py Coverage: {file_data.get('summary', {}).get('percent_covered', 'N/A')}%\n"
                            )
            ...
            f.write(
                f"• HTML Coverage Report: {coverage_json.with_suffix('').name}/\n"
            )
