        ...
        if os.path.exists(self.data_file):
            try:
                with open(self.data_file, "r") as f:
                    data = json.load(f)
                    for blocker_id, blocker_data in data.get(
                        "blockers", {}
                    ).items():
                        # Convert actions
                        actions = [
                            BlockerAction(**action)
                            for action in blocker_data.get("actions", [])
                        ]
                        blocker_data["actions"] = actions

                        # Convert metrics
                        metrics_data = blocker_data.get("metrics", {})
                        blocker_data["metrics"] = BlockerMetrics(
                            **metrics_data
                        )

                        # Convert enums
                        blocker_data["priority"] = Priority(
                            blocker_data["priority"]
                        )
                        blocker_data["status"] = BlockerStatus(
                            blocker_data["status"]
                        )
                        blocker_data["risk_level"] = RiskLevel(
                            blocker_data["risk_level"]
                        )

                        self.blockers[blocker_id] = BlockerTracker(
                            **blocker_data
                        )
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical_count": sum(
                ...
                if b.priority == Priority.CRITICAL
            ),
    ...
    def get_overdue_count(self) -> int:
        ...
        for blocker in self.blockers.values():
            if blocker.status not in [
                ...
                try:
                    estimated_date = datetime.datetime.fromisoformat(
                        blocker.estimated_completion
                    ).date()
...
"""
        ...
        for blocker_id, blocker in self.blockers.items():
            ...
            report += (
                f"- **Target Completion:** {blocker.estimated_completion}\n"
            )
            ...
            if blocker.next_steps:
                report += (
                    f"- **Next Steps:** {', '.join(blocker.next_steps[-3:])}\n"
                )
...
def main():
    ...
    tracker.update_action_status(
        ...
        BlockerStatus.COMPLETED,
        actual_hours=1.5,
        notes="Requirements file updated successfully",
    )
    ...
    tracker.add_next_step(
        "VD-001", "Resolve matplotlib version compatibility issue"
    )
