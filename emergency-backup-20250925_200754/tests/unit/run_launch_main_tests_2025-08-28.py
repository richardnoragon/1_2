...
def run_tests():
    ...
    test_command = [
        ...
        "-c",
        "pytest_launch_main_2025-08-28.ini",
        "-v",
        "--tb=short",
        "--strict-markers",
    ...
    if coverage_json_path.exists():
        try:
            ...
            summary["coverage_summary"] = {
                ...
                "branches_covered": coverage_data.get("totals", {}).get(
                    "covered_branches", 0
                ),
                "branches_total": coverage_data.get("totals", {}).get(
                    "num_branches", 0
                ),
            }
        except Exception as e:
    ...
    with open(text_summary_file, "w", encoding="utf-8") as f:
        ...
        if "coverage_summary" in summary:
            ...
            if "total_coverage" in cov_sum:
                ...
                f.write(
                    f"Lines Covered: {cov_sum['lines_covered']}/{cov_sum['lines_total']}\n"
                )
                f.write(
                    f"Branches Covered: {cov_sum['branches_covered']}/{cov_sum['branches_total']}\n\n"
                )
        ...
        f.write(
            "• result_launch_main_summary_2025-08-28.json (Summary JSON)\n"
        )
...
def main():
    ...
    print("=" * 60)
    print(
        f"Test execution completed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
