...
def test_configuration():
    ...
    return {
        "general": {"last_opened_dir": "", "theme": "default"},
        "viewer": {"zoom_factor": 1.0, "default_page": 1, "auto_fit": True},
        "performance": {"cache_size": 10, "prefetch_pages": 2},
    }
...
def setup_test_environment(
    mock_pyqt5, mock_fitz, mock_config_manager, mock_log_config
):
    ...
    class Reporter:
        ...
        def save_results(self, filename):
            with open(filename, "w") as f:
                json.dump(
                    {"test_session": TEST_TIMESTAMP, "results": results},
                    f,
                    indent=2,
                )
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "pdf: PDF processing related tests")
    config.addinivalue_line("markers", "ui: User interface related tests")
    config.addinivalue_line(
...
pytestmark = [pytest.mark.pdf, pytest.mark.unit]
