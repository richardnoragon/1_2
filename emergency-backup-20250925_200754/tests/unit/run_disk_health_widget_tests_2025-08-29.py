...
class DiskHealthWidgetTestRunner:
    ...
    def run_individual_test_file(self, test_file):
        ...
        try:
            ...
            if result.returncode == 0:
                print(
                    f"✓ {test_file} completed successfully ({execution_time:.2f}s)"
                )
            else:
                print(
                    f"✗ {test_file} failed (return code: {result.returncode})"
                )
    ...
    def run_combined_tests(self):
        ...
        test_patterns = [
            str(self.test_directory / pattern) for pattern in self.test_files
        ]
        ...
        try:
            ...
            print(
                f"Combined tests completed (return code: {result.returncode})"
            )
    ...
    def generate_execution_log(self):
        ...
        with open(log_file, "w") as f:
            ...
            for test_file, results in self.test_results.items():
                f.write(f"\n{test_file}:\n")
                f.write(
                    f"  Return Code: {results.get('return_code', 'N/A')}\n"
                )
                f.write(
    ...
    def generate_summary_report(self):
        ...
        summary_file = (
            self.results_directory / self.result_files["summary_report"]
        )
