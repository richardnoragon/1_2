...
class Phase4TestRunner:
    ...
    def __init__(self, base_path: str = None):
        self.base_path = (
            Path(base_path) if base_path else Path(__file__).parent
        )
    ...
    def run_optimized_tests(
        ...
        print(
            f"Parallel Execution: {'Enabled' if max_workers != 1 else 'Disabled'}"
        )
        print(
            f"Flaky Detection: {'Enabled' if enable_flaky_detection else 'Disabled'}"
        )
        print()
        ...
        try:
            ...
            optimization_result = (
                self.resource_optimizer.optimize_test_environment(
                    optimization_level
                )
            )
            ...
            if enable_flaky_detection:
                ...
                flaky_analysis = (
                    self.flaky_detector.generate_flaky_test_report()
                )
                ...
                flaky_test_names = [
                    ft["test_name"]
                    for ft in flaky_analysis.get("flaky_test_details", [])
                ]
                test_files = [
                    tf
                    for tf in test_files
                    if Path(tf).stem not in flaky_test_names
                ]

                if flaky_test_names:
                    print(
                        f"⚠️ Temporarily excluding {len(flaky_test_names)} flaky tests"
                    )
            ...
            if max_workers == 1:
                ...
                execution_result = self._run_parallel_optimized_tests(
            ...
            self.results["recommendations"] = (
                self._generate_phase4_recommendations()
            )
    ...
    def _run_sequential_tests(self, test_files: List[str]) -> Dict:
        ...
        for test_file in test_files:
            ...
            print(
                f"  {test_file}: {'PASS' if test_result.get('success') else 'FAIL'} ({test_duration:.2f}s)"
            )

        results["duration"] = time.time() - start_time
        return results

    def _run_parallel_optimized_tests(
        self, test_files: List[str], max_workers: int = None
    ) -> Dict:
        """Run tests with full Phase 4 optimization"""
        ...
        print(
            f"📋 Execution plan: {len(execution_plan['test_groups'])} parallel groups"
        )
        print(
            f"⏱️ Estimated duration: {execution_plan['estimated_duration']:.2f}s"
        )
        ...
        execution_result.update(
            {
                ...
                "optimization_effectiveness": execution_result.get(
    ...
    def _analyze_performance_improvements(
        ...
        analysis = {
            "execution_type": execution_result.get(
        ...
        if execution_result.get("execution_type") == "parallel_optimized":
            ...
            analysis["optimization_metrics"] = {
                "parallel_efficiency": opt_metrics.get(
            ...
            if speedup > 1:
                ...
                analysis["performance_gains"] = {
                    ...
                    "efficiency_rating": (
                        "excellent"
                        if speedup > 3
                        else "good" if speedup > 2 else "moderate"
                    ),
                }

        # Analyze resource efficiency from optimizer
        if (
            hasattr(self.resource_optimizer, "usage_history")
            and self.resource_optimizer.usage_history
        ):
            recent_usage = self.resource_optimizer.usage_history[-10:]
            avg_memory = sum(m.memory_mb for m in recent_usage) / len(
                recent_usage
            )
            avg_cpu = sum(m.cpu_percent for m in recent_usage) / len(
                recent_usage
            )
            ...
            analysis["resource_efficiency"] = {
                ...
                - (avg_memory / self.resource_optimizer.max_memory_mb * 100),
                "cpu_efficiency": 100
                - (avg_cpu / self.resource_optimizer.max_cpu_percent * 100),
            }
    ...
    def _validate_maintenance_procedures(self) -> Dict:
        ...
        validation_results["procedures_validated"].append(
        ...
        validation_results["validation_status"][
            "reporting_systems"
        ] = self._validate_reporting_systems()

        # Calculate overall maintenance health
        all_validations = list(
            validation_results["validation_status"].values()
        )
        healthy_count = sum(
            1 for v in all_validations if v.get("status") == "healthy"
        )
    ...
    def _validate_optimization_systems(self) -> Dict:
        ...
        try:
            test_schedule = self.scheduler.schedule_optimized_execution(
                ["test_dummy.py"]
            )
            ...
            if test_schedule.get("test_groups"):
                ...
                validation["issues_found"].append(
                    "scheduler_not_generating_groups"
                )
        except Exception as e:
        ...
        try:
            opt_result = self.resource_optimizer.optimize_test_environment(
                "balanced"
            )
            ...
            if opt_result.get("strategies_applied"):
                ...
                validation["issues_found"].append(
                    "optimizer_no_strategies_applied"
                )
        except Exception as e:
    ...
    def _validate_monitoring_systems(self) -> Dict:
        ...
        try:
            ...
            if self.resource_optimizer.usage_history:
                validation["monitoring_capabilities"].append(
                    "resource_monitoring"
                )
            else:
    ...
    def _generate_phase4_recommendations(self) -> List[Dict]:
        ...
        if (
            perf_improvements.get("optimization_metrics", {}).get(
                "speedup_factor", 1
            )
            > 2
        ):
...
"""
    ...
    def _format_recommendations_section(self) -> str:
        ...
        for i, rec in enumerate(recommendations, 1):
            formatted.append(
                f"{i}. **{rec['title']}** ({rec['priority']}) - {rec['description']}"
            )
    ...
    def _print_execution_summary(self):
        ...
        if perf.get("optimization_metrics"):
            ...
            print(
                f"Parallel Efficiency: {opt.get('parallel_efficiency', 0):.1%}"
            )
        ...
        print(
            f"Flaky Tests: {flaky_count} {'detected' if flaky_count > 0 else '(none detected)'}"
        )
        ...
        print(
            "✅ Phase 4 optimization and maintenance validation completed successfully!"
        )
...
def main():
    ...
    try:
        ...
        maintenance_health = results.get("maintenance_validation", {}).get(
            "maintenance_health"
        )
