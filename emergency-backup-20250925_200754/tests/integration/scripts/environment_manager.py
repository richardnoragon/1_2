...
class EnvironmentManager:
    ...
    def load_environment_config(self, env_name: str) -> Dict[str, Any]:
        ...
        if not config_path.exists():
            raise FileNotFoundError(
                f"Configuration not found for environment: {env_name}"
            )
    ...
    def create_environment(self, env_name: str, force: bool = False) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create environment {env_name}: {str(e)}"
            )
            return False

    def _initialize_database(
        self, env_name: str, config: Dict[str, Any], force: bool = False
    ) -> bool:
        """Initialize SQLite database for the environment."""
        db_config = config.get("database", {})
        db_location = db_config.get(
            "location", f"tests/integration/data/test_{env_name}.db"
        )
        ...
        try:
            ...
            if db_config.get("initialization", {}).get(
            ...
            self.logger.error(
                f"Failed to initialize database for {env_name}: {str(e)}"
            )
            return False
    ...
    def _load_sample_database_data(
        self, conn: sqlite3.Connection, env_name: str
    ):
    ...
    def _create_sample_data(
        self, env_name: str, config: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            for i in range(pdf_count):
                ...
                pdf_file = (
                    pdf_dir / f"sample_{i:03d}.txt"
                )  # Using .txt for simplicity
            ...
            self.logger.error(
                f"Failed to create sample data for {env_name}: {str(e)}"
            )
            return False
    ...
    def _create_environment_status(
        self, env_name: str, config: Dict[str, Any]
    ):
        ...
        status_file = (
            self.artifacts_path / env_name / "environment_status.json"
        )
    ...
    def validate_environment(self, env_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Environment validation completed for {env_name}: {validation_results['overall_status']}"
            )
            ...
            self.logger.error(
                f"Environment validation failed for {env_name}: {str(e)}"
            )
    ...
    def _validate_file_system(
        ...
        try:
            ...
            if not env_data_path.exists():
                return {
                    "status": "unhealthy",
                    "error": "Environment data directory not found",
                }
            ...
            if not os.access(env_data_path, os.R_OK | os.W_OK):
                return {
                    "status": "unhealthy",
                    "error": "Insufficient directory permissions",
                }
    ...
    def cleanup_environment(
        self, env_name: str, preserve_config: bool = True
    ) -> bool:
        ...
        try:
            ...
            if not preserve_config:
                ...
                if env_artifacts_path.exists():
                    ...
                    self.logger.info(
                        f"Removed artifacts directory: {env_artifacts_path}"
                    )

            self.logger.info(
                f"Successfully cleaned up environment: {env_name}"
            )
            return True

        except Exception as e:
            self.logger.error(
                f"Failed to clean up environment {env_name}: {str(e)}"
            )
            return False
    ...
    def backup_environment(self, env_name: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create backup for {env_name}: {str(e)}"
            )
            return ""
        ...
        for env_name in self.environments:
            try:
                ...
                environments.append(
                    {
                        "name": env_name,
                        "purpose": config.get("environment", {}).get(
                            "purpose", "Unknown"
                        ),
                        "created": is_created,
                        "metrics": metrics,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RFU Integration Test Environment Manager"
    )
    parser.add_argument(
        "action",
        choices=["create", "validate", "cleanup", "backup", "list"],
    ...
    if args.action == "list":
        ...
        if args.action == "create":
            ...
            success = manager.cleanup_environment(
                args.environment, args.preserve_config
            )
            if success:
                print(
                    f"Successfully cleaned up environment: {args.environment}"
                )
            else:
            ...
            if backup_path:
                ...
                print(
                    f"Failed to create backup for environment: {args.environment}"
                )
                sys.exit(1)
