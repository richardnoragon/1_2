...
Examples:
  ...
  python run_tests.py --performance             # Run performance tests
    ...
    parser.add_argument(
        "--html-report", action="store_true", help="Generate HTML reports"
    )

    parser.add_argument(
        "--json-report", action="store_true", help="Generate JSON reports"
    )
    ...
    try:
        if args.ci_cd:
            ...
            if summary.get("success_rate", 0) < args.fail_under:
                print(
                    f"❌ Coverage below threshold: {summary.get('success_rate', 0):.1f}% < {args.fail_under}%"
                )
                exit_code = 1
            ...
            if summary.get("total_failed", 0) > args.max_failures:
                print(
                    f"❌ Too many failures: {summary.get('total_failed', 0)} > {args.max_failures}"
                )
                exit_code = 1
            ...
            if summary.get("overall_status") != "passed":
                print(
                    f"❌ Overall test status: {summary.get('overall_status')}"
                )
                exit_code = 1
            ...
            if result["status"] == "passed":
                ...
                exit_code = 1
                print(
                    f"❌ {args.suite.title()} tests failed: {result['status']}"
                )
        ...
        if args.verbose or not args.quiet:
            ...
            if output_dir.exists():
                reports = list(output_dir.glob("*.html")) + list(
