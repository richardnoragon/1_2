...
Modern PyQt5 interface for the Content Management System Directory tool,
implemented with the shared StandardWindow architecture.
"""
...
class CMSDWindow(StandardWindow):
    ...
    def __init__(self):
        ...
        super().__init__(
            title="Content Management System Directory", icon_path=self._get_cmsd_icon()
        )
    ...
    def _get_cmsd_icon(self) -> str:
        """Get CMSD icon path."""
        icon_path = os.path.join(os.path.dirname(__file__), "..", "icons", "cmsd.png")
    ...
    def _move_files_right(self) -> None:
        ...
        if hasattr(self, "listView"):
            ...
            if result.success:
                self.show_status_message(f"Copied {len(result.processed_files)} files")
                self._update_directory_views()
            else:
                error_msg = "\n".join(
                    [f"{file}: {error}" for file, error in result.failed_files]
                )
