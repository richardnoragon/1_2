...
from pdf_security_engine import (
    SecuritySettings, DigitalSignature, EncryptionLevel
)
...
class PDFDigitalSignatureDialog(BaseSecurityDialog):
    ...
    def validate_input(self) -> Tuple[bool, str]:
        ...
        if (self.private_key_edit.text() and 
                not os.path.exists(self.private_key_edit.text())):
...
class PDFSecurityInfoDialog(BaseSecurityDialog):
    ...
    def populate_info(self):
        ...
        self.encryption_layout.addRow(
            "Encrypted:", 
            QLabel("Yes" if encrypted else "No")
        )
        ...
        if encrypted:
            ...
            self.encryption_layout.addRow(
                "Password Required:",
                QLabel("Yes" if needs_password else "No")
            )
