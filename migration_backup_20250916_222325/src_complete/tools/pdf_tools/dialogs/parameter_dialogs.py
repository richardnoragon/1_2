...
from PyQt5.QtWidgets import (
    ...
    QProgressBar, QTextEdit, QTabWidget, QWidget, QFrame,
    QScrollArea, QSlider, QSplitter, QTreeWidget, QTreeWidgetItem
)
from PyQt5.QtCore import Qt, pyqtSignal, QThread, QTimer, QSize
from PyQt5.QtGui import QFont, QPixmap, QIcon, QPalette, QColor, QDragEnterEvent, QDropEvent
...
class PDFSignDialog(QDialog):
    ...
    def init_ui(self):
        ...
        output_file_layout = QHBoxLayout()
        self.output_file_edit = QLineEdit()
        self.output_file_edit.setPlaceholderText("Output file will be auto-generated...")
    ...
    def get_sign_options(self) -> Dict[str, Any]:
        ...
        if self.first_page_radio.isChecked():
            ...
            if pages_text:
                try:
                    ...
                    pages = [int(p.strip()) - 1 for p in pages_text.split(',') if p.strip().isdigit()]
                except ValueError:
                    pages = "all"  # fallback
        ...
        options = {
            ...
            'certificate_file': self.cert_file_edit.text().strip() if self.digital_sig_cb.isChecked() else None
        }
