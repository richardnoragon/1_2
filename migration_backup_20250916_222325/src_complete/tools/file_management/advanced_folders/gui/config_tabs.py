...
try:
    from .constants import (Accessibility, Colors, FileTypes, Fonts, Icons,
                            Layout, SearchConfig, Styles, Validation)
except ImportError:
    # Fallback constants
    class Colors:
        PRIMARY_BLUE = "#3498db"
...
class GeneralConfigTab(BaseConfigTab):
    ...
    def _create_basic_info_section(self):
        ...
        self.name_edit.setPlaceholderText("Enter a unique name for this folder configuration")
        ...
        self.description_edit.setPlaceholderText("Optional description for this folder configuration")
    ...
    def _create_directories_section(self):
        ...
        self.directories_list.itemSelectionChanged.connect(self._on_directory_selection_changed)
    ...
    def validate(self):
        ...
        if not name:
            ...
            errors.append(f"Folder name cannot exceed {Validation.MAX_FOLDER_NAME_LENGTH} characters")
        
        # Validate description length
        description = self.description_edit.toPlainText().strip()
        if len(description) > Validation.MAX_DESCRIPTION_LENGTH:
            errors.append(f"Description cannot exceed {Validation.MAX_DESCRIPTION_LENGTH} characters")
    ...
    def set_read_only(self, read_only=True):
        ...
        self.remove_directory_btn.setEnabled(not read_only and bool(self.directories_list.selectedItems()))
...
class SearchConfigTab(BaseConfigTab):
    ...
    def _setup_ui(self):
        ...
        placeholder_label.setStyleSheet(f"color: {Colors.TEXT_SECONDARY}; padding: 50px;")
