...
class SoftwareDeinstaller(MaintenanceToolBase):
    ...
    def _get_common_leftover_paths(self) -> List[str]:
        ...
        if self.platform_support.platform_type.value == "windows":
            ...
            paths.extend([
                ...
                os.path.join(system_paths.get('userprofile', ''), 'Documents'),
                'C:\\ProgramData',
                'C:\\Users\\Public'
            ])
    ...
    def _load_configuration(self):
        ...
        config_file = Path("software_maintenance/config/deinstaller_config.json")
        ...
        if config_file.exists():
            try:
                ...
                self.create_restore_points = config.get('create_restore_points', True)
                self.backup_before_removal = config.get('backup_before_removal', True)
                self.deep_scan_enabled = config.get('deep_scan_enabled', True)
                self.auto_cleanup_leftovers = config.get('auto_cleanup_leftovers', False)
                
                self.log_info("De-installer configuration loaded successfully")
                
            except Exception as e:
                self.log_warning(f"Failed to load de-installer configuration: {e}")
    ...
    def _load_uninstall_history(self):
        ...
        history_file = Path("software_maintenance/config/uninstall_history.json")
            ...
            try:
                ...
                for session_data in history_data:
                    session = UninstallSession(
                        ...
                        successful_removals=session_data['successful_removals'],
                ...
                self.log_info(f"Loaded {len(self.uninstall_history)} uninstall sessions")
    ...
    def _save_uninstall_history(self):
        ...
        try:
            history_data = [session.to_dict() for session in self.uninstall_history]
    ...
    def scan_installed_software(self, include_system: bool = False) -> Dict[str, SoftwareInfo]:
        """
        ...
        try:
            ...
            if success:
                self.detected_software = self.software_detector.detected_software
                self.log_info(f"Found {len(self.detected_software)} installed applications")
    ...
    def analyze_software_for_removal(self, software_name: str) -> Optional[UninstallAnalysis]:
        """
        ...
        try:
            ...
            if estimated_size == 0.0 and software_info.install_location:
                estimated_size = self._calculate_directory_size(software_info.install_location)
            ...
            self.log_info(f"Analysis completed for {software_name}: {complexity} complexity")
            return analysis
    ...
    def _calculate_directory_size(self, directory_path: str) -> float:
        ...
        try:
            ...
            self.log_warning(f"Could not calculate directory size for {directory_path}: {e}")
            return 0.0
            ...
            for hive, base_path in search_paths:
                try:
                    with winreg.OpenKey(hive, base_path) as key:
                        self._search_registry_recursive(key, base_path, software_name, registry_entries, hive)
                except FileNotFoundError:
                    continue
                    
        except Exception as e:
            self.log_warning(f"Error searching registry for {software_name}: {e}")
        
        return registry_entries
    
    def _search_registry_recursive(self, key, path: str, software_name: str, 
                                  results: List[str], hive, max_depth: int = 3):
                    ...
                    try:
                        with winreg.OpenKey(key, subkey_name) as subkey:
                            ...
                            self._search_registry_recursive(
                                subkey, subpath, software_name, results, hive, max_depth - 1
                            )
    ...
    def _find_potential_leftovers(self, software_name: str) -> List[LeftoverItem]:
        ...
        try:
            ...
            self.log_warning(f"Error finding leftovers for {software_name}: {e}")
    ...
    def _create_leftover_item(self, item_path: str) -> Optional[LeftoverItem]:
        ...
        try:
            ...
            self.log_warning(f"Error creating leftover item for {item_path}: {e}")
            return None
    ...
    def _is_safe_to_remove(self, item_path: str) -> bool:
        ...
        try:
            ...
            user_dirs = ['AppData', 'Documents', 'Desktop', '.config', '.local']
    ...
    def _find_dependencies(self, software_name: str) -> List[str]:
        ...
        try:
            ...
            for other_software in self.detected_software.keys():
                if (other_software != software_name and 
                    software_name.lower() in other_software.lower()):
                    dependencies.append(other_software)
                    
        except Exception as e:
            self.log_warning(f"Error finding dependencies for {software_name}: {e}")
        
        return dependencies
    
    def _determine_uninstall_complexity(self, software_info: SoftwareInfo,
                                       registry_entries: List[str],
                                       leftover_files: List[LeftoverItem],
    ...
    def create_pre_removal_backup(self, software_name: str) -> Optional[str]:
        ...
        try:
            if software_name in self.detected_software:
                ...
                if software_info.install_location and Path(software_info.install_location).exists():
                    ...
                    if backup_id:
                        self.log_info(f"Created backup for {software_name}: {backup_id}")
                        return backup_id
                ...
                if software_info.registry_key:
                    ...
                    if backup_id:
                        self.log_info(f"Created registry backup for {software_name}: {backup_id}")
                        return backup_id
    ...
    def uninstall_software(self, software_name: str, 
                          remove_leftovers: bool = None) -> bool:
        """
        ...
        try:
            ...
            if success:
                ...
                if remove_leftovers or (remove_leftovers is None and self.auto_cleanup_leftovers):
                ...
                if backup_id:
                    self.log_info(f"Backup available for restoration: {backup_id}")
    ...
    def _execute_uninstall(self, software_info: SoftwareInfo) -> bool:
        ...
        try:
            ...
            if software_info.uninstall_string:
                return self._run_uninstall_command(software_info.uninstall_string)
            ...
            self.log_warning(f"No uninstall method found for {software_info.name}, attempting manual removal")
            return self._manual_removal(software_info)
            
        except Exception as e:
            self.log_error(f"Error executing uninstall for {software_info.name}: {e}")
            return False
    ...
    def _run_uninstall_command(self, uninstall_string: str) -> bool:
        ...
        try:
            ...
            if '/S' not in args and '/SILENT' not in args and '/quiet' not in args:
                args += ' /S'
    ...
    def _uninstall_via_package_manager(self, software_name: str, package_manager: str) -> bool:
        """Uninstall software via package manager."""
        try:
            if package_manager == 'chocolatey':
                result = subprocess.run(['choco', 'uninstall', software_name, '-y'], 
                                      capture_output=True, timeout=300)
                return result.returncode == 0
            
            elif package_manager == 'winget':
                result = subprocess.run(['winget', 'uninstall', software_name, '--silent'], 
                                      capture_output=True, timeout=300)
                return result.returncode == 0
            
            elif package_manager == 'homebrew':
                result = subprocess.run(['brew', 'uninstall', software_name], 
                                      capture_output=True, timeout=300)
                return result.returncode == 0
            
            elif package_manager == 'apt':
                result = subprocess.run(['sudo', 'apt', 'remove', software_name, '-y'], 
                                      capture_output=True, timeout=300)
                return result.returncode == 0
            
            else:
                self.log_warning(f"Unsupported package manager: {package_manager}")
                return False
        ...
        try:
            ...
            if software_info.install_location and Path(software_info.install_location).exists():
                try:
                    ...
                    self.log_info(f"Removed installation directory: {software_info.install_location}")
                except Exception as e:
                    self.log_error(f"Failed to remove installation directory: {e}")
                    success = False
            
            # Remove registry entries
            if software_info.registry_key and self.platform_support.platform_type.value == "windows":
                try:
                    self._remove_registry_key(software_info.registry_key)
                    self.log_info(f"Removed registry key: {software_info.registry_key}")
                except Exception as e:
    ...
    def _remove_registry_key(self, registry_key: str):
        ...
        try:
            ...
            if registry_key.startswith('HKEY_LOCAL_MACHINE'):
                ...
                raise ValueError(f"Unsupported registry hive in: {registry_key}")
            ...
            for leftover in leftovers:
                if leftover.is_safe_to_remove:
                    try:
                        ...
                        self.log_warning(f"Failed to remove leftover {leftover.path}: {e}")
            
            self.log_info(f"Cleaned up {removed_count} leftover items for {software_name}")
            
        except Exception as e:
            self.log_error(f"Error cleaning up leftovers for {software_name}: {e}")
    
    def uninstall_multiple_software(self, software_list: List[str],
                                   remove_leftovers: bool = None) -> Dict[str, bool]:
        """
        ...
        try:
            for i, software_name in enumerate(software_list):
                ...
                if software_name in self.detected_software:
                    ...
                    if space_before == 0.0 and software_info.install_location:
                        space_before = self._calculate_directory_size(software_info.install_location)
                
                success = self.uninstall_software(software_name, remove_leftovers)
            ...
            self.log_info(f"Batch uninstall completed: {session.successful_removals}/{total_removals} successful")
    ...
    def rollback_removal(self, software_name: str, backup_id: str) -> bool:
        ...
        try:
            ...
            if success:
                self.log_info(f"Successfully rolled back removal of {software_name}")
                return True
            else:
                self.log_error(f"Failed to rollback removal of {software_name}")
                return False
                
        except Exception as e:
            self.log_error(f"Error rolling back removal of {software_name}: {e}")
            return False
    
    def analyze_disk_space_recovery(self, software_list: List[str]) -> Dict[str, float]:
        """
        ...
        try:
            for software_name in software_list:
                if software_name in self.detected_software:
                    ...
                    if estimated_space == 0.0 and software_info.install_location:
                        estimated_space = self._calculate_directory_size(software_info.install_location)
            ...
            self.log_info(f"Disk space analysis completed for {len(software_list)} applications")
    ...
    def get_uninstall_statistics(self) -> Dict[str, any]:
        ...
        total_removals = sum(session.successful_removals for session in self.uninstall_history)
        total_failures = sum(session.failed_removals for session in self.uninstall_history)
        total_space_recovered = sum(session.space_recovered_mb for session in self.uninstall_history)
        
        # Recent activity (last 30 days)
        from datetime import timedelta
        recent_cutoff = datetime.now() - timedelta(days=30)
        recent_sessions = [s for s in self.uninstall_history if s.start_time >= recent_cutoff]
        recent_removals = sum(session.successful_removals for session in recent_sessions)
        recent_space_recovered = sum(session.space_recovered_mb for session in recent_sessions)
    ...
    def execute(self, scan_software: bool = True, analyze_all: bool = False,
                include_system: bool = False, save_report: bool = True) -> bool:
        """
        ...
        try:
            ...
            self.log_info("Software uninstallation analysis completed successfully")
            return True
