...
class YumPackageManager(PackageManager):
    ...
    def is_available(self) -> bool:
        ...
        return shutil.which('yum') is not None or shutil.which('dnf') is not None
...
class CrossPlatformPackageInstaller:
    ...
    def __init__(self, verbose: bool = False):
        ...
        self.logger.info(f"Available package managers: {[pm.name for pm in self.package_managers]}")
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler(f'package_installation_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
            ]
        )
    ...
    def _load_package_definitions(self) -> Dict[str, Dict]:
        ...
        return {
            'visualization': {
                'matplotlib': {
                    'pip': PackageInfo('matplotlib', '3.7.0', 'Python plotting library'),
                    'apt': PackageInfo('python3-matplotlib', description='Matplotlib for Python 3'),
                    'yum': PackageInfo('python3-matplotlib', description='Matplotlib for Python 3'),
                    'homebrew': PackageInfo('matplotlib', description='Python plotting library')
                },
                'numpy': {
                    'pip': PackageInfo('numpy', '1.24.0', 'Numerical computing library'),
                    'apt': PackageInfo('python3-numpy', description='NumPy for Python 3'),
                    'yum': PackageInfo('python3-numpy', description='NumPy for Python 3'),
                    'homebrew': PackageInfo('numpy', description='Numerical computing library')
                },
                'seaborn': {
                    'pip': PackageInfo('seaborn', '0.12.0', 'Statistical data visualization'),
                    'apt': PackageInfo('python3-seaborn', description='Seaborn for Python 3'),
                    'yum': PackageInfo('python3-seaborn', description='Seaborn for Python 3'),
                    'homebrew': PackageInfo('seaborn', description='Statistical data visualization')
                },
                'plotly': {
                    'pip': PackageInfo('plotly', '5.15.0', 'Interactive plotting library'),
                    'apt': PackageInfo('python3-plotly', description='Plotly for Python 3'),
                    'yum': PackageInfo('python3-plotly', description='Plotly for Python 3'),
                    'homebrew': PackageInfo('plotly', description='Interactive plotting library')
                },
                'bokeh': {
                    'pip': PackageInfo('bokeh', '3.0.0', 'Interactive visualization library'),
                    'apt': PackageInfo('python3-bokeh', description='Bokeh for Python 3'),
                    'yum': PackageInfo('python3-bokeh', description='Bokeh for Python 3'),
                    'homebrew': PackageInfo('bokeh', description='Interactive visualization library')
                },
                'dash': {
                    'pip': PackageInfo('dash', '2.10.0', 'Web application framework'),
                    'apt': PackageInfo('python3-dash', description='Dash for Python 3'),
                    'yum': PackageInfo('python3-dash', description='Dash for Python 3'),
                    'homebrew': PackageInfo('dash', description='Web application framework')
                }
            },
            'scientific': {
                'scipy': {
                    'pip': PackageInfo('scipy', '1.10.0', 'Scientific computing library'),
                    'apt': PackageInfo('python3-scipy', description='SciPy for Python 3'),
                    'yum': PackageInfo('python3-scipy', description='SciPy for Python 3'),
                    'homebrew': PackageInfo('scipy', description='Scientific computing library')
                },
                'pandas': {
                    'pip': PackageInfo('pandas', '2.0.0', 'Data analysis library'),
                    'apt': PackageInfo('python3-pandas', description='Pandas for Python 3'),
                    'yum': PackageInfo('python3-pandas', description='Pandas for Python 3'),
                    'homebrew': PackageInfo('pandas', description='Data analysis library')
                },
                'scikit-learn': {
                    'pip': PackageInfo('scikit-learn', '1.3.0', 'Machine learning library'),
                    'apt': PackageInfo('python3-sklearn', description='Scikit-learn for Python 3'),
                    'yum': PackageInfo('python3-scikit-learn', description='Scikit-learn for Python 3'),
                    'homebrew': PackageInfo('scikit-learn', description='Machine learning library')
                }
            },
            'network': {
                'networkx': {
                    'pip': PackageInfo('networkx', '3.0', 'Network analysis library'),
                    'apt': PackageInfo('python3-networkx', description='NetworkX for Python 3'),
                    'yum': PackageInfo('python3-networkx', description='NetworkX for Python 3'),
                    'homebrew': PackageInfo('networkx', description='Network analysis library')
                },
                'igraph': {
                    'pip': PackageInfo('python-igraph', '0.10.0', 'Graph analysis library'),
                    'apt': PackageInfo('python3-igraph', description='IGraph for Python 3'),
                    'yum': PackageInfo('python3-igraph', description='IGraph for Python 3'),
                    'homebrew': PackageInfo('python-igraph', description='Graph analysis library')
                }
            }
        }
    
    def get_package_for_manager(self, package_name: str, manager: PackageManager) -> Optional[PackageInfo]:
    ...
    def install_package(self, package_name: str, preferred_manager: Optional[str] = None) -> InstallationResult:
        ...
        if not target_manager:
            ...
            for manager in self.package_managers:
                package_info = self.get_package_for_manager(package_name, manager)
                if package_info:
                    target_manager = manager
                    break
        ...
        package_info = self.get_package_for_manager(package_name, target_manager)
        if not package_info:
            return InstallationResult(
                package=package_name,
                success=False,
        ...
        if target_manager.check_package_installed(package_info.name):
            installed_version = target_manager.get_installed_version(package_info.name)
            self.logger.info(f"Package {package_name} already installed (version: {installed_version})")
        ...
        if result.success:
            ...
            self.logger.error(f"Failed to install {package_name}: {result.message}")
        
        return result
    
    def install_packages(self, package_names: List[str], 
                        parallel: bool = True, 
                        max_workers: int = 4) -> List[InstallationResult]:
        ...
        if parallel and len(package_names) > 1:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_package = {
                    executor.submit(self.install_package, package_name): package_name
                ...
                for future in concurrent.futures.as_completed(future_to_package):
                    ...
                    try:
                        ...
                        self.logger.error(f"Exception installing {package_name}: {e}")
