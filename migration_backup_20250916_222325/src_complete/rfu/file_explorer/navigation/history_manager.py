...
class HistoryDatabase:
        ...
        try:
            with self.lock:
                ...
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (
                    entry.path, entry.timestamp, entry.access_count,
                    entry.last_accessed, entry.is_favorite, tags_json, entry.notes
                ))
    ...
    def delete_entry(self, path: str) -> bool:
        ...
        try:
            with self.lock:
                conn = sqlite3.connect(self.db_path)
                conn.execute('DELETE FROM navigation_history WHERE path = ?', (path,))
                conn.commit()
                conn.close()
                return True
...
class HistoryManager(QObject):
    ...
    def toggle_favorite(self, path: str) -> bool:
        ...
        if success:
            ...
            self.history_changed.emit()
            self.logger.info(f"Favorite toggled for {path}: {entry.is_favorite}")
        
        return entry.is_favorite
    
    def add_favorite(self, path: str, notes: str = "", tags: List[str] = None) -> bool:
        """
        ...
        if entry is None:
            entry = HistoryEntry(path=path, is_favorite=True, notes=notes, tags=tags)
        else:
    ...
    def _auto_cleanup(self) -> None:
        ...
        deleted_count = self.database.cleanup_old_entries(self.auto_cleanup_days)
        if deleted_count > 0:
            self.history_changed.emit()
