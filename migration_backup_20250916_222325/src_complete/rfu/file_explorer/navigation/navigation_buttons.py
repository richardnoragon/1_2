...
from PyQt5.QtWidgets import (QAction, QApplication, QButtonGroup, QHBoxLayout,
                             QMenu, QToolButton, QToolTip, QWidget)
...
class NavigationState:
    ...
    def __post_init__(self):
        ...
        if self.current_path:
            self.can_go_up = bool(Path(self.current_path).parent != Path(self.current_path))
...
class NavigationButtonGroup(QObject):
    ...
    def _create_back_button(self, parent: QWidget, layout: QHBoxLayout) -> None:
    ...
    def _create_forward_button(self, parent: QWidget, layout: QHBoxLayout) -> None:
    ...
    def _create_home_button(self, parent: QWidget, layout: QHBoxLayout) -> None:
    ...
    def _create_refresh_button(self, parent: QWidget, layout: QHBoxLayout) -> None:
    ...
    def _setup_back_button_menu(self, button: QToolButton) -> None:
        ...
        button.customContextMenuRequested.connect(lambda pos: menu.exec_(button.mapToGlobal(pos)))
    ...
    def _setup_home_button_menu(self, button: QToolButton) -> None:
        ...
        user_home_action.triggered.connect(lambda: self._navigate_to_special_dir('home'))
        menu.addAction(user_home_action)
        
        # Desktop
        desktop_action = QAction("Desktop", menu)
        desktop_action.triggered.connect(lambda: self._navigate_to_special_dir('desktop'))
        menu.addAction(desktop_action)
        
        # Documents
        documents_action = QAction("Documents", menu)
        documents_action.triggered.connect(lambda: self._navigate_to_special_dir('documents'))
    ...
    def _populate_hierarchy_menu(self, menu: QMenu) -> None:
        ...
        for parent_path in parents:
            ...
            action.triggered.connect(lambda checked, p=str(parent_path): self.navigate_to_path.emit(p))
    ...
    def _navigate_to_special_dir(self, dir_type: str) -> None:
        ...
        if dir_type in special_dirs:
            ...
            if target_path.exists():
                ...
                self.logger.warning(f"Special directory not found: {target_path}")
    ...
    def enable_button(self, button_name: str, enabled: bool = True) -> None:
        ...
        if button_name in self.button_widgets:
            ...
            self.button_state_changed.emit(button_name, enabled)
            self.logger.debug(f"Button {button_name} {'enabled' if enabled else 'disabled'}")
    ...
    def trigger_button(self, button_name: str) -> None:
        ...
        """
        if button_name in self.button_widgets and self.get_button_state(button_name):
            button = self.button_widgets[button_name]
            button.click()
            self.logger.debug(f"Button {button_name} triggered programmatically")
    ...
    def get_all_button_states(self) -> Dict[str, bool]:
        ...
        """
        return {name: button.isEnabled() for name, button in self.button_widgets.items()}
