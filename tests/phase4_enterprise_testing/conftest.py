...
class TestMetrics:
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "peak_memory_mb": (
                max(self.memory_usage) if self.memory_usage else 0
            ),
            "avg_memory_mb": (
    ...
    for attempt in range(cleanup_attempts):
        try:
            ...
            if attempt == cleanup_attempts - 1:
                print(
                    f"Warning: Could not clean up temp directory {temp_path}: {e}"
                )
            else:
...
def memory_monitor():
    ...
    if final_stats["growth"] > TEST_CONFIG["MEMORY_LIMIT_MB"]:
        print(
            f"Warning: Memory growth exceeded limit: {final_stats['growth']:.1f}MB"
        )
    ...
    for name, content in structure.items():
        ...
        if isinstance(content, dict):
            ...
            created_paths.update(
                {f"{name}/{k}": v for k, v in sub_paths.items()}
            )
...
def assert_performance_benchmark(
    actual_time: float, benchmark_name: str, tolerance: float = 0.2
):
