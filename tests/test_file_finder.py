...
# MIGRATION UPDATE: File Finder relocated to finder package under src.tools
from src.tools.file_management.finder.file_finder import FileFinder
...
class TestFileFinder(BaseGuiTest):
        ...
        self.assertTrue(
            self.wait_for(
                lambda: all(
                    ...
                    <= os.path.getsize(
                        os.path.join(
                            self.test_dir, self.finder.results_list.item(i).text()
                        )
                    )
                    <= 102400
                    for i in range(self.finder.results_list.count())
    ...
    def test_result_actions(self):
        ...
        self.assertTrue(
            self.wait_for(lambda: os.path.exists(self.app.clipboard().text()))
        )
    ...
    def test_invalid_search(self):
        ...
        self.assertTrue(
            self.verify_status_message(
                self.finder.status_bar, "Please select a search directory"
            )
        )
        ...
        # Verify error message
        self.assertTrue(
            self.verify_status_message(self.finder.status_bar, "Invalid search pattern")
        )
    ...
    def test_search_cancellation(self):
        ...
        self.assertTrue(
            self.verify_status_message(self.finder.status_bar, "Search cancelled")
        )
