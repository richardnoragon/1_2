...
class PerformanceBenchmarkRunner:
    ...
    def run_benchmark(self, test_name: str, analyzer: SizeAnalyzer, 
                     directory: str) -> PerformanceMetrics:
    ...
    def generate_report(self) -> str:
        ...
        files_per_sec = [r.files_per_second for r in self.results if r.files_per_second > 0]
        ...
        if files_per_sec:
            report.append(f"Average Throughput: {statistics.mean(files_per_sec):.1f} files/sec")
        ...
        for result in self.results:
            ...
            report.append(f"  Bandwidth: {result.bytes_per_second/1024/1024:.1f} MB/sec")
...
class TestCoreAnalysisEnginePerformance:
    ...
    def test_small_dataset_performance(self, qapp, benchmark_runner, small_dataset):
        ...
        assert metrics.duration < benchmark_runner.baseline_metrics['max_analysis_time_small']
        assert metrics.files_per_second >= benchmark_runner.baseline_metrics['min_files_per_second']
        assert metrics.memory_usage_mb <= benchmark_runner.baseline_metrics['max_memory_usage_mb']
    ...
    def test_medium_dataset_performance(self, qapp, benchmark_runner, medium_dataset):
        ...
        # Verify performance requirements
        assert metrics.duration < benchmark_runner.baseline_metrics['max_analysis_time_medium']
    ...
    def test_large_dataset_performance(self, qapp, benchmark_runner, large_dataset):
        ...
        # Verify performance requirements
        assert metrics.duration < benchmark_runner.baseline_metrics['max_analysis_time_large']
    ...
    def test_memory_efficiency_benchmark(self, qapp, benchmark_runner):
        ...
        try:
            ...
            for temp_dir, label, expected_count in datasets:
                ...
                memory_per_file = metrics.memory_usage_mb / expected_count if expected_count > 0 else 0
...
class TestCoreAnalysisEngineRegressionBenchmarks:
    ...
    def test_performance_regression_detection(self, qapp):
        ...
        try:
            ...
            assert avg_duration <= max_acceptable_duration, \
                f"Performance regression: {avg_duration:.2f}s > {max_acceptable_duration}s"
            
            assert avg_throughput >= min_acceptable_throughput, \
