...
try:
    ...
    class PaneConfiguration:
        def __init__(
            self, pane_id, pane_type="file_explorer", title="File Explorer"
        ):
...
class MultiPaneFileExplorer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.available_layouts = {
            1: ["single"],  # Single pane: single view only
            2: [
                "horizontal",
                "vertical",
            ],  # Two panes: horizontal/vertical splits
            3: ["horizontal", "vertical", "grid"],  # Three panes: all options
            4: ["horizontal", "vertical", "grid"],  # Four panes: all options
        }
    ...
    def setup_theme_system(self):
        ...
        try:
            if THEME_MANAGER_AVAILABLE and self.config_manager:
                ...
                if ThemeManager:
                    ThemeManager.add_theme_changed_callback(
    ...
    def apply_current_theme(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Applied {self.current_theme} theme to all components"
            )
    ...
    def _update_layout_combo_options(self, layout_combo):
        ...
        self.logger.debug(
            "Updating layout combo: pane_count=%s, "
    ...
    def _setup_single_pane_combo(self, layout_combo):
        ...
        layout_combo.setEnabled(True)  # Keep enabled but non-selectable
        layout_combo.setCurrentIndex(0)  # Set to the only option
        layout_combo.setToolTip("Single view mode - only one layout available")
    ...
    def _restore_layout_selection(self, layout_combo, available, current_text):
        ...
        if self._try_restore_previous_selection(
            layout_combo, available, current_text
        ):
            return
    ...
    def _try_restore_previous_selection(
        self, layout_combo, available, current_text
    ):
        ...
        if internal_name in available:
            ...
            if index >= 0:
                ...
                self.logger.debug(
                    "Restored layout selection: %s", current_text
                )
                return True
        return False
    ...
    def _try_set_current_layout_mode(self, layout_combo, available):
        ...
        if index >= 0:
            layout_combo.setCurrentIndex(index)
            self.logger.debug(
                "Set layout combo to current mode: %s", display_name
            )
        else:
    ...
    def setup_enhanced_toolbar(self, parent_layout):
        ...
        if not EnhancedToolbar:
            ...
            self.logger.info(
                "EnhancedToolbar not available, using simple toolbar"
            )
        ...
        self.enhanced_toolbar.add_action_button(
            "duplicate_finder",
            "Find Duplicates",
            tooltip="Launch Duplicate Finder",
        )
    ...
    def setup_main_content_area(self, parent_layout):
        ...
        try:
            ...
            left_panel = self._setup_left_panel(main_splitter)
            self._setup_center_pane_area(main_splitter)
            right_panel = self._setup_right_panel(main_splitter)

            # Configure splitter layout
            self._configure_splitter_sizes(
                main_splitter, left_panel, right_panel
            )
            self._finalize_main_content_setup(parent_layout, main_splitter)

        except Exception as e:
            self.logger.error(f"Error setting up main content area: {e}")
            self._create_minimal_fallback_layout(parent_layout)

    def _create_main_splitter(self):
        """Create the main horizontal splitter with fallback handling."""
        try:
    ...
    def _setup_left_panel(self, main_splitter):
        """Setup and add left panel to main splitter."""
        self.logger.info("Creating left panel...")
        left_panel = self.create_left_panel()
        if left_panel:
            main_splitter.addWidget(left_panel)
            self.logger.info(
                f"Left panel added with {left_panel.count()} tabs"
            )
        else:
            self.logger.error("Failed to create left panel")
        return left_panel

    def _setup_center_pane_area(self, main_splitter):
        """Setup center pane splitter area."""
        self.logger.info("Creating center pane splitter...")
        self.pane_splitter = self._create_pane_splitter()
        self._verify_pane_splitter()
    ...
    def _create_pane_splitter(self):
        """Create the pane splitter with fallback handling."""
        ...
        try:
            ...
            return self._create_fallback_pane_splitter()

    def _create_fallback_pane_splitter(self):
        """Create pane splitter using fallback method."""
        try:
            pane_splitter = QSplitter()
            pane_splitter.setOrientation(Qt.Horizontal)
    ...
    def _verify_pane_splitter(self):
        """Verify pane splitter was created properly."""
        ...
        self.logger.info(
            f"pane_splitter is None: {self.pane_splitter is None}"
        )

        if self.pane_splitter and hasattr(self.pane_splitter, "isVisible"):
            self.logger.info(
                f"pane_splitter.isVisible(): {self.pane_splitter.isVisible()}"
            )

    def _setup_right_panel(self, main_splitter):
        """Setup and add right panel to main splitter."""
        self.logger.info("Creating right panel...")
        right_panel = self.create_right_panel()
        if right_panel:
            main_splitter.addWidget(right_panel)
            self.logger.info(
                f"Right panel added with {right_panel.count()} tabs"
            )
        else:
            self.logger.warning("No right panel created")
        return right_panel

    def _configure_splitter_sizes(
        self, main_splitter, left_panel, right_panel
    ):
        """Configure splitter sizes based on available panels."""
    ...
    def _finalize_main_content_setup(self, parent_layout, main_splitter):
        """Finalize the main content area setup."""
        ...
        if self.pane_splitter is not None:
            ...
            self.logger.error(
                "CRITICAL: pane_splitter is None at end of setup!"
            )
        self.logger.info("Main content area setup completed successfully")

    def _create_minimal_fallback_layout(self, parent_layout):
        """Create minimal fallback layout when main setup fails."""
        try:
            self.logger.error("Creating minimal fallback layout")
            ...
            self.logger.error(f"Error creating fallback layout: {e}")
            # Last resort - just add an empty widget
            parent_layout.addWidget(QWidget())
    ...
    def create_left_panel(self):
        ...
        try:
            ...
            self.logger.info(
                f"Left panel created with {left_panel.count()} tabs"
            )
            return left_panel
        ...
        try:
            ...
            if QuickPreviewWidget:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to create preview widget: {e}"
                    )
            else:
            ...
            if FilePropertyPanel:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to create properties panel: {e}"
                    )
            else:
            ...
            if tabs_added > 0:
                ...
                self.logger.warning(
                    "No tabs were added to right panel, returning None"
                )
                return None
    ...
    def create_recent_locations_widget(self):
        ...
        recent_tree.itemDoubleClicked.connect(
            self._on_recent_location_activated
        )
        return recent_tree
    ...
    def create_enhanced_tools_widget(self):
        ...
        for category, tools in discovered_tools.items():
            ...
            for tool_info in tools:
                try:
                    ...
                    tool_item.setToolTip(
                        0, f"Double-click to launch {tool_name}"
                    )
        ...
        try:
            tools_tree.itemDoubleClicked.connect(
                self._on_discovered_tool_activated
            )
    ...
    def create_enhanced_bookmark_widget(self):
        ...
        if BookmarkManager:
            try:
                ...
                # Connect signals
                bookmark_tree.itemDoubleClicked.connect(
                ...
                self.logger.error(
                    f"Error creating enhanced bookmark widget: {e}"
                )
    ...
    def create_simple_bookmark_widget(self):
        ...
        bookmark_tree.itemDoubleClicked.connect(
            self._on_simple_bookmark_activated
        )
        return bookmark_tree
    ...
    def create_simple_toolbar(self, parent_layout):
        ...
        self.pane_count_combo.currentTextChanged.connect(
            self.on_pane_count_changed
        )
    ...
    def on_toolbar_action(self, action_id: str):
        ...
        try:
            ...
            self.logger.error(
                f"Error executing toolbar action {action_id}: {e}"
            )
        ...
        try:
            ...
            if tool_name in tool_map:
                ...
                self.statusBar().showMessage(
                    f"Tool not implemented: {tool_name}", 3000
                )
    ...
    def load_configuration(self):
        ...
        if self.config_manager:
            try:
                ...
                try:
                    config = (
                        self.config_manager.get_section("file_explorer") or {}
                    )
                ...
                if THEME_MANAGER_AVAILABLE:
                    try:
                        ...
                        if saved_theme != self.current_theme:
                            ...
                            self.logger.info(
                                f"Loaded and applied theme: {saved_theme}"
                            )
                    except Exception as e:
    ...
    def _add_panes(self, count: int):
        ...
        self.logger.info(
            f"Adding {count} panes. Current pane count: {len(self.panes)}"
        )
        ...
        for _ in range(count):
            try:
                ...
                self.logger.info(
                    f"Created FileExplorerPane {len(self.panes) + 1}"
                )
            except Exception as e:
                ...
                self.logger.info(
                    f"Created fallback pane {len(self.panes) + 1}"
                )
    ...
    def _create_file_explorer_pane(
        self, pane_number: int
    ) -> "FileExplorerPane":
        """Create a proper FileExplorerPane instance."""
        try:
            # Check if we have the proper FileExplorerPane implementation
            if not hasattr(FileExplorerPane, "navigate_to") and not hasattr(
                FileExplorerPane, "set_path"
            ):
                # We have the fallback class, so use the functional fallback pane instead
                self.logger.info(
                    f"Using functional fallback pane for pane {pane_number}"
                )
    ...
    def _create_simple_fallback_pane(self, pane_number: int) -> QWidget:
        ...
        self.logger.info(
            f"Created enhanced fallback pane {pane_number} with theming"
        )
        ...
        try:
            ...
            if system == "Windows":
                ...
                for mount_point in mount_points:
                    if os.path.exists(mount_point) and os.path.ismount(
                        mount_point
                    ):
                        drives.append(
        ...
        try:
            if item and item.parent() is None:  # Top-level item
                ...
                if stored_path:
                    ...
                    for icon in [self.FOLDER_ICON] + list(
                        self.FILE_ICONS.values()
                    ):
                ...
                for item in path.iterdir():
                    try:
                        ...
                        if item.is_dir():
                            ...
                            file_icon = self.FILE_ICONS.get(
                                extension, self.FILE_ICONS["default"]
                            )
                            display_name = f"{file_icon} {item.name}"

                        tree_item = QTreeWidgetItem(
                            [display_name, size_str, type_str, modified_str]
                        )
                        ...
                        if item.is_dir():
                            ...
                            tree_item.setBackground(
                                0, QBrush(QColor(230, 240, 250))
                            )
                            font = tree_item.font(0)
                            font.setBold(True)
                            tree_item.setFont(0, font)
                            tree_item.setForeground(
                                0, QBrush(QColor(25, 25, 112))
                            )  # Midnight blue
                        else:
    ...
    def _update_pane_layout(self):
        ...
        self.logger.info(
            f"_update_pane_layout called with {len(self.panes)} "
        ...
        try:
            if self.pane_splitter is None:
                self.logger.error(
                    "Cannot update layout: pane_splitter is None"
                )
                return
            ...
            if len(valid_panes) != len(self.panes):
                self.logger.warning(
                    f"Removed {len(self.panes) - len(valid_panes)} "
                    f"invalid panes"
                )
            ...
            self.logger.info(
                f"Applying layout: {self.layout_mode} for "
            ...
            try:
                if len(self.panes) == 1 or self.layout_mode == "single":
                    ...
                    self.logger.warning(
                        f"Invalid layout combination: {self.layout_mode} "
                ...
                self.logger.error(
                    f"Error applying {self.layout_mode} layout: "
                    f"{layout_error}"
                )
                layout_applied = False
            ...
            if not layout_applied:
                try:
                    self.logger.warning(
                        "Layout application failed, trying "
                        "horizontal fallback"
                    )
                    self.layout_mode = "horizontal"
                    self._create_horizontal_layout()
                    ...
                    self.logger.error(
                        f"Fallback layout failed: {fallback_error}"
                    )
    ...
    def _safe_add_widget_to_splitter(self, widget):
        ...
        try:
            ...
            if hasattr(self.pane_splitter, "addWidget"):
                ...
                self.logger.warning(
                    "pane_splitter does not support widget addition"
                )
    ...
    def _create_grid_layout(self):
        ...
        if len(self.panes) < 3:
            ...
            self.logger.warning(
                "Grid layout requires at least 3 panes, falling back to horizontal"
            )
            self._create_horizontal_layout()
            return
        ...
        if self.pane_splitter is None:
            self.logger.error(
                "Cannot create grid layout: pane_splitter is None"
            )
            return
        ...
        for row in range(rows):
            try:
                ...
                row_splitter.setChildrenCollapsible(
                    False
                ...
                for pane_idx in range(start_idx, end_idx):
                    if pane_idx < len(self.panes):
                        ...
                        panes_in_row += 1
                        self.logger.debug(
                            f"Added pane {pane_idx + 1} to row {row + 1}"
                        )
                ...
                if panes_in_row > 0:
                    ...
                    self.logger.debug(
                        f"Added row {row + 1} with {panes_in_row} panes"
                    )
                else:
        ...
        if row_splitters:
            ...
            for i, splitter in enumerate(row_splitters):
                if splitter.count() > 0:
                    ...
                    self.logger.debug(
                        f"Set equal column sizes for row {i + 1}: {col_sizes}"
                    )
        ...
        if total_widgets == len(self.panes):
            self.logger.info(
                f"Grid layout created successfully: {len(row_splitters)} rows, {total_widgets} panes"
            )
        else:
            self.logger.warning(
                f"Grid layout incomplete: expected {len(self.panes)} panes, got {total_widgets}"
            )
    ...
    def _create_horizontal_layout(self):
        ...
        self.logger.info(
            f"Creating horizontal layout for {len(self.panes)} " f"panes"
        )
        ...
        if hasattr(self.pane_splitter, "count"):
            ...
            if final_count != len(self.panes):
                self.logger.warning(
                    f"Expected {len(self.panes)} panes in "
    ...
    def _create_vertical_layout(self):
        ...
        self.logger.info(
            f"Creating vertical layout for {len(self.panes)} " f"panes"
        )
            ...
            if hasattr(splitter, "count") and hasattr(splitter, "widget"):
                ...
                for widget in widgets_to_remove:
                    try:
                        ...
                        widget.setParent(None)
                        self.logger.debug(
                            f"Removed widget from splitter: {type(widget).__name__}"
                        )
                    except RuntimeError as e:
                        if WIDGET_DELETED_ERROR in str(e):
                            self.logger.debug(
                                f"Widget already deleted during clear: {e}"
                            )
                        else:
                            raise
                    ...
                    while layout.count():
                        ...
                        if child.widget():
                            try:
                                ...
                                if WIDGET_DELETED_ERROR in str(e):
                                    self.logger.debug(
                                        f"Widget already deleted during layout clear: {e}"
                                    )
                                else:
                                    raise
    ...
    def on_layout_mode_changed(self, text):
        ...
        self.logger.info(
            f"Available layouts for {len(self.panes)} panes: {available}"
        )
        ...
        if mode_valid:
            ...
            self.logger.info(
                f"Layout mode changed from {old_mode} to {new_mode}"
            )
            ...
            try:
                ...
                if hasattr(self, "statusBar"):
                    self.statusBar().showMessage(
                        f"Layout change failed, reverted to {old_display}",
                        3000,
                    )
        else:
            self.logger.warning(
            ...
            if hasattr(self, "statusBar"):
                self.statusBar().showMessage(
                    f"Layout {text} not available for {len(self.panes)} panes",
                    3000,
                )
        ...
        try:
            ...
            if get_enhanced_config_manager:
                ...
                if enhanced_config:
                    ...
                    self.logger.debug(
                        "Configuration saved with enhanced manager"
                    )
                    return
    ...
    def launch_checksum(self):
        ...
        self._launch_tool(
            "Checksum", "src.tools.analysis.check_sum", "ChecksumGUI"
        )
    ...
    def _launch_tool(self, tool_name: str, module_name: str, class_name: str):
        ...
        try:
            ...
            for i, strategy in enumerate(import_strategies, 1):
                try:
                    self.logger.debug(
                        f"Trying import strategy {i} for {tool_name}"
                    )
                    module = strategy()
                    if module:
                        ...
                        if self._validate_tool_class(tool_class, tool_name):
                            ...
                            if tool_instance:
                                self.logger.info(
                                    f"Tool {tool_name} created with strategy {i}"
                                )
                                break
                        else:
                            self.logger.warning(
                                f"Tool class validation failed for {tool_name}"
                            )
            ...
            if tool_instance:
                ...
                if hasattr(self, "statusBar") and self.statusBar():
                    self.statusBar().showMessage(
                        f"{tool_name} launched successfully", 3000
                    )
                ...
                self.logger.error(
                    f"Failed to launch {tool_name}: {last_error}"
                )
    ...
    def _validate_launch_environment(self, tool_name: str) -> bool:
        ...
        try:
            ...
            if not self.isVisible():
                self.logger.warning(
                    f"Parent window not visible, launching {tool_name} anyway"
                )
            ...
            try:
                ...
                if memory_percent > 90:
                    self.logger.warning(
                        f"High memory usage ({memory_percent}%) when launching {tool_name}"
                    )
            except ImportError:
            ...
            self.logger.error(
                f"Environment validation failed for {tool_name}: {e}"
            )
            return False
    ...
    def _validate_tool_class(self, tool_class, tool_name: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(
                    f"{tool_name} constructor parameters: {list(sig.parameters.keys())}"
                )
            except Exception as e:
                self.logger.warning(
                    f"Could not inspect {tool_name} constructor: {e}"
                )

            return True
        except Exception as e:
            self.logger.error(
                f"Tool class validation failed for {tool_name}: {e}"
            )
            return False
    ...
    def _post_launch_configuration(self, tool_instance, tool_name: str):
        ...
        try:
            ...
            if hasattr(tool_instance, "setWindowState"):
                ...
                tool_instance.setWindowState(
                    tool_instance.windowState() & ~Qt.WindowMinimized
                    | Qt.WindowActive
                )

            self.logger.debug(
                f"Post-launch configuration completed for {tool_name}"
            )

        except Exception as e:
            self.logger.warning(
                f"Post-launch configuration failed for {tool_name}: {e}"
            )
    ...
    def new_tab(self):
        """Create a new tab."""
        self.statusBar().showMessage(
            "New tab functionality will be implemented", 2000
        )
    ...
    def open_preferences(self):
        ...
        try:
            if SettingsDialog is not None and SETTINGS_DIALOG_AVAILABLE:
                ...
                self.logger.warning(
                    "Settings dialog not available - module not loaded"
                )
    ...
    def _on_bookmark_activated(self, item, column):
        ...
        try:
            ...
            if (
                path
                and self.panes
                and self.active_pane_index < len(self.panes)
            ):
                active_pane = self.panes[self.active_pane_index]
    ...
    def _create_tool_instance(self, tool_class, tool_name):
        ...
        try:
            ...
            creation_strategies = [
                ...
                lambda: self._try_create_with_window_type(
                    tool_class, tool_name
                ),
            ...
            for i, strategy in enumerate(creation_strategies, 1):
                try:
                    self.logger.debug(
                        f"Trying creation strategy {i} for {tool_name}"
                    )
                    tool_instance = strategy()
                    if tool_instance:
                        self.logger.info(
                            f"Tool {tool_name} created successfully with strategy {i}"
                        )
                        break
                except Exception as e:
                    last_error = e
            ...
            if not tool_instance:
                self.logger.error(
                    f"All creation strategies failed for {tool_name}: {last_error}"
                )
                self._show_user_error(
                    "Tool Launch Error",
            ...
            if (
                hasattr(tool_instance, "setParent")
                and tool_instance.parent() != self
            ):
                try:
            ...
            self.logger.error(
                f"Unexpected error creating tool instance {tool_name}: {e}"
            )
            self._show_user_error(
                "Tool Launch Error",
    ...
    def _configure_tool_instance(self, tool_instance, tool_name):
        ...
        try:
            ...
            self.logger.debug(
                f"Tool instance {tool_name} configured successfully"
            )

        except Exception as e:
            self.logger.warning(
                f"Error configuring tool instance {tool_name}: {e}"
            )
    ...
    def _launch_tool_by_name(self, tool_name):
        ...
        if clean_name in method_mappings:
            try:
                ...
                self.logger.error(
                    f"Error calling method for {clean_name}: {e}"
                )
    ...
    def _create_minimal_tool_fallback(self, tools_tree):
        ...
        try:
            ...
            for tool_name, callback in basic_tools:
                ...
                tool_item.setToolTip(0, f"Click to {tool_name}")
    ...
    def _launch_tool_by_name(self, tool_name):
        ...
        if clean_name in method_mappings:
            try:
                method_mappings[clean_name]()
                self.logger.info(
                    f"Launched tool via method mapping: {clean_name}"
                )
            except Exception as e:
                self.logger.error(
                    f"Error calling method for {clean_name}: {e}"
            ...
            self.logger.warning(
                f"No method mapping found for tool: {clean_name}"
            )
    ...
    def _on_discovered_tool_activated(self, item, column):
        try:
            ...
            if item.parent():  # Only handle leaf items (actual tools)
                ...
                if isinstance(tool_data, dict):
                    ...
                    if tool_name:
                        ...
                        self.logger.warning(
                            "No tool data or name found for activation"
                        )
            else:
            ...
            self._show_user_error(
                "Tool Launch Error", f"Failed to activate tool: {e}"
            )
    ...
    def _discover_tools_from_directory(self):
        ...
        try:
            if not tools_dir:
                self.logger.warning(
                    "No tools directory found, using fallback tools"
                )
        ...
        try:
            ...
            for item in directory.iterdir():
                if (
                    ...
                    tool_info = self._analyze_python_file_for_tool(
                        item, default_icon
                    )
                    if tool_info:
            ...
            for pattern in gui_class_patterns:
                ...
                if matches:
                    ...
                    gui_matches = [
                        m
                        for m in matches
                        if "GUI" in m or "App" in m or "Window" in m
                    ]
                    if gui_matches:
    ...
    def _launch_discovered_tool(self, tool_data):
        ...
        try:
            ...
            for strategy in import_strategies:
                try:
                    self.logger.debug(
                        f"Trying import strategy for {tool_name}"
                    )
                    module = strategy()
                    if module:
                        tool_class = getattr(module, class_name)
    ...
    def _show_directory_info(self):
        ...
        try:
            ...
            if active_pane:
                current_path = getattr(
                    active_pane, "_current_path", Path.home()
                )
    ...
    def _show_bookmark_context_menu(self, position):
        ...
        try:
            ...
            open_action.triggered.connect(
                lambda: self._on_bookmark_activated(item, 0)
            )
            ...
            delete_action.triggered.connect(
                lambda: self._delete_bookmark(item)
            )
    ...
    def _open_bookmark_in_other_pane(self, item):
        ...
        try:
            ...
            if path and len(self.panes) > 1:
                # Find another pane
                other_pane_index = (self.active_pane_index + 1) % len(
                    self.panes
                )
    ...
    def _sync_panes(self):
        ...
        try:
            ...
            self.statusBar().showMessage(
                f"Synchronized panes to: {active_path}", 3000
            )
        ...
        try:
            if len(self.panes) < 2:
                QMessageBox.warning(
                    self,
                    "Compare Error",
                    "Need at least two panes for comparison",
                )
                return
            ...
            if not path1 or not path2:
                QMessageBox.warning(
                    self, "Compare Error", "Both panes must have valid paths"
                )
                return
            ...
            try:
                files1 = set(
                    item.name for item in path1.iterdir() if item.is_file()
                )
                files2 = set(
                    item.name for item in path2.iterdir() if item.is_file()
                )
                ...
                message += (
                    f"Only in {path1.name}: {len(only_in_pane1)} files\n"
                )
                message += (
                    f"Only in {path2.name}: {len(only_in_pane2)} files\n"
                )
                message += f"Common files: {len(files1 & files2)} files"

                if only_in_pane1:
                    message += f"\n\nUnique to {path1.name}:\n" + "\n".join(
                        list(only_in_pane1)[:10]
                    )
                ...
                QMessageBox.warning(
                    self, "Compare Error", f"Error comparing directories: {e}"
                )
    ...
    def _fallback_navigation_back(self, pane):
        ...
        if current_path:
            ...
            if parent_path != current_path and hasattr(
                pane, "navigate_to_path"
            ):
            ...
            for pane in self.panes:
                if hasattr(pane, "_file_list"):
                    ...
                    file_list.customContextMenuRequested.connect(
                        lambda pos, p=pane: self._show_file_context_menu(
                            pos, p
                        )
                    )
    ...
    def _show_quick_filter(self):
        ...
        try:
            ...
            text, ok = QInputDialog.getText(
                self, "Quick Filter", "Filter files by name:"
            )
            if ok and text:
    ...
    def _perform_search(
        self, query, case_sensitive, use_regex, search_subdirs
    ):
        ...
        try:
            ...
            if use_regex:
                ...
                try:
                    ...
                    QMessageBox.warning(
                        self, "Search Error", "Invalid regular expression"
                    )
                    return
            else:
            ...
            QMessageBox.warning(
                self, "Search Error", f"Error during search: {e}"
            )
    ...
    def _apply_filter_to_active_pane(self, filter_text):
        ...
        try:
            ...
            self.statusBar().showMessage(
                f"Filter applied: {filter_text}", 3000
            )
    ...
    def _show_file_context_menu(self, position, pane):
        ...
        try:
            ...
            if item:
                ...
                open_action = QAction("Open", self)
                open_action.triggered.connect(
                    lambda: self._open_selected_file(pane)
                )
                menu.addAction(open_action)
                ...
                delete_action.triggered.connect(
                    lambda: self._delete_selected_files(pane)
                )
                menu.addAction(delete_action)

                properties_action = QAction("Properties", self)
                properties_action.triggered.connect(
                    lambda: self._show_file_properties(pane)
                )
            ...
            new_folder_action.triggered.connect(
                lambda: self._create_new_folder(pane)
            )
    ...
    def _delete_selected_files(self, pane):
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                self.statusBar().showMessage(
                    f"Deleted {len(selected_files)} items", 3000
                )
        ...
        try:
            ...
            if ok and name:
                ...
                try:
                    ...
                    QMessageBox.warning(
                        self, "Create Folder", f"Error creating folder: {e}"
                    )
    ...
    def _show_file_properties(self, pane):
        ...
        try:
            ...
            layout.addRow(
                "Type:", QLabel("Directory" if file_path.is_dir() else "File")
            )
            ...
            try:
                ...
                layout.addRow(
                    "Modified:", QLabel(modified.strftime("%Y-%m-%d %H:%M:%S"))
                )

                created = datetime.fromtimestamp(stat.st_ctime)
                layout.addRow(
                    "Created:", QLabel(created.strftime("%Y-%m-%d %H:%M:%S"))
                )

            except Exception as e:
                layout.addRow(
                    "Error:", QLabel(f"Could not read file stats: {e}")
                )
        ...
        try:
            ...
            if file_path.suffix.lower() in [
                ...
                try:
                    ...
                    QMessageBox.warning(
                        self, "Preview Error", f"Could not preview file: {e}"
                    )
            else:
                QMessageBox.information(
                    self,
                    "Preview",
                    f"Preview not available for {file_path.suffix} files",
                )
    ...
    def _setup_enhanced_shortcuts(self):
        ...
        try:
            ...
            self.shortcut_manager.register_shortcut(
                "Alt+Left", self.go_back, "Go Back"
            )
            self.shortcut_manager.register_shortcut(
                "Alt+Right", self.go_forward, "Go Forward"
            )
            self.shortcut_manager.register_shortcut(
                "Alt+Up", self.go_up, "Go Up"
            )
            self.shortcut_manager.register_shortcut(
                "F5", self.refresh_current_pane, "Refresh"
            )

            # Search and tools
            self.shortcut_manager.register_shortcut(
                "Ctrl+F", self.show_search, "Search"
            )
            self.shortcut_manager.register_shortcut(
                "Ctrl+B", self.bookmark_current_location, "Bookmark"
            )

            # Pane management
            self.shortcut_manager.register_shortcut(
                "Ctrl+1", lambda: self.set_pane_count(1), "1 Pane"
            )
            self.shortcut_manager.register_shortcut(
                "Ctrl+2", lambda: self.set_pane_count(2), "2 Panes"
            )
            self.shortcut_manager.register_shortcut(
                "Ctrl+3", lambda: self.set_pane_count(3), "3 Panes"
            )
            self.shortcut_manager.register_shortcut(
                "Ctrl+4", lambda: self.set_pane_count(4), "4 Panes"
            )

            # Tools
            self.shortcut_manager.register_shortcut(
                "Ctrl+Shift+F", self.launch_file_finder, "File Finder"
            )
            self.shortcut_manager.register_shortcut(
                "Ctrl+Shift+D",
                self.launch_duplicate_finder,
            ...
            self.shortcut_manager.register_shortcut(
                "Ctrl+Shift+S", self.launch_size_analyzer, "Size Analyzer"
            )
    ...
    def launch_copy_move_sync(self):
        ...
        self._launch_tool(
            "Copy/Move/Sync",
            "src.tools.file_operations.copy_move_sync_delete",
    ...
    def launch_disk_usage(self):
        ...
        self._launch_tool(
            "Disk Usage", "src.tools.analysis.disk_usage", "DiskUsageGUI"
        )
    ...
    def launch_file_integrity(self):
        ...
        self._launch_tool(
            "File Integrity",
            "src.tools.security.file_integrity",
            "FileIntegrityGUI",
        )
    ...
    def launch_page_admin(self):
        ...
        self._launch_tool(
            "Page Administration",
            "src.tools.pdf_tools.page_admin",
            "PageAdminGUI",
        )
    ...
    def launch_pdf_conversion(self):
        ...
        self._launch_tool(
            "PDF Conversion",
            "src.tools.pdf_tools.conversion",
            "PDFConversionGUI",
        )
    ...
    def launch_file_transfer(self):
        ...
        self._launch_tool(
            "File Transfer",
            "src.tools.network.file_transfer",
            "FileTransferGUI",
        )
    ...
    def launch_remote_access(self):
        ...
        self._launch_tool(
            "Remote Access",
            "src.tools.network.remote_access",
            "RemoteAccessGUI",
        )
