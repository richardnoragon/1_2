...
class EnvironmentSetup:
    ...
    def check_python_version(self) -> bool:
        ...
        self.log(f"Python {version.major}.{version.minor}.{version.micro} detected. Compatible.", "INFO")
        return True
    ...
    def check_git_repository(self) -> bool:
        ...
        if not git_dir.exists():
            self.log("Not in a git repository. This may affect CI/CD setup.", "WARN")
            return False
    ...
    def setup_virtual_environment(self, force: bool = False) -> bool:
        ...
        if venv_path.exists():
            if force:
                ...
                self.log("Virtual environment already exists. Use --force to recreate.")
                return True
    ...
    def install_dependencies(self) -> bool:
        ...
        if not pip_executable.exists():
            pip_executable = "pip"
            self.log("Virtual environment not found. Using system pip.", "WARN")
        ...
        if requirements_file.exists():
            ...
            self.log("No requirements.txt found. Skipping main dependencies.", "WARN")
        
        # Install test requirements
        test_requirements_file = self.base_path / "requirements-test.txt"
        if test_requirements_file.exists():
            self.run_command([str(pip_executable), "install", "-r", str(test_requirements_file)])
        else:
            self.log("No test requirements found. Installing minimal test dependencies.", "WARN")
    ...
    def setup_environments(self, environments: List[str] = None) -> bool:
        ...
        if not env_manager_path.exists():
            self.log("Environment manager script not found. Skipping environment setup.", "WARN")
            return False
            ...
            for env_name in environments:
                ...
                if success:
                    ...
                    if validation['overall_status'] == 'healthy':
                        ...
                        self.log(f"Environment {env_name} validation failed: {validation}", "WARN")
                else:
                    self.log(f"Failed to create environment: {env_name}", "ERROR")
                    return False
    ...
    def setup_monitoring(self) -> bool:
        ...
        if not config_file.exists():
            self.log("Monitoring config not found. Skipping monitoring setup.", "WARN")
            return False
        ...
        if monitor_script.exists():
            ...
            self.log("Monitoring script not found. Skipping monitoring test.", "WARN")
            return False
    ...
    def setup_ci_cd(self) -> bool:
        ...
        if workflow_file.exists():
            ...
            self.log("Integration test workflow not found. Please ensure it's properly configured.", "WARN")
    ...
    def run_initial_tests(self) -> bool:
        ...
        try:
            test_command = [
                ...
                str(self.base_path),
                "-v", "--tb=short",
                "-m", "smoke or not smoke",  # Run all tests if no smoke tests marked
                "--timeout=60"
            ]
            ...
            if result.returncode == 0:
                ...
                self.log(f"Some tests failed (exit code: {result.returncode}). Setup may need attention.", "WARN")
    ...
    def generate_setup_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "monitoring": {
                "configured": (self.base_path / "configs" / "monitoring_config.yaml").exists(),
                "operational": True
            },
            "ci_cd": {
                "configured": (self.project_root / ".github" / "workflows" / "integration-tests.yml").exists()
            }
        }
    ...
    def setup_all(self, force: bool = False, environments: List[str] = None) -> bool:
        ...
        if success:
            ...
            if os.name == 'nt':
                self.log(f"   {self.project_root / 'venv' / 'Scripts' / 'activate.bat'}")
            else:
                self.log(f"   source {self.project_root / 'venv' / 'bin' / 'activate'}")
            
            self.log("2. Run integration tests:")
            self.log("   pytest tests/integration/ -v")
            
            self.log("3. Start monitoring (optional):")
            self.log("   python tests/integration/monitoring/system_monitor.py start --daemon")
            
            self.log("4. Check environment status:")
            self.log("   python tests/integration/scripts/environment_manager.py list")
            
        else:
            self.log("=" * 60)
            self.log("Setup completed with errors. Please review the log above.")
            self.log("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='RFU Integration Testing Environment Setup')
