...
class SystemMonitor:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.config_path = (
            config_path
            or self.base_path / "configs" / "monitoring_config.yaml"
        )
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "alerting": {
                ...
                "webhook": {"enabled": False, "url": "", "timeout": 30},
            },
            "retention": {"metrics_days": 30, "alerts_days": 90},
        }
    ...
    def start_monitoring(self):
        ...
        self.alert_thread = threading.Thread(
            target=self._alerting_loop, daemon=True
        )
    ...
    def _collect_environment_metrics(self) -> Dict[str, Any]:
        ...
        environments = self.config.get("monitoring", {}).get(
            "environments", []
        )
        env_metrics = {}
        ...
        for env_name in environments:
            try:
                ...
                self.logger.error(
                    f"Error collecting metrics for environment {env_name}: {e}"
                )
        ...
        for env_name in environments:
            try:
                db_path = (
                    ...
                    / f"tests/integration/data/test_{env_name}.db"
                )
                ...
                db_metrics[env_name] = {
                    ...
                    "tables": tables,
                    "total_records": sum(
                        count for count in tables.values() if count >= 0
                    ),
                ...
                self.logger.error(
                    f"Error collecting database metrics for {env_name}: {e}"
                )
    ...
    def _cleanup_old_metrics(self):
        ...
        retention_days = self.config.get("retention", {}).get(
            "metrics_days", 30
        )
        ...
        for metrics_file in self.monitoring_data_path.glob("metrics_*.json"):
            try:
                ...
                if file_date < cutoff_date:
                    metrics_file.unlink()
                    self.logger.debug(
                        f"Cleaned up old metrics file: {metrics_file}"
                    )
    ...
    def _store_alert(self, alert: Dict[str, Any]):
        ...
        daily_alerts = (
            alerts_dir / f"alerts_{datetime.now().strftime('%Y%m%d')}.jsonl"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RFU Integration Test System Monitor"
    )
    parser.add_argument(
        "action",
        choices=["start", "status", "history", "alerts"],
