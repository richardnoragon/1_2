...
class Phase3TestRunner:
    ...
    def run_tests(
        ...
        print("=" * 70)
        print(
            "RFU Phase 3 Advanced Testing - End-to-End & Performance/Security"
        )
        print("=" * 70)
        ...
        for category, test_file in tests_to_run.items():
            ...
            test_result = self._run_single_test(
                category, test_file, verbose, skip_slow
            )
    ...
    def _generate_security_metrics(self):
        ...
        for category in security_categories:
            if category in self.results["test_categories"]:
                ...
                if result["status"] == "PASSED":
                    security_metrics["security_tests_passed"] += result.get(
                        "tests_run", 0
                    )
        ...
        if total_security_tests > 0:
            security_metrics["security_score"] = (
                security_metrics["security_tests_passed"]
    ...
    def _generate_compliance_status(self):
        ...
        week9_10_passed = all(
            self.results["test_categories"].get(category, {}).get("status")
            == "PASSED"
        ...
        compliance["phase3_requirements_met"] = (
            week9_10_passed and week11_12_passed
        )
        ...
        compliance["compliance_percentage"] = (
            sum(compliance_factors) / len(compliance_factors)
        ) * 100
    ...
    def _print_summary(self):
        ...
        for category, result in self.results["test_categories"].items():
            ...
            print(
                f"{category:35} {status:8} ({tests_run:2d} tests, {duration:5.2f}s)"
            )
        ...
        security_metrics = self.results.get("security_metrics", {})
        print(
            f"Security Score: {security_metrics.get('security_score', 0):.1f}%"
        )
        print(
            f"Compliance Level: {security_metrics.get('compliance_level', 'UNKNOWN')}"
        )
