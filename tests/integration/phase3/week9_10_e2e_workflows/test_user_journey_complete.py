...
try:
    from src.config.config_manager import ConfigManager as RFUConfigManager
    from src.log_manager import LogManager as RFULogManager
...
class TestFileOperationsWorkflows:
    ...
    def test_file_catalog_to_metadata_workflow(self):
        ...
        assert image_metadata.name == "ImageMetadata", "Image metadata tool failed"
        ...
        assert metadata_result["status"] == "success", "Metadata extraction failed"
        ...
        success_results = [r for r in catalog_results if r["status"] == "success"]
        assert len(success_results) == len(catalog_results), "Catalog workflow failed"
        assert workflow_time < 30.0, f"Workflow took too long: {workflow_time}s"

        self.test_suite.test_results["file_operations_workflows"][
            "catalog_metadata"
        ] = "PASS"
        self.test_suite.workflow_timings["catalog_metadata_workflow"] = workflow_time
    ...
    def test_duplicate_finder_to_secure_delete_workflow(self):
        ...
        assert (
            duplicate_finder.name == "DuplicateFinder"
        ...
        assert secure_delete.name == "SecureDelete", "Secure delete failed to open"
        ...
        assert os.path.exists(original_file), "Original file was incorrectly deleted"

        workflow_time = time.time() - start_time
        assert workflow_time < 20.0, f"Workflow took too long: {workflow_time}s"

        self.test_suite.test_results["file_operations_workflows"][
            "duplicate_to_delete"
        ] = "PASS"
        self.test_suite.workflow_timings["duplicate_to_delete_workflow"] = workflow_time
    ...
    def test_comprehensive_tab_navigation_workflow(self):
        ...
        for category_name, tool_functions in tool_categories:
            ...
            for tool_function in tool_functions:
                ...
                assert (
                    interaction_result["status"] == "success"
        ...
        self.test_suite.workflow_timings["comprehensive_navigation_workflow"] = (
            workflow_time
        )
...
class TestCrossCategoryWorkflows:
    ...
    def test_complete_security_workflow(self):
        ...
        assert monitor_result["status"] == "success", "Workflow monitoring failed"
        ...
        self.test_suite.workflow_timings["complete_security_workflow"] = workflow_time
    ...
    report = {
        ...
        "workflow_categories": {
            ...
            "hub_navigation_workflows": {
                ...
                "test_count": 1,
                "critical_paths": ["Comprehensive Tab Navigation"],
            },
            "cross_category_workflows": {
                "description": "Complex inter-category tool combinations",
                "test_count": 1,
                "critical_paths": ["Complete Security Workflow"],
            },
        },
