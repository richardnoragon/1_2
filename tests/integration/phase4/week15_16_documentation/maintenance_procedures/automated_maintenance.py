...
class AutomatedMaintenanceSystem:
    ...
    def __init__(self, base_path: str = None, config_path: str = None):
        ...
        self.maintenance_tasks = {
            ...
            "monthly": [self.monthly_comprehensive_review],
        }
    ...
    def run_daily_maintenance(self) -> Dict:
        ...
        failed_tasks = [
            name
            for name, result in results["task_results"].items()
...
def main():
    ...
    parser.add_argument(
        "--task",
        choices=["daily", "weekly", "monthly", "setup"],
        default="daily",
    ...
    try:
        if args.task == "daily":
            ...
            print(
                f"  Baselines updated: {len(baseline_result.get('baselines_updated', []))}"
            )
            print(
                f"  Recommendations: {len(analysis_result.get('recommendations', []))}"
            )
            return 0
            ...
            health_score = review_result.get("system_health_summary", {}).get(
                "avg_daily_health_score", 0
            )
            time_savings = review_result.get("optimization_roi", {}).get(
                "time_savings_hours_per_month", 0
            )
