...
class Phase2TestRunner:
    ...
    def run_tests(self, test_categories=None, verbose=False, generate_html=True):
        ...
        if test_categories:
            tests_to_run = {k: v for k, v in ALL_TESTS.items() if k in test_categories}
        else:
    ...
    def _generate_performance_metrics(self):
        ...
        if category_times:
            ...
            total_tests = sum(r.get('tests_run', 0) for r in self.results['test_categories'].values())
            if total_tests > 0:
                metrics['average_test_time'] = metrics['total_execution_time'] / total_tests
    ...
    def _print_summary(self):
        ...
        for category, result in self.results['test_categories'].items():
            ...
            print(f"{category:30} {status:8} ({tests_run:2d} tests, {duration:5.2f}s)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='RFU Phase 2 Integration Test Runner')
