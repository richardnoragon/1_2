...
class NetworkComplexTestRunner:
    ...
    def run_test_suite(self, test_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            json_result_file = self.results_dir / f'result_{Path(test_file).stem}.json'
            json_results = {}
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "test_files_failed": sum(1 for r in all_results if not r["success"]),
    ...
    def generate_comprehensive_report(self, test_summary: Dict[str, Any]) -> str:
...
"""
    ...
    def update_utilities_overview(self, test_summary: Dict[str, Any]) -> bool:
        ...
        try:
            overview_file = self.project_root / "tests" / "unit" / "utilities-overview.md"
            ...
            if network_section_start == -1:
                print("⚠️  Network Complex Module section not found in utilities-overview.md")
                return False
...
"""
            ...
            if next_section_start == -1:
                ...
                updated_content = content[:network_section_start] + updated_section
            else:
                # Replace the section
                updated_content = content[:network_section_start] + updated_section + content[next_section_start:]
    ...
    def save_results(self, test_summary: Dict[str, Any]) -> None:
        ...
        # Save JSON summary
        summary_file = self.results_dir / f"network_complex_test_summary_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.json"
        with open(summary_file, 'w') as f:
            json.dump(test_summary, f, indent=2, default=str)
        
        # Generate and save comprehensive report
        report_content = self.generate_comprehensive_report(test_summary)
        report_file = self.reports_dir / f"network_complex_comprehensive_report_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.md"
...
def main():
    ...
    try:
        ...
        print("=" * 80)
        print(f"✅ Test Files Passed: {test_summary['test_files_passed']}/{test_summary['test_files_run']}")
        print(f"✅ Individual Tests Passed: {test_summary['total_passed']}/{test_summary['total_tests']}")
        print(f"📈 Overall Pass Rate: {test_summary['pass_rate']:.1f}%")
        print(f"⏱️  Total Duration: {test_summary['total_duration']:.2f} seconds")
