...
def test_original_analyzer():
    ...
    try:
        ...
        for i in range(10):
            ...
            print(f"  Operation {i+1}: {'+' if memory_change > 0 else ''}{memory_change:.3f} MB")
...
def main():
    ...
    if original_results and optimized_results:
        ...
        print(f"  Total Memory Change: {original_results['total_memory_change_mb']:.3f} MB")
        print(f"  Average per Operation: {original_results['avg_memory_change_mb']:.3f} MB")
        
        print(f"\nOptimized Analyzer:")
        print(f"  Total Memory Change: {optimized_results['total_memory_change_mb']:.3f} MB")
        print(f"  Average per Operation: {optimized_results['avg_memory_change_mb']:.3f} MB")
        
        # Calculate improvement
        memory_improvement = original_results['total_memory_change_mb'] - optimized_results['total_memory_change_mb']
        avg_improvement = original_results['avg_memory_change_mb'] - optimized_results['avg_memory_change_mb']
        
        if original_results['total_memory_change_mb'] > 0:
            improvement_percent = (memory_improvement / original_results['total_memory_change_mb']) * 100
        else:
            improvement_percent = 0
        
        print(f"\nIMPROVEMENT ANALYSIS:")
        print(f"  Memory Reduction: {memory_improvement:.3f} MB ({improvement_percent:.1f}%)")
        ...
        if optimized_results['avg_memory_change_mb'] < 0.5:
            print(f"  ✓ Avg memory per operation < 0.5 MB: {optimized_results['avg_memory_change_mb']:.3f} MB")
            criteria_met += 1
        else:
            print(f"  ✗ Avg memory per operation >= 0.5 MB: {optimized_results['avg_memory_change_mb']:.3f} MB")
        ...
        if optimized_results['total_memory_change_mb'] < 5.0:
            print(f"  ✓ Total memory increase < 5 MB: {optimized_results['total_memory_change_mb']:.3f} MB")
            criteria_met += 1
        else:
            print(f"  ✗ Total memory increase >= 5 MB: {optimized_results['total_memory_change_mb']:.3f} MB")
        
        # Criterion 4: Consistent operation success
        successful_ops = sum(1 for op in optimized_results['operations'] if op['success'])
