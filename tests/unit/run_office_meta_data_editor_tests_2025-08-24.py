...
def parse_test_results():
    ...
    json_report_path = os.path.join(
        TEST_CONFIG["output_dir"], TEST_CONFIG["reports"]["json"]
    )
    ...
    coverage_json_path = os.path.join(
        TEST_CONFIG["output_dir"], TEST_CONFIG["reports"]["coverage_json"]
    )
    ...
    if os.path.exists(coverage_json_path):
        try:
            ...
            print(
                f"  ✓ Parsed coverage report: {results_summary['coverage']['coverage_percent']:.1f}%"
            )
...
def generate_summary_report(test_results, results_summary):
    ...
    summary_path = os.path.join(
        TEST_CONFIG["output_dir"], TEST_CONFIG["reports"]["summary"]
    )
    ...
    try:
        with open(summary_path, "w") as f:
            ...
            if test_results:
                f.write(
                    f"Execution Time: {test_results['execution_time']:.2f} seconds\n"
                )
