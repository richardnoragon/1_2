...
class Phase6ConcurrentUserSimulation:
    ...
    def create_shared_dataset(self, dataset_id: str, size: str = "medium") -> str:
        ...
        for i in range(file_count):
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"Shared file {i} - dataset {dataset_id} - timestamp: {datetime.now()}\n"
                )
        ...
        print(
            f"Created shared dataset {dataset_id} with {total_files} files at: {test_dir}"
        )

        return test_dir

    def simulate_user_session(
        self, user_id: int, dataset_path: str, operations_count: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_results["successful_operations"] > 0:
                successful_ops = [
                    op for op in user_results["operations"] if op["success"]
                ]
                user_results["avg_operation_time"] = sum(
                    op["duration_sec"] for op in successful_ops
                ) / len(successful_ops)
    ...
    def run_concurrent_simulation(
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
                ...
                for future in as_completed(future_to_user):
                    ...
                    try:
                        ...
                        print(
                            f"User {user_id:2d}: {result['successful_operations']}/{operations_per_user} ops "
            ...
            successful_user_results = [
                r for r in user_results if r["successful_operations"] > 0
            ]
            if successful_user_results:
                avg_user_operation_time = sum(
                    r["avg_operation_time"] for r in successful_user_results
                ) / len(successful_user_results)
            else:
            ...
            return {
                ...
                "performance_analysis": {
                    ...
                    "concurrent_users_completed": len(
                        [r for r in user_results if "session_error" not in r]
                    ),
                },
    ...
    def generate_concurrent_test_report(self, results: Dict[str, Any]) -> str:
        ...
        successful_users = [
            ...
            if "session_error" not in u and u["successful_operations"] > 0
        ]
        ...
        if (
            threshold["threshold_met"]
            and performance["overall_success_rate_percent"] >= 95
        ):
            ...
            recommendation = (
                "Memory compliant but concurrent performance needs improvement"
            )
        else:
