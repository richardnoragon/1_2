...
def mock_standard_window():
    ...
    # Provide lightweight stand-ins for EnhancedEditor import side effects.
    class _StubEnhancedEditor:  # noqa: D401 - simple stub class
        """Minimal replacement for the real EnhancedEditor class."""

        def __init__(self, *args, **kwargs):  # pragma: no cover - simple stub
            pass

    class _StubTextEditor:  # pragma: no cover - minimal type placeholder
        pass

    stub_package = types.ModuleType("enhanced_editor")
    stub_package.EnhancedEditor = _StubEnhancedEditor
    stub_package.TextEditor = _StubTextEditor
    stub_package.__all__ = ["EnhancedEditor", "TextEditor"]
    stub_package.__path__ = []  # Mark as package so relative imports succeed

    stub_module = types.ModuleType("enhanced_editor_module")
    stub_module.EnhancedEditor = _StubEnhancedEditor
    stub_module.TextEditor = _StubTextEditor
    ...
    previous_flag = os.environ.get("RFU_SKIP_ENHANCED_EDITOR_IMPORT")
    os.environ["RFU_SKIP_ENHANCED_EDITOR_IMPORT"] = "1"
    ...
    try:
        ...
        if previous_flag is None:
            os.environ.pop("RFU_SKIP_ENHANCED_EDITOR_IMPORT", None)
        else:
            os.environ["RFU_SKIP_ENHANCED_EDITOR_IMPORT"] = previous_flag
...
class TestSecureDeleteGUI:
    ...
    def test_init_with_standard_window_available(
        self, mock_pyqt5, mock_standard_window
    ):
    ...
    def test_setup_menu_callbacks(self, secure_delete_gui):
        ...
        secure_delete_gui.menu_manager.register_callback.assert_has_calls(
            expected_calls
        )
    ...
    @patch(f"{SECURE_DELETE_MODULE}.QFileDialog")
    def test_select_files_success(
        self, mock_file_dialog, secure_delete_gui, temp_test_files
    ):
    ...
    @patch(f"{SECURE_DELETE_MODULE}.QFileDialog")
    def test_select_folder_success(
        self, mock_file_dialog, secure_delete_gui, temp_test_dir
    ):
    ...
    @patch(f"{SECURE_DELETE_MODULE}.QMessageBox")
    ...
    @patch(f"{SECURE_DELETE_MODULE}.QMessageBox")
    def test_secure_delete_confirmation_cancelled(
        self, mock_message_box, secure_delete_gui
    ):
        """Test secure delete when confirmation is cancelled."""
        # Setup
        secure_delete_gui.selected_files = ["test_file.txt"]
        secure_delete_gui.method_combo = MagicMock()
        secure_delete_gui.method_combo.currentText.return_value = (
    ...
    @patch(f"{SECURE_DELETE_MODULE}.threading.Thread")
    @patch(f"{SECURE_DELETE_MODULE}.QMessageBox")
    def test_secure_delete_confirmation_accepted(
        self, mock_message_box, mock_thread, secure_delete_gui
    ):
    ...
    @patch(f"{SECURE_DELETE_MODULE}.threading.Thread")
    @patch(f"{SECURE_DELETE_MODULE}.QMessageBox")
    def test_secure_delete_various_methods(
        self, mock_message_box, mock_thread, secure_delete_gui
    ):
            ...
            with patch("builtins.print") as mock_print:
                with patch("sys.exit") as mock_exit:
                    ...
                    try:
                        ...
                        module_key = SECURE_DELETE_MODULE
                        if module_key in sys.modules:
                            importlib.reload(sys.modules[module_key])
                    except SystemExit:
                        pass

    @patch(f"{SECURE_DELETE_MODULE}.QApplication")
    @patch(f"{SECURE_DELETE_MODULE}.SecureDeleteGUI")
...
class TestSecureDeleteIntegration:
    ...
    def test_full_workflow_file_selection_to_deletion(
        self, mock_pyqt5, temp_test_files
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                # Mock file dialog
                with patch(f"{SECURE_DELETE_MODULE}.QFileDialog") as mock_dialog:
                    ...
                    with patch(f"{SECURE_DELETE_MODULE}.QMessageBox") as mock_msg:
    ...
    def test_error_handling_edge_cases(self, secure_delete_gui):
        ...
        with patch(f"{SECURE_DELETE_MODULE}.QMessageBox") as mock_msg:
            ...
            try:
                ...
                pytest.fail(
                    f"secure_delete should handle invalid paths gracefully: {e}"
                )
...
class TestSecureDeletePerformance:
    ...
    def test_large_file_list_performance(self, secure_delete_gui):
        ...
        assert (
            duration < 1.0
        ), "clear_selection should complete quickly even with large file lists"
