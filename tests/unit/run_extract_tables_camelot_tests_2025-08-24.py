...
class TestRunner:
    ...
    def __init__(self):
        ...
        # File paths
        self.test_file = (
            self.test_dir / f"test_extract_tables_camelot_{self.timestamp}.py"
        )
        self.config_file = (
            self.test_dir
            / f"pytest_extract_tables_camelot_{self.timestamp}.ini"
        )
        self.conftest_file = (
            self.test_dir
            / f"conftest_extract_tables_camelot_{self.timestamp}.py"
        )

        # Output files
        self.html_report = (
            self.test_dir
            / f"result_extract_tables_camelot_{self.timestamp}.html"
        )
        self.json_report = (
            self.test_dir
            / f"result_extract_tables_camelot_{self.timestamp}.json"
        )
        self.junit_report = (
            self.test_dir
            / f"result_extract_tables_camelot_{self.timestamp}_junit.xml"
        )
        self.coverage_html = (
            self.test_dir
            / f"result_extract_tables_camelot_coverage_{self.timestamp}"
        )
        self.coverage_json = (
            self.test_dir
            / f"result_extract_tables_camelot_coverage_{self.timestamp}.json"
        )
        self.execution_summary = (
            self.test_dir
            / f"result_extract_tables_camelot_execution_summary_{self.timestamp}.txt"
        )
...
"""
    ...
    def generate_test_report_summary(self):
        ...
        summary_file = (
            self.test_dir
            / f"result_extract_tables_camelot_project_completion_{self.timestamp}.json"
        )
        try:
