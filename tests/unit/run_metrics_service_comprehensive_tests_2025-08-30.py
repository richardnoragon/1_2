...
def setup_environment():
    ...
    sys.path.insert(
        0,
        r"c:\Users\richardi\1_2\src\utilities\network\network_connectivity_complex\core",
    )
...
def generate_summary_report(success, execution_time):
    ...
    summary_file = os.path.join(
        OUTPUT_DIR, f"result_metrics_service_summary_2025-08-30.json"
    )
    ...
    markdown_file = os.path.join(
        OUTPUT_DIR, f"result_metrics_service_summary_2025-08-30.md"
    )
...
def validate_outputs():
    ...
    coverage_dir = os.path.join(
        OUTPUT_DIR, "coverage_metrics_service_2025-08-30"
    )
...
def main():
    ...
    try:
        ...
        if success and outputs_valid:
            ...
            print(
                f"❌ Output validation: {'PASSED' if outputs_valid else 'FAILED'}"
            )
            return_code = 1
