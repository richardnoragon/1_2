...
class PlatformUtilsTestRunner:
    ...
    def __init__(self):
        ...
        self.test_file = (
            self.base_dir / f"test_platform_utils_{self.test_date}.py"
        )
        self.config_file = (
            self.base_dir / f"pytest_platform_utils_{self.test_date}.ini"
        )
        ...
        self.coverage_html_dir = (
            self.base_dir / f"{self.results_prefix}_coverage"
        )
        self.coverage_json = (
            self.base_dir / f"{self.results_prefix}_coverage.json"
        )
        self.summary_report = (
            self.base_dir / f"{self.results_prefix}_summary.json"
        )
                ...
                try:
                    with open(self.json_report, "r") as f:
                        ...
                        test_results["test_summary"] = {
                            ...
                            "errors": json_data.get("summary", {}).get(
                                "error", 0
                            ),
                ...
                try:
                    with open(self.coverage_json, "r") as f:
                        ...
                        test_results["coverage_summary"] = {
                            ...
                            "missing_statements": coverage_data.get(
                                "totals", {}
                            ).get("missing_lines", 0),
    ...
    def generate_summary_report(self, test_results: Dict[str, Any]) -> None:
        ...
        summary = {
            "test_execution_summary": {
                ...
                "execution_timestamp": test_results["execution_info"][
                    "timestamp"
                ],
        ...
        if "test_summary" in test_results:
            ...
            summary["test_results"]["pass_rate"] = (
                ...
                / max(test_results["test_summary"]["total_tests"], 1)
                * 100
            )
        ...
        if "coverage_parse_error" in test_results:
            summary["coverage_parse_error"] = test_results[
        ...
        print(
            f"Timestamp: {summary['test_execution_summary']['execution_timestamp']}"
        )
        print(
            f"Execution Time: {summary['test_execution_summary']['total_execution_time']}"
        )
        print(
            f"Overall Success: {summary['test_execution_summary']['overall_success']}"
        )
        ...
        if "coverage_analysis" in summary:
            print(f"\nCoverage Analysis:")
            print(
                f"  Coverage: {summary['coverage_analysis']['coverage_percent']:.1f}%"
            )
            print(
                f"  Total Statements: {summary['coverage_analysis']['total_statements']}"
            )
            print(
                f"  Covered Lines: {summary['coverage_analysis']['covered_lines']}"
            )
            print(
                f"  Missing Statements: {summary['coverage_analysis']['missing_statements']}"
            )
