...
def generate_execution_summary(test_results_file, coverage_file, output_file):
    ...
    if os.path.exists(test_results_file):
        try:
            ...
            summary["test_results"] = {
                "error": f"Could not read test results: {e}"
            }
    ...
    if os.path.exists(coverage_file):
        try:
            ...
            summary["coverage_results"] = {
                "error": f"Could not read coverage results: {e}"
            }
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        if (
            "test_results" in summary
            and "error" not in summary["test_results"]
        ):
        ...
        if (
            "coverage_results" in summary
            and "error" not in summary["coverage_results"]
        ):
...
def main():
    ...
    test_command = [
        ...
        "pytest",
        "-c",
        "pytest_convert_to_docx_2025-08-24.ini",
    ...
    try:
        ...
        if (
            ...
            results = summary["test_results"]
            print(
                f"Test Summary: {results['passed']} passed, {results['failed']} failed, {results['skipped']} skipped"
            )
