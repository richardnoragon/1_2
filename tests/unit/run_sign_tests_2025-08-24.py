...
class SignTestRunner:
    ...
    def __init__(self):
        ...
        self.requirements_file = self.test_dir / f"requirements_test_sign_{self.test_date}.txt"
        
        # Output files
        self.html_report = self.reports_dir / f"result_sign_{self.test_date}.html"
        self.json_report = self.reports_dir / f"result_sign_{self.test_date}.json"
        self.junit_report = self.reports_dir / f"result_sign_{self.test_date}_junit.xml"
        self.coverage_html = self.reports_dir / f"result_sign_coverage_{self.test_date}"
        self.coverage_json = self.reports_dir / f"result_sign_coverage_{self.test_date}.json"
        self.execution_summary = self.reports_dir / f"result_sign_execution_summary_{self.test_date}.txt"
    ...
    def check_dependencies(self):
        ...
        try:
            ...
            dependencies = [
                'pytest', 'pytest_html', 'pytest_cov', 
                'pikepdf', 'fitz', 'PIL', 'OpenSSL'
            ]
    ...
    def run_tests(self, install_deps=False, verbose=True):
        ...
        if not self.check_dependencies():
            if install_deps:
                ...
                print("Dependencies missing. Use --install-deps to install them.")
                return False
    ...
    def generate_execution_summary(self, result, duration):
        ...
        # Write JSON summary
        json_summary_file = self.reports_dir / f"result_sign_summary_{self.test_date}.json"
