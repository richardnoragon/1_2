...
def mock_qt_widgets():
    ...
    with patch('highlight.QApplication') as mock_app, \
        ...
        mock_dialog.getOpenFileName = Mock(return_value=('test.pdf', 'PDF Files (*.pdf)'))
        ...
        'pages': [
            {
                ...
                'lines': ['First page content with test data', 'Another line here']
            },
            {
                'number': 1,
                'text': 'Second page has more test information\nFinal line of document',
                'lines': ['Second page has more test information', 'Final line of document']
            }
        ],
        'metadata': {
...
def test_command_args():
    ...
    return {
        'file_highlight': [
            'highlight.py', '-i', '/test/file.pdf', '-s', 'test', '-a', 'Highlight'
        ],
        'file_redact': [
            'highlight.py', '-i', '/test/file.pdf', '-s', 'sensitive', '-a', 'Redact'
        ],
        'folder_recursive': [
            'highlight.py', '-i', '/test/folder', '-s', 'test', '-r', 'true'
        ],
        'remove_highlights': [
            'highlight.py', '-i', '/test/file.pdf', '-a', 'Remove'
        ]
    }
...
def color_test_cases():
    ...
    return {
        ...
        'unknown': {'rgb': (1, 1, 0), 'name': 'invalid_color'}  # Should default to yellow
    }
