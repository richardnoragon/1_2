        ...
        try:
            ...
            for package_import, package_name in packages_to_check:
                try:
                    ...
                    self.logger.warning(
                        f"⚠ {package_name} not found - some features may be limited"
                    )
            ...
            for path in self.test_dir.glob(pattern):
                if path.is_file():
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Could not remove directory {path}: {e}"
                        )
    ...
    def run_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            execution_info = {
                ...
                "memory_peak": (
                    max(
                        memory_before.get("rss", 0), memory_after.get("rss", 0)
                    )
                    if "error" not in memory_before
                    and "error" not in memory_after
                    else 0
                ),
            }

            self.logger.info(
                f"Test execution completed in {execution_info['execution_time']:.2f} seconds"
            )
    ...
    def analyze_results(
        ...
        if analysis.get("coverage_analysis", {}).get("line_coverage", 0) < 90:
            analysis["recommendations"].append(
                "Consider increasing test coverage to above 90%"
            )

        if execution_info.get("execution_time", 0) > 120:
            analysis["recommendations"].append(
                "Test execution time is high - consider optimizing slow tests"
            )

        if execution_info.get("memory_peak", 0) > 100 * 1024 * 1024:  # 100MB
            analysis["recommendations"].append(
                "High memory usage detected - review memory-intensive tests"
            )
...
"""
        ...
        if not analysis.get("recommendations"):
            summary_text += (
                "- No specific recommendations - tests executed successfully\n"
            )
...
"""
        ...
        # Save text summary
        text_summary_path = (
            self.test_dir / f"result_{self.base_name}_summary.txt"
        )
    ...
    def run(self):
        ...
        try:
            ...
            self.logger.info(
                f"Total workflow execution time: {total_time:.2f} seconds"
            )
