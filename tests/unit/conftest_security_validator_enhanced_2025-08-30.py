...
class EnhancedTestDataGenerator:
    ...
    def get_cached_or_generate(self, cache_key: str, generator_func, *args, **kwargs):
    ...
    def _generate_stress_test_domains(self, count: int) -> List[str]:
        ...
        tlds = ['.com', '.org', '.net', '.edu', '.gov', '.mil', '.int', '.io', '.ai', '.co']
        domains = []
    ...
    if hasattr(session.config, '_test_execution_data'):
        ...
        if execution_times:
            execution_data['session_statistics'] = {
                ...
                'slow_tests': [
                    test for test in session.config._test_execution_data
                    if test['execution_time_milliseconds'] > ENHANCED_TEST_CONFIG['performance_threshold_ms']
                ]
            }
        
        # Save to file
        output_file = Path(__file__).parent / 'result_security_validator_2025-08-30_session_data.json'
