...
def mock_pyqt5():
    ...
    with patch.dict(
        ...
        yield {"widgets": mock_widgets, "core": mock_core, "gui": mock_gui}
...
def mock_clipboard():
    ...
    with patch(
        "src.tools.security.simple_password_generator.QApplication"
    ) as mock_qapp:
...
class TestSimplePasswordGeneratorGUI:
    ...
    def test_generate_password_success(self, mock_pyqt5, password_generator_gui):
        ...
        with patch(
            "src.tools.security.simple_password_generator.secrets"
        ) as mock_secrets:
            ...
            password_generator_gui.password_display.setText.assert_called_once_with(
                "A" * 16
            )

    def test_generate_password_empty_charset_warning(
        self, mock_pyqt5, password_generator_gui
    ):
    ...
    def test_generate_password_various_lengths(
        ...
        for length in lengths:
            ...
            with patch(
                ...
                password_generator_gui.password_display.setText.assert_called_with(
                    "X" * length
                )

    def test_generate_multiple_passwords_success(
        self, mock_pyqt5, password_generator_gui
    ):
        ...
        with patch(
            ...
            password_generator_gui.multiple_display.setText.assert_called_once_with(
                expected_passwords
            )

    def test_generate_multiple_passwords_empty_charset_warning(
        self, mock_pyqt5, password_generator_gui
    ):
    ...
    def test_copy_password_success(
        self, mock_pyqt5, password_generator_gui, mock_clipboard
    ):
    ...
    def test_character_set_all_combinations(self, password_generator_gui):
        ...
        for upper, lower, numbers, symbols in combinations:
            ...
            if not any([upper, lower, numbers, symbols]):
                assert (
                    chars == ""
                ), f"Expected empty for combination {(upper, lower, numbers, symbols)}"
            else:
                if upper:
                    assert any(c in string.ascii_uppercase for c in chars)
                if lower:
...
class TestSimplePasswordGeneratorSecurity:
    ...
    def test_password_randomness_distribution(self, password_generator_gui):
        ...
        # Setup mocks
        password_generator_gui.length_spin = MagicMock()
        password_generator_gui.length_spin.value.return_value = (
            100  # Large enough for distribution testing
        )
        password_generator_gui.password_display = MagicMock()
        password_generator_gui.get_character_set = MagicMock()
        password_generator_gui.get_character_set.return_value = (
        ...
        with patch("src.tools.security.simple_password_generator.secrets", secrets):
            ...
            generated_password = (
                password_generator_gui.password_display.setText.call_args[0][0]
            )
            ...
            assert (
                len(unique_chars) > 1
            ), "Password should contain multiple different characters"
...
class TestSimplePasswordGeneratorIntegration:
    ...
    def test_complete_password_generation_workflow(
        self, mock_pyqt5, password_generator_gui, mock_clipboard
    ):
    ...
    def test_multiple_password_generation_workflow(
        self, mock_pyqt5, password_generator_gui
    ):
...
class TestSimplePasswordGeneratorPerformance:
    ...
    def test_multiple_password_generation_performance(self, password_generator_gui):
        ...
        assert (
            duration < 2.0
        ), f"Multiple password generation took too long: {duration:.3f}s"
...
class TestDataValidator:
    ...
    def validate_password_format(password, expected_length, allowed_chars):
        ...
        if len(password) != expected_length:
            return (
                False,
                f"Length mismatch: expected {expected_length}, got {len(password)}",
            )
    ...
    def validate_multiple_passwords(passwords_text, expected_count, expected_length):
        ...
        if len(passwords) != expected_count:
            return (
                False,
                f"Count mismatch: expected {expected_count}, got {len(passwords)}",
            )
        ...
        for i, password in enumerate(passwords):
            if len(password) != expected_length:
                return (
                    False,
                    f"Password {i+1} length mismatch: expected {expected_length}, got {len(password)}",
                )
