...
def run_enhanced_ocr_tests():
    ...
    cmd = [
        ...
        "-c",
        "pytest_ocr_enhanced_complete_2025-09-01.ini",
        "--verbose",
        "--tb=short",
        "--show-capture=no",
        "--durations=10",
...
def generate_test_summary(exit_code):
    ...
    if html_report.exists():
        print(
            f"   📋 HTML Report: {html_report.name} ({html_report.stat().st_size} bytes)"
        )
    else:
    ...
    if json_report.exists():
        print(
            f"   📊 JSON Report: {json_report.name} ({json_report.stat().st_size} bytes)"
        )
    else:
