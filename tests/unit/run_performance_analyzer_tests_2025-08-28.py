...
def run_tests():
    ...
    print(f"Starting Performance Analyzer Test Suite - {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80)
    ...
    try:
        ...
        json_report_file = current_dir / "result_performance_analyzer_json_report_2025-08-28.json"
        ...
        coverage_file = current_dir / "result_performance_analyzer_coverage_2025-08-28.json"
        ...
        summary_file = current_dir / "result_performance_analyzer_execution_summary_2025-08-28.json"
        with open(summary_file, 'w') as f:
            json.dump(summary, f, indent=2)
        
        print("=" * 80)
        print(f"Test execution completed - {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        if summary.get('test_results'):
            ...
            print(f"Tests: {tr['total_tests']} total, {tr['passed']} passed, {tr['failed']} failed")
        
        if summary.get('coverage'):
            cov = summary['coverage']
            print(f"Coverage: {cov['total_coverage']:.1f}% ({cov['lines_covered']}/{cov['total_lines']} lines)")
        
        print("\nGenerated Reports:")
        print(f"- HTML Report: result_performance_analyzer_html_report_2025-08-28.html")
        print(f"- JSON Report: result_performance_analyzer_json_report_2025-08-28.json")
        print(f"- Coverage HTML: result_performance_analyzer_coverage_2025-08-28/")
        print(f"- Coverage JSON: result_performance_analyzer_coverage_2025-08-28.json")
        print(f"- Execution Summary: result_performance_analyzer_execution_summary_2025-08-28.json")
