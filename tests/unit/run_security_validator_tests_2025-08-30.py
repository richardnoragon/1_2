...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            "result_security_validator_test_execution_2025-08-30.log"
        ),
    ],
)
...
class SecurityValidatorTestRunner:
    ...
    def __init__(self):
        ...
        logger.info(
            f"SecurityValidator Test Runner initialized at {self.start_time}"
        )
    ...
    def build_pytest_command(self, args):
        ...
        # Output reports
        if args.html:
            cmd.extend(["--html", self.html_report, "--self-contained-html"])

        if args.json:
            cmd.extend(
                ["--json-report", "--json-report-file", self.json_report]
            )

        if args.junit:
            cmd.extend(["--junit-xml", self.junit_report])
    ...
    def run_tests(self, args):
        ...
        try:
            ...
            logger.error(
                f"Test execution timed out after {args.timeout} seconds"
            )
            return {
                "returncode": -1,
    ...
    def cleanup_old_results(self):
        ...
        for pattern in patterns:
            for file_path in self.test_dir.glob(pattern):
                if (
                    file_path.name != self.html_report
                    and file_path.name != self.json_report
                    and file_path.name != self.junit_report
                ):
                    try:
                        file_path.unlink()
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Test execution completed in {total_duration:.2f} seconds"
        )
        ...
        if test_result["success"] and output_validation:
            ...
            logger.error(
                "❌ Test execution failed or output validation failed"
            )
            sys.exit(1)
