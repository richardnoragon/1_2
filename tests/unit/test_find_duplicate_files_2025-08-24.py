...
try:
    ...
    pytest.skip(
        f"Cannot import find_duplicate_files module: {e}", allow_module_level=True
    )
...
class TestDuplicateFinderApp:
    ...
    def duplicate_finder_app(self, app):
        ...
        with patch(
            "src.tools.analysis.duplicate_finder.find_duplicate_files.STANDARD_WINDOW_AVAILABLE",
            False,
        ):
    ...
    def test_select_directory_success(self, duplicate_finder_app):
        ...
        with patch(
            "PyQt5.QtWidgets.QFileDialog.getExistingDirectory", return_value=test_dir
        ):
    ...
    def test_prepare_scan(self, duplicate_finder_app):
        ...
        assert (
            ...
            in duplicate_finder_app.results_list.item(0).text()
        )
    ...
    def test_display_results_no_duplicates(self, duplicate_finder_app):
        ...
        assert duplicate_finder_app.results_list.count() == 1
        assert (
            "No duplicates found." in duplicate_finder_app.results_list.item(0).text()
        )
...
class TestEdgeCases:
    ...
    def test_scan_with_permission_errors(self, duplicate_finder_app):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                duplicate_finder_app, "_get_file_hash", side_effect=mock_get_hash
            ):
                duplicates = duplicate_finder_app._scan_for_duplicates()
