...
class Phase8MetricsGenerator:
    ...
    def ensure_results_directory(self):
        ...
        (self.results_dir / "execution_reports").mkdir(parents=True, exist_ok=True)
    ...
    def calculate_line_coverage(self) -> Dict[str, Any]:
        ...
        for suite_file, suite_info in self.test_suites.items():
            ...
            line_coverage_results[suite_file] = {
                ...
                "coverage_percentage": result.get("line_coverage_percent", 0.0),
                "missing_lines": result.get("missing_lines", []),
                "quality_assessment": self.assess_line_coverage_quality(result)
            }
        
        # Calculate overall line coverage
        total_lines = sum(r["total_lines"] for r in line_coverage_results.values())
        covered_lines = sum(r["covered_lines"] for r in line_coverage_results.values())
        overall_percentage = (covered_lines / total_lines * 100) if total_lines > 0 else 0.0
        ...
        return {
            ...
            "quality_gates": {
                ...
                "achieved_quality": "EXCELLENT" if overall_percentage >= 95.0 else 
                                  "GOOD" if overall_percentage >= 80.0 else "NEEDS_IMPROVEMENT"
            }
        }
    ...
    def calculate_branch_coverage(self) -> Dict[str, Any]:
        ...
        total_branches = sum(r["total_branches"] for r in branch_coverage_data.values())
        covered_branches = sum(r["covered_branches"] for r in branch_coverage_data.values())
        overall_percentage = (covered_branches / total_branches * 100) if total_branches > 0 else 0.0
    ...
    def calculate_function_coverage(self) -> Dict[str, Any]:
        ...
        total_functions = sum(r["total_functions"] for r in function_coverage_data.values())
        tested_functions = sum(r["tested_functions"] for r in function_coverage_data.values())
        overall_percentage = (tested_functions / total_functions * 100) if total_functions > 0 else 0.0
    ...
    def calculate_class_coverage(self) -> Dict[str, Any]:
        ...
        total_classes = sum(r["total_classes"] for r in class_coverage_data.values())
        tested_classes = sum(r["tested_classes"] for r in class_coverage_data.values())
        overall_percentage = (tested_classes / total_classes * 100) if total_classes > 0 else 0.0
    ...
    def calculate_module_coverage(self) -> Dict[str, Any]:
        ...
        total_modules = sum(r["total_modules"] for r in module_coverage_data.values())
        tested_modules = sum(r["tested_modules"] for r in module_coverage_data.values())
        overall_percentage = (tested_modules / total_modules * 100) if total_modules > 0 else 0.0
    ...
    def assess_line_coverage_quality(self, coverage_result: Dict[str, Any]) -> str:
    ...
    def analyze_test_execution_times(self) -> Dict[str, Any]:
        ...
        total_execution_time = sum(e["total_execution_time"] for e in execution_times.values())
    ...
    def save_metrics_to_files(self, coverage_metrics: Dict[str, Any], 
        ...
        coverage_file = self.results_dir / "coverage" / f"coverage_metrics_{timestamp}.json"
        with open(coverage_file, 'w') as f:
            json.dump(coverage_metrics, f, indent=2, default=str)
        
        # Save performance metrics
        performance_file = self.results_dir / "performance" / f"performance_metrics_{timestamp}.json"
        with open(performance_file, 'w') as f:
            json.dump(performance_metrics, f, indent=2, default=str)
        
        # Save quality metrics
        quality_file = self.results_dir / "performance" / f"quality_metrics_{timestamp}.json"
...
def main():
    ...
    try:
        ...
        # Key metrics summary
        print("\nKey Metrics Summary:")
        print(f"  Overall Line Coverage: {coverage_metrics['coverage_types']['line_coverage']['overall_coverage_percentage']:.1f}%")
        print(f"  Overall Function Coverage: {coverage_metrics['coverage_types']['function_coverage']['overall_function_coverage']:.1f}%")
        print(f"  Total Execution Time: {performance_metrics['execution_times']['overall_execution_time']:.2f}s")
        print(f"  Security Test Success Rate: {quality_metrics['security_validation']['security_test_results']['security_test_success_rate']:.1f}%")
