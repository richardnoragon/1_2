...
def generate_execution_summary(success, execution_time):
    ...
    summary = {
        ...
        "generated_files": {
            ...
            "execution_summary": get_output_filename(
    ...
    summary_json_file = test_dir / get_output_filename(
    ...
    summary_text_file = test_dir / get_output_filename(
        "execution_summary", "txt"
    )
    with open(summary_text_file, "w") as f:
        ...
        f.write(f"=" * 50 + "\n\n")
        f.write(
            f"Execution Timestamp: {summary['test_execution']['timestamp']}\n"
        )
        f.write(
            f"Target Module: {summary['test_execution']['target_module']}\n"
        )
...
def validate_output_files():
    ...
    expected_dirs = [get_output_filename("coverage")]
