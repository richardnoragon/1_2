...
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/custom-properties">
    ...
    def test_read_ole_metadata(self, app, sample_files):
        ...
        assert "error" in metadata
        assert (
            "OLE metadata extraction requires additional libraries" in metadata["error"]
        )
...
class TestOfficeMetaDataEditorGUI:
    ...
    with patch(
        "src.tools.metadata.office_metadata.office_meta_data_editor.StandardWindow"
    ):
    ...
    def test_setup_menu_callbacks(self, window):
        ...
        calls = [
            call[0][0] for call in window.menu_manager.register_callback.call_args_list
        ]
        expected_callbacks = [
            "new_metadata_session",
            "save_file",
            "open_file",
            "export_data",
            "import_data",
    ...
    def test_browse_folder_non_recursive(
        self, mock_isfile, mock_listdir, mock_dialog, window
    ):
        ...
        expected_files = ["/test/folder/file1.docx", "/test/folder/file3.xlsx"]
    ...
    def test_start_operation_read(self, window):
        ...
        with patch.object(window, "worker", None):
            with patch(
                "src.tools.metadata.office_metadata.office_meta_data_editor.OfficeMetadataWorker"
    ...
    def test_start_operation_write(self, window):
        ...
        with patch.object(window, "worker", None):
            with patch(
                ...
                mock_worker_class.assert_called_with(
                    window.selected_files, "write", updates
                )
    ...
    def test_add_result_docx(self, window):
        ...
        metadata = {"file_info": {"size": 1024, "modified": "2025-08-24T10:00:00"}}
    ...
    def test_add_result_pdf(self, window):
        ...
        metadata = {
            "file_info": {"size": 2048000, "modified": "2025-08-24T10:00:00"}  # 2MB
        }
    ...
    def test_add_result_unknown_type(self, window):
        ...
        metadata = {"file_info": {"size": 512, "modified": "invalid-date"}}
    ...
    def test_export_metadata_text(self, mock_file, mock_dialog, window):
        ...
        window.current_metadata = {"/test/file1.docx": {"title": "Test Document 1"}}
    ...
    @patch(
        ...
        read_data='{"selected_files": ["test.docx"], "recursive_search": true}',
    )
    @patch("json.load")
    ...
    def test_close_event_with_worker(self, window):
        ...
        with patch.object(
            window.__class__.__bases__[0], "closeEvent"
        ) as mock_super_close:
...
class TestMainFunction:
    ...
    @patch("src.tools.metadata.office_metadata.office_meta_data_editor.sys.exit")
    @patch("src.tools.metadata.office_metadata.office_meta_data_editor.QApplication")
    ...
    with patch(
        "src.tools.metadata.office_metadata.office_meta_data_editor.OfficeMetaDataEditorGUI"
    ) as mock_gui:
        mock_window = Mock()
        mock_gui.return_value = mock_window

        # Import and call main function
        from src.tools.metadata.office_metadata.office_meta_data_editor import main
    ...
    def test_ooxml_metadata_malformed_xml(self, app):
        ...
        try:
            ...
            with zipfile.ZipFile(docx_path, "w") as zf:
                # Write malformed XML
                zf.writestr(
                    "docProps/core.xml", "<?xml version='1.0'?><invalid>malformed"
                )
