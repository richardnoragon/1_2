...
class TestUtilityWindow:
    ...
    def test_utility_window_init_with_pyqt5(
        self, mock_qmainwindow, mock_parent_hub, mock_utility_widget
    ):
        ...
        assert (
            window.title == "Test Tool"
        )  # This would be set by the mocked QMainWindow

    @patch("rfu.hub.PYQT5_AVAILABLE", False)
    def test_utility_window_init_without_pyqt5(
        self, mock_parent_hub, mock_utility_widget
    ):
    ...
    @patch("rfu.hub.PYQT5_AVAILABLE", True)
    @patch("rfu.hub.QMainWindow")
    def test_clone_menu_bar_success(
        self, mock_qmainwindow, mock_parent_hub, mock_utility_widget
    ):
    ...
    @patch("rfu.hub.PYQT5_AVAILABLE", True)
    @patch("rfu.hub.QMainWindow")
    def test_clone_menu_bar_exception(
        self, mock_qmainwindow, mock_parent_hub, mock_utility_widget
    ):
    ...
    @patch("rfu.hub.PYQT5_AVAILABLE", True)
    @patch("rfu.hub.QMainWindow")
    def test_register_delegated_callbacks(
        self, mock_qmainwindow, mock_parent_hub, mock_utility_widget
    ):
...
class TestRFUHub:
    ...
    def test_hub_init_with_pyqt5(self, mock_qmainwindow, mock_get_config, mock_get_log):
        ...
        with patch.object(RFUHub, "_setup_gui"), patch.object(
            RFUHub, "_setup_hub_integration"
        ):
            hub = RFUHub()
    ...
    def test_setup_gui(self, mock_qmainwindow, mock_get_config, mock_get_log):
        ...
        with patch("rfu.hub.SimpleMenuManager") as mock_menu_manager, patch.object(
            RFUHub, "_setup_menu_callbacks"
        ), patch.object(RFUHub, "_get_application_icon"), patch.object(
            RFUHub, "create_analysis_tab"
        ), patch.object(
            RFUHub, "create_file_operations_tab"
        ), patch.object(
            RFUHub, "create_metadata_tab"
        ), patch.object(
            RFUHub, "create_network_tab"
        ), patch.object(
            RFUHub, "create_pdf_tools_tab"
        ), patch.object(
            RFUHub, "create_privacy_tab"
        ), patch.object(
            RFUHub, "create_security_tab"
        ), patch.object(
            RFUHub, "create_system_tab"
        ), patch.object(
            RFUHub, "create_logs_tab"
        ):
    ...
    def test_register_tool_success(self):
        ...
        with patch("rfu.hub.get_log_manager"), patch(
    ...
    def test_unregister_tool_exception(self):
        ...
        with patch("rfu.hub.get_log_manager"), patch(
            ...
            with patch.object(
                hub, "registered_tools", side_effect=Exception("Test error")
            ):
                result = hub.unregister_tool("test_tool")
    ...
    def test_update_tool_progress(self):
        ...
        with patch("rfu.hub.get_log_manager"), patch(
            ...
            assert (
                hub.tool_status["test_tool"]["current_operation"] == "Processing files"
            )
    ...
    def test_show_with_pyqt5(self):
        ...
        with patch("rfu.hub.get_log_manager"), patch(
            "rfu.hub.get_config_manager"
        ), patch("rfu.hub.PYQT5_AVAILABLE", True), patch("rfu.hub.QMainWindow"):

            hub = RFUHub()

            with patch.object(hub, "_setup_gui"), patch.object(
                hub, "_setup_hub_integration"
    ...
    def test_show_command_line_interface(self):
        ...
        with patch("rfu.hub.get_log_manager"), patch(
            "rfu.hub.get_config_manager"
        ), patch("rfu.hub.PYQT5_AVAILABLE", False), patch(
            "builtins.print"
        ) as mock_print:
            ...
            assert any(
                "RICHARD'S FILE UTILITIES - COMMAND LINE MODE" in msg
    ...
    def test_get_application_icon_exists(self):
        ...
        with patch("rfu.hub.get_log_manager"), patch(
            "rfu.hub.get_config_manager"
        ), patch("rfu.hub.PYQT5_AVAILABLE", True), patch("rfu.hub.QMainWindow"), patch(
...
class TestRFUHubToolOpening:
    ...
    def test_open_tool_methods_success(self, hub_instance):
        ...
        tool_methods = [
            (
                "open_file_catalog",
                "..utilities.file_operations.file_catalog",
                "FileCatalogGUI",
            ),
            (
                "open_file_touch",
                "..utilities.file_operations.file_touch",
                "FileTouchGUI",
            ),
            (
                "open_file_splitter",
                "..utilities.file_operations.file_splitter",
                "FileSplitJoinGUI",
            ),
            (
                "open_secure_delete",
                "..utilities.file_operations.secure_delete",
                "SecureDeleteGUI",
            ),
            (
                "open_compression_tools",
                "..utilities.file_operations.compression",
                "CompressionGUI",
            ),
            (
                "open_duplicate_finder",
                "..utilities.file_operations.duplicate_finder",
                "DuplicateFinderGUI",
            ),
        ]
    ...
    def test_open_tool_methods_import_error(self, hub_instance):
        ...
        if hasattr(hub_instance, method_name):
            with patch(
                "rfu.hub.FileCatalogGUI", side_effect=ImportError("Module not found")
            ):
                method = getattr(hub_instance, method_name)
                method()
...
class TestRFUHubLogManagement:
    ...
    def test_load_recent_logs_no_entries(self, hub_instance):
        ...
        hub_instance.log_display.setText.assert_called_with(
    ...
    def test_load_recent_logs_exception(self, hub_instance):
        ...
        hub_instance.logger.handlers[0].get_recent_logs.side_effect = Exception(
            "Test error"
        )
    ...
    def test_save_logs_success(self, hub_instance):
        ...
        with patch("rfu.hub.QFileDialog") as mock_dialog, patch(
    ...
    def test_save_logs_exception(self, hub_instance):
        ...
        with patch("rfu.hub.QFileDialog") as mock_dialog, patch(
            "builtins.open", create=True, side_effect=Exception("Test error")
        ):
...
class TestRFUHubButtonCreation:
    ...
    def test_create_styled_tool_button_primary(self, mock_button, hub_instance):
        ...
        with patch.object(hub_instance, "_setup_gui"), patch.object(
            hub_instance, "_setup_hub_integration"
        ):

            button = hub_instance._create_styled_tool_button(
                "Test Button", "Test tooltip", mock_callback, primary=True
            )
    ...
    def test_create_styled_tool_button_secondary(self, mock_button, hub_instance):
        ...
        with patch.object(hub_instance, "_setup_gui"), patch.object(
            ...
            button = hub_instance._create_styled_tool_button(
                "Test Button", "Test tooltip", mock_callback, primary=False
            )
    ...
    def test_create_styled_tool_button_no_pyqt5(self, hub_instance):
        ...
        button = hub_instance._create_styled_tool_button(
            "Test Button", "Test tooltip", Mock(), primary=True
        )
