...
class ExtractTextTestRunner:
    ...
    def __init__(self):
        ...
        # Result file paths
        self.result_files = {
            'html_report': self.test_dir / f"result_extract_text_{self.date_suffix}.html",
            'json_report': self.test_dir / f"result_extract_text_{self.date_suffix}.json",
            'coverage_html': self.test_dir / f"result_extract_text_coverage_{self.date_suffix}",
            'coverage_json': self.test_dir / f"result_extract_text_coverage_{self.date_suffix}.json",
            'junit_xml': self.test_dir / f"result_extract_text_{self.date_suffix}.xml",
            'summary_txt': self.test_dir / f"result_extract_text_summary_{self.date_suffix}.txt",
            'execution_log': self.test_dir / f"result_extract_text_execution_{self.date_suffix}.log"
        }
    ...
    def run_tests(self):
        ...
        cmd = [
            ...
            "--durations=10",
            "-m", "not gui"  # Skip GUI tests by default unless display available
        ]
    ...
    def generate_summary_report(self):
        ...
        if self.result_files['json_report'].exists():
            try:
                ...
                for test in tests:
                    ...
                    elif 'edge' in test_name or 'stress' in test_name or 'concurrent' in test_name:
                        summary_data['test_categories']['edge_case_tests'] += 1
                    elif 'error' in test_name or 'exception' in test_name or 'failure' in test_name:
                        summary_data['test_categories']['error_handling_tests'] += 1
        ...
        with open(self.result_files['summary_txt'], 'w') as f:
            ...
            f.write(f"Target path: {summary_data['test_execution']['target_path']}\n")
            ...
            if summary_data['results']:
                ...
                if any(categories.values()):
                    ...
                    f.write("-" * 40 + "\n")
                    f.write(f"Function tests: {categories['function_tests']}\n")
                    f.write(f"GUI tests: {categories['gui_tests']}\n")
                    f.write(f"Edge case tests: {categories['edge_case_tests']}\n")
                    f.write(f"Error handling tests: {categories['error_handling_tests']}\n\n")
        ...
        summary_json_path = self.test_dir / f"result_extract_text_summary_{self.date_suffix}.json"
    ...
    def print_results_summary(self, summary_data):
        ...
        if summary_data['results']:
            ...
            categories = summary_data['test_categories']
            if any(categories.values()):
                print(f"📋 Categories: {categories['function_tests']} function, {categories['gui_tests']} GUI, {categories['edge_case_tests']} edge cases, {categories['error_handling_tests']} error handling")
            ...
            if summary_data['coverage']:
                ...
                if 'extract_text_coverage' in coverage:
                    ext_cov = coverage['extract_text_coverage']
                    print(f"🎯 extract_text.py: {ext_cov['percent']:.1f}% ({ext_cov['covered']}/{ext_cov['lines']} lines)")
