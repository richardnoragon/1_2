...
def setup_environment():
    ...
    if not (cwd / "tests" / "unit").exists():
        print("Error: tests/unit directory not found. Please run from project root.")
        sys.exit(1)
...
def generate_summary_report(results_dir, success, stdout, stderr):
    ...
    json_results_file = results_dir / f"result_browser_detector_{date_str}_results.json"
    if json_results_file.exists():
        try:
            with open(json_results_file, 'r') as f:
                json_data = json.load(f)
                
            summary_data["test_results"]["total_tests"] = json_data.get("summary", {}).get("total", 0)
            summary_data["test_results"]["passed"] = json_data.get("summary", {}).get("passed", 0)
            summary_data["test_results"]["failed"] = json_data.get("summary", {}).get("failed", 0)
            summary_data["test_results"]["skipped"] = json_data.get("summary", {}).get("skipped", 0)
            summary_data["test_results"]["errors"] = json_data.get("summary", {}).get("error", 0)
            
            if "duration" in json_data:
                summary_data["test_execution"]["execution_time"] = f"{json_data['duration']:.2f} seconds"
        
        except Exception as e:
            print(f"Warning: Could not parse JSON results: {e}")
    
    # Try to parse coverage results if available
    coverage_json_file = results_dir / f"result_browser_detector_{date_str}_coverage.json"
    if coverage_json_file.exists():
        try:
            ...
            if "totals" in coverage_data:
                ...
                summary_data["coverage"]["total_statements"] = totals.get("num_statements", 0)
                summary_data["coverage"]["missing_statements"] = totals.get("missing_lines", 0)
                summary_data["coverage"]["line_coverage"] = totals.get("percent_covered", 0)
        
        except Exception as e:
            print(f"Warning: Could not parse coverage data: {e}")
    
    # Save comprehensive summary
    summary_file = results_dir / f"result_browser_detector_{date_str}_summary.json"
...
"""
    ...
    markdown_file = results_dir / f"result_browser_detector_{date_str}_documentation.md"
