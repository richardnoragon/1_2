...
try:
    ...
    def create_signature_images(output_dir: str):
        ...
        for filename, size, color, text in signatures:
            ...
            draw.rectangle(
                [5, 5, size[0] - 5, size[1] - 5], outline=color, width=2
            )
    ...
    def create_test_data():
        ...
        with open(test_data_dir / "test_text_file.txt", "w") as f:
            f.write(
                "This is a test text file that should be ignored during PDF processing."
            )
    ...
    def create_minimal_test_data():
        ...
        for i, filename in enumerate(
            ["sample_single_page.pdf", "sample_multi_page.pdf"], 1
        ):
        ...
        for filename in [
            ...
            with open(test_data_dir / filename, "wb") as f:
                if filename.endswith(".jpg"):
                    f.write(
                        b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x01\x00H\x00H\x00\x00\xff\xfe\x00\x13Created with Python\xff\xd9"
                    )
                else:
                    f.write(
