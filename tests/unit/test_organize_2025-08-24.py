...
This module contains comprehensive test coverage for all classes and
functions in organize.py, including edge cases, error handling, and
mocked data interactions.
...
from src.tools.file_management.organizer.organize import (  # noqa: E402
    OrganizeRule,
    OrganizeWindow,
    RuleEditDialog,
    RulesDialog,
...
ORGANIZER_MODULE = "src.tools.file_management.organizer.organize"
...
class TestOrganizeRule:
    ...
    def test_organize_rule_creation_with_defaults(self):
        ...
        rule = OrganizeRule(name="TestRule", pattern="*.txt", destination="Documents")
    ...
    def test_organize_rule_creation_with_explicit_enabled(self):
        ...
        rule = OrganizeRule(
            name="TestRule", pattern="*.pdf", destination="PDFs", enabled=False
        )
...
class TestOrganizeWindow:
    ...
    @patch(f"{ORGANIZER_MODULE}.uic.loadUi")
    @patch(f"{ORGANIZER_MODULE}.Path.exists")
    def test_organize_window_initialization(
        self,
        mock_exists,
        mock_loadui,
        qapp,
        ...
        with patch.object(OrganizeWindow, "_setup_icons"), patch.object(
            OrganizeWindow, "_connect_signals"
        ), patch.object(OrganizeWindow, "_set_initial_state"), patch.object(
            OrganizeWindow, "_load_default_rules"
        ), patch.object(
            OrganizeWindow, "show"
        ):
    ...
    def test_init_models(self, qapp):
        ...
        with patch.object(OrganizeWindow, "_setup_ui"), patch.object(
            OrganizeWindow, "_setup_icons"
        ), patch.object(OrganizeWindow, "_connect_signals"), patch.object(
            OrganizeWindow, "_set_initial_state"
        ), patch.object(
            OrganizeWindow, "_load_default_rules"
        ), patch.object(
            OrganizeWindow, "show"
    ...
    def test_setup_ui_success(self, mock_exists, mock_loadui, qapp):
        ...
        with patch.object(OrganizeWindow, "_init_models"), patch.object(
            OrganizeWindow, "_setup_icons"
        ), patch.object(OrganizeWindow, "_connect_signals"), patch.object(
            OrganizeWindow, "_set_initial_state"
        ), patch.object(
    ...
    @patch(f"{ORGANIZER_MODULE}.uic.loadUi")
    @patch(f"{ORGANIZER_MODULE}.Path.exists")
    @patch(f"{ORGANIZER_MODULE}.show_error_dialog")
    @patch("sys.exit")
    def test_setup_ui_file_not_found(
        self,
        mock_exit,
        mock_error,
        mock_exists,
    ...
    @patch(f"{ORGANIZER_MODULE}.get_existing_directory")
    ...
    def test_get_file_list_non_recursive(
        self,
        qapp,
        temp_directory,
        sample_files,
    ...
    def test_organize_single_file_matching_rule(self, qapp, temp_directory):
        ...
        with patch.object(OrganizeWindow, "_setup_ui"), patch.object(
            ...
            window = OrganizeWindow()
            window._current_dir = temp_directory
            window._rules = [OrganizeRule("Documents", "*.txt", "Documents", True)]
    ...
    def test_organize_single_file_disabled_rule(self, qapp, temp_directory):
        ...
        with patch.object(OrganizeWindow, "_setup_ui"), patch.object(
            ...
            window = OrganizeWindow()
            window._current_dir = temp_directory
            # Provide a disabled rule to confirm it is skipped
            window._rules = [OrganizeRule("Documents", "*.txt", "Documents", False)]
    ...
    def test_move_file_to_destination_duplicate_handling(
        self,
        qapp,
        temp_directory,
    ):
    ...
    def test_move_file_nonexistent_source(self, qapp, temp_directory):
        ...
        with patch.object(OrganizeWindow, "_setup_ui"), patch.object(
            ...
            result = window._move_file_to_destination(
                nonexistent_file,
                "Documents",
            )
    ...
    @patch(f"{ORGANIZER_MODULE}.QMessageBox.information")
...
class TestMainFunction:
    ...
    @patch(f"{ORGANIZER_MODULE}.QApplication")
    @patch(f"{ORGANIZER_MODULE}.OrganizeWindow")
    @patch("sys.exit")
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_organize_rule_with_special_characters(self):
        ...
        rule = OrganizeRule("Test@#$%", "*.txt;*.pdf;*.doc*", "Folder/With/Path")
    ...
    def test_file_list_with_permission_error(self, qapp, temp_directory):
        ...
        with patch.object(OrganizeWindow, "_setup_ui"), patch.object(
            ...
            f"{ORGANIZER_MODULE}.show_error_dialog"
        ) as mock_error:
...
class TestPerformanceAndMemory:
    ...
    def test_memory_cleanup_after_organization(self, qapp, temp_directory):
        ...
        with patch.object(OrganizeWindow, "_setup_ui"), patch.object(
            ...
            for i in range(100):
                window._organized_files.append((f"source_{i}.txt", f"dest_{i}.txt"))
