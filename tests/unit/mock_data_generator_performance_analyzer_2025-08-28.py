...
class MockDataGenerator:
    ...
    def generate_measurements(
        ...
        total_measurements = scenario_config.duration_hours * measurements_per_hour
    ...
    def generate_trend_data(
        ...
        for i in range(num_points):
            ...
            value = max(0, value)
            if metric in [PerformanceMetric.PACKET_LOSS, PerformanceMetric.BANDWIDTH_UTILIZATION]:
                value = min(100, value)
    ...
    def generate_performance_degradation_sequence(
        ...
        for scenario_name, time_fraction in phases:
            ...
            for i in range(phase_measurements):
                timestamp = base_time - timedelta(
                    hours=current_time + (i * phase_duration / phase_measurements)
                )
...
def get_test_dataset(dataset_name: str) -> List[PerformanceMeasurement]:
    ...
    datasets = {
        ...
        "multi_interface": lambda: [
            m for measurements in generator.generate_multi_interface_data().values()
...
if __name__ == "__main__":
    ...
    measurements = generator.generate_measurements("good", measurements_per_hour=2)
