...
def run_specific_test_class(test_file: str, test_class: str) -> Dict[str, Any]:
    ...
    # Build pytest arguments
    args = [str(test_path) + f"::{test_class}", "-v", "--tb=short"]
...
def run_validation_tests() -> Dict[str, Any]:
    ...
    args = [str(test_dir), "-k", "validation", "-v", "--tb=short"]
...
def run_database_tests() -> Dict[str, Any]:
    ...
    args = [str(test_dir), "-k", "repository or database", "-v", "--tb=short"]
...
def run_comprehensive_test_suite() -> Dict[str, Any]:
    ...
    test_categories = [
        ...
        (
            "Error Handling",
            lambda: run_specific_test_class("test_error_handling", ""),
        ),
    ...
    print(
        f"Overall Result: {'SUCCESS' if success_count == total_categories else 'FAILURE'}"
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "all":
            ...
            print(
                "Available commands: all, unit, performance, validation, database, coverage"
            )
    else:
