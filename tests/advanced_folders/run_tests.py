...
class TestRunner:
    ...
    def generate_test_report(self, output_file: Optional[Path] = None) -> int:
        ...
        if output_file is None:
            output_file = (
                self.project_root / "test_report_advanced_folders.html"
            )
        ...
        if result == 0:
            ...
            print(
                f"✓ Coverage report: {self.project_root}/htmlcov/advanced_folders/index.html"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Advanced Folders Test Runner"
    )
    parser.add_argument(
        "--test-type",
    ...
    parser.add_argument(
        "--pattern", "-k", help="Run tests matching specific pattern"
    )
    parser.add_argument(
