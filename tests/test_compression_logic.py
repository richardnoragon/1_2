...
class TestCompressionLogic(unittest.TestCase):
    ...
    def test_password_protected_zip(self):
        ...
        self.compressor._decompress_zip(
    ...
    def test_error_handling_nonexistent_archive(self):
        ...
        with self.assertRaises((FileNotFoundError, OSError)):
            self.compressor._decompress_zip(
                nonexistent, output_dir, password=""
            )
...
class TestCompressionIntegration(unittest.TestCase):
    ...
    def test_full_compression_decompression_cycle(self):
        ...
        formats_to_test = [
            ("ZIP", ".zip", "_compress_zip", "_decompress_zip"),
            ("TAR.GZ", ".tar.gz", "_compress_targz", "_decompress_targz"),
            ("TAR.BZ2", ".tar.bz2", "_compress_tarbz2", "_decompress_tarbz2")
        ]
        
        # Create compression instance
        compressor = CompressDecompressApp()
        
        for format_name, extension, compress_method, decompress_method in (
            formats_to_test
        ):
            with self.subTest(format=format_name):
                ...
                compress_func = getattr(compressor, compress_method)
                decompress_func = getattr(compressor, decompress_method)
                
                if format_name == "ZIP":
                    compress_func(self.source_dir, archive_file, "", 6)
                else:
                    compress_func(self.source_dir, archive_file, 6)
                ...
                if format_name == "ZIP":
                    decompress_func(archive_file, extract_dir, "")
                else:
                    decompress_func(archive_file, extract_dir)
    ...
    def test_compression_ratio_comparison(self):
        ...
        for fmt in formats:
            ...
            if fmt == "zip":
                compressor._compress_zip(self.source_dir, archive_file, "", 6)
            elif fmt == "tar.gz":
                compressor._compress_targz(self.source_dir, archive_file, 6)
            elif fmt == "tar.bz2":
                compressor._compress_tarbz2(self.source_dir, archive_file, 6)
