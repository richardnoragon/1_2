#!/usr/bin/env python3
"""
Comprehensive Import/Export and Backup/Restore Test Suite

Tests all import/export and backup/restore functionality for Advanced Folders.
"""
...
from utilities.file_management.advanced_folders.core.backup_restore import (
...
from utilities.file_management.advanced_folders.core.import_export import (
...
def test_import_export_functionality():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager._config_file = temp_path / "test_config.json"
        ...
        print(f"  ✓ Created {len(manager.list_folders())} test configurations")
        ...
        assert len(export_data['advanced_folders_export']['configurations']) == 2
        ...
        import_manager._config_file = temp_path / "import_config.json"
        ...
        assert len(import_manager.list_folders()) == 2, "Should import 2 configurations"
        
        print("  ✓ Import from JSON functionality working")
        
        # Import from ZIP
        import_manager2 = FolderConfigurationManager()
        import_manager2._config_file = temp_path / "import_config2.json"
        ...
        assert len(import_manager2.list_folders()) == 2, "Should import 2 configurations from ZIP"
...
def test_conflict_resolution():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        original_manager._config_file = temp_path / "original_config.json"
        ...
        conflict_manager = FolderConfigurationManager()
        conflict_manager._config_file = temp_path / "conflict_config.json"
        ...
        assert len(configurations_after_skip) == 1, "Skip should not add new configurations"
        
        print("  ✓ Skip conflict resolution working")
        
        # Test 'rename' strategy
        rename_success, rename_messages = conflict_ie_manager.import_configurations(
            export_path,
            conflict_resolution='rename'
        )
        
        # Should now have both configurations with different names
        configurations_after_rename = conflict_manager.list_folders()
        assert len(configurations_after_rename) == 2, "Rename should add configuration with new name"
        
        # Check that names are different
        names = {config.name for config in configurations_after_rename}
        assert len(names) == 2, "Should have two different names"
...
def test_backup_restore_functionality():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager = FolderConfigurationManager()
        manager._config_file = temp_path / "backup_test_config.json"
        ...
        print(f"  ✓ Created {len(manager.list_folders())} configurations for backup")
        
        # Initialize backup manager
        backup_manager = BackupManager(manager, backup_dir)
        
        # Test manual backup creation
        backup_path = backup_manager.create_backup("Manual test backup")
        ...
        assert len(backups) == 2, "Should have 2 backups"
        
        manual_backups = backup_manager.list_backups('manual')
        auto_backups = backup_manager.list_backups('auto')
        assert len(manual_backups) == 1, "Should have 1 manual backup"
        assert len(auto_backups) == 1, "Should have 1 automatic backup"
        ...
        # Create new manager for restore testing
        restore_manager = FolderConfigurationManager()
        restore_manager._config_file = temp_path / "restore_test_config.json"
        ...
        assert len(restored_configs) >= 2, "Should have at least 2 restored configurations"
        
        print("  ✓ Restore functionality working")
        
        # Test backup statistics
        stats = backup_manager.get_backup_statistics()
        assert stats['total_backups'] >= 2, "Should show at least 2 backups in stats"
        assert stats['total_size_bytes'] > 0, "Should show non-zero backup size"
...
def test_automatic_backup_scheduler():
    """Test automatic backup scheduling."""
    print("Testing automatic backup scheduler...")
    
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        
        manager = FolderConfigurationManager()
        manager._config_file = temp_path / "scheduler_test.json"
        
        # Create a test configuration
        manager.create_folder(
            name="Scheduler Test",
            description="Test for scheduler",
            directory_paths=["C:\\Scheduler\\Test"]
        )
        
        backup_manager = BackupManager(manager, temp_path / "backups")
        
        # Configure automatic backup
        backup_manager.configure_automatic_backup(enabled=True, interval_hours=1)
        
        # Check scheduler status
        assert backup_manager.auto_scheduler._enabled, "Auto backup should be enabled"
        assert backup_manager.auto_scheduler._backup_interval.total_seconds() == 3600, "Interval should be 1 hour"
        
        # Test that backup is due (since none have been created)
        assert backup_manager.auto_scheduler.is_backup_due(), "First backup should be due"
        
        print("  ✓ Automatic backup scheduler configuration working")
        
        # Perform automatic backup
        backup_success = backup_manager.auto_scheduler.perform_automatic_backup()
        assert backup_success, "Automatic backup should succeed"
        
        # Check that backup is no longer due
        assert not backup_manager.auto_scheduler.is_backup_due(), "Backup should not be due after completion"
        
        print("  ✓ Automatic backup execution working")
...
def main():
    ...
    print("="*70)
    print("Advanced Folders - Import/Export & Backup/Restore Test Suite")
    print("="*70)
    ...
    tests = [
        ...
        test_automatic_backup_scheduler
    ]
    ...
    print("="*70)
    print("Import/Export & Backup/Restore Test Summary")
    print("="*70)
    ...
    if failed == 0:
        ...
        print("✓ Automatic backup scheduling operational")
