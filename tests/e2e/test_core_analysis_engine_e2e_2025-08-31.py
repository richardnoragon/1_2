...
class TestCompleteAnalysisWorkflows:
        ...
        structure = {
            ...
            'target': {
                'classes': {
                    'com': {
                        'example': {
                            'Main.class': b'\xca\xfe\xba\xbe' + b'fake class file' * 50  # Simulate compiled class
                        }
                    }
                }
            },
    ...
    def test_complete_project_analysis_workflow(self, qapp, real_world_dataset):
        ...
        if len(result['largest_files']) > 1:
            ...
            for i in range(len(result['largest_files']) - 1):
                assert result['largest_files'][i]['size'] >= result['largest_files'][i + 1]['size']
    ...
    def test_multi_filter_analysis_workflow(self, qapp, real_world_dataset):
        ...
        config_result = analyzer.analyze_directory(
            real_world_dataset,
            include_extensions=['.properties', '.yaml', '.yml']
        )
        ...
        all_result = analyzer.analyze_directory(
            real_world_dataset,
            top_files_count=5
        )
