...
class IntegrationTestSuite:
    ...
    def test_basic_imports(self):
        ...
        import_tests = [
            ...
            (
                "file_utilities_2.gui.check_sum_standardized",
                "GUI standardized",
            ),
    ...
    def test_specific_exports(self):
        ...
        try:
            ...
            from file_utilities_2.core.check_sum import (
                ChecksumLogic as CoreLogic,
            )
            from file_utilities_2.core.check_sum import (
                VALID_ALGORITHMS as CoreAlgorithms,
            )
            ...
            for algorithm in ["md5", "sha1", "sha256", "sha512"]:
                ...
                for filename, file_info in self.test_files.items():
                    checksummer = ChecksumLogic(
                        file_info["path"], algorithm, mode="calculate_file"
                    )
                    ...
                    if algorithm == "md5":
                        ...
                        result = checksummer.calculate_sha256(
                            file_info["path"]
                        )
        ...
        try:
            ...
            for file_path, checksum in results.items():
                ...
                if checksum == expected:
                    ...
                    print(
                        f"  ❌ {filename}: Expected {expected}, got {checksum}"
                    )
                    return False
        ...
        try:
            ...
            for filename, file_info in self.test_files.items():
                ...
                result = checksummer.verify_file(
                    file_info["path"], file_info["sha256"], "sha256"
                )
                ...
                result = checksummer.verify_file(
                    file_info["path"], "invalid_checksum_value", "sha256"
                )

                if not result:
                    print(
                        f"  ✅ {filename}: Invalid checksum correctly rejected"
                    )
                else:
                    print(
                        f"  ❌ {filename}: Invalid checksum incorrectly accepted"
                    )
                    return False
            ...
            for signal_name in signals_to_check:
                if hasattr(checksummer, signal_name):
                    ...
                    if hasattr(signal, "connect"):
                        print(
                            f"  ✅ {signal_name}: Signal exists and connectable"
                        )
                    else:
