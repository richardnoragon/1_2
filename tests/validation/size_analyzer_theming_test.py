...
try:
    ...
    from file_utilities_2.gui.themes import (
        ThemeManager,
        Colors,
        Fonts,
        Spacing,
...
class ThemingValidator:
    ...
    def setup_test_environment(self):
        ...
        try:
            ...
            self.log_error(
                "Environment Setup", f"Failed to setup test environment: {e}"
            )
            return False
    ...
    def log_warning(self, test_name: str, message: str):
        """Log warning."""
        self.test_results.append(
            {"test": test_name, "status": "⚠️ WARN", "message": message}
        )
    ...
    def test_theme_manager_functionality(self):
        ...
        try:
            ...
            if instance1 is instance2:
                ...
                self.log_error(
                    "ThemeManager Singleton", "Singleton pattern not working"
                )
            ...
            if ThemeManager._current_theme == "dark":
                self.log_success(
                    "Theme Switching", "Theme switching functionality works"
                )
            else:
                self.log_error(
                    "Theme Switching", "Theme switching not working"
                )
            ...
            if not missing_colors:
                self.log_success(
                    "Color Definitions", "All required colors defined"
                )
            else:
                self.log_error(
                    "Color Definitions", f"Missing colors: {missing_colors}"
                )
            ...
            if not missing_fonts:
                self.log_success(
                    "Font Definitions", "All required fonts defined"
                )
            else:
                self.log_error(
                    "Font Definitions", f"Missing fonts: {missing_fonts}"
                )
    ...
    def test_size_analyzer_initialization(self):
        ...
        try:
            ...
            if self.size_analyzer:
                self.log_success(
                    "GUI Initialization",
                    "Size Analyzer GUI created successfully",
                )
            else:
                self.log_error(
                    "GUI Initialization", "Failed to create Size Analyzer GUI"
                )
                return False
            ...
            if isinstance(self.size_analyzer, StandardWindow):
                self.log_success(
                    ...
                    "Properly inherits from StandardWindow",
                )
            else:
                self.log_error(
                    "StandardWindow Inheritance",
                    "Does not inherit from StandardWindow",
                )

            # Test window properties
            min_size = self.size_analyzer.minimumSize()
            if (
                min_size.width() >= Dimensions.UTILITY_WINDOW_MIN_WIDTH
                and min_size.height() >= Dimensions.UTILITY_WINDOW_MIN_HEIGHT
            ):
                self.log_success(
                    "Window Dimensions", "Minimum window size set correctly"
                )
            else:
                self.log_warning(
                    "Window Dimensions",
    ...
    def test_ui_component_styling(self):
        ...
        if not self.size_analyzer:
            self.log_error(
                "Component Styling", "Size Analyzer not initialized"
            )
            return
        ...
        try:
            ...
            if not missing_components:
                self.log_success(
                    "Required Components", "All required components present"
                )
            else:
                self.log_error(
                    "Required Components", f"Missing: {missing_components}"
                )
            ...
            if (
                len(present_enhanced) >= 3
            ):  # At least most components should be present
                self.log_success(
                    "Enhanced Components",
            ...
            if hasattr(self.size_analyzer, "browse_button"):
                ...
                if Colors.BUTTON_PRIMARY in button_style or button_style:
                    self.log_success(
                        "Button Styling", "Buttons have custom styling applied"
                    )
                else:
                    self.log_warning(
                        "Button Styling", "Buttons may not have proper styling"
                    )

            # Test input field styling
            if hasattr(self.size_analyzer, "directory_line_edit"):
                input_style = (
                    self.size_analyzer.directory_line_edit.styleSheet()
                )
                if Colors.WINDOW_BACKGROUND in input_style or input_style:
                    self.log_success(
                        "Input Styling", "Input fields have custom styling"
                    )
                else:
                    self.log_warning(
                        "Input Styling",
                        "Input fields may not have proper styling",
                    )
    ...
    def test_progress_visualization(self):
        ...
        if not self.size_analyzer:
            self.log_error(
                "Progress Visualization", "Size Analyzer not initialized"
            )
            return
            ...
            if hasattr(self.size_analyzer, "progress_bar"):
                ...
                if Colors.ACCENT in style or style:
                    self.log_success(
                        ...
                        "Progress bar has themed styling",
                    )
                else:
                    self.log_warning(
                        "Progress Bar Styling",
                        "Progress bar styling may be missing",
                    )
                ...
                if progress_bar.value() == 50:
                    self.log_success(
                        ...
                        "Progress bar value setting works",
                    )
                else:
                    self.log_error(
                        "Progress Bar Functionality",
                        "Progress bar value setting failed",
                    )
            else:
                self.log_warning(
                    "Progress Bar", "Progress bar component not found"
                )

            # Test progress label
            if hasattr(self.size_analyzer, "progress_label"):
                self.log_success(
                    "Progress Label", "Progress label component present"
                )
            else:
                self.log_warning(
                    "Progress Label", "Progress label component not found"
                )

            # Test progress group
            if hasattr(self.size_analyzer, "progress_group"):
                self.log_success(
                    "Progress Group", "Progress group container present"
                )
            else:
                self.log_warning(
                    "Progress Group", "Progress group container not found"
                )
    ...
    def test_responsive_design(self):
        ...
        if not self.size_analyzer:
            self.log_error(
                "Responsive Design", "Size Analyzer not initialized"
            )
            return
        ...
        try:
            ...
            if (
                current_size.width() >= new_width - 50
                and current_size.height() >= new_height - 50
            ):
                self.log_success(
                    "Window Resizing", "Window resizing works correctly"
                )
            else:
                self.log_warning(
                    "Window Resizing",
            ...
            if hasattr(self.size_analyzer, "resultsSplitter"):
                ...
                if splitter.count() >= 2:
                    self.log_success(
                        "Splitter Layout",
                        "Results splitter configured correctly",
                    )
                else:
                    self.log_warning(
                        "Splitter Layout",
                        "Results splitter may not be configured",
                    )

            # Test size policies
            size_policy = self.size_analyzer.sizePolicy()
            if (
                size_policy.horizontalPolicy() != 0
                and size_policy.verticalPolicy() != 0
            ):
                self.log_success(
                    "Size Policies", "Window has proper size policies"
                )
            else:
                self.log_warning(
                    "Size Policies", "Size policies may not be optimal"
                )
    ...
    def test_theme_consistency(self):
        ...
        if not self.size_analyzer:
            self.log_error(
                "Theme Consistency", "Size Analyzer not initialized"
            )
            return
        ...
        try:
            ...
            if callback_count > 0:
                self.log_success(
                    "Theme Callbacks",
                    f"Theme callbacks registered: {callback_count}",
                )
            else:
                self.log_warning(
                    "Theme Callbacks", "No theme callbacks registered"
                )
            ...
            if window_style:
                self.log_success(
                    "Window Theming", "Window has theme styling applied"
                )
            else:
                self.log_warning(
                    "Window Theming", "Window may not have theme styling"
                )
            ...
            if (
                ...
                self.log_success(
                    "Color Consistency", "Color scheme is properly defined"
                )
            else:
                self.log_warning(
                    "Color Consistency", "Color scheme may have issues"
                )
    ...
    def test_icon_and_resource_paths(self):
        ...
        try:
            ...
            if icon_path and os.path.exists(icon_path):
                ...
                self.log_warning(
                    "Icon Path", f"Icon not found, using fallback: {icon_path}"
                )
            ...
            ui_file_path = os.path.join(
                os.path.dirname(
                    self.size_analyzer.__class__.__module__.replace(
                        ".", os.sep
                    )
                ),
                "size_analyzer.ui",
            ...
            if os.path.exists(ui_file_path):
                ...
                self.log_warning(
                    "UI File",
                    "UI file may not be found, using programmatic creation",
                )
    ...
    def generate_test_summary(self):
        ...
        passed_tests = len(
            [r for r in self.test_results if "✅" in r["status"]]
        )
        failed_tests = len(
            [r for r in self.test_results if "❌" in r["status"]]
        )
        warning_tests = len(
            [r for r in self.test_results if "⚠️" in r["status"]]
        )
