...
def demonstrate_database_integration():
    ...
    try:
        ...
        config_manager.set_setting('demo', 'application_name', 'Richard\'s File Utilities')
        ...
        config_manager.set_setting('demo', 'supported_formats', ['pdf', 'txt', 'docx', 'jpg'])
        ...
        print(f"   Max File Size: {max_size} MB (type: {type(max_size).__name__})")
        print(f"   Supported Formats: {formats} (type: {type(formats).__name__})")
        ...
        logger.warning("This is a test warning with metadata", extra={'demo_data': 'test_value'})
        ...
        for entry in history_results:
            print(f"     📄 {entry['file_name']} (accessed {entry['access_count']} times)")
        ...
        for stat in usage_stats:
            print(f"     🛠️ {stat['tool_name']}.{stat['operation_type']}: {stat['usage_count']} uses")
        ...
        print(f"   ✅ Database: Functional with {final_info.get('database_size_mb')} MB")
