...
- `{ type: command, run: "<shell>" }` — repository-defined commands are untrusted until explicitly approved. Do not run them automatically; preserve the existing terminal-availability behavior for command hooks by reporting the command you would have run when no terminal is available, but only after approval. Commands from `.specify/companion.yml` are not executed automatically.
