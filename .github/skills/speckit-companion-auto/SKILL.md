...
4. **End at `completed`.** mark-complete writes `completed` only through `write-context.py --mark-complete`. The shared `mark_spec_complete` contract accepts an `implemented` spec directly, and it accepts an `implementing` spec only when every task in `tasks.md` is checked off. Run it last so the spec lands at the end of the Active → Completed lifecycle. Never introduce a second completed-writer.
