...
- **Pause at every review gate, unless unattended.** When `unattended: true` is set, record the checkpoint and continue without waiting for approval pauses. Otherwise, where the workflow marks a `gate` (e.g. review-spec, review-plan), stop and wait for approval rather than running past it. Only continue once the gate is approved.
