            ...
            for filename in filenames:
                ...
                if os.path.isfile(file_path) and os.access(file_path, os.R_OK):
                    ...
                    if len(files) >= max_files:
                        msg = f"Warning: Reached maximum file limit of " \
                              f"{max_files}. Stopping search."
                        print(msg)
                        return files
...
def open_files_in_vscode(files, batch_size=10):
    ...
    if len(files) > 20:
        msg = f"This will open {len(files)} files. Continue? (y/N): "
        response = input(msg)
    ...
    for i in range(0, len(files), batch_size):
        ...
        try:
            ...
            print("Make sure VS Code is installed and added to PATH.")
...
def main():
    ...
    """
    description = 'Open all files from a folder and subfolders in VS Code'
    ...
    parser.add_argument('--batch-size', type=int, default=10,
                        help='Number of files to open in each batch '
                             '(default: 10)')
    parser.add_argument('--list-only', action='store_true',
                        help='Only list files that would be opened, '
