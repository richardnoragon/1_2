    ...
    for dir_path in utility_dirs:
        ...
        if not os.path.exists(init_file):
            ...
            with open(init_file, 'w') as f:
                f.write(f'"""Utilities package for Richard\'s File Utilities - {os.path.basename(dir_path)} module."""\n')
...
def update_main_py():
    ...
    if 'Network Tools' not in updated_content:
        ...
        if pdf_tab_pos != -1:
            ...
            '''
            updated_content = updated_content[:pdf_tab_pos] + new_tabs + updated_content[pdf_tab_pos:]
...
'''
            updated_content = updated_content[:method_end] + new_launchers + updated_content[method_end:]
