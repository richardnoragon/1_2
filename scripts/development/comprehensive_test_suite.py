...
class ToolTestSuite:
    ...
    def __init__(self):
        ...
        self.tools = {
            ...
            "office_meta_data_editor": {
                "module": "office_meta_data_editor",
    ...
    def run_class_test(self, tool_name: str, tool_info: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            if hasattr(module, expected_class):
                ...
                return False, f"❌ Class '{expected_class}' not found. Available: {available_classes}"
        except Exception as e:
            return False, f"❌ Class test error: {str(e)}"

    def run_instantiation_test(self, tool_name: str, tool_info: Dict) -> Tuple[bool, str]:
    ...
    def run_tool_tests(self, tool_name: str, tool_info: Dict) -> Dict:
        ...
        if import_success:
            ...
            if class_success:
                inst_success, inst_msg = self.run_instantiation_test(tool_name, tool_info)
                results["tests"]["instantiation"] = {"success": inst_success, "message": inst_msg}
                print(f"  Instantiation: {inst_msg}")
                
                # Test 4: Integration Test
                integ_success, integ_msg = self.run_integration_test(tool_name, tool_info)
                results["tests"]["integration"] = {"success": integ_success, "message": integ_msg}
                ...
                results["tests"]["instantiation"] = {"success": False, "message": "Skipped - class test failed"}
                results["tests"]["integration"] = {"success": False, "message": "Skipped - class test failed"}
                results["overall_success"] = False
                self.failed_tests += 1
        else:
            results["tests"]["class"] = {"success": False, "message": "Skipped - import failed"}
            results["tests"]["instantiation"] = {"success": False, "message": "Skipped - import failed"}
            results["tests"]["integration"] = {"success": False, "message": "Skipped - import failed"}
            results["overall_success"] = False
            self.failed_tests += 1
    ...
    def print_summary(self, results: Dict):
        ...
        failed_tools = [name for name, data in results["tools"].items() 
