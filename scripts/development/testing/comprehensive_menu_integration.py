...
def update_tool_to_standard_window(file_path, class_name, window_type="utility"):
    ...
    try:
        ...
        content = content.replace(
            f"class {class_name}(QMainWindow):", f"class {class_name}(StandardWindow):"
        )
        ...
        if f"class {class_name}(StandardWindow):" in content:
            ...
            for line in lines:
                if f"def __init__(self" in line and not init_updated:
                    ...
                    "self.setWindowTitle(" in line
                    or "self.setGeometry(" in line
                    or "self.setCentralWidget(" in line
                ):
...
def main():
    ...
    tools_to_update = [
        ...
        {
            "path": "src/tools/file_operations/rename/rename.py",
            "class": "RenameWindow",
    ...
    for tool_info in tools_to_update:
        ...
        if file_path.exists():
            ...
            if update_tool_to_standard_window(
                file_path, tool_info["class"], tool_info["type"]
            ):
                success_count += 1
        else:
