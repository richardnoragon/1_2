...
class EnhancedRFUMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.statusBar().showMessage(
            "Richard's File Utilities - Enhanced Menu System Ready", 3000
        )
    ...
    def _register_menu_callbacks(self):
        ...
        self.menu_manager.register_callback(
            "show_preferences", self.show_main_preferences
        )
    ...
    def _add_file_tools_menu(self):
        ...
        if (
            not hasattr(self.menu_manager, "tools_menu")
            or self.menu_manager.tools_menu is None
        ):
        ...
        file_tools_menu.addAction("🔍 File Finder").triggered.connect(
            self.open_file_finder
        )
        file_tools_menu.addAction("📋 Catalog Files").triggered.connect(
            self.open_catalog
        )
        file_tools_menu.addAction("✏️ Rename Files").triggered.connect(self.open_rename)
        file_tools_menu.addAction("📂 Organize Files").triggered.connect(
            self.open_organize
        )
        ...
        # File operations
        file_tools_menu.addAction("📦 Compress/Decompress").triggered.connect(
            self.open_compress
        )
    ...
    def _add_security_tools_menu(self):
        ...
        encryption_menu.addAction("🔒 Encrypt Files").triggered.connect(
            self.open_encryption
        )
        encryption_menu.addAction("🔓 Decrypt Files").triggered.connect(
            self.open_decryption
        )
        encryption_menu.addAction("🔑 Manage Keys").triggered.connect(
            self.open_key_manager
        )

        # Security analysis
        analysis_menu = security_menu.addMenu("🛡️ Security &Analysis")
        analysis_menu.addAction("🔍 Security Scan").triggered.connect(
            self.open_security_scan
        )
        analysis_menu.addAction("📊 Audit Report").triggered.connect(
            self.open_audit_report
        )
        analysis_menu.addAction("⚠️ Vulnerability Check").triggered.connect(
            self.open_vuln_check
        )
    ...
    def _add_analysis_tools_menu(self):
        ...
        # File analysis
        analysis_menu.addAction("📏 Size Analyzer").triggered.connect(
            self.open_size_analyzer
        )
        analysis_menu.addAction("🔍 Duplicate Finder").triggered.connect(
            self.open_duplicate_finder
        )
        analysis_menu.addAction("📁 Empty Folders").triggered.connect(
            self.open_empty_folders
        )
        analysis_menu.addAction("✅ Checksum Validator").triggered.connect(
            self.open_checksum
        )
        ...
        analysis_menu.addAction("🕒 File Timeline").triggered.connect(
            self.open_file_timeline
        )
        analysis_menu.addAction("🏷️ Metadata Viewer").triggered.connect(
            self.open_metadata_viewer
        )
    ...
    def _add_utilities_menu(self):
        ...
        system_menu.addAction("🧹 Temp Cleaner").triggered.connect(
            self.open_temp_cleaner
        )
        system_menu.addAction("📊 Performance Monitor").triggered.connect(
            self.open_perf_monitor
        )
    ...
    def _create_central_widget(self):
        ...
        # Footer
        footer_label = QLabel(
            "Use the comprehensive menu system above for advanced features and operations"
        )
    ...
    def _create_tool_buttons(self):
        ...
        tools = [
            ...
            (
                "📋 Catalog Files",
                "Create detailed file catalogs and reports",
                self.open_catalog,
            ),
            (
                "✏️ Rename Files",
            ...
            (
                "✂️ Split/Join Files",
                "Split large files or join file parts",
                self.open_file_splitter,
            ),
            (
                "️ Secure Delete",
    ...
    self.launch_tool("Rename", "src.tools.file_operations.rename", "RenameWindow")
    ...
    def open_compress(self):
        ...
        self.launch_tool(
            "Compress",
            "src.utilities.file_operations.compression",
    ...
    def open_file_splitter(self):
        ...
        self.launch_tool(
            "File Splitter",
            "src.utilities.file_operations.file_splitter",
            "FileSplitJoinGUI",
        )
    ...
    def open_pdf_security(self):
        ...
        QMessageBox.information(
            self, "PDF Security", "PDF Security tool with menu integration coming soon!"
        )
    ...
    def open_key_manager(self):
        """Open Key Manager."""
        QMessageBox.information(
            self, "Key Manager", "Encryption Key Manager coming soon!"
        )
    ...
    def open_audit_report(self):
        ...
        QMessageBox.information(
            self, "Audit Report", "Security Audit Report generator coming soon!"
        )

    def open_vuln_check(self):
        """Open Vulnerability Checker."""
        QMessageBox.information(
            self, "Vulnerability Check", "Vulnerability Checker coming soon!"
        )
    ...
    def show_system_info(self):
        ...
        if self.menu_manager:
            ...
            QMessageBox.information(
                self, "System Info", "System information display coming soon!"
            )

    def open_temp_cleaner(self):
        """Open Temp File Cleaner."""
        QMessageBox.information(
            self, "Temp Cleaner", "Temporary File Cleaner coming soon!"
        )

    def open_perf_monitor(self):
        """Open Performance Monitor."""
        QMessageBox.information(
            self, "Performance Monitor", "Performance Monitor coming soon!"
        )

    def open_disk_usage(self):
        """Open Disk Usage Analyzer."""
        QMessageBox.information(self, "Disk Usage", "Disk Usage Analyzer coming soon!")

    def open_file_timeline(self):
        """Open File Timeline Analyzer."""
        QMessageBox.information(
            self, "File Timeline", "File Timeline Analyzer coming soon!"
        )

    def open_metadata_viewer(self):
        """Open Metadata Viewer."""
        QMessageBox.information(
            self, "Metadata Viewer", "File Metadata Viewer coming soon!"
        )
    ...
    def export_project_data(self):
        """Export project data."""
        QMessageBox.information(
            self, "Export Data", "Export project data functionality coming soon!"
        )

    def import_project_data(self):
        """Import project data."""
        QMessageBox.information(
            self, "Import Data", "Import project data functionality coming soon!"
        )
    ...
    def show_main_preferences(self):
        ...
        if self.menu_manager:
            ...
            try:
                ...
                QMessageBox.information(
                    self, "Preferences", "Main application preferences coming soon!"
                )
        else:
            QMessageBox.information(
                self, "Preferences", "Main application preferences coming soon!"
    ...
    def show_main_options(self):
        ...
        QMessageBox.information(
            self, "Options", "Main application options coming soon!"
        )
