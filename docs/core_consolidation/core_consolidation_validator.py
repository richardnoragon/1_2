...
class CoreConsolidationValidator:
    ...
    def test_directory_structure(self) -> bool:
        ...
        if self.core_rfu_path.exists():
            ...
            self.log_message(
                f"WARNING: core_rfu still exists: {self.core_rfu_path}", "WARNING"
            )
    ...
    def test_constants_functionality(self) -> bool:
        ...
        try:
            from core.constants import APP_NAME, JSON_FILES_FILTER, SECURITY_HIGH
            ...
            if not isinstance(SECURITY_HIGH, str):
                self.log_message(
                    f"SECURITY_HIGH is not a string: {type(SECURITY_HIGH)}", "ERROR"
                )
                return False

            if not isinstance(JSON_FILES_FILTER, str):
                self.log_message(
                    f"JSON_FILES_FILTER is not a string: {type(JSON_FILES_FILTER)}",
                    "ERROR",
                )
                return False
    ...
    def test_subdirectory_imports(self) -> bool:
        ...
        subdirectory_tests = [
            (
                "core.directory_security.directory_security_manager",
        ...
        for module_path, class_name in subdirectory_tests:
            try:
                ...
                if hasattr(module, class_name):
                    self.log_message(
                        f"✅ Subdirectory import successful: {module_path}.{class_name}"
                    )
                    passed += 1
                else:
                    self.log_message(
                        f"❌ Class not found: {module_path}.{class_name}", "WARNING"
                    )
            except ImportError as e:
                self.log_message(
                    f"❌ Subdirectory import failed: {module_path} - {e}", "WARNING"
                )
            except Exception as e:
                self.log_message(
                    f"❌ Unexpected error importing {module_path}: {e}", "ERROR"
                )
        ...
        if success_rate >= 0.5:  # 50% success rate is acceptable
            ...
            self.log_message(
                f"❌ Too many subdirectory import failures: {passed}/{total}", "ERROR"
            )
            return False
    ...
    def test_configuration_imports(self) -> bool:
        ...
        for module_path in config_tests:
            try:
                ...
                self.log_message(
                    f"❌ Configuration import failed: {module_path} - {e}", "WARNING"
                )
            except Exception as e:
                self.log_message(
                    f"❌ Unexpected error importing {module_path}: {e}", "ERROR"
        ...
        if passed > 0:
            self.log_message(
                f"✅ Configuration imports: {passed}/{len(config_tests)} successful"
            )
            return True
        else:
    ...
    def test_database_imports(self) -> bool:
        ...
        for module_path in database_tests:
            try:
                ...
                self.log_message(
                    f"❌ Database import failed: {module_path} - {e}", "WARNING"
                )
            except Exception as e:
                self.log_message(
                    f"❌ Unexpected error importing {module_path}: {e}", "WARNING"
                )
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        self.run_test(
            "Error Handler Functionality", self.test_error_handler_functionality
        )
    ...
    def generate_validation_report(self) -> str:
        ...
        if success_rate >= 90:
            report.append(
                "✅ Migration appears to be successful. All core functionality is working."
            )
        elif success_rate >= 75:
            report.append(
                "⚠️ Migration mostly successful with minor issues. Review failed tests."
            )
        elif success_rate >= 50:
            report.append(
                "⚠️ Migration partially successful. Some functionality may be impacted."
            )
        else:
            report.append(
                "❌ Migration has significant issues. Consider rollback and investigation."
            )
