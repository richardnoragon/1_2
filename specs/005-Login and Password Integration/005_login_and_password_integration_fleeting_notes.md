...
> **Status Update: 2025-11-28** — specs/006-baseline-login-password is now **100% complete** (T001–T117).
> The items below are deferred to specs/007-upgrade-to-login or future maintenance cycles.

- [ ] Plan the sunset path for the remaining `network_org` shim components (consumer inventory, migration checklist, removal window); `network_base.py` was deleted on 2025-11-23 so the follow-up now targets the other shim breadcrumbs. **→ Deferred to maintenance backlog**
- [ ] Audit the remaining legacy suites (security validator, performance analyzer, diagnostics runners) for lingering `utilities.*` references and migrate them to `src.tools...`. **→ Deferred to maintenance backlog**
- [ ] Restore or replace the historical `log_config` dependency that the OCR suites expect, then verify OpenCV/numpy integration still works. **→ Deferred to maintenance backlog**
- [ ] Reintroduce (or modernize) the thin logging shim so OCR tests can patch the canonical logger entry point without import errors. **→ Deferred to maintenance backlog**
- [ ] After the network + logging shims are retired, rerun `shell: Run Tests` to capture the next wave of failures. **→ Blocked on above items**
- [ ] `FN-MFA-SECURE-DISPATCH`: route every `mfa_secret_encrypted` write path (and associated PreferenceManager wiring) through `SecureSecretDispatcher`, update fixtures/tests, and document the enforced encryption flow before enabling MFA. **→ Deferred to specs/007 Phase 4**
- [ ] (!) Schedule the PyQt5 GUI validation pass once the harness is ready. **→ Deferred to specs/007 Phase 4**
- [ ] (!) Execute the GUI login/preferences validation once the PyQt harness exists so the PreferenceManager wiring stays covered. **→ Deferred to specs/007 Phase 4**
...
## Account Structure Overview (2025-11-26)

### Account Types

There are **four account roles** in the system:

| Role       | Description                                                            |
| ---------- | ---------------------------------------------------------------------- |
| `dev`      | Developer access with full debugging/diagnostic capabilities           |
| `admin`    | Administrator access with user management and configuration privileges |
| `user`     | Standard user access for regular application functionality             |
| `readonly` | Read-only access with no write/modification capabilities               |

### Always-Available Accounts

Two accounts are **always available** and can be used at any time to access the application:

| Account Type | Purpose                                                                     |
| ------------ | --------------------------------------------------------------------------- |
| **dev**      | Guaranteed developer access for debugging and development operations        |
| **admin**    | Guaranteed administrator access for user management and recovery operations |

These accounts ensure that operators are never completely locked out of the system.

### Break-Glass Accounts

Two **break-glass accounts** exist for emergency recovery scenarios:

| Account Type | Purpose                                                                   |
| ------------ | ------------------------------------------------------------------------- |
| **dev**      | Emergency developer access when primary dev account is compromised/locked |
| **admin**    | Emergency admin access when primary admin account is compromised/locked   |

Break-glass accounts should:

- Be used **only** in emergency situations
- Have their usage audited and reviewed
- Have credentials stored securely (e.g., in a sealed envelope or secure vault)
- Trigger an immediate password rotation after use
...
- [ ] All roles ('dev', admin`,`operator`,`user`,`readonly`) are defined in`approval_policy.json`
...
- [ ] Emergency accounts (`breakglass_dev`) and ('breakglass_admin') are defined in policy
