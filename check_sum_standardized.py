...
class ChecksumWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        calculate_btn = QPushButton("Calculate Checksum")
        ThemeManager.style_primary_button(calculate_btn)
        calculate_btn.clicked.connect(self.calculate_checksum)
        
        verify_btn = QPushButton("Verify Checksum")
        ThemeManager.style_secondary_button(verify_btn)
        verify_btn.clicked.connect(self.verify_checksum)
        
        clear_btn = QPushButton("Clear")
        ThemeManager.style_secondary_button(clear_btn)
    ...
    def calculate_checksum(self):
        ...
        # Create and start checksum thread
        self.checksum_thread = ChecksumThread(file_path, algorithm)
    ...
    def verify_checksum(self):
        ...
        # Create and start checksum thread
        self.checksum_thread = ChecksumThread(file_path, algorithm, expected_checksum)
    ...
    def on_checksum_calculated(self, result):
        ...
        self.results_text.setPlainText(f"Algorithm: {result.get('algorithm', '')}\n"
    ...
    def on_checksum_verified(self, result):
        ...
        if is_valid:
            self.results_text.setPlainText(f"✅ Checksum verification PASSED\n"
                                         f"Algorithm: {result.get('algorithm', '')}\n"
                                         f"File: {result.get('file_path', '')}\n"
                                         f"Expected: {expected}\n"
                                         f"Actual: {actual}")
        else:
            self.results_text.setPlainText(f"❌ Checksum verification FAILED\n"
                                         f"Algorithm: {result.get('algorithm', '')}\n"
                                         f"File: {result.get('file_path', '')}\n"
                                         f"Expected: {expected}\n"
                                         f"Actual: {actual}")
...
class ChecksumThread(QThread):
    """Thread for calculating checksums without blocking the UI."""
    ...
    def run(self):
        """Calculate checksum in separate thread."""
        try:
            ...
            # If expected checksum provided, verify
            if self.expected_checksum:
                result['is_valid'] = checksum.lower() == self.expected_checksum.lower()
                result['expected'] = self.expected_checksum
