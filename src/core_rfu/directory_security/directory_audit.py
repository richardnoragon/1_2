...
class DirectoryAuditLogger:
    ...
    def log_access_denied(self, user_id: str, resource_id: str,
    ...
    def log_permission_change(self, user_id: str, target_user: str,
                             action: str, permission_details: Dict[str, Any],
                             success: bool) -> str:
        """
        ...
        try:
            ...
            event_type = (AuditEventType.PERMISSION_GRANT if action == 'grant'
    ...
    def log_pii_detection(self, user_id: str, resource_id: str,
                         pii_indicators: List[str], sensitivity_level: int,
                         anonymized_path: str) -> str:
        """
        ...
        try:
            ...
            severity = (AuditSeverity.CRITICAL if sensitivity_level >= 5
                       else AuditSeverity.WARNING if sensitivity_level >= 3
    ...
    def generate_audit_report(self, start_time: datetime, end_time: datetime,
                             include_pii_events: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for event in events:
                ...
                # User tracking
                report['unique_users'].add(self._anonymize_user_id(event.user_id))
            ...
            if len(events) > 0:
                report['success_rate'] = (successful_events / len(events)) * 100
            ...
            report['top_operations'] = dict(
                sorted(report['top_operations'].items(),
            ...
            if include_pii_events:
                ...
                report['pii_events'] = {
                    ...
                    'high_sensitivity': len([e for e in pii_events 
    ...
    def _create_audit_event(self, event_type: AuditEventType,
                           severity: AuditSeverity, user_id: str,
                           resource_id: Optional[str], action: str,
                           success: bool, details: Dict[str, Any]) -> AuditEvent:
        ...
        for key, value in details.items():
            if isinstance(value, str):
                ...
                for pattern, replacement in self.pii_patterns.items():
                    anonymized_value = anonymized_value.replace(pattern, replacement)
    ...
    def _parse_audit_event_from_db(self, db_result: tuple) -> Optional[AuditEvent]:
        ...
        try:
            return AuditEvent(
                ...
                anonymized_data=json.loads(db_result[9]) if db_result[9] else None
            )
