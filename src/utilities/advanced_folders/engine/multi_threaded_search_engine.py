...
class SearchEngineMetrics:
    ...
    def update_search_completed(self, search_time: float,
                               files_processed: int):
...
class SearchWorker(QThread):
    ...
    def __init__(self, worker_id: str, task_queue: queue.Queue, 
                 config: SearchEngineConfig, parent=None):
    ...
    def _execute_search_task(self, task: SearchTask) -> SearchResult:
        ...
        try:
            ...
            if not task.directory_path.exists():
                raise FileNotFoundError(f"Directory not found: {task.directory_path}")
            
            # Scan directory for files
            files_to_search = list(self._scan_directory(task.directory_path))
            total_scanned = len(files_to_search)
            
            self.logger.debug(f"Task {task.task_id}: Scanning {total_scanned} files")
            
            # Process files in chunks for better progress reporting
            chunk_size = self.config.chunk_size
            for i, file_chunk in enumerate(self._chunk_list(files_to_search, chunk_size)):
                if self.stop_event.is_set():
                    break
                
                # Process chunk
                chunk_results = self._process_file_chunk(file_chunk, task.search_parameters)
                found_files.extend(chunk_results)
                
                # Update progress
                progress = min(100, int(((i + 1) * chunk_size / total_scanned) * 100))
                self.progress_updated.emit(task.task_id, (i + 1) * chunk_size, total_scanned)
    ...
    def _process_file_chunk(self, files: List[Path], 
                          search_params: SearchParameter) -> List[FileMetadata]:
    ...
    def _file_matches_criteria(self, file_path: Path, 
                             search_params: SearchParameter) -> bool:
        ...
        if hasattr(search_params, 'name_pattern') and search_params.name_pattern:
            if search_params.name_pattern.lower() not in file_path.name.lower():
                return False
        
        # Check file extension
        if hasattr(search_params, 'file_extensions') and search_params.file_extensions:
            if file_path.suffix.lower() not in [ext.lower() for ext in search_params.file_extensions]:
                return False
        
        # Check file size
        if hasattr(search_params, 'size_range') and search_params.size_range:
            file_size = file_path.stat().st_size
            if not (search_params.size_range.min_size <= file_size <= search_params.size_range.max_size):
                return False
        
        # Content search (if enabled and applicable)
        if (self.config.enable_content_search and 
            hasattr(search_params, 'content_pattern') and 
            search_params.content_pattern):
            return self._search_file_content(file_path, search_params.content_pattern)
    ...
    def _search_file_content(self, file_path: Path, pattern: str) -> bool:
        ...
        try:
            ...
            if file_path.suffix.lower() not in ['.txt', '.md', '.py', '.js', '.html', '.css']:
                return False
            ...
            self.logger.debug(f"Content search failed for {file_path}: {str(e)}")
            return False
    ...
    def _extract_file_metadata(self, file_path: Path) -> FileMetadata:
        ...
        try:
            ...
            self.logger.warning(f"Failed to extract metadata for {file_path}: {str(e)}")
    ...
    def stop_worker(self):
        ...
        if self.current_task:
            self.logger.debug(f"Cancelling current task {self.current_task.task_id}")
...
class MultiThreadedSearchEngine(QObject):
    ...
    def __init__(self, config: Optional[SearchEngineConfig] = None, parent=None):
    ...
    def _initialize_engine(self):
        ...
        try:
            ...
            for i in range(self.config.max_workers):
                ...
                worker = SearchWorker(worker_id, self.task_queue, self.config, self)
            ...
            self.logger.debug(f"Initialized {len(self.workers)} worker threads")
    ...
    def submit_search(self, directory_path: Union[str, Path], 
                     search_parameters: SearchParameter,
        ...
        if search_id is None:
            search_id = f"search_{int(time.time() * 1000)}_{len(self.active_searches)}"
        ...
        self.logger.info(f"Submitted search task {search_id} for {directory_path}")
        return search_id
    ...
    def _handle_task_completed(self, task_id: str, result: SearchResult):
        ...
        self.logger.info(f"Search {task_id} completed: {len(result.files)} files found")
