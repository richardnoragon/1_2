...
class CPUTracker:
    ...
    def __init__(self, enable_per_core: bool = True, 
        ...
        try:
            if PSUTIL_AVAILABLE:
                ...
                if self.platform_impl:
                    ...
                    if platform_info:
                        self._cpu_info.architecture = platform_info.get('architecture')
            ...
            try:
                ...
                if freq:
                    freq_info.update({
                        ...
                        'cpu_freq_min': round(freq.min, 2) if freq.min else None,
                        'cpu_freq_max': round(freq.max, 2) if freq.max else None
                    })
                    
                    # Calculate frequency utilization
                    if freq.max and freq.max > 0:
                        utilization = (freq.current / freq.max) * 100
                        freq_info['cpu_freq_utilization'] = round(utilization, 2)
            ...
            if hasattr(psutil, 'sensors_temperatures'):
                ...
                if cpu_temps:
                    ...
                    if len(self._temperature_history) > 100:
                        self._temperature_history = self._temperature_history[-100:]
        ...
        try:
            ...
            if hasattr(psutil, 'getloadavg'):
                ...
                if self._cpu_info:
                    ...
                    load_info.update({
                        'load_per_core_1min': round(load_avg[0] / cores, 2),
                        'load_per_core_5min': round(load_avg[1] / cores, 2),
                        'load_per_core_15min': round(load_avg[2] / cores, 2)
                    })
        ...
        try:
            ...
            if 'cpu_per_core' in metrics:
                ...
                if len(per_core) > 1:
                    ...
                    variance = sum((usage - avg_usage) ** 2 for usage in per_core) / len(per_core)
            ...
            if len(self._cpu_usage_history) >= 2:
                recent_usage = [
                    point['cpu_percent'] for point in self._cpu_usage_history[-10:]
            ...
            error_handler.handle_error(e, "CPUTracker._calculate_derived_metrics")
    ...
    def get_cpu_usage_history(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def get_temperature_history(self, limit: Optional[int] = None) -> List[float]:
    ...
    def get_frequency_history(self, limit: Optional[int] = None) -> List[Dict[str, float]]:
