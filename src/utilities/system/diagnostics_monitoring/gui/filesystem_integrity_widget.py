...
try:
    from PyQt5.QtWidgets import (
        ...
        QSpinBox, QSplitter, QFrame, QScrollArea,
...
class ScanWorker(QThread):
    ...
    def __init__(self, integrity_monitor: IntegrityMonitor, scan_config: Dict[str, Any]):
...
class FilesystemIntegrityWidget(QWidget):
    ...
    def create_overview_tab(self) -> QWidget:
        ...
        self.platform_label = QLabel(f"Platform: {self.platform_detector.platform.value}")
    ...
    def create_repair_recommendations_tab(self) -> QWidget:
        ...
        self.recommendations_table.setHorizontalHeaderLabels([
            "Priority", "Action", "Description", "Command", "Auto", "Time Est."
        ])
    ...
    def create_history_tab(self) -> QWidget:
        ...
        self.history_table.setHorizontalHeaderLabels([
            "Date", "Type", "Duration", "Files", "Errors", "Status", "Actions"
        ])
    ...
    def populate_scan_history(self):
        ...
        try:
            ...
            for row, scan in enumerate(self.scan_history):
                ...
                self.history_table.setItem(
                    row, 2, QTableWidgetItem(f"{scan.get('duration_seconds', 0):.1f}s")
                )
                
                stats = scan.get('statistics', {})
                self.history_table.setItem(
                    row, 3, QTableWidgetItem(str(stats.get('files_scanned', 0)))
                )
                self.history_table.setItem(
                    row, 4, QTableWidgetItem(str(stats.get('errors_encountered', 0)))
                )
                self.history_table.setItem(
