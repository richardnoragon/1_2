...
def ocr_img(
    ...
    try:
        logger.info("Starting OCR on image%s", f": {input_file}" if input_file else "")
        logger.debug("Parameters - Action: %s, Search: %s, Generate output: %s", 
                    action, search_str, generate_output)
...
def parse_args():
    ...
    parser.add_argument("-g", "--generate-output", action="store_true",
...
class OcrUI(QtWidgets.QMainWindow):
    def __init__(self):
        try:
            ...
            self.previewView.setRenderHint(QtWidgets.QPainter.SmoothPixmapTransform)
    ...
    def dropEvent(self, event):
        ...
        for url in event.mimeData().urls():
            ...
            if os.path.isfile(file_path):
                if file_path.lower().endswith(('.pdf', '.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
                    files.append(file_path)
            elif os.path.isdir(file_path):
                # If directory, add all supported files in it
                for root, _, filenames in os.walk(file_path):
                    for filename in filenames:
                        if filename.lower().endswith(('.pdf', '.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
            ...
            if self.batchModeCheckbox.isChecked():
                ...
                if choice == QMessageBox.Yes:
                    ...
                    if folder:
                        ...
                        if self.input_files:
                            self.inputFileEdit.setText(f"Selected folder: {folder} ({len(self.input_files)} files)")
                            self.is_batch = True
                            self.processButton.setEnabled(True)
                        else:
                            QMessageBox.warning(self, "Warning", "No supported files found in selected folder!")
                else:
                    ...
                    if filenames:
                        ...
                        self.inputFileEdit.setText(f"Selected {len(filenames)} files")
                        self.is_batch = True
                        self.processButton.setEnabled(True)
            else:
                # Single file mode
    ...
    def process_file(self):
        try:
            if not self.input_files:
                ...
                QMessageBox.warning(self, "Warning", "Please select file(s) first")
                return
            ...
            if self.pagesEdit.text():
                try:
                    pages = tuple(int(p.strip()) for p in self.pagesEdit.text().split(','))
                    logger.info("Processing specific pages: %s", pages)
                except ValueError:
                    logger.error("Invalid page numbers format: %s", self.pagesEdit.text())
                    QMessageBox.warning(self, "Warning", "Invalid page numbers format! Use comma-separated numbers.")
                    return
            ...
            for idx, input_file in enumerate(self.input_files, 1):
                try:
                    ...
                    self.statusBar().showMessage(f"Processing file {idx} of {total_files}: {os.path.basename(input_file)}")
    ...
    def preview_file(self, file_path):
        ...
        try:
            ...
            if not pixmap.isNull():
                ...
                self.previewView.fitInView(self.preview_scene.sceneRect(), QtCore.Qt.KeepAspectRatio)
    ...
    def show_next_preview(self):
        ...
        if self.input_files and self.current_preview_item < len(self.input_files) - 1:
    ...
    def update_navigation_buttons(self):
        ...
        if len(self.input_files) > 1:
            ...
            self.next_button.setEnabled(self.current_preview_item < len(self.input_files) - 1)
        else:
            self.prev_button.hide()
            self.next_button.hide()
