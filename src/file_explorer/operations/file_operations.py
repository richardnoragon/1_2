...
class FileOperation:
    ...
    def __post_init__(self):
        ...
        if (
            self.operation_type
            in (OperationType.COPY, OperationType.MOVE, OperationType.SYNC)
            and not self.destination_path
        ):
            raise ValueError(
                f"{self.operation_type.name} requires destination path"
            )
...
class FileOperationManager(QObject):
    ...
    def _check_dependencies(self, operation: FileOperation) -> None:
        ...
        for dep_id in operation.dependencies:
            if dep_id in self.active_operations:
                ...
                if dep_operation.status not in (
    ...
    def _calculate_operation_size(
        self, operation: FileOperation, result: FileOperationResult
    ) -> None:
        """
    ...
    def _execute_copy_operation(
        ...
        for source_path_str in operation.source_paths:
            ...
            if source_path.is_file():
                ...
                self._copy_directory_with_progress(
                    source_path, dest_path, result
                )

    def _copy_file_with_progress(
        self, source: Path, dest_dir: Path, result: FileOperationResult
    ) -> None:
        ...
        with open(source, "rb") as src, open(dest_file, "wb") as dst:
            while True:
                ...
                if (
                    self.active_operations[result.operation_id].status
                    == OperationStatus.CANCELLED
                ):
                    dst.close()
                    dest_file.unlink()  # Delete partial file
                    raise FileOperationError(
                        "Operation cancelled", operation_id=result.operation_id
                    )
        ...
        for item in source.rglob("*"):
            if item.is_file():
                ...
                self._copy_file_with_progress(
                    item, dest_file.parent, temp_result
                )
        ...
        for source_path_str in operation.source_paths:
            ...
            try:
                ...
                result.warnings.append(
                    f"Failed to delete original {source_path}: {e}"
                )
                self.logger.warning(
                    f"Failed to delete original after move: {e}"
                )
    ...
    def _handle_operation_error(
        ...
        self.logger.error(
            f"Operation failed: {operation.operation_id} - {error}"
        )
    ...
    def _attempt_error_recovery(
        ...
        self.logger.info(
            f"Attempting error recovery for: {operation.operation_id}"
        )
        ...
        self.logger.info(
            f"Error recovery not implemented for: {type(error).__name__}"
        )
    ...
    def _start_resource_monitor(self) -> None:
        ...
        monitor_thread = threading.Thread(
            target=resource_monitor, name="RFU-ResourceMonitor", daemon=True
        )
...
class OperationThrottle:
    ...
    def should_throttle(self) -> bool:
        ...
        if (
            current_time - self.last_operation_time
            < self.min_operation_interval
        ):
            return True
