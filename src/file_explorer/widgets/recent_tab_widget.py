...
class RecentTabWidget(QWidget):
    ...
    def _create_tools_section(self):
        ...
        self.tools_tree.setHeaderLabels(["Tool Name", "Category", "Last Used", "Usage Count"])
    ...
    def _load_configuration(self):
        ...
        if self.config_manager:
            try:
                ...
                self.cleanup_interval_hours = recent_config.get('cleanup_interval_hours', 24)
                self.max_item_age_days = recent_config.get('max_item_age_days', 30)
                
                self.logger.info(f"Configuration loaded: max_dirs={self.max_recent_directories}, max_tools={self.max_recent_tools}")
    ...
    def _load_recent_data(self):
        ...
        try:
            ...
            if recent_file.exists():
                ...
                self.logger.info(f"Loaded {len(self.recent_directories)} directories and {len(self.recent_tools)} tools")
            else:
        ...
        try:
            ...
            for dir_data in self.recent_directories:
                try:
                    ...
                    if not dir_path.exists():
                        self.logger.debug(f"Removing inaccessible directory: {dir_data['path']}")
                        continue
                    
                    # Check age
                    last_accessed = datetime.fromisoformat(dir_data['last_accessed'])
                    if last_accessed < cutoff_time and dir_data['path'] not in self.pinned_directories:
                    ...
                    self.logger.warning(f"Error checking directory {dir_data.get('path')}: {e}")
                    continue
            ...
            for tool_data in self.recent_tools:
                try:
                    ...
                    if last_used < cutoff_time and tool_data['name'] not in self.pinned_tools:
                    ...
                    self.logger.warning(f"Error checking tool {tool_data.get('name')}: {e}")
                    continue
            ...
            if dirs_removed > 0 or tools_removed > 0:
                self.logger.info(f"Cleanup completed: removed {dirs_removed} directories and {tools_removed} tools")
    ...
    def add_recent_directory(self, path: str):
        ...
        try:
            ...
            if existing_index >= 0:
                ...
                self.recent_directories[existing_index]['last_accessed'] = current_time.isoformat()
            ...
            if len(self.recent_directories) > self.max_recent_directories:
                ...
                self.recent_directories = [
                    item for item in self.recent_directories
                    if item['path'] in self.pinned_directories
                ][:self.max_recent_directories//2] + self.recent_directories[:self.max_recent_directories//2]
    ...
    def add_recent_tool(self, tool_name: str, category: str = "Unknown", module_path: str = ""):
        ...
        try:
            ...
            if existing_index >= 0:
                # Update existing entry
                self.recent_tools[existing_index]['last_used'] = current_time.isoformat()
            ...
            if len(self.recent_tools) > self.max_recent_tools:
                ...
                ][:self.max_recent_tools//2] + self.recent_tools[:self.max_recent_tools//2]
        ...
        try:
            ...
            sorted_dirs = sorted(
                ...
                key=lambda x: (
                    ...
                    -datetime.fromisoformat(x['last_accessed']).timestamp()  # Most recent first
                )
            )
            ...
            for dir_data in sorted_dirs:
                try:
                    ...
                    if path in self.pinned_directories:
                        item.setIcon(0, self.style().standardIcon(self.style().SP_DialogSaveButton))
                        for col in range(3):
            ...
            self.logger.debug(f"Refreshed directories display with {len(sorted_dirs)} items")
        ...
        try:
            ...
            sorted_tools = sorted(
                ...
                key=lambda x: (
                    ...
                    -datetime.fromisoformat(x['last_used']).timestamp()  # Most recent first
                )
            )
                ...
                try:
                    ...
                    if name in self.pinned_tools:
                        ...
                        if 'file' in category.lower():
                            ...
                            item.setIcon(0, self.style().standardIcon(self.style().SP_VistaShield))
                        else:
                            item.setIcon(0, self.style().standardIcon(self.style().SP_ComputerIcon))
                    
                    # Set tooltip
                    item.setToolTip(0, f"Tool: {name}\nCategory: {category}\nLast used: {last_used.strftime('%Y-%m-%d %H:%M:%S')}\nUsage count: {usage_count}")
        ...
        try:
            if item:
                ...
                if path and Path(path).exists():
                    ...
                    QMessageBox.warning(self, "Directory Not Found", f"The directory no longer exists:\n{path}")
    ...
    def _show_directory_context_menu(self, position):
        ...
        try:
            ...
            navigate_action.triggered.connect(lambda: self._on_directory_activated(item, 0))
    ...
    def _remove_directory(self, path: str):
        ...
        self.recent_directories = [d for d in self.recent_directories if d['path'] != path]
        if path in self.pinned_directories:
            self.pinned_directories.remove(path)
        self._save_recent_data()
        self._refresh_directories_display()
    ...
    def _clear_recent_directories(self):
        ...
        try:
            ...
            self.recent_directories = [
                d for d in self.recent_directories
            ...
            QMessageBox.information(self, "Cleared", "Recent directories cleared (pinned items preserved)")
    ...
    def _clear_recent_tools(self):
        ...
        try:
            ...
            self.recent_tools = [
                t for t in self.recent_tools
                if t['name'] in self.pinned_tools
            ]
            
            self._save_recent_data()
            self._refresh_tools_display()
            
            QMessageBox.information(self, "Cleared", "Recent tools cleared (pinned items preserved)")
...
if __name__ == '__main__':
    ...
    widget.add_recent_tool("File Finder", "File Management", "src.tools.file_management.file_finder")
    widget.add_recent_tool("Duplicate Finder", "Analysis", "src.tools.analysis.duplicate_finder")
    widget.add_recent_tool("Secure Delete", "Security", "src.tools.security.secure_delete")
