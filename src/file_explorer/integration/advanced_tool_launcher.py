...
from .tool_integration_framework import (FileContext, ToolCapability,
                                         ToolInterface, ToolMetadata,
...
class ToolInstanceManager:
    ...
    def register_instance(
        ...
        with self._lock:
            ...
            self.logger.info(f"Registered tool instance: {metadata.name} (ID: {instance_id})")
            return instance_id
        ...
        with self._lock:
            if instance in self.instances:
                ...
                if instance_id in self.instance_metadata:
                    self.instance_metadata[instance_id]['last_accessed'] = time.time()
                    self.instance_metadata[instance_id]['access_count'] += 1
    
    def get_instance_info(self, instance: ToolInterface) -> Optional[Dict[str, Any]]:
...
class ContextProcessor:
    ...
    def process_context(
        ...
        try:
            ...
            enhanced_context.metadata['tool_category'] = metadata.category.value
            ...
            enhanced_context.metadata['security_level'] = metadata.security_level
        ...
        try:
            ...
            for file_path in context.selected_files:
                if file_path.exists() and file_path.is_file():
                    ...
                    if suffix:
                        file_patterns[suffix] = file_patterns.get(suffix, 0) + 1
            
            context.metadata['file_patterns'] = file_patterns
            
            # Analyze directory structure
            if context.has_subdirectories:
                dir_info = self._analyze_directory_structure(context.selected_files)
                context.metadata['directory_analysis'] = dir_info
            
            # Check for special files
            special_files = self._identify_special_files(context.selected_files)
            if special_files:
    ...
    def _analyze_directory_structure(self, files: List[Path]) -> Dict[str, Any]:
        """Analyze directory structure."""
...
class LaunchQueue:
    ...
    def add_launch(self, config: LaunchConfiguration) -> None:
        ...
        with self._lock:
            ...
            self.logger.info(f"Added {config.tool_name} to launch queue (priority: {config.priority})")
...
class AdvancedToolLauncher(QObject if PYQT_AVAILABLE else object):
    ...
    def __init__(self, registry: ToolRegistry):
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=4, 
            thread_name_prefix="ToolLauncher"
        )
    ...
    def launch_tool(
        ...
        **kwargs
        ...
        try:
            ...
            if PYQT_AVAILABLE:
                self.tool_launch_requested.emit(tool_name, asdict(launch_config))
    ...
    def _validate_launch_request(self, config: LaunchConfiguration) -> bool:
        ...
        try:
            ...
            # Validate context
            if not config.context.selected_files and not config.context.current_directory:
                self.logger.warning(f"Empty context for tool: {config.tool_name}")
    ...
    def _check_resource_limits(self, metadata: ToolMetadata) -> bool:
        ...
        try:
            ...
            if queue_status['running'] >= self.config['max_concurrent_launches']:
                self.logger.warning("Maximum concurrent launches reached")
                return False
            
            # Check memory requirements
            required_memory = metadata.resource_requirements.get('memory_mb', 0)
            if required_memory > 2048:  # 2GB limit
                self.logger.warning(f"Tool requires too much memory: {required_memory}MB")
                return False
    ...
    def _execute_launch(self, config: LaunchConfiguration) -> LaunchResult:
        ...
        try:
            ...
            metadata = self.registry.get_tool_metadata(config.tool_name)
            if not metadata:
                raise ValueError(f"Tool metadata not found: {config.tool_name}")
            
            # Update status
            self.registry.update_tool_status(config.tool_name, ToolStatus.LOADING)
            if PYQT_AVAILABLE:
                self.tool_status_changed.emit(config.tool_name, ToolStatus.LOADING.value)
            ...
            if not tool_instance:
                raise RuntimeError(f"Failed to load tool instance: {config.tool_name}")
            
            # Initialize tool
            self.registry.update_tool_status(config.tool_name, ToolStatus.INITIALIZING)
            if PYQT_AVAILABLE:
                self.tool_status_changed.emit(config.tool_name, ToolStatus.INITIALIZING.value)
            
            if not tool_instance.initialize(enhanced_context):
                raise RuntimeError(f"Tool initialization failed: {config.tool_name}")
            ...
            # Execute tool
            self.registry.update_tool_status(config.tool_name, ToolStatus.RUNNING)
            if PYQT_AVAILABLE:
                self.tool_status_changed.emit(config.tool_name, ToolStatus.RUNNING.value)
            ...
            self.registry.update_tool_status(config.tool_name, ToolStatus.ERROR)
    ...
    def _execute_tool_instance(
        ...
        try:
            if hasattr(instance, 'show') and callable(instance.show):
                ...
                self.logger.warning(f"Tool has no execution method: {config.tool_name}")
                
        except Exception as e:
            self.logger.error(f"Tool execution failed: {config.tool_name} - {e}")
            raise
    ...
    def _collect_performance_metrics(
        ...
        try:
            ...
            if len(self.performance_metrics[tool_name]) > 20:
                self.performance_metrics[tool_name] = self.performance_metrics[tool_name][-20:]
        ...
        try:
            ...
            stats = {
                ...
                'success_rate': (
                    ...
                    if self.launch_history else 0
                ),
                'average_launch_time': (
                    sum(r.duration for r in successful_launches) / len(successful_launches)
                    if successful_launches else 0
                ),
                'queue_status': self.launch_queue.get_queue_status(),
                'active_instances': len(self.instance_manager.get_all_instances())
            }
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            for instance_info in self.instance_manager.get_all_instances():
                self.instance_manager.cleanup_instance(instance_info['instance'])
...
def create_advanced_tool_launcher(registry: ToolRegistry) -> AdvancedToolLauncher:
