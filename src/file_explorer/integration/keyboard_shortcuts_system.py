...
class ShortcutValidator:
    ...
    def validate_shortcut(self, shortcut: ShortcutDefinition) -> Tuple[bool, List[str]]:
        ...
        sequence_valid, sequence_errors = self._validate_key_sequence(shortcut.key_sequence)
        ...
        for platform_name, sequence in shortcut.platform_specific.items():
            ...
            if not seq_valid:
                errors.extend([f"Platform {platform_name}: {error}" for error in seq_errors])
    ...
    def _validate_key_sequence(self, sequence: str) -> Tuple[bool, List[str]]:
        ...
        for pattern in self.invalid_patterns:
            if pattern in sequence.lower():
                errors.append(f"Key sequence contains invalid pattern: {pattern}")
    ...
    def _get_reserved_sequences(self) -> Set[str]:
        ...
        common_reserved = {
            ...
            'alt+f4', 'ctrl+alt+del', 'windows', 'alt+tab'
        }
...
class ConflictResolver:
    ...
    def _resolve_by_priority(self, conflict: ShortcutConflict) -> ShortcutConflict:
    ...
    def _resolve_by_context(self, conflict: ShortcutConflict) -> ShortcutConflict:
    ...
    def _resolve_by_frequency(self, conflict: ShortcutConflict) -> ShortcutConflict:
    ...
    def _resolve_manually(self, conflict: ShortcutConflict) -> ShortcutConflict:
...
class ShortcutRegistry:
    ...
    def __init__(self):
        ...
        self.key_sequence_map: Dict[str, List[str]] = {}  # sequence -> shortcut_ids
    ...
    def register_shortcut(self, shortcut: ShortcutDefinition) -> bool:
        ...
        with self._lock:
            try:
                ...
                if not is_valid:
                    self.logger.error(f"Invalid shortcut {shortcut.id}: {errors}")
                    return False
                ...
                self.logger.info(f"Registered shortcut: {shortcut.id} ({shortcut.key_sequence})")
                return True
                
            except Exception as e:
                self.logger.error(f"Failed to register shortcut {shortcut.id}: {e}")
                return False
    ...
    def get_shortcuts_by_context(self, context: ShortcutContext) -> List[ShortcutDefinition]:
    ...
    def get_shortcuts_by_sequence(self, key_sequence: str) -> List[ShortcutDefinition]:
    ...
    def search_shortcuts(self, query: str) -> List[ShortcutDefinition]:
        ...
        for shortcut in self.shortcuts.values():
            if (query_lower in shortcut.name.lower() or
                ...
                query_lower in shortcut.key_sequence.lower()):
                results.append(shortcut)
        
        return results
    
    def _check_conflicts(self, new_shortcut: ShortcutDefinition) -> List[ShortcutDefinition]:
        ...
        for existing in existing_shortcuts:
            if (existing.context == new_shortcut.context or
                existing.context == ShortcutContext.GLOBAL or
                new_shortcut.context == ShortcutContext.GLOBAL):
                conflicts.append(existing)
        
        # Check alternate sequences
        for alt_sequence in new_shortcut.alternate_sequences:
            alt_shortcuts = self.get_shortcuts_by_sequence(alt_sequence.lower())
    ...
    def get_registry_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'contexts': {ctx.value: len(shortcuts) for ctx, shortcuts in self.context_shortcuts.items()},
            'categories': {cat.value: len(shortcuts) for cat, shortcuts in self.category_shortcuts.items()},
            'conflicts': len(self.conflicts),
            'unresolved_conflicts': len([c for c in self.conflicts if not c.auto_resolved]),
...
class ShortcutManager(QObject if PYQT_AVAILABLE else object):
    ...
    if PYQT_AVAILABLE:
        ...
        conflict_detected = pyqtSignal(str, list)  # key_sequence, conflicting_ids
    ...
    def _load_configuration(self) -> Dict[str, Any]:
        ...
        if ConfigManager:
            try:
                ...
                self.logger.warning(f"Failed to load shortcut configuration: {e}")
    ...
    def _create_qt_shortcut(self, shortcut: ShortcutDefinition) -> None:
        ...
        try:
            ...
            qt_shortcut = QShortcut(
                QKeySequence(shortcut.get_platform_sequence()),
                parent
            )
                ...
                try:
                    if shortcut.enabled and shortcut.callback:
                        ...
                        self.shortcut_activated.emit(
                            shortcut.id, 
                            shortcut.get_platform_sequence()
                        )
                        
                except Exception as e:
                    self.logger.error(f"Shortcut execution failed {shortcut.id}: {e}")
            ...
            self.logger.error(f"Failed to create Qt shortcut for {shortcut.id}: {e}")
    
    def _confirm_shortcut_execution(self, shortcut: ShortcutDefinition) -> bool:
    ...
    def pop_context(self) -> None:
        ...
        if self.context_stack:
            ...
            self.logger.debug(f"Context popped to: {self.current_context.value}")
    
    def get_shortcuts_for_current_context(self) -> List[ShortcutDefinition]:
        """Get shortcuts applicable to current context."""
        context_shortcuts = self.registry.get_shortcuts_by_context(self.current_context)
        global_shortcuts = self.registry.get_shortcuts_by_context(ShortcutContext.GLOBAL)
    ...
    def _initialize_default_shortcuts(self) -> None:
        ...
        default_shortcuts = [
            ...
            },
            {
                ...
                'key_sequence': 'Ctrl+D',
                'callback': lambda: self.logger.info("Launch Duplicate Finder"),
                'category': ShortcutCategory.TOOL_LAUNCH,
                ...
                for shortcut_id, shortcut_data in user_shortcuts.items():
                    try:
                        ...
                        shortcut = ShortcutDefinition(
                            ...
                            context=ShortcutContext(shortcut_data.get('context', 'global')),
                            category=ShortcutCategory(shortcut_data.get('category', 'custom')),
                            priority=ShortcutPriority(shortcut_data.get('priority', 'default')),
                            enabled=shortcut_data.get('enabled', True),
                            visible=shortcut_data.get('visible', True),
                            tooltip=shortcut_data.get('tooltip', ''),
                            alternate_sequences=shortcut_data.get('alternate_sequences', []),
                            platform_specific=shortcut_data.get('platform_specific', {}),
                            requires_confirmation=shortcut_data.get('requires_confirmation', False),
                        ...
                        self.logger.error(f"Failed to load user shortcut {shortcut_id}: {e}")
    ...
    def save_user_shortcuts(self) -> bool:
        ...
        try:
            ...
            for shortcut_id, shortcut in self.registry.shortcuts.items():
                if (shortcut.priority == ShortcutPriority.USER or
                    shortcut.category == ShortcutCategory.CUSTOM):
            ...
            if ConfigManager:
                config_manager = ConfigManager()
                config_manager.set_setting('shortcuts', 'user_shortcuts', user_shortcuts)
    ...
    def import_shortcuts(self, file_path: Path, merge: bool = True) -> bool:
        ...
        try:
            ...
            imported_count = 0
            for shortcut_id, shortcut_data in import_data.get('shortcuts', {}).items():
                try:
                    # Create shortcut definition
                    shortcut = ShortcutDefinition(
                        id=shortcut_id,
                        ...
                        context=ShortcutContext(shortcut_data.get('context', 'global')),
                        category=ShortcutCategory(shortcut_data.get('category', 'custom')),
                        priority=ShortcutPriority(shortcut_data.get('priority', 'user')),
                        enabled=shortcut_data.get('enabled', True),
                        visible=shortcut_data.get('visible', True),
                        tooltip=shortcut_data.get('tooltip', ''),
                        alternate_sequences=shortcut_data.get('alternate_sequences', []),
                    ...
                    self.logger.error(f"Failed to import shortcut {shortcut_id}: {e}")
            
            self.logger.info(f"Imported {imported_count} shortcuts from: {file_path}")
