...
class FileContext:
    ...
    def __post_init__(self):
        ...
        if self.selected_files:
            ...
            self.file_types = {
                f.suffix.lower() for f in self.selected_files if f.suffix
            }
            self.has_subdirectories = any(
                f.is_dir() for f in self.selected_files if f.exists()
            )
...
class ToolRegistry:
    ...
    def register_tool(self, tool_metadata: ToolMetadata) -> bool:
        ...
        with self._lock:
            try:
                ...
                self.logger.error(
                    f"Failed to register tool {tool_metadata.name}: {e}"
                )
                return False
    ...
    def search_tools(self, query: str) -> List[ToolMetadata]:
        ...
        for tool in self.tools.values():
            if (
                ...
                or any(
                    query_lower in keyword.lower() for keyword in tool.keywords
                )
            ):
                results.append(tool)
    ...
    def update_tool_status(self, tool_name: str, status: ToolStatus) -> None:
        ...
        with self._lock:
            ...
            self.logger.debug(
                f"Tool {tool_name} status updated to {status.value}"
            )
        ...
        with self._lock:
            if tool_name in self.usage_stats:
                ...
                stats["average_duration"] = (
                    (current_avg * (launch_count - 1)) + duration
                ) / launch_count
...
class ToolDiscovery:
    ...
    def _discover_from_filesystem(self) -> List[ToolMetadata]:
        ...
        for base_path in self.discovery_paths:
            ...
            for py_file in base_path.rglob("*.py"):
                ...
                try:
                    tool_metadata = self._extract_tool_metadata_from_file(
                        py_file
                    )
                    if tool_metadata:
    ...
    def _discover_from_modules(self) -> List[ToolMetadata]:
        ...
        module_patterns = ["src.tools.*.*.*.py", "src.tools.*.*"]
        ...
        for pattern in module_patterns:
            try:
                ...
                self.logger.debug(
                    f"Module discovery failed for pattern {pattern}: {e}"
                )
    ...
    def _discover_from_configuration(self) -> List[ToolMetadata]:
        ...
        if ConfigManager:
            try:
                ...
                tool_configs = config.get_setting(
    ...
    def _should_exclude_file(self, file_path: Path) -> bool:
        ...
        for pattern in self.exclude_patterns:
            if pattern.startswith("*") and file_path.name.endswith(
                pattern[1:]
            ):
                return True
    ...
    def _extract_tool_metadata_from_file(
        self, file_path: Path
        ...
        try:
            ...
            if "class" in content and (
                "GUI" in content or "Window" in content or "Tool" in content
            ):
                # Extract class names
                import re

                class_pattern = (
                    r"class\s+(\w+(?:GUI|Window|Tool|App))\s*\([^)]*\):"
                )
    ...
    def _create_metadata_from_config(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create metadata from config for {tool_name}: {e}"
            )
            return None
...
class ToolLauncher(QObject):
    ...
    def launch_tool(
        ...
        try:
            ...
            # Security validation
            if not self._security_validator.validate_tool_launch(
                metadata, context
            ):
                self.logger.error(
                ...
                self.tool_launch_failed.emit(
                    tool_name, "Security validation failed"
                )
                return False
            ...
            future = self.thread_pool.submit(
                self._launch_tool_async, tool_name, context, **kwargs
            )
    ...
    def _launch_tool_async(
        ...
        try:
            ...
            if not tool_instance:
                raise Exception(
                    f"Failed to load tool instance for {tool_name}"
                )

            # Update status
            self.registry.update_tool_status(
                tool_name, ToolStatus.INITIALIZING
            )
            self.tool_status_changed.emit(
                tool_name, ToolStatus.INITIALIZING.value
            )
            ...
            if hasattr(tool_instance, "show"):
                QApplication.instance().postEvent(
                    QApplication.instance(), lambda: tool_instance.show()
                )
            ...
            self._performance_monitor.record_launch_metrics(
                tool_name, duration, success
            )
    ...
    def _record_launch_attempt(
        self, tool_name: str, context: FileContext
    ) -> None:
...
class ToolSecurityValidator:
    ...
    def validate_tool_launch(
        self, metadata: ToolMetadata, context: FileContext
    ) -> bool:
        ...
        try:
            ...
            if not self._validate_resource_limits(
        ...
        if ConfigManager:
            try:
                ...
                policies = config.get_setting(
                    "security", "tool_policies", default_policies
                )
    ...
    def _validate_file_access(
        ...
        if context.selection_count > policies["max_file_count"]:
            self.logger.warning(
                f"File count exceeds limit: {context.selection_count}"
            )
            return False
        ...
        if context.total_size > policies["max_total_size"]:
            self.logger.warning(
                f"Total size exceeds limit: {context.total_size}"
            )
            return False

        # Check file extensions
        blocked_extensions = policies.get("blocked_extensions", set())
        if blocked_extensions and context.file_types.intersection(
            blocked_extensions
        ):
            self.logger.warning(
                f"Blocked file types detected: {context.file_types.intersection(blocked_extensions)}"
            )
            return False
...
class ToolIntegrationManager:
    ...
    def _initialize_system(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Tool integration system initialized: "
                f"{registration_count} tools registered from {len(discovered_tools)} discovered"
            )

        except Exception as e:
            self.logger.error(
                f"Failed to initialize tool integration system: {e}"
            )
    ...
    def get_tools_for_context(
        ...
        for metadata in self.registry.tools.values():
            ...
            if metadata.file_type_filters:
                if not any(
                    file_type in metadata.file_type_filters
            ...
            if (
                context.has_subdirectories
                and ToolCapability.DIRECTORY_OPERATIONS
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_launches": len(
                [
                    h
                    for h in self.launcher.get_launch_history()
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error during tool integration system shutdown: {e}"
            )
