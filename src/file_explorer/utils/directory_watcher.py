...
class EventFilter:
    ...
    def __init__(self):
        ...
        # Event type filters
        self.allowed_event_types: Set[FileSystemEventType] = set(
            FileSystemEventType
        )
...
class EventDebouncer:
    ...
    def __init__(self, debounce_interval: float = 0.5):
        ...
        # Event storage
        self._pending_events: DefaultDict[str, WatchEvent] = defaultdict(
            lambda: None
        )
    ...
    def _setup_timer(self):
        ...
        if QT_AVAILABLE:
            ...
            self._processing_timer.timeout.connect(
    ...
    def add_event(self, event: WatchEvent):
        ...
        with self._lock:
            ...
            self.logger.debug(
                f"Added event for {event.path}: {event.event_type.name}"
            )
...
class WatchdogEventHandler(FileSystemEventHandler):
    ...
    def __init__(self, watcher: "DirectoryWatcher"):
        ...
        self.logger = logging.getLogger(
            "RFU.FileExplorer.WatchdogEventHandler"
        )
...
class DirectoryWatcher(QObject):
    ...
    def __init__(
        self, debounce_interval: float = 0.5, max_events_per_batch: int = 100
    ):
        """
    ...
    def _setup_monitoring(self):
        ...
        if WATCHDOG_AVAILABLE:
            ...
            self.logger.warning(
                "Watchdog not available, monitoring will be limited"
            )
        ...
        try:
            ...
            with self._lock:
                if path_str in self._watched_paths:
                    self.logger.debug(
                        f"Path already being monitored: {path_str}"
                    )
                    return True

                if self._observer and self._event_handler:
                    self._observer.schedule(
                        self._event_handler, path_str, recursive=recursive
                    )
                ...
                self.logger.info(
                    f"Added watch path: {path_str} (recursive={recursive})"
                )
                return True
    ...
    def _process_raw_event(self, event: WatchEvent):
        ...
        try:
            ...
            self.logger.debug(
                f"Processed raw event: {event.event_type.name} - {event.path}"
            )
    ...
    def _handle_debounced_events(self, events: List[WatchEvent]):
        ...
        try:
            ...
            if len(events) > self.max_events_per_batch:
                self.logger.warning(
                    f"Large event batch ({len(events)}), processing in chunks"
                )
        ...
        try:
            ...
            for event in events:
                try:
                    ...
                    self.logger.warning(
                        f"Error processing individual event: {e}"
                    )
