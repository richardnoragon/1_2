...
from .widget_lifecycle_manager import (WidgetLifecycleManager,
                                       ...
                                       is_widget_valid, register_widget,
                                       safe_destroy_widget,
...
class EnhancedBasePaneWidget:
    ...
    def __init__(self, config, parent=None):
        ...
        # Widget tracking
        self._widget_ids: Dict[str, str] = {}  # Maps widget name to lifecycle ID
        ...
        try:
            ...
            if self._title_widget:
                self._title_widget_id = register_widget(
                    self._title_widget, 
                    f"TitleWidget_{self.config.pane_id}"
                )
            ...
            self.logger.error(f"Failed to setup widgets for pane {self.config.pane_id}: {e}")
    ...
    def _create_title_widget(self) -> Optional[QWidget]:
        ...
        try:
            ...
            self._title_label_id = register_widget(
                title_label, 
                f"TitleLabel_{self.config.pane_id}"
            )
    ...
    def safe_update_title_display(self):
        ...
        try:
            ...
            if not is_widget_valid(self._title_label_id):
                self.logger.warning(f"Title label widget is no longer valid for pane {self.config.pane_id}")
                return
            ...
            if success:
                self.logger.debug(f"Updated title display for pane {self.config.pane_id}")
            else:
                self.logger.warning(f"Failed to update title display for pane {self.config.pane_id}")
                
        except Exception as e:
            self.logger.error(f"Error updating title display for pane {self.config.pane_id}: {e}")
    ...
    def safe_close_pane(self) -> bool:
        ...
        try:
            ...
            if QT_AVAILABLE and hasattr(self, 'paneClosed') and self.paneClosed:
            ...
            self.logger.error(f"Failed to close pane {self.config.pane_id}: {e}")
            return False
        ...
        try:
            ...
            for widget_name in widget_destroy_order:
                ...
                if widget_id and is_widget_valid(widget_id):
                    ...
                    if success:
                        self.logger.debug(f"Destroyed {widget_name} widget for pane {self.config.pane_id}")
                    else:
                        self.logger.warning(f"Failed to destroy {widget_name} widget for pane {self.config.pane_id}")
            ...
            self.logger.debug(f"Widget cleanup completed for pane {self.config.pane_id}")
            
        except Exception as e:
            self.logger.error(f"Error during widget cleanup for pane {self.config.pane_id}: {e}")
    ...
    def set_modified(self, modified: bool = True):
        ...
        try:
            if self._is_modified != modified:
                ...
                if QT_AVAILABLE and hasattr(self, 'paneModified') and self.paneModified:
                    self.paneModified.emit(self.config.pane_id)
                
                self.safe_update_title_display()
                
        except Exception as e:
            self.logger.error(f"Error setting modified state for pane {self.config.pane_id}: {e}")
    ...
    def activate(self):
        ...
        try:
            if not self._is_active:
                ...
                self._last_access_time = datetime.now()
                self.config.state = getattr(self.config, 'PaneState', {}).get('ACTIVE', 'ACTIVE')
                
                if QT_AVAILABLE and hasattr(self, 'paneActivated') and self.paneActivated:
                    self.paneActivated.emit(self.config.pane_id)
                
                self._on_activated()
                
        except Exception as e:
            self.logger.error(f"Error activating pane {self.config.pane_id}: {e}")
    ...
    def deactivate(self):
        ...
        try:
            if self._is_active:
                self._is_active = False
                self.config.state = getattr(self.config, 'PaneState', {}).get('INACTIVE', 'INACTIVE')
                
                if QT_AVAILABLE and hasattr(self, 'paneDeactivated') and self.paneDeactivated:
                    self.paneDeactivated.emit(self.config.pane_id)
                
                self._on_deactivated()
                
        except Exception as e:
            self.logger.error(f"Error deactivating pane {self.config.pane_id}: {e}")
    ...
    def is_valid(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error checking pane validity for {self.config.pane_id}: {e}")
            return False
...
class EnhancedPaneManager:
    ...
    def create_pane(self, config, parent=None) -> str:
        ...
        try:
            ...
            if not pane.is_valid():
                self.logger.error(f"Failed to create valid pane widgets for {config.pane_id}")
                return ""
    ...
    def remove_pane(self, pane_id: str) -> bool:
        ...
        try:
            ...
            if not pane.is_valid():
                self.logger.warning(f"Pane {pane_id} is already invalid, forcing removal")
            else:
                # Try to close pane normally
                if not pane.safe_close_pane():
                    self.logger.warning(f"Pane {pane_id} could not be closed normally, forcing removal")
    ...
    def get_pane(self, pane_id: str) -> Optional[EnhancedBasePaneWidget]:
        ...
        try:
            ...
            if pane and not pane.is_valid():
                self.logger.warning(f"Pane {pane_id} is invalid, removing from management")
    ...
    def validate_all_panes(self) -> Dict[str, bool]:
        ...
        try:
            ...
            invalid_count = sum(1 for valid in validation_results.values() if not valid)
            if invalid_count > 0:
                self.logger.warning(f"Found {invalid_count} invalid panes during validation")
