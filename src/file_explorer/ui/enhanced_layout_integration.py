...
from .responsive_layout_manager import (
    ContentHierarchyManager,
    LayoutImplementor,
    LayoutType,
...
class EnhancedLayoutIntegration(QObject):
    ...
    def _integrate_with_explorer(self):
        ...
        self.main_explorer.apply_responsive_layout = (
    ...
    def _enhanced_update_pane_layout(self):
        ...
        try:
            ...
            current_mode = getattr(
                self.main_explorer, "layout_mode", "horizontal"
            )
    ...
    def _enhanced_validate_and_update_layout(self):
        ...
        try:
            ...
            if current_responsive and current_responsive in [
                opt[0] for opt in available
            ]:
    ...
    def apply_responsive_layout(
        ...
        try:
            ...
            if not panes:
                self.logger.warning(
                    "No panes available for layout application"
                )
                return False
    ...
    def get_layout_recommendations(self, pane_count: int) -> Dict[str, Any]:
        ...
        try:
            ...
            recommended = self.layout_manager.get_recommended_layout(
                pane_count
            )
            ...
            return {
                ...
                "available_layouts": [
                    {
                        "type": lt.value,
                        "display_name": self.layout_manager.layout_configs[
                            lt
                        ].display_name,
                        "description": self.layout_manager.layout_configs[
                            lt
                        ].description,
                    }
                    for lt in available
    ...
    def _responsive_to_legacy_mode(
    ...
    def _on_constraints_updated(
        self, pane_count: int, available_layouts: List
    ):
        """Handle constraint updates from responsive manager."""
        # Convert to display format
        options = [
            (lt, self.layout_manager.layout_configs[lt].display_name)
    ...
    def _update_layout_options_for_viewport(
        self, pane_count: int, viewport_type: ViewportType
    ):
        ...
        try:
            ...
            if current_responsive and current_responsive not in available:
                ...
                if recommended:
                    new_legacy_mode = self._responsive_to_legacy_mode(
                        recommended
                    )
                    if new_legacy_mode:
            ...
            self.logger.error(
                f"Error updating layout options for viewport: {e}"
            )
    ...
    def apply_responsive_layout(
        ...
        try:
            ...
            if not panes or not container:
                self.logger.error(
                    "Missing panes or container for layout application"
                )
                return False
            ...
            if success:
                ...
                self.logger.info(
                    f"Applied responsive layout: {layout_type.value}"
                )
        ...
        try:
            ...
            return {
                ...
                "current_layout": {
                    ...
                    "display_name": (
                        self.layout_manager.layout_configs[
                            current_layout
                        ].display_name
                        if current_layout
                        else None
...
def create_enhanced_layout_integration(
    main_explorer,
) -> EnhancedLayoutIntegration:
    """
