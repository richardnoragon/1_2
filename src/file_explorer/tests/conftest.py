...
class MockFileSystem:
    ...
    def create_directory_structure(self):
        ...
        files = [
            ...
            (
                "Documents/Projects/Python/config.ini",
                "[DEFAULT]\ndebug = true\nlog_level = INFO",
            ),
            # Web projects
            (
            ...
            (
                "Documents/Archive/backup_2023.tar.gz",
                b"\x1f\x8b\x08fake_tar_gz_data",
            ),
            # Pictures (binary data simulation)
            (
                "Pictures/2023/vacation.jpg",
                b"\xff\xd8\xff\xe0fake_jpeg_header",
            ),
            ...
            (
                ...
                b"RIFF\x00\x00\x00\x00AVIfake_avi_data",
            ),
            # Downloads
...
class PerformanceMonitor:
    ...
    def assert_performance(self, name: str, max_duration: float):
        ...
        ), f"{name} took {duration:.3f}s, expected <= {max_duration:.3f}s"
...
class MemoryMonitor:
    ...
    def assert_memory_limit(self, max_increase_mb: int):
        ...
        ), f"Memory increased by {increase_mb:.1f}MB, expected <= {max_increase_mb}MB"
...
class TestDataGenerator:
    ...
    @staticmethod
    def generate_large_directory(
        base_path: Path, num_files: int = 1000, num_dirs: int = 100
    ):
    ...
    def generate_mixed_content_directory(base_path: Path):
        ...
        content_types = [
            ...
            (
                "images",
                ".jpg",
                lambda i: b"fake_image_data_" + str(i).encode(),
            ),
            (
                "documents",
                ".pdf",
                lambda i: b"fake_pdf_data_" + str(i).encode(),
            ),
            (
                "archives",
                ".zip",
                lambda i: b"fake_zip_data_" + str(i).encode(),
            ),
            (
                "code",
                ".py",
                lambda i: f"# Python file {i}\nprint('Hello {i}')",
            ),
        ]
    ...
    @staticmethod
    def generate_nested_directory_structure(
        base_path: Path, depth: int = 5, width: int = 3
    ):
            ...
            for i in range(width):
                ...
                for j in range(2):
                    ...
                    file_path.write_text(
                        f"Content at depth {current_depth}, file {j}"
                    )
...
def large_directory():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        TestDataGenerator.generate_large_directory(
            base_path, 100, 20
        )  # Smaller for CI
        yield base_path
...
def nested_directory():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        base_path = Path(temp_dir)
        TestDataGenerator.generate_nested_directory_structure(
            base_path, 4, 3
        )  # Smaller for CI
        yield base_path
