        ...
        for _ in range(count):
            if self.panes:
                ...
                if pane_id and is_widget_valid(pane_id):
                    ...
                    if not success:
                        self.logger.warning(
                            f"Failed to safely remove pane, widget may be deleted"
                        )
                    ...
                    try:
                        ...
                        if "wrapped C/C++ object" in str(e):
                            self.logger.warning(
                                f"Widget already deleted during removal: {e}"
                            )
                        else:
                            raise
    ...
    try:
        ...
        for widget in widgets_to_remove:
            try:
                ...
                if "wrapped C/C++ object" in str(e):
                    self.logger.warning(
                        f"Widget already deleted during layout update: {e}"
                    )
                else:
                    raise
        ...
        for pane in self.panes:
            ...
            if pane_id and is_widget_valid(pane_id):
                ...
                try:
                    ...
                    if "wrapped C/C++ object" in str(e):
                        self.logger.warning(
                            f"Skipping deleted pane widget: {e}"
                        )
                        continue
                    raise
            else:
        ...
        self.logger.debug(
            f"Successfully updated layout for {len(self.panes)} panes"
        )
...
def safe_add_widget_to_splitter(self, widget):
    ...
    try:
        ...
        if widget_id and is_widget_valid(widget_id):
            ...
            if not success:
                self.logger.warning(
                    "Failed to add widget to splitter through lifecycle manager"
                )
        else:
            ...
            try:
                ...
                if "wrapped C/C++ object" in str(e):
                    self.logger.warning(
                        f"Cannot add deleted widget to splitter: {e}"
                    )
                else:
                    raise
    ...
    try:
        if len(self.panes) >= 4:
                ...
                if pane_id and is_widget_valid(pane_id):
                    ...
                    try:
                        ...
                        if "wrapped C/C++ object" not in str(e):
                            raise
                        self.logger.warning(
                            f"Skipping deleted pane in grid layout: {e}"
                        )
            ...
            if len(valid_panes) >= 4:
                ...
                bottom_splitter_id = register_widget(
                    bottom_splitter, "BottomSplitter"
                )
                ...
                try:
                    ...
                    if "wrapped C/C++ object" in str(e):
                        self.logger.warning(
                            f"Widget deletion during grid creation: {e}"
                        )
                ...
                self.logger.warning(
                    f"Insufficient valid panes for grid layout: {len(valid_panes)}"
                )
...
def enhanced_save_configuration(self):
    ...
    try:
        ...
        if hasattr(self, "saveState"):
            enhanced_config.set_setting(
                "file_explorer", "window_state", self.saveState()
            )
        ...
        if hasattr(self, "pane_splitter") and self.pane_splitter:
            try:
                ...
                enhanced_config.set_setting(
                    "file_explorer", "splitter_state", splitter_state
                )
            except RuntimeError as e:
                if "wrapped C/C++ object" in str(e):
                    self.logger.warning(
                        f"Cannot save splitter state, widget deleted: {e}"
                    )
                else:
                    raise
...
def enhanced_cleanup(self):
    ...
    try:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to save configuration during cleanup: {e}"
            )
            ...
            if pane_id:
                ...
                try:
                    ...
                    if "wrapped C/C++ object" not in str(e):
                        raise
                    self.logger.warning(
                        f"Widget already deleted during cleanup: {e}"
                    )
        ...
        if hasattr(self, "pane_splitter") and self.pane_splitter:
            try:
                ...
                if "wrapped C/C++ object" not in str(e):
                    raise
                self.logger.warning(
                    f"Splitter already deleted during cleanup: {e}"
                )
