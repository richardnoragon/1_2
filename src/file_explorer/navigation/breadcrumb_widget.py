            ...
            if path_obj.exists() and path_obj.is_dir():
                ...
                for subdir in subdirs[:20]:  # Limit to prevent huge menus
                    ...
                    action.triggered.connect(
                        lambda checked, p=str(
                            subdir
                        ): self.segment_selected.emit(p)
                    )
                ...
                if len(subdirs) > 20:
                    menu.addSeparator()
                    more_action = QAction(
                        f"... and {len(subdirs) - 20} more", menu
                    )
...
class BreadcrumbWidget(QWidget):
    ...
    def _update_breadcrumb_display(self) -> None:
        ...
        for i, segment in enumerate(self.segments):
            ...
            if (
                ...
                and self.show_dropdowns
                and not segment.is_current
            ):
                dropdown = BreadcrumbDropdown(segment, self)
                dropdown.segment_selected.connect(
    ...
    def _create_segment_widget(self, segment: BreadcrumbSegment) -> QWidget:
        ...
        if segment.path == "...":
            ...
            label.clicked.connect(
                lambda: self._on_segment_clicked(segment.path)
            )
            label.right_clicked.connect(
                lambda: self._on_segment_right_clicked(segment.path)
            )
    ...
    def show_root_menu(self) -> None:
        ...
        for drive in drives:
            ...
            action.triggered.connect(
                lambda checked, d=drive: self.navigation_requested.emit(d)
            )
            menu.addAction(action)
        ...
        # Show menu
        menu.exec_(
            self.root_button.mapToGlobal(QPoint(0, self.root_button.height()))
        )
    ...
    def show_full_path_menu(self) -> None:
        ...
        for i, part in enumerate(parts):
            ...
            action.triggered.connect(
                lambda checked, p=current_path: self.navigation_requested.emit(
                    p
                )
            )
            menu.addAction(action)
    ...
    def show_context_menu(self, position: QPoint) -> None:
        ...
        if segment_path:
            copy_segment_action = QAction(
                f"Copy '{Path(segment_path).name}' Path", menu
            )
            copy_segment_action.triggered.connect(
                lambda: self.copy_path_to_clipboard(segment_path)
            )
    ...
    def _get_segment_at_position(self, position: QPoint) -> Optional[str]:
        ...
        for widget in self.segment_widgets:
            ...
            ) and widget.geometry().contains(position):
    ...
    def open_in_file_manager(self) -> None:
        ...
        if self.current_path:
            ...
            try:
                ...
                self.logger.info(
                    f"Opened in file manager: {self.current_path}"
                )
