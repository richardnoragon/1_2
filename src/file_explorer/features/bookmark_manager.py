...
try:
    from src.config_manager import get_config_manager as _get_config_manager
    ...
    def validate(self) -> bool:
        ...
        try:
            if self.is_folder:
                ...
                self.metadata.validation_status = (
                    BookmarkValidationStatus.VALID
                )
            ...
            if self.bookmark_type == BookmarkType.URL:
                ...
                self.metadata.validation_status = (
                    BookmarkValidationStatus.PENDING
                )
            ...
            if self.bookmark_type == BookmarkType.SEARCH:
                ...
                if self.path:
                    ...
                    self.metadata.validation_status = (
                        BookmarkValidationStatus.INVALID
                    )
            ...
            if path_obj.exists():
                if (
                    self.bookmark_type == BookmarkType.FILE
                    ...
                    self.bookmark_type == BookmarkType.DIRECTORY
                    and path_obj.is_dir()
                ):
                    self.metadata.validation_status = (
                        BookmarkValidationStatus.VALID
                    ...
                    self.metadata.validation_status = (
                        BookmarkValidationStatus.WARNING
                    )
...
class BookmarkDatabase:
            ...
            with self.db_lock:
                ...
                """,
                    (
                        ...
                        bookmark.name,
                        bookmark.path,
                        bookmark.bookmark_type.name,
                        bookmark.parent_id,
                        bookmark.order_index,
                        bookmark.is_folder,
                        bookmark.is_expanded,
                ...
                """,
                    (
                        ...
                        bookmark.metadata.access_count,
        ...
        try:
            with self.db_lock:
                ...
                for bid in all_ids:
                    ...
                    cursor.execute(
                        "DELETE FROM bookmark_metadata WHERE bookmark_id = ?",
                        (bid,),
                    )
                    cursor.execute(
                        "DELETE FROM bookmark_tags WHERE bookmark_id = ?",
                    ...
                    cursor.execute(
                        "DELETE FROM bookmark_collection_members WHERE bookmark_id = ?",
                        (bid,),
                    )
                ...
                self.logger.debug(
                    f"Deleted bookmark {bookmark_id} and {len(descendants)} children"
                )
                return True
    ...
    def _find_descendants(self, bookmark_id: str) -> List[str]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to find descendants of {bookmark_id}: {e}"
            )
    ...
    def search_bookmarks(
        self, query: str, search_tags: bool = True, search_notes: bool = True
    ) -> List[str]:
...
class BookmarkManager(QObject):
    ...
    bookmark_moved = pyqtSignal(
        str, str, str
    )  # bookmark_id, old_parent, new_parent
    ...
    def add_bookmark(
        ...
        bookmark_type: BookmarkType = BookmarkType.DIRECTORY,
        parent_id: Optional[str] = None,
        metadata: Optional[BookmarkMetadata] = None,
    ) -> str:
        """
        Add new bookmark.
    ...
    def move_bookmark(
        ...
        try:
            ...
            if new_parent_id:
                ...
                bookmark.order_index = new_parent.children_ids.index(
                    bookmark_id
                )
            else:
                if new_index is not None:
                ...
                bookmark.order_index = self.root_bookmark_ids.index(
                    bookmark_id
                )
            ...
            if self.bookmark_db.save_bookmark(bookmark):
                self.bookmark_moved.emit(
                    bookmark_id, old_parent_id, new_parent_id
                )
    ...
    def get_root_bookmarks(self) -> List[Bookmark]:
        ...
        return [
            self.bookmarks[bid]
            for bid in self.root_bookmark_ids
            if bid in self.bookmarks
        ]
    ...
    def get_children(self, bookmark_id: str) -> List[Bookmark]:
        ...
        return [
            self.bookmarks[bid]
            for bid in bookmark.children_ids
            if bid in self.bookmarks
        ]

    def search_bookmarks(
        self, query: str, include_tags: bool = True, include_notes: bool = True
    ) -> List[Bookmark]:
        """
        Search bookmarks.
    ...
    def access_bookmark(self, bookmark_id: str) -> bool:
        ...
        try:
            ...
            if len(self.recent_bookmarks) > self.max_recent_bookmarks:
                self.recent_bookmarks = self.recent_bookmarks[
                    : self.max_recent_bookmarks
                ]
            ...
            self.logger.error(
                f"Failed to record bookmark access {bookmark_id}: {e}"
            )
            return False
    ...
    def get_recent_bookmarks(self, limit: int = 10) -> List[Bookmark]:
        ...
        return [
            self.bookmarks[bid]
            for bid in self.recent_bookmarks[:limit]
            if bid in self.bookmarks
        ]
    ...
    def export_bookmarks(
        self, file_path: str, format_type: str = "json"
    ) -> bool:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            folder_count = sum(
                1 for b in self.bookmarks.values() if b.is_folder
            )
            file_bookmarks = sum(
                1
            ...
            favorite_count = sum(
                1 for b in self.bookmarks.values() if b.is_favorite
            )
            ...
            valid_count = sum(
                1
                for b in self.bookmarks.values()
                if b.metadata.validation_status
                == BookmarkValidationStatus.VALID
            )
            invalid_count = sum(
                1
