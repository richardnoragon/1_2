...
with file_utilities_2 integration, including user preferences, default
...
class FileSplitterConfig:
    ...
    def set(self, key: str, value: Any, save: bool = True):
        ...
        self.logger.debug(f"Configuration updated: {key} = {value} (was {old_value})")
    ...
    def validate_config(self) -> Dict[str, str]:
        ...
        if log_level not in valid_levels:
            errors["log_level"] = f"Invalid log level (must be one of {valid_levels})"

        # Validate output directory if specified
        output_dir = self.get_default_output_directory()
        if output_dir and not os.path.isdir(output_dir):
            errors["default_output_dir"] = "Default output directory does not exist"
    ...
    def __str__(self) -> str:
        ...
        custom_settings = summary["custom_settings"]
        summary_text = (
            f"FileSplitterConfig({total_settings} settings, "
            f"{custom_settings} custom)"
        )
        return summary_text

    def __repr__(self) -> str:
        """Detailed string representation."""
        settings_count = len(self._config)
        return (
            "FileSplitterConfig("
            f"config_path='{self.config_path}', settings={settings_count})"
        )
