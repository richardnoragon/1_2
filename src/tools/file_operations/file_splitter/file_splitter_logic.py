...
def _validate_and_sanitize_path(
    path: str, base_dir: Optional[str] = None, operation_type: str = "read"
) -> str:
    """
        ...
        for pattern in dangerous_patterns:
            if pattern in check_path:
                ...
                raise FileSplitterSecurityError(
                    f"Path contains suspicious traversal pattern: {pattern}"
                )
    ...
    if base_dir:
        try:
            ...
            if not canonical_path.startswith(base_canonical):
                logger.warning(
                    "Path outside boundary detected: %s not in %s",
                    canonical_path,
                    base_canonical,
                )
                raise FileSplitterSecurityError(
                    "Path outside allowed directory boundary: " f"{canonical_path}"
                )
        except (OSError, ValueError) as e:
    ...
    if operation_type == "write":
        ...
        if os.path.isabs(path) and not base_dir:
            ...
            for root in drive_roots:
                if path_upper.startswith(root.upper()):
                    ...
                    is_allowed = any(
                        path_upper.startswith(allowed) for allowed in allowed_paths
                    )
                    if not is_allowed:
                        logger.warning(
                            "Absolute path write blocked: %s", canonical_path
                        )
                        raise FileSplitterSecurityError(
                            "Absolute path write not allowed: " f"{canonical_path}"
                        )
        ...
        for sys_dir in system_dirs:
            try:
                sys_canonical = (
                    str(Path(sys_dir).resolve()) if os.path.exists(sys_dir) else sys_dir
                )
                if canonical_path.startswith(sys_canonical):
                    logger.warning(
                        "Attempted write to system directory: %s",
                        canonical_path,
                    )
                    raise FileSplitterSecurityError(
                        "Write operation to system directory not allowed: " f"{sys_dir}"
                    )
...
def _create_secure_temp_dir() -> str:
    ...
    try:
        ...
        raise FileSplitterSecurityError(
            f"Failed to create secure temporary directory: {e}"
        )
...
class FileSplitterLogic(QObject):
    ...
    def __init__(self, config_manager=None, logger=None):
        ...
        self.logger.info(
            "FileSplitterLogic initialized with file_utilities_2 integration"
        )
    ...
    def stop(self) -> None:
        ...
        if self.hub_connector:
            self.hub_connector.report_status_to_hub(
                "stopped", {"stop_time": datetime.now().isoformat()}
            )
    ...
    def _report_progress_to_hub(self, current: int, total: int, message: str):
        ...
        if self.hub_connector and self.config.get("enable_hub_reporting", True):
            ...
            self.hub_connector.report_progress_to_hub(
                percentage, f"File Splitter: {message}"
            )
    ...
    def split_file(
        ...
        try:
            ...
            try:
                validated_input = _validate_and_sanitize_path(
                    input_filepath, operation_type="read"
                )
            except FileSplitterSecurityError as e:
                raise FileSplitterIOError(
                    f"Input file path validation failed: {e}"
                ) from e
            ...
            try:
                validated_output_dir = _validate_and_sanitize_path(
                    output_dir, operation_type="write"
                )
            except FileSplitterSecurityError as e:
                raise FileSplitterIOError(
                    f"Output directory path validation failed: {e}"
                ) from e
            ...
            with open(input_filepath, "rb") as infile:
                for i in range(num_chunks):
                    ...
                    # SECURITY: Validate chunk path to avoid traversal
                    ...
                    progress_msg = (
                        f"Writing chunk {chunk_num}/{num_chunks}: " f"{chunk_filename}"
                    )
                    self.progress_updated.emit(chunks_created, num_chunks, progress_msg)
                    ...
                    try:
                        with open(chunk_filepath, "wb") as outfile:
                            ...
                            while bytes_written_this_chunk < chunk_size:
                                ...
                                this_chunk = chunk_size - bytes_written_this_chunk
                                total = file_size - bytes_written_total

                                # Determine read size
                                read_size = min(buffer_size, this_chunk, total)
                                ...
                                self.operation_stats["bytes_processed"] = (
                                    bytes_written_total
                                )
                        ...
                        error_msg = (
                            f"Error writing chunk {chunk_filename}: " f"{write_error}"
                        )
                        self._report_error_to_hub(
                            error_msg,
                            {
                    ...
                    self.progress_updated.emit(chunks_created, num_chunks, msg)
                    self._report_progress_to_hub(chunks_created, num_chunks, msg)
            ...
            if not self._is_running:
                ...
                self.logger.info(
                    "Split operation completed: %s chunks created",
                    chunks_created,
                )
    ...
    def join_files(self, first_chunk_path: str, output_filepath: str) -> None:
        """
        ...
        try:
            ...
            try:
                validated_chunk_path = _validate_and_sanitize_path(
                    first_chunk_path, operation_type="read"
                )
            except FileSplitterSecurityError as e:
                raise FileSplitterIOError(
                    f"First chunk path validation failed: {e}"
                ) from e

            if not os.path.exists(validated_chunk_path):
                raise FileSplitterIOError(
                    f"Input file not found: {validated_chunk_path}"
                )

            # SECURITY: Validate output file path
            try:
                validated_output_path = _validate_and_sanitize_path(
                    output_filepath, operation_type="write"
                )
            except FileSplitterSecurityError as e:
                raise FileSplitterIOError(
                    f"Output file path validation failed: {e}"
                ) from e
            ...
            if os.path.exists(meta_filepath):
                try:
                    ...
                    if metadata is not None:
                        ...
                        metadata_label = original_filename or "file"
                        self.progress_updated.emit(
                            0,
                            1,
                            f"Loaded metadata for '{metadata_label}'",
                        )
                        self.logger.info(
                            f"Loaded metadata: {num_chunks} chunks expected"
                        )
                except (json.JSONDecodeError, KeyError) as e:
                    self.progress_updated.emit(
                        0,
                        1,
                        f"Warning: Metadata file invalid ({e}), "
            ...
            if not metadata:
                ...
                # Confirm first part number matches expected value (1)
                try:
                    if int(parts[1]) != 1:
                        raise FileSplitterValidationError(
                ...
                self.progress_updated.emit(
                    0,
                    num_chunks,
                    f"Found {num_chunks} potential chunks based on pattern.",
                )
            ...
            if output_dir and not os.path.exists(output_dir):
                try:
                    ...
                    raise FileSplitterIOError(
                        "Could not create output directory " f"'{output_dir}': {e}"
                    ) from e
            ...
            self.progress_updated.emit(
                0,
                num_chunks,
                f"Starting join operation for {num_chunks} chunks...",
            )
            self._report_progress_to_hub(
                0,
                num_chunks,
                ...
                with open(output_filepath, "wb") as outfile:
                    for i in range(num_chunks):
                        ...
                        if not os.path.exists(chunk_filepath):
                            raise FileSplitterIOError(
                                "Missing chunk required for join: " f"{chunk_filename}"
                            )

                        read_message = (
                            f"Reading chunk {chunk_num}/{num_chunks}: "
                            f"{chunk_filename}"
                        )
                        self.progress_updated.emit(
                            chunks_processed,
                        ...
                        try:
                            ...
                            raise FileSplitterIOError(
                                "Error reading chunk " f"{chunk_filename}: {read_error}"
                            ) from read_error
                        except Exception as e:
                            raise FileSplitterIOError(
                        ...
                        finished_message = (
                            f"Finished processing chunk {chunk_num}/" f"{num_chunks}"
                        )
                        self.progress_updated.emit(
                            chunks_processed,
                            num_chunks,
            ...
            if not self._is_running:
                ...
                if expected_total_size is not None and final_size >= 0:
                    if final_size == expected_total_size:
                        verification_msg = (
                            " Final size ("
                            f"{final_size} bytes) matches expected size."
                        )
                        self.logger.info("File size verification passed")
                    else:
                        verification_msg = (
                            " WARNING: Final size ("
                            f"{final_size} bytes) does NOT match expected "
                            f"size ({expected_total_size} bytes)!"
                        )
                ...
                if self.hub_connector:
                    self.hub_connector.report_status_to_hub(
                        "completed",
                        {
                            ...
                            "size_verification": (
                                final_size == expected_total_size
...
class FileSplitterWorkerThread(QThread):
    ...
    def run(self):
        ...
        try:
            self.logger.info(
                f"Starting {self.operation_type} operation in worker thread"
            )
