    ...
    def _setup_highlighting_rules(self):
        ...
        if self.language == DocumentType.PYTHON:
            keywords = [
                ...
                'finally', 'for', 'while', 'import', 'from', 'as',
                'return', 'yield', 'break', 'continue', 'pass', 'and',
                'or', 'not', 'in', 'is', 'lambda', 'with', 'global',
                'nonlocal', 'assert', 'del', 'raise'
            ]
            
            for keyword in keywords:
                self.highlighting_rules.append((
                    re.compile(r'\b' + keyword + r'\b'),
                    keyword_format
                ))
...
class TextEditor(QPlainTextEdit):
    ...
    def apply_settings(self):
        ...
        self.setTabStopWidth(self.settings.tab_width * 
                           QFontMetrics(font).width(' '))
        self.setLineWrapMode(QPlainTextEdit.WidgetWidth if 
                           self.settings.word_wrap else 
...
class EnhancedEditor(StandardWindow):
    ...
    def setup_menu_callbacks(self):
        ...
        if hasattr(self, 'menu_manager'):
            ...
            self.menu_manager.register_callback('text_statistics', self.show_text_statistics)
    ...
    def perform_search(self, editor: TextEditor, search_text: str, 
    ...
    def update_document_properties(self):
        ...
        if document:
            ...
            self.doc_type_label.setText(document.get('document_type', DocumentType.TEXT).value.title())
    ...
    def load_settings(self):
        ...
        self.settings.font_family = settings.value('font_family', self.settings.font_family)
        self.settings.font_size = int(settings.value('font_size', self.settings.font_size))
        self.settings.tab_width = int(settings.value('tab_width', self.settings.tab_width))
        self.settings.use_spaces = settings.value('use_spaces', self.settings.use_spaces, type=bool)
        self.settings.word_wrap = settings.value('word_wrap', self.settings.word_wrap, type=bool)
        self.settings.line_numbers = settings.value('line_numbers', self.settings.line_numbers, type=bool)
        self.settings.syntax_highlighting = settings.value('syntax_highlighting', self.settings.syntax_highlighting, type=bool)
