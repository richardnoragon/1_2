...
def _validate_and_sanitize_path(
    ...
    if base_dir:
        try:
            ...
            if not canonical_path.startswith(base_canonical):
                logger.warning(
                    f"Path outside boundary detected: {canonical_path} not in {base_canonical}"
                )
                raise FileSplitterSecurityError(
    ...
    if operation_type == "write":
        ...
        if os.path.isabs(path) and not base_dir:
            ...
            for root in drive_roots:
                if path_upper.startswith(root.upper()):
                    ...
                    is_allowed = any(
                        path_upper.startswith(allowed)
                        for allowed in allowed_paths
                    )
                    if not is_allowed:
                        logger.warning(
                            f"Absolute path write blocked: {canonical_path}"
                        )
                        raise FileSplitterSecurityError(
        ...
        for sys_dir in system_dirs:
            try:
                sys_canonical = (
                    str(Path(sys_dir).resolve())
                    if os.path.exists(sys_dir)
                    else sys_dir
                )
                if canonical_path.startswith(sys_canonical):
                    logger.warning(
                        f"Attempted write to system directory: {canonical_path}"
                    )
                    raise FileSplitterSecurityError(
...
class FileSplitterLogic(QObject):
    ...
    def _report_progress_to_hub(self, current: int, total: int, message: str):
        ...
        """
        if self.hub_connector and self.config.get(
            "enable_hub_reporting", True
        ):
            ...
            self.hub_connector.report_status_to_hub(
                "processing",
                {
                    ...
                    "chunks_processed": self.operation_stats[
                        "chunks_processed"
                    ],
    ...
    def split_file(
        ...
        try:
            self.logger.info(
                f"Starting file split operation: {input_filepath}"
            )
            ...
            if not os.path.exists(validated_input):
                raise FileSplitterIOError(
                    f"Input file not found: {validated_input}"
                )
            ...
            try:
                if not os.path.exists(validated_output_dir):
                    os.makedirs(
                        validated_output_dir, mode=0o755
                    )  # Secure permissions
            except OSError as e:
                raise FileSplitterIOError(
            ...
            if not self._is_running:
                ...
                self.logger.info(
                    f"Split operation completed: {chunks_created} chunks created"
                )
    ...
    def join_files(self, first_chunk_path: str, output_filepath: str) -> None:
        ...
        try:
            self.logger.info(
                f"Starting file join operation: {first_chunk_path}"
            )
            ...
            if output_dir:
                try:
                    ...
                    os.makedirs(
                        validated_output_dir, mode=0o755, exist_ok=True
                    )
                except OSError as e:
                    raise FileSplitterIOError(
                        f"Cannot create output directory: {e}"
            ...
            if os.path.exists(meta_filepath):
                try:
                    ...
                    if metadata is not None:
                        ...
                        self.progress_updated.emit(
                            0,
                            1,
                            f"Loaded metadata for '{original_filename or 'file'}'",
                        )
                        self.logger.info(
                ...
                self.logger.info(
                    "No metadata file found, using manual detection"
                )
            ...
            if not metadata:
                ...
                chunk_pattern = (
                    f"{base_filename_inferred}.part{{:0{padding}d}}"
                )
                ...
                for i in range(1, 10000):  # Check up to 9999 chunks
                    check_path = os.path.join(
                        chunk_dir, chunk_pattern.format(i)
                    )
                ...
                self.logger.info(
                    f"Detected {num_chunks} chunks using pattern matching"
                )
                ...
                with open(output_filepath, "wb") as outfile:
                    for i in range(num_chunks):
                        ...
                        try:
                            ...
                            error_msg = (
                                f"Chunk file path validation failed: {e}"
                            )
                            self._report_error_to_hub(
                                error_msg,
                                {
                        ...
                        chunks_processed += 1
                        self.operation_stats["chunks_processed"] = (
                            chunks_processed
                        )
                        self.progress_updated.emit(
                            chunks_processed,
            ...
            if not self._is_running:
                ...
                self.logger.info(
                    f"Join operation completed: {output_filepath}"
                )
    ...
    def run(self):
        ...
        try:
            ...
            if self.operation_type == "split":
                ...
                self.logger.error(
                    f"Unknown operation type: {self.operation_type}"
                )
