...
class SystemCleanupGUI(SystemDiagnosticsGUI):
                                ...
                                if title_layout:
                                    ...
                                    if desc_widget:
                                        ...
                                        if isinstance(desc_label, QLabel):
                                            desc_label.setText(
                                                "Comprehensive system cleanup and optimization tools"
                                            )
        except Exception as e:
    ...
    def create_temp_files_advanced_group(self) -> QGroupBox:
        ...
        self.temp_age_spinbox.setToolTip(
            "0 = delete all temp files, >0 = only delete files older than specified days"
        )
        ...
        self.temp_size_spinbox.setToolTip(
            "0 = delete all sizes, >0 = only delete files larger than specified MB"
        )
        ...
        self.temp_system_checkbox.setToolTip(
            "Include Windows system temp directories (may require admin privileges)"
        )
        ...
        self.temp_backup_checkbox.setToolTip(
            "Create backup of files before deletion (uses additional disk space)"
        )
        ...
        self.temp_secure_checkbox.setToolTip(
            "Securely overwrite file data before deletion (slower but more secure)"
        )
    ...
    def create_safety_backup_tab(self):
        ...
        try:
            ...
            self.restore_point_checkbox.setToolTip(
                "Create a system restore point before performing cleanup operations"
            )
            ...
            self.confirm_operations_checkbox.setToolTip(
                "Show confirmation dialogs before performing potentially destructive operations"
            )
    ...
    def estimate_quick_cleanup(self):
        ...
        try:
            ...
            if (
                self.quick_cleanup_options["temp_files"].isChecked()
                and "temp_files" in self.cleanup_tools
            ):

                temp_tool = self.cleanup_tools["temp_files"]
                temp_size = temp_tool.estimate_cleanup_size(
                    max_age_days=0, min_size_bytes=0, include_system_temp=True
                )
            ...
            # Display results
            results_text = (
                f"Estimated space to be freed: {self._format_size(total_size)}\n\n"
            )
            ...
            self.quick_results_text.setPlainText(
                f"Error estimating cleanup size: {str(e)}"
            )
    ...
    def preview_temp_cleanup(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Preview Error", f"Error generating preview:\n{str(e)}"
            )
    ...
    def execute_temp_cleanup(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Cleanup Error", f"Error executing temp files cleanup:\n{str(e)}"
            )
        ...
        try:
            ...
            if self.current_worker is not None:
                QMessageBox.warning(
                    ...
                    "Another cleanup operation is currently running. Please wait for it to complete.",
                )
                return

            # Create and configure worker thread
            self.current_worker = CleanupWorker(cleanup_tool, params)
            ...
            QMessageBox.critical(
                self, "Cleanup Error", f"Failed to start cleanup operation:\n{str(e)}"
            )
