...
class WindowsDiskMonitor(MonitorBase):
    ...
    def _initialize_platform_specific(self) -> None:
        ...
        try:
            ...
            if self.wmi_enabled:
                ...
                self.logger.warning("WMI not available - limited functionality")
            ...
            self.logger.error(f"Failed to initialize Windows disk monitoring: {e}")
                    ...
                    try:
                        if disk.DriveType == 3:  # Local disk
                            disk_info = {
                                ...
                                'media_type': disk.MediaType or 'Unknown',
                                'volume_serial_number': disk.VolumeSerialNumber or '',
                                'compressed': disk.Compressed or False,
                                'supports_disk_quotas': disk.SupportsDiskQuotas or False
                            }
                            
                            # Calculate usage percentages
                            if disk_info['size_bytes'] > 0:
                                used_bytes = disk_info['size_bytes'] - disk_info['free_bytes']
                                disk_info['used_bytes'] = used_bytes
                                disk_info['used_percent'] = (used_bytes / disk_info['size_bytes']) * 100
                                disk_info['free_percent'] = (disk_info['free_bytes'] / disk_info['size_bytes']) * 100
                            else:
    ...
    def _enrich_disk_information(self) -> None:
        ...
        try:
            ...
            for partition in self.wmi_connection.Win32_DiskPartition():
                try:
                    ...
                    if device_id in self._physical_disks:
                        if 'partitions_info' not in self._physical_disks[device_id]:
                            self._physical_disks[device_id]['partitions_info'] = []
                        ...
                        partition_info = {
                            ...
                            'primary_partition': partition.PrimaryPartition or False
                        }
                        
                        self._physical_disks[device_id]['partitions_info'].append(partition_info)
            ...
            for perf_disk in self.wmi_connection.Win32_PerfRawData_PerfDisk_PhysicalDisk():
                try:
                    if perf_disk.Name and perf_disk.Name != '_Total':
                        ...
                        for device_id, disk_info in self._physical_disks.items():
                            if disk_name in device_id or str(disk_info.get('index', '')) in disk_name:
                                disk_info['performance_object'] = perf_disk.Name
                                break
                                
                except Exception as e:
                    self.logger.warning(f"Error processing performance disk: {e}")
                    continue
    ...
    def _collect_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for device_id, disk_info in self._physical_disks.items():
                try:
                    physical_data = self._collect_physical_disk_data(device_id, disk_info)
                    if physical_data:
                        data['physical_disks'].append(physical_data)
                        
                except Exception as e:
                    self.logger.error(f"Error collecting data for physical disk {device_id}: {e}")
                    continue
            ...
            for device_id, disk_info in self._logical_disks.items():
                try:
                    logical_data = self._collect_logical_disk_data(device_id, disk_info)
                    if logical_data:
                        ...
                        # Count health status
                        health_status = logical_data.get('health_status', 'unknown')
                    ...
                    self.logger.error(f"Error collecting data for logical disk {device_id}: {e}")
                    continue
            ...
            # Update summary
            data['summary']['total_physical_disks'] = len(data['physical_disks'])
    ...
    def _collect_physical_disk_data(self, device_id: str, disk_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if smart_data:
                ...
                data['health_status'] = smart_data.get('overall_health', 'unknown')
            ...
            self.logger.error(f"Error collecting physical disk data for {device_id}: {e}")
            return None
    
    def _collect_logical_disk_data(self, device_id: str, disk_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
                        ...
                        if disk.DeviceID == device_id:
                            if disk.Size and disk.FreeSpace:
                                ...
                                data.update({
                                    ...
                                    'free_percent': (free_bytes / size_bytes) * 100
                                })
                            break
                except Exception as e:
                    self.logger.warning(f"Error updating space usage for {device_id}: {e}")
            ...
            self.logger.error(f"Error collecting logical disk data for {device_id}: {e}")
            return None
    
    def _get_windows_smart_data(self, device_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error getting Windows SMART data for {device_id}: {e}")
            return None
    ...
    def _get_wmi_smart_data(self, device_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                for item in self.wmi_connection.MSStorageDriver_FailurePredictStatus():
                    if str(item.InstanceName).endswith(f'_{disk_index}'):
                        smart_data['prediction_failure'] = item.PredictFailure
                        smart_data['overall_health'] = 'failed' if item.PredictFailure else 'passed'
                        break
            except Exception:
                pass
            ...
            try:
                for item in self.wmi_connection.MSStorageDriver_FailurePredictData():
                    if str(item.InstanceName).endswith(f'_{disk_index}'):
                        if item.VendorSpecific:
                            # Parse vendor-specific SMART data
                            vendor_data = self._parse_vendor_smart_data(item.VendorSpecific)
            ...
            try:
                for item in self.wmi_connection.MSStorageDriver_FailurePredictThresholds():
                    if str(item.InstanceName).endswith(f'_{disk_index}'):
                        if item.VendorSpecific:
                            # Parse threshold data
                            threshold_data = self._parse_threshold_data(item.VendorSpecific)
            ...
            if len(vendor_data) >= 512:
                ...
                for i in range(2, 362, 12):  # Each attribute is 12 bytes
                    if i + 12 <= len(vendor_data):
                        ...
                        raw_value = int.from_bytes(vendor_data[i + 5:i + 11], 'little')
    ...
    def _get_smartctl_data_windows(self, device_id: str) -> Optional[Dict[str, Any]]:
    ...
    def _get_volume_information(self, device_id: str) -> Optional[Dict[str, Any]]:
        """Get volume-specific information.
        ...
        try:
            ...
            if PYWIN32_AVAILABLE:
                try:
                    ...
                    volume_name, volume_serial, max_component_length, file_system_flags, file_system_name = \
                    ...
                    self.logger.warning(f"Error getting volume info for {device_id}: {e}")
            ...
            if self.wmi_enabled and self.wmi_connection:
                try:
                    ...
                    self.logger.warning(f"Error getting WMI volume info for {device_id}: {e}")
            
            return volume_info if volume_info else None
            
        except Exception as e:
            self.logger.error(f"Error getting volume information for {device_id}: {e}")
            return None
    ...
    def _collect_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for counter_name, counter_info in self.performance_counters.items():
                try:
                    ...
                    # Get counter value
                    counter_type, counter_value = win32pdh.GetFormattedCounterValue(
                    ...
                    self.logger.warning(f"Error collecting counter {counter_name}: {e}")
                    continue
    ...
    def _determine_physical_disk_health(self, disk_data: Dict[str, Any]) -> str:
    ...
    def _validate_data(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            required_fields = ['timestamp', 'platform', 'physical_disks', 'logical_disks', 'summary']
            ...
            if not isinstance(data['physical_disks'], list) or not isinstance(data['logical_disks'], list):
                return False
            ...
            required_summary_fields = [
                'total_physical_disks', 'total_logical_disks', 'healthy_disks',
                'warning_disks', 'critical_disks'
            ]
    ...
    def get_windows_disk_details(self, device_id: str, disk_type: str = 'logical') -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error getting Windows disk details for {device_id}: {e}")
            return None
    ...
    def refresh_windows_disks(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error refreshing Windows disk information: {e}")
            error_handler.handle_error(e, "WindowsDiskMonitor.refresh_disks")
            return False
    
    def get_disk_performance_history(self, device_id: str, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            filtered_history = [
                ...
                if datetime.fromisoformat(entry.get('timestamp', '')).timestamp() > cutoff_time
            ]
            
            return filtered_history
            
        except Exception as e:
            self.logger.error(f"Error getting disk performance history for {device_id}: {e}")
            return []
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            if self.performance_counters:
                for counter_info in self.performance_counters.values():
                    try:
                        ...
                        self.logger.warning(f"Error closing performance counter: {e}")
            ...
            self.logger.error(f"Error during Windows disk monitor cleanup: {e}")