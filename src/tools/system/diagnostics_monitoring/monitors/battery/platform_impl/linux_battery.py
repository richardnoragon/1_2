...
class LinuxBatteryMonitor(MonitorBase):
    ...
    def _initialize_platform_specific(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to initialize Linux battery monitoring: {e}"
            )
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["upower", "--version"], capture_output=True, timeout=5
                )
    ...
    def _discover_linux_batteries(self) -> None:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "LinuxBatteryMonitor.discover_batteries"
            )
                ...
                if os.path.exists(type_file):
                    try:
                        ...
                        if device_type.lower() == "battery":
                            battery_data = self._read_sysfs_battery_data(
                                device, device_path
                            )
                            if battery_data:
                                self._batteries[device] = battery_data
                                self.logger.debug(
                                    f"Added battery via sysfs: {device}"
                                )
                        else:
                            # Store other power supply info (AC adapters, etc.)
                            supply_data = self._read_sysfs_supply_data(
                                device, device_path
                            )
                            if supply_data:
                        ...
                        self.logger.warning(
                            f"Error reading device {device}: {e}"
                        )
                        continue
    ...
    def _read_sysfs_battery_data(
        self, device: str, device_path: str
        ...
        try:
            ...
            self.logger.error(
                f"Error reading sysfs battery data for {device}: {e}"
            )
            return None

    def _calculate_sysfs_derived_values(
        ...
        try:
            ...
            if (
                "charge_full" in battery_data
                and "charge_full_design" in battery_data
            ):
                ...
                if charge_full_design > 0:
                    ...
                    battery_data["health_percent"] = min(
                        100.0, max(0.0, health_percent)
                    )

            elif (
                "energy_full" in battery_data
                and "energy_full_design" in battery_data
            ):
            ...
            for voltage_field in ["voltage_now", "voltage_min_design"]:
                if voltage_field in battery_data:
                    ...
                    if isinstance(voltage_uv, int) and voltage_uv > 100000:
                        battery_data[f"{voltage_field}_volts"] = (
    ...
    def _read_sysfs_supply_data(
        ...
        try:
            ...
            self.logger.error(
                f"Error reading sysfs supply data for {device}: {e}"
            )
            return None
    ...
    def _discover_batteries_upower(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(
                ["upower", "-e"], capture_output=True, text=True, timeout=10
            )
            ...
            for device in devices:
                if "BAT" in device or "battery" in device.lower():
                    ...
                    if battery_data:
                        ...
                        self.logger.debug(
                            f"Added/updated battery via upower: {device_id}"
                        )

        except Exception as e:
            self.logger.error(f"Error discovering batteries via upower: {e}")

    def _get_upower_battery_data(
        ...
        try:
            ...
            battery_data = {"device_path": device_path, "source": "upower"}
            ...
            for line in result.stdout.split("\n"):
                ...
                if ":" in line:
                    ...
                    if key in ["percentage"]:
                        try:
                            battery_data["capacity_percent"] = float(
                                value.rstrip("%")
                            )
                        except ValueError:
                            pass
        ...
        try:
            ...
            for battery_dir in os.listdir(self.acpi_battery_path):
                battery_path = os.path.join(
                    self.acpi_battery_path, battery_dir
                )

                if os.path.isdir(battery_path):
                    battery_data = self._read_acpi_battery_data(
                    ...
                    if battery_data:
                        ...
                        self.logger.debug(
                            f"Added/updated battery via ACPI: {device_id}"
                        )

        except Exception as e:
            self.logger.error(f"Error discovering batteries via ACPI: {e}")

    def _read_acpi_battery_data(
        self, battery_name: str, battery_path: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _get_sysfs_current_status(self, battery_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            device_name = battery_id.replace("upower_", "").replace(
                "acpi_", ""
            )
            ...
            current_data = self._read_sysfs_battery_data(
            ...
            if current_data:
                return {
                    ...
                    "current_temperature": current_data.get(
                        "temperature_celsius"
                    ),
    ...
    def _get_upower_current_status(self, battery_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not device_path:
                ...
                device_path = (
                    f"/org/freedesktop/UPower/devices/battery_{device_name}"
                )
            ...
            if device_path:
                ...
                if current_data:
                    return {
                        "upower_capacity_percent": current_data.get(
                            "capacity_percent"
                        ),
                        "upower_charging_state": current_data.get(
                            "charging_state"
                        ),
            ...
            if os.path.exists(thermal_path):
                ...
                for zone in os.listdir(thermal_path):
                    if zone.startswith("thermal_zone"):
                        ...
                        if os.path.exists(temp_file) and os.path.exists(
                            type_file
                        ):
                            try:
                                ...
                                thermal_zones.append(
                                    {
                                        "zone": zone,
                                        "type": zone_type,
                                        "temperature_celsius": temp_millicelsius
                                        / 1000.0,
                                    }
                                )
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting Linux battery health status: {e}"
            )
            return {
                "status": "error",
