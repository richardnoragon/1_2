...
class PerformanceMonitor(MonitorBase):
    ...
    def _initialize_platform_specific(self) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"Platform-specific implementation not available: {e}"
            )
        except Exception as e:
            self.logger.error(
                f"Error initializing platform-specific components: {e}"
            )
            error_handler.handle_error(
                e, "PerformanceMonitor._initialize_platform_specific"
            )
    ...
    def _initialize_trackers(self) -> None:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "PerformanceMonitor._initialize_trackers"
            )
    ...
    def _calculate_derived_metrics(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if len(self._cpu_history) >= 2:
                recent_cpu = [
                    point["cpu_percent"] for point in self._cpu_history[-10:]
                ]
            ...
            if len(self._memory_history) >= 2:
                recent_memory = [
                    ...
                    for point in self._memory_history[-10:]
                ]
            ...
            data["responsiveness_score"] = (
                self._calculate_responsiveness_score(data)
            )
    ...
    def _validate_data(self, data: Dict[str, Any]) -> bool:
        ...
        if "cpu_percent" in data:
            if not (0 <= data["cpu_percent"] <= 100):
                self.logger.warning(
                    f"Invalid CPU percentage: {data['cpu_percent']}"
                )
                return False
        ...
        if "memory_percent" in data:
            if not (0 <= data["memory_percent"] <= 100):
                self.logger.warning(
                    f"Invalid memory percentage: {data['memory_percent']}"
                )
                return False
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "memory_health": "good",
            "load_health": "good",
            "responsiveness_score": current_data.get(
    ...
    def get_performance_summary(
        self, time_range: timedelta = timedelta(minutes=10)
    ) -> Dict[str, Any]:
        ...
        recent_cpu = [
            point
            for point in self._cpu_history
            if point["timestamp"] >= cutoff_time
        ]
        recent_memory = [
            point
            for point in self._memory_history
            if point["timestamp"] >= cutoff_time
        ]
        recent_load = [
            load for load in self._load_history[-60:]
        ]  # Last 60 points
        ...
        if recent_memory:
            memory_values = [
                point["memory_percent"] for point in recent_memory
            ]
