...
class WindowsPerformanceImpl:
    ...
    def _initialize_wmi(self) -> None:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "WindowsPerformanceImpl._initialize_wmi"
            )
        ...
        try:
            ...
            for name, path in counter_paths.items():
                try:
                    ...
                    self.logger.warning(
                        f"Failed to initialize counter {name}: {e}"
                    )
            ...
            error_handler.handle_error(
                e, "WindowsPerformanceImpl._initialize_performance_counters"
            )
    ...
    def get_cpu_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "WindowsPerformanceImpl.get_cpu_info"
            )
        ...
        try:
            ...
            if "cpu_usage" in self._counter_handles:
                try:
                    ...
                    self.logger.debug(
                        f"Performance counter CPU usage failed: {e}"
                    )
            ...
            if "context_switches" in self._counter_handles:
                try:
                    ...
                    metrics["context_switches_per_sec"] = round(
                        switches_per_sec, 2
                    )
                except Exception as e:
            ...
            error_handler.handle_error(
                e, "WindowsPerformanceImpl.get_cpu_metrics"
            )
        ...
        try:
            ...
            if "memory_available" in self._counter_handles:
                try:
                    ...
                    memory_info["available_mb_perfcounter"] = round(
                        available_mb, 2
                    )
                except Exception as e:
            ...
            if "memory_committed" in self._counter_handles:
                try:
                    ...
                    memory_info["committed_gb"] = round(
                        committed_bytes / (1024**3), 2
                    )
                except Exception as e:
            ...
            error_handler.handle_error(
                e, "WindowsPerformanceImpl.get_memory_details"
            )
    ...
    def _get_memory_pool_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "memory_pool_nonpaged" in self._counter_handles:
                ...
                pool_info["nonpaged_pool_mb"] = round(
                    nonpaged_pool / (1024**2), 2
                )
            ...
            if self._wmi_available and self._wmi_conn:
                    ...
                    memory_info.update(
                        {
                            ...
                            "physical_memory_total_gb": round(
                                total_capacity / (1024**3), 2
                            ),
                ...
                if os_info:
                    ...
                    if hasattr(os, "TotalVisibleMemorySize"):
                        ...
                        memory_info["visible_memory_gb"] = round(
                            visible_memory / (1024**3), 2
                        )

                    if hasattr(os, "FreePhysicalMemory"):
                        free_memory = int(os.FreePhysicalMemory) * 1024
                        memory_info["free_physical_memory"] = free_memory
                        memory_info["free_physical_memory_gb"] = round(
                            free_memory / (1024**3), 2
                        )
    ...
    def _get_cpu_temperature_wmi(self) -> Dict[str, Any]:
        ...
        try:
            if self._wmi_available and self._wmi_conn:
                ...
                thermal_zones = (
                    self._wmi_conn.Win32_PerfRawData_Counters_ThermalZoneInformation()
                )
                ...
                if temperatures:
                    temp_info.update(
                        {
                            "cpu_temperature_wmi": round(
                                sum(temperatures) / len(temperatures), 1
                            ),
                            "cpu_temperature_zones": [
                                round(temp, 1) for temp in temperatures
                            ],
                            "cpu_temperature_max_wmi": round(
                                max(temperatures), 1
                            ),
                            "cpu_temperature_min_wmi": round(
                                min(temperatures), 1
                            ),
                        }
                    )
    ...
    def get_process_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "WindowsPerformanceImpl.get_process_metrics"
            )
        ...
        try:
            ...
            for process in processes:
                if process.Name and process.Name != "_Total":
                    ...
                    if (
                        hasattr(process, "PageFaultsPersec")
                        and process.PageFaultsPersec
                    ):
                ...
                process_info.update(
                    {
                        ...
                        "avg_handles_per_process": round(
                            total_handles / process_count, 2
                        ),
    ...
    def get_additional_metrics(self) -> Dict[str, Any]:
        ...
        try:
                    ...
                    if hasattr(os, "LastBootUpTime"):
                        ...
                        if boot_time:
                            try:
                                ...
                                uptime_seconds = (
                                    datetime.now() - boot_datetime
                                ).total_seconds()
                                metrics.update(
                                    {
                                        "uptime_seconds_wmi": uptime_seconds,
                                        "uptime_hours_wmi": round(
            ...
            error_handler.handle_error(
                e, "WindowsPerformanceImpl.get_additional_metrics"
            )
                ...
                fallback_info.update(
                    {
                        ...
                        "memory_percent": round(
                            (
                                (
                                    memory_status["TotalPhys"]
                                    - memory_status["AvailPhys"]
                                )
                                / memory_status["TotalPhys"]
                            )
                            * 100,
