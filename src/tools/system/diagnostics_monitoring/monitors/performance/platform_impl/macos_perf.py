...
class MacOSPerformanceImpl:
    ...
    def get_cpu_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "MacOSPerformanceImpl.get_cpu_metrics"
            )
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    if "CPU die temperature" in line:
                        ...
                        if match:
                            temp_info["cpu_temperature_powermetrics"] = float(
                                match.group(1)
                            )
    ...
    def get_memory_details(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "MacOSPerformanceImpl.get_memory_details"
            )
                        ...
                        if len(parts) == 2:
                            ...
                            try:
                                ...
                                if "pages" in key:
                                    ...
                                    memory_info[f"vm_stat_{key}_bytes"] = (
                                        bytes_value
                                    )
                                    memory_info[f"vm_stat_{key}_mb"] = round(
                                        bytes_value / (1024**2), 2
                                    )
    ...
    def get_process_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "MacOSPerformanceImpl.get_process_metrics"
            )
                            ...
                            process_info.update(
                                {
                                    ...
                                    "running_processes_top": int(
                                        match.group(2)
                                    ),
                                    "sleeping_processes_top": int(
                                        match.group(3)
                                    ),
                                }
                            )
    ...
    def get_additional_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "MacOSPerformanceImpl.get_additional_metrics"
            )
    ...
    def get_fallback_cpu_info(self) -> Dict[str, Any]:
        ...
        try:
            if self._tools_available["sysctl"]:
                ...
                if result.returncode == 0:
                    fallback_info["cpu_logical_cores"] = int(
                        result.stdout.strip()
                    )
            ...
            if self._tools_available["top"]:
                ...
                if result.returncode == 0:
                    ...
                    for line in lines:
                        if "Processes:" in line:
                            match = re.search(
                                r"Processes:\s*(\d+)\s*total", line
                            )
                            if match:
                                fallback_info["total_processes"] = int(
                                    match.group(1)
                                )
                            break
