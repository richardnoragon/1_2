...
class ScannerEngine:
    ...
    def __init__(self):
        ...
        # Progress tracking
        self._progress_callback: Optional[Callable[[Dict[str, Any]], None]] = (
            None
        )
        self._scan_stats = {
            "files_scanned": 0,
    ...
    def run_scan(
        ...
        try:
            ...
            duration = (
                end_time - self._scan_stats["start_time"]
            ).total_seconds()
    ...
    def _estimate_total_files(
        self, scan_paths: List[str], config: Dict[str, Any]
    ) -> None:
    ...
    def _estimate_path_files(
        self, path: str, max_depth: int, current_depth: int
    ) -> int:
        ...
        try:
            ...
            if current_depth < max_depth - 1 and estimated_dirs > 0:
                ...
                if len(sample_dirs) > 0:
                    subdir_files = (subdir_files * estimated_dirs) // len(
                        sample_dirs
                    )
    ...
    def _execute_scan(
        ...
        if parallel_workers > 1 and len(scan_paths) > 1:
            ...
            scan_results = self._parallel_scan(
                scan_paths, config, scan_results
            )
        else:
            # Sequential scanning
            for path in scan_paths:
    ...
    def _parallel_scan(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit scan tasks
            future_to_path = {
                executor.submit(self._scan_path_isolated, path, config): path
                for path in scan_paths
            }
    ...
    def _scan_path_isolated(
        self, path: str, config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _scan_file(
        self, file_path: str, config: Dict[str, Any], results: Dict[str, Any]
    ) -> None:
        """Scan a regular file.
    ...
    def _scan_symlink(
        self, link_path: str, config: Dict[str, Any], results: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            if config.get("follow_symlinks", False) and link_info.get(
                "target_exists"
            ):
                target_path = link_info.get("target")
                if target_path and not self._is_circular_link(
    ...
    def _apply_filters(
        self, entries: List[str], config: Dict[str, Any]
    ) -> List[str]:
    ...
    def _merge_scan_results(
        self, main_results: Dict[str, Any], path_results: Dict[str, Any]
    ) -> None:
    ...
    def _update_progress(self) -> None:
        ...
        try:
            ...
            if self._scan_stats["estimated_total"] > 0:
                scanned = (
                    self._scan_stats["files_scanned"]
                    + self._scan_stats["directories_scanned"]
                )
                progress = min(
                    100.0,
                    (scanned / self._scan_stats["estimated_total"]) * 100,
                )
