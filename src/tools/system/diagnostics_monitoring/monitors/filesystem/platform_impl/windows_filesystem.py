...
class WindowsFilesystemImpl:
    ...
    def get_filesystem_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.handle_error(
                e, "WindowsFilesystemImpl.get_filesystem_info"
            )
        ...
        try:
            ...
            for drive in drives:
                try:
                    ...
                    volumes.append({"drive": drive, "error": str(e)})
        ...
        try:
            ...
            if os.path.exists(drive):
                try:
                    ...
                    self.logger.error(
                        f"Error getting disk usage for {drive}: {e}"
                    )
    ...
    def _get_filesystem_type(self, drive: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting filesystem type for {drive}: {e}"
            )
            return "Unknown"
            ...
            if self.tools["fsutil"]:
                ...
                if result.returncode == 0:
                    ...
                    cluster_match = re.search(
                        r"Bytes Per Cluster\s*:\s*(\d+)", output
                    )
                    if cluster_match:
                    ...
                    mft_match = re.search(
                        r"Mft Valid Data Length\s*:\s*(\d+)", output
                    )
                    if mft_match:
    ...
    def run_chkdsk(
        self, drive: str, fix_errors: bool = False, scan_only: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            if not self.tools["chkdsk"]:
                return {"success": False, "error": "chkdsk tool not available"}
