...
class CorruptionDetector:
    ...
    def analyze_scan_results(
        ...
        try:
            ...
            error_handler.handle_error(
                e, "CorruptionDetector.analyze_scan_results"
            )
            return {
                "error": str(e),
                "timestamp": datetime.now().isoformat(),
        ...
        for file_path, timestamp_info in timestamps.items():
            try:
                ...
                self._check_timestamp_anomalies(
                    file_path, timestamp_info, analysis
                )
    ...
    def _analyze_structural_integrity(
        ...
        try:
            ...
            total_items = len(scan_results.get("files", [])) + len(
                scan_results.get("directories", [])
            )
    ...
    def _check_directory_metadata(
        self, dir_info: Dict[str, Any], analysis: Dict[str, Any]
    ) -> None:
    ...
    def _check_permission_anomalies(
        ...
        file_ext = Path(file_path).suffix.lower()
        if file_ext in [".exe", ".sh", ".py", ".pl"] and not perm_info.get(
            "owner_execute", False
        ):
            self._add_corruption(
                analysis,
    ...
    def _check_timestamp_anomalies(
        ...
        future_threshold = now + (
            self.thresholds["timestamp_future_days"] * 24 * 3600
        )
        ...
        past_threshold = now - (
            self.thresholds["timestamp_past_years"] * 365 * 24 * 3600
        )
    ...
    def _generate_analysis_summary(self, analysis: Dict[str, Any]) -> None:
        ...
        if stats["files_analyzed"] > 0:
            corruption_rate = (
                stats["corruptions_found"] / stats["files_analyzed"]
            )
        ...
        if corruptions:
            ...
            for corruption in corruptions:
                ...
                type_counts[corruption_type] = (
                    type_counts.get(corruption_type, 0) + 1
                )
        ...
        if stats["corruptions_found"] == 0:
            recommendations.append(
                "No corruption detected. Filesystem appears healthy."
            )
    ...
    def get_corruption_statistics(
        ...
        for corruption in corruptions:
            ...
            type_stats[corruption_type] = (
                type_stats.get(corruption_type, 0) + 1
            )
        ...
        return {
            ...
            "corruption_rate": analysis.get("statistics", {}).get(
                "corruption_rate", 0.0
            ),
            "overall_health": analysis.get("summary", {}).get(
                "overall_health", "unknown"
            ),
        }
