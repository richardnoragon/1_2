...
class SoftwareDetector(MaintenanceToolBase):
    ...
    def scan_registry_software(self) -> Dict[str, SoftwareInfo]:
        ...
        software_dict = {}
        self.update_status(
            "Scanning Windows registry for installed software..."
        )
                ...
                with winreg.OpenKey(hive, subkey) as key:
                    ...
                    while True:
                        try:
                            ...
                            software_info = self._read_registry_entry(
                                hive, f"{subkey}\\{subkey_name}"
                            )
    ...
    def _read_registry_entry(
        self, hive, subkey_path: str
    ) -> Optional[SoftwareInfo]:
        ...
        try:
            with winreg.OpenKey(hive, subkey_path) as key:
                ...
                if values.get("EstimatedSize"):
                    try:
                        size_mb = (
                            float(values["EstimatedSize"]) / 1024
                        )  # KB to MB
            ...
            self.log_warning(
                f"Error reading registry entry {subkey_path}: {e}"
            )
            return None

    def scan_package_manager_software(
        self, manager: str
            ...
            if result.returncode == 0:
                for line in result.stdout.strip().split("\n"):
                    if "|" in line:
                        ...
                        unique_key = (
                            f"{software_info.name}_{software_info.version}"
                        )
                        software_dict[unique_key] = software_info
    ...
    def _scan_scoop(self) -> Dict[str, SoftwareInfo]:
        ...
        try:
            result = subprocess.run(
                ["scoop", "list"], capture_output=True, text=True, timeout=30
            )
    ...
    def filter_system_components(
        self,
        software_dict: Dict[str, SoftwareInfo],
        ...
        for key, software in software_dict.items():
            ...
            is_system = any(
                component.lower() in software.name.lower()
    ...
    def execute(
        self, include_system: bool = False, save_results: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            steps = ["Registry Scan"] + [
                f"{pm.title()} Scan" for pm in self.package_managers
            ]
            if save_results:
            ...
            self.log_info(
                f"Software detection completed. Found {len(self.detected_software)} applications"
            )
            return True
    ...
    def get_software_statistics(self) -> Dict[str, any]:
        ...
        total_size = sum(
            s.size_mb for s in self.detected_software.values() if s.size_mb
        )
        ...
        return {
            ...
            "system_components": sum(
                1
                for s in self.detected_software.values()
