...
class WingetSource(UpdateSource):
    ...
    def execute_update(self, software_name: str) -> bool:
        ...
        try:
            result = subprocess.run(
                ["winget", "upgrade", software_name, "--accept-source-agreements", "--accept-package-agreements"],
                capture_output=True,
                text=True,
                timeout=300
            )
                    ...
                    if '/' in line:
                        ...
                        if len(parts) >= 2:
                            ...
                            if '[upgradable from:' in line:
                                ...
                                current_start = line.find('[upgradable from: ') + 18
                                current_end = line.find(']', current_start)
                                current = line[current_start:current_end]
                                
                                if name_arch in software_list or not software_list:
                                    update_info = UpdateInfo(
...
class UpdateSourceManager(MaintenanceToolBase):
    ...
    def _initialize_sources(self):
        ...
        self.log_info(f"Initialized update sources: {list(self.sources.keys())}")
    ...
    def check_all_updates(self, software_list: List[str] = None) -> Dict[str, UpdateInfo]:
        """
            ...
            try:
                ...
                for update in updates:
                    ...
                    if key not in self.available_updates:
                        ...
                        if self._compare_versions(update.available_version, 
                                                existing.available_version) > 0:
                            self.available_updates[key] = update
                
                self.log_info(f"Found {len(updates)} updates from {source_name}")
                
            except Exception as e:
                self.log_error(f"Error checking updates from {source_name}: {e}")
        
        self.log_info(f"Total available updates: {len(self.available_updates)}")
    ...
    def get_changelog(self, software_name: str) -> str:
        ...
        if source_name in self.sources:
            ...
            changelog = source.get_changelog(software_name, 
    ...
    def execute_batch_update(self, software_list: List[str]) -> Dict[str, bool]:
    ...
    def execute(self, software_list: List[str] = None, 
        ...
        try:
            ...
            self.update_status(f"Update check completed. Found {len(updates)} updates")
            return True
