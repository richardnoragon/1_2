...
try:
    ...
    print(
        "Warning: cryptography library not available. Install it for secure encryption."
    )
                ...
                while True:
                    ...
                    while len(message_data) < length:
                        chunk = client_socket.recv(
                            min(length - len(message_data), 4096)
                        )
                        if not chunk:
                            break
                        message_data += chunk
...
class PathSecurity:
    ...
    def _check_file_stats(path: Path, result: Dict[str, Any]) -> bool:
        ...
        if file_size > PathSecurity.MAX_FILE_SIZE:
            result["reason"] = (
                f"File too large (max {PathSecurity.MAX_FILE_SIZE} bytes)"
            )
            return False
...
class TransferClient(QThread):
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                hello_msg = TransferProtocol.create_message(
                    TransferProtocol.MSG_HELLO,
                    {
                        ...
                        "version": TransferProtocol.VERSION,
                    },
                )
                sock.send(hello_msg)
            ...
            try:
                with open(file_path, "rb") as f:
                    ...
                    while True:
                        ...
                        base_progress = i * 100
                        total_progress = int(
                            (base_progress + file_progress) / total_files
                        )
    ...
    def send_config(self, sock: socket.socket):
        ...
        config_msg = TransferProtocol.create_message(
            TransferProtocol.MSG_CONFIG_DATA, config_data
        )
    ...
    def send_collection(self, sock: socket.socket):
        ...
        collection_msg = TransferProtocol.create_message(
            TransferProtocol.MSG_COLLECTION_DATA, collection
        )
...
class NetworkTransferGUI(StandardWindow):
    ...
    def _setup_menu_callbacks(self):
        ...
        if hasattr(self, "menu_manager"):
            ...
            self.menu_manager.register_callback(
                "export_data", self.export_transfer_settings
            )
            self.menu_manager.register_callback(
                "import_data", self.import_transfer_settings
            )
    ...
    def refresh_view(self):
        ...
        QMessageBox.information(
            self, "Refresh", "Transfer interface refreshed successfully."
        )
    ...
    def create_send_tab(self):
        ...
        self.target_host = TextInput(
            ...
            accessible_name="Transfer target host",
        )
    ...
    def create_receive_tab(self):
        ...
        self.receive_path = TextInput(
            "Receive Path",
            str(Path.home() / "Downloads" / "RFU_Transfers"),
            accessible_name="Transfer receive path",
        )
    ...
    def create_collections_tab(self):
        ...
        self.new_collection_name = TextInput(
            "Collection Name",
            "",
            accessible_name="New collection name",
        )
    ...
    def select_files(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Error", f"Failed to select files: {e}")
    ...
    def select_folder(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Error", f"Failed to select folder: {e}")
    ...
    def _check_forbidden_directories(self, normalized_path: Path) -> bool:
        ...
        for forbidden in forbidden_dirs:
            try:
                ...
                if (
                    str(normalized_path).startswith(str(forbidden_resolved))
                    or normalized_path == forbidden_resolved
                ):
                    return False
    ...
    def _add_folder_files_recursive(
        ...
        if current_depth > max_depth:
            self.logger.warning(
                f"Maximum recursion depth ({max_depth}) exceeded at: {current_path}"
            )
            return
    ...
    def transfer_settings(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Transfer Error",
                f"Failed to transfer settings: {str(e)}",
            )
    ...
    def _collect_configs_from_path(
        self,
        config_path: Path,
    ...
    def send_selected_files(self):
        ...
        transfer_data = {"type": "files", "files": files}
    ...
    def transfer_collection(self):
        ...
        transfer_data = {"type": "collection", "collection": collection}
    ...
    def create_collection(self):
        ...
        QMessageBox.information(
            self, "Collection", f"Collection '{name}' created successfully."
        )
