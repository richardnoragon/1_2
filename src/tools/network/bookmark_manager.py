...
class BookmarkModel:
    ...
    def _validate_url(self, url: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if not url.startswith(protocols):
                url = "https://" + url
                self.logger.debug(
                    f"Added https:// protocol to URL: {original_url} -> {url}"
                )
    ...
    def search_bookmarks(self, query: str, field: str = "all") -> List[Dict]:
        ...
        try:
            ...
            conn.close()
            self.logger.debug(
                f"Search returned {len(bookmarks)} bookmarks for query: {query}"
            )
            return bookmarks
        except sqlite3.Error as e:
...
class BookmarkImporter:
    ...
    def _extract_bookmarks_recursive(
        obj: Any, bookmarks: List[Dict], folder: str = "Imported"
    ) -> None:
...
class BookmarkManagerGUI(QMainWindow):
    ...
    def filter_by_tag(self, item):
        ...
        if tag == "All Tags":
            ...
            self.populate_table(bookmarks)
            self.status_bar.showMessage(
                f"Showing {len(bookmarks)} bookmarks with tag '{tag}'"
            )
    ...
    def _show_import_error(self, error: Exception) -> None:
        ...
        QMessageBox.critical(
            self, "Import Error", f"Failed to import bookmarks: {str(error)}"
        )
        ...
        if file_path:
            try:
                ...
                if success:
                    ...
                    QMessageBox.warning(
                        self, "Export Error", "Failed to export bookmarks"
                    )
                ...
                QMessageBox.critical(
                    self,
                    "Export Error",
                    f"Failed to export bookmarks: {str(e)}",
                )
