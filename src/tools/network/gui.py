...
class NetworkToolsWindow(StandardWindow):
    ...
    def create_bandwidth_monitor_tab(self) -> QWidget:
        ...
        self.lbl_download_speed.setStyleSheet("font-weight: bold; color: #4CAF50;")
        ...
        self.lbl_upload_speed.setStyleSheet("font-weight: bold; color: #FF9800;")
        ...
        if file_path:
            try:
                with open(file_path, 'w') as f:
                    ...
                    self.export_table_data(f, "Port Scan Results", self.table_scan_results)
                    self.export_table_data(f, "WiFi Networks", self.table_wifi)
                    self.export_table_data(f, "Discovered Hosts", self.table_hosts)
                    self.export_table_data(f, "Connectivity Results", self.table_connectivity)
