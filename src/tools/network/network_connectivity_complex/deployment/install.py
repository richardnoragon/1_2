...
class NetworkConnectivityInstaller:
    ...
    def _get_default_install_dir(self) -> Path:
        ...
        if system == "windows":
            ...
            if self._is_admin():
                ...
                return (
                    Path.home() / "AppData" / "Local" / "NetworkConnectivity"
                )
    ...
    def check_prerequisites(self) -> Tuple[bool, List[str]]:
        ...
        if not self._check_permissions():
            errors.append(
                f"Insufficient permissions to write to {self.install_dir}"
            )
        ...
        if success:
            ...
            self.logger.error(
                f"✗ Prerequisites check failed: {len(errors)} errors"
            )
            for error in errors:
    ...
    def install_dependencies(self) -> bool:
        ...
        try:
            ...
            requirements_file = (
                self.source_dir / "deployment" / "requirements.txt"
            )
        ...
        try:
            ...
            if not config_file.exists():
                ...
                with open(config_file, "w") as f:
                    yaml.dump(
                        default_config, f, default_flow_style=False, indent=2
                    )
            ...
            if not logging_config_file.exists():
                ...
                with open(logging_config_file, "w") as f:
                    yaml.dump(
                        logging_config, f, default_flow_style=False, indent=2
                    )
        ...
        return {
            ...
            "handlers": {
                ...
                "file": {
                    ...
                    "formatter": "detailed",
                    "filename": str(
                        self.install_dir / "logs" / "network_connectivity.log"
                    ),
    ...
    def _create_windows_shortcuts(self):
        ...
        try:
            ...
            shortcut_path = os.path.join(
                desktop, "Network Connectivity Hub.lnk"
            )
            ...
            shortcut.IconLocation = str(
                self.install_dir / "gui" / "icons" / "hub.ico"
            )
            shortcut.save()
...
"""
        ...
        (desktop_dir / "network-connectivity-hub.desktop").write_text(
            desktop_entry
        )
    ...
    def validate_installation(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            self.logger.debug(
                f"Successfully imported network_connectivity v{network_connectivity.__version__}"
            )
        ...
        if success:
            ...
            self.logger.error(
                f"✗ Installation validation failed: {len(errors)} errors"
            )
            for error in errors:
                self.logger.error(f"  - {error}")
    ...
    def install(self) -> bool:
        ...
        """
        self.logger.info(
            "Starting Network Connectivity Toolkit installation..."
        )
        ...
        if not prereq_success:
            self.logger.error(
                "Prerequisites check failed. Installation aborted."
            )
            return False
        ...
        if not self.install_dependencies():
            self.logger.error(
                "Dependency installation failed. Installation aborted."
            )
            return False
        ...
        if not self.create_configuration():
            self.logger.error(
                "Configuration creation failed. Installation aborted."
            )
            return False
        ...
        if not self.create_shortcuts():
            self.logger.warning(
                "Shortcut creation failed, but installation continues."
            )
        else:
