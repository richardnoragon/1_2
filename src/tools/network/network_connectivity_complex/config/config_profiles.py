...
class ConfigProfileManager:
    ...
    def create_profile_from_template(
        self, template_name: str, profile_name: str = None
    ) -> bool:
    ...
    def export_template(self, template_name: str, file_path: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Exported template {template_name} to {file_path}"
            )
            return True
    ...
    def import_template(
        self, file_path: str, template_name: str = None
    ) -> bool:
        ...
        try:
            ...
            if "template" not in import_data:
                raise ValueError(
                    "Invalid template file: missing template section"
                )

            template_data = import_data["template"]

            # Convert string back to enum
            template_data["use_case"] = ProfileUseCase(
