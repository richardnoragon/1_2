...
class ServiceDetector:
    ...
    def detect_service(self, host: str, port: int, timeout: float = 3.0) -> PortInfo:
        ...
        try:
            ...
            if banner:
                ...
                if service_info:
                    port_info.service = service_info.get("service", port_info.service)
                    port_info.version = service_info.get("version")
                    port_info.confidence = 0.9
            
            # Special handling for HTTP services
            if port in [80, 443, 8080, 8443] or (port_info.service and 'http' in port_info.service):
                http_info = self._probe_http_service(host, port, timeout)
                if http_info:
                    port_info.service = http_info.get("service", port_info.service)
                    port_info.version = http_info.get("version", port_info.version)
    ...
    def _grab_banner(self, host: str, port: int, timeout: float) -> Optional[str]:
    ...
    def _analyze_banner(self, port: int, banner: str) -> Optional[Dict[str, Any]]:
    ...
    def _probe_http_service(self, host: str, port: int, timeout: float) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response:
                return self._parse_http_response(response.decode('utf-8', errors='ignore'))
...
class VulnerabilityAssessment:
    ...
    def assess_vulnerabilities(self, scan_result: ScanResult) -> List[VulnerabilityInfo]:
    ...
    def _check_service_vulnerabilities(self, port_info: PortInfo) -> List[VulnerabilityInfo]:
    ...
    def _check_insecure_configurations(self, port_info: PortInfo) -> List[VulnerabilityInfo]:
...
class ScanEngine:
            ...
            with ThreadPoolExecutor(max_workers=max_threads) as executor:
                ...
                for future in as_completed(future_to_port):
                    ...
                    if progress_callback:
                        progress_callback(completed, total_ports, f"Scanned port {port}")
    ...
    def _tcp_connect_scan(self, host: str, port: int, timeout: float) -> PortState:
    ...
    def _update_stats(self, port_state: PortState):
        ...
        elif port_state in [PortState.FILTERED, PortState.OPEN_FILTERED, PortState.CLOSED_FILTERED]:
    ...
    def generate_csv_report(
        ...
        try:
            ...
            with open(file_path, 'w', newline='', encoding='utf-8') as csvfile:
                fieldnames = [
                    'target', 'scan_time', 'scan_type', 'port', 'protocol',
                    'state', 'service', 'version', 'banner', 'response_time',
                    'vulnerabilities'
                ]
                    ...
                    for port in result.ports:
                        ...
                        for vuln in result.vulnerabilities:
                            if vuln.port == port.port:
                                vulns.append(f"{vuln.severity}: {vuln.description}")
...
class PortScanner(NetworkToolBase):
    ...
    def _load_configuration(self):
        ...
        try:
            ...
            self.logger.warning(f"Invalid security level: {security_level_str}")
        
        # Load scan settings
        self.default_timeout = self.get_tool_config('scan_timeout', 3000) / 1000.0
        self.max_threads = self.get_tool_config('max_threads', 50)
        self.enable_service_detection = self.get_tool_config('enable_service_detection', True)
        self.enable_vulnerability_scan = self.get_tool_config('enable_vulnerability_scan', False)
        self.save_scan_results = self.get_tool_config('save_scan_results', True)
        
        # Load port lists
        self.common_ports = self.get_tool_config('common_ports', [
    ...
    def _scan_target_operation(self, **kwargs) -> NetworkOperationResult:
        ...
        scan_result = self.scan_engine.scan_target(
            ...
            scan_policy,
            max_threads,
            lambda current, total, msg: self.progress_updated.emit(current, total, msg)
        )
        
        # Enhance with service detection if enabled
        if kwargs.get('enable_service_detection', self.enable_service_detection):
            self._enhance_with_service_detection(scan_result)
        
        # Perform vulnerability assessment if enabled
        if kwargs.get('enable_vulnerability_scan', self.enable_vulnerability_scan):
            vulnerabilities = self.vulnerability_assessment.assess_vulnerabilities(scan_result)
            scan_result.vulnerabilities = [asdict(vuln) for vuln in vulnerabilities]
        ...
        for port_info in scan_result.ports:
            if port_info.state == PortState.OPEN:
                try:
                    ...
                    self.logger.debug(f"Service detection failed for {port_info.port}: {e}")
        ...
        if isinstance(ports_spec, str):
            if ports_spec.lower() == 'common':
                ...
                try:
                    ...
                    raise ValueError(f"Invalid port specification: {ports_spec}")
        
        raise ValueError(f"Invalid port specification type: {type(ports_spec)}")
    ...
    def _get_statistics_operation(self, **kwargs) -> NetworkOperationResult:
        ...
        statistics = self.result_manager.get_statistics(target=target, hours=hours)
    ...
    def validate_parameters(self, **kwargs) -> bool:
        ...
        valid_operations = [
            'scan_target', 'get_results', 'generate_report',
            'get_statistics', 'stop_scan'
        ]
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'current_scan': asdict(self._current_scan) if self._current_scan else None,
    ...
    def scan_target(
        ...
        if operation_result.success:
            ...
            raise RuntimeError(f"Scan failed: {operation_result.error_message}")
