...
class SpeedCalculator:
    ...
    def __init__(
        self, interface_name: str, platform_detector: PlatformNetworkDetector
    ):
...
class HistoricalDataManager:
    ...
    def get_measurements(
        ...
        if start_time:
            filtered_data = [
                m for m in filtered_data if m.timestamp >= start_time
            ]

        if end_time:
            filtered_data = [
                m for m in filtered_data if m.timestamp <= end_time
            ]

        if interface_name:
            filtered_data = [
                m for m in filtered_data if m.interface_name == interface_name
            ]

        return filtered_data

    def get_statistics(
        self, interface_name: Optional[str] = None, hours: int = 1
    ) -> Dict[str, float]:
        ...
        measurements = self.get_measurements(
            start_time=start_time, interface_name=interface_name
        )
...
class AlertManager:
    ...
    def set_threshold(
        self, interface_name: str, threshold_type: str, value: float
    ):
    ...
    def _trigger_alert(
        ...
        for recent_alert in self.recent_alerts:
            if (
                ...
                and recent_alert.interface_name == interface_name
                and (now - recent_alert.timestamp).total_seconds()
                < self.alert_cooldown
            ):
    ...
    def get_recent_alerts(
        self, hours: int = 1, interface_name: Optional[str] = None
    ) -> List[BandwidthAlert]:
        """Get recent alerts.
        ...
        try:
            ...
            if include_statistics and measurements:
                ...
                data["statistics"] = self.data_manager.get_statistics(
                    interface_name=interface_name, hours=hours
                )
...
class BandwidthMonitor(NetworkToolBase):
    ...
    def _load_configuration(self):
        ...
        self.monitoring_interval = (
            self.get_tool_config("monitoring_interval", 1000) / 1000.0
    ...
    def _start_monitoring_operation(self, **kwargs) -> NetworkOperationResult:
        ...
        invalid_interfaces = [
            name for name in interface_names if name not in available_names
        ]
        ...
        while self._monitoring_active:
            try:
                ...
                for (
                    interface_name,
                    calculator,
                ) in self.speed_calculators.items():
                    measurement = calculator.calculate_speed()
    ...
    def set_monitoring_interval(self, interval_seconds: float):
        ...
        if interval_seconds > 0:
            ...
            self.set_tool_config(
                "monitoring_interval", int(interval_seconds * 1000)
            )
