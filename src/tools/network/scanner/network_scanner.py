...
try:
    from src.gui.standard_window import StandardWindow as _BaseWindow
except ImportError:
    # Fallback for standalone execution
    _BaseWindow = QMainWindow


class NetworkScannerGUI(_BaseWindow):
    ...
    def refresh_view(self):
        ...
        QMessageBox.information(
            self, "Refresh", "Scanner interface refreshed successfully."
        )
    ...
    def test_basic_connectivity(self, target):
        ...
        except OSError as exc:  # OSError captures generic socket failures
            self.results_text.append(f"✗ Connectivity test failed: {exc}")
