...
for code, name in PIL_TAGS.items():
    ...
    if not found:
        ...
        if 0xFFFE < code:  # Treat higher codes generally
            if 'Exif' not in ALL_KNOWN_TAGS: 
                ALL_KNOWN_TAGS['Exif'] = {}
            if code not in ALL_KNOWN_TAGS['Exif']: 
                ALL_KNOWN_TAGS['Exif'][code] = name
        else:
            if '0th' not in ALL_KNOWN_TAGS: 
                ALL_KNOWN_TAGS['0th'] = {}
            if code not in ALL_KNOWN_TAGS['0th']: 
                ALL_KNOWN_TAGS['0th'][code] = name
...
def parse_exif_value(value_str: str, original_type: type,
                     tag_code: int) -> Any:
    ...
    if isinstance(original_type, bytes):
        ...
        try:
            ...
            raise ValueError(
                f"Cannot encode '{value_str}' back to bytes for this tag.")
    elif isinstance(original_type, int):
        try:
            return int(value_str)
        except ValueError:
            raise ValueError(f"Invalid integer value: '{value_str}'")
    elif (isinstance(original_type, tuple) and len(original_type) == 2 and
          all(isinstance(x, int) for x in original_type)):
...
class ImageMetadataLogic(QObject):
    ...
    def load_image_metadata(self, file_path: str,
        ...
        if self._hub_connector:
            self._hub_connector.report_status_to_hub("loading_metadata", {
                "file_path": file_path,
        ...
        try:
            ...
            self.original_exif_dict = piexif.load(file_path)
            ...
            if self._hub_connector:
                self._hub_connector.report_status_to_hub("metadata_loaded", {
                    "file_path": file_path,
    ...
    def save_image_metadata(self, file_path: str, modified_data: Dict[str, Any]) -> bool:
        """
        Save metadata modifications to image file.
        
        Args:
            file_path: Path to image file
            modified_data: Metadata updates to apply
        ...
        try:
            ...
            if not os.path.exists(file_path):
                raise FileNotFoundError(f"File not found (was it moved/deleted?): {file_path}")
            ...
            for ifd_name, tags in modified_data.items():
                if ifd_name == 'thumbnail' or ifd_name == 'file_info': 
                    continue
                ...
                for tag_code, tag_info in tags.items():
                    original_value = tag_info.get('original_value', None)
            ...
            try:
                ...
                if not exif_bytes:
                    self.metadata_saved.emit(True, f"All EXIF data removed from {os.path.basename(file_path)}.")
                else:
                    self.metadata_saved.emit(True, f"EXIF data successfully saved to {os.path.basename(file_path)}.")
                return True
    ...
    def _process_metadata_for_display(self, exif_dict: Dict[str, Any], include_thumbnails: bool = False) -> Dict[str, Any]:
...
class ImageMetadataWorker(QThread):
    """
    Worker thread for running image metadata operations without blocking the GUI.
    """
    ...
    def __init__(self, logic: ImageMetadataLogic, file_path: str, operation: str = "load", **kwargs):
        """
