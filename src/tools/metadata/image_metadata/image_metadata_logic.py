...
try:
    ...
    class MockPiexif:
        ...
        # Provide camelCase alias for compatibility with real piexif
        ExifIFD.UserComment = ExifIFD.user_comment
        ...
        def insert(exif_bytes, file_path):
            # Fallback stub keeps interface parity with piexif
            pass
...
def format_exif_value(value: Any) -> str:  # noqa: C901
                ...
                for tag_code, tag_info in tags.items():
                    ...
                    if (
                        ...
                        try:
                            ...
                            # Handle helper cases (like UserComment)
                            # after the parsed value is available
                            if tag_code == piexif.ExifIFD.UserComment and isinstance(
                                parsed_value, str
                            ):
                                new_exif_dict[ifd_name][tag_code] = (
                                    piexif.helper.UserComment.dump(parsed_value)
                                )
                            else:
                            ...
                            raise ValueError(
                                (
                                    ...
                                    f"'{tag_name_for_error}' "
                                    f"({ifd_name}/{tag_code}): {e}"
                                )
                            )
            ...
            try:
                ...
                if not exif_bytes:
                    self.metadata_saved.emit(
                        True,
                        f"All EXIF data removed from {filename}.",
                    )
                else:
                    self.metadata_saved.emit(
                        True,
...
class ImageMetadataWorker(QThread):
    """Background worker for image metadata operations."""
