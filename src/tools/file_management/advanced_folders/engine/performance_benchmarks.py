        ...
        with self.lock:
            ...
            if self.disk_io:
                ...
                stats["disk_io"] = {
                    ...
                    "avg_read_rate_mbps": (
                        statistics.mean(read_bytes)
                        / (1024 * 1024)
                        / self.interval
                    ),
                    "avg_write_rate_mbps": (
                        statistics.mean(write_bytes)
                        / (1024 * 1024)
                        / self.interval
                    ),
                }
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            config = SearchEngineConfig(
                max_workers=4, queue_size=1000, result_batch_size=100
            )
            ...
            with MemoryProfiler().profile() as profiler:
                ...
                while time.time() < end_time:
                    ...
                    try:
                        ...
                        # Add metrics
                        result.add_metric(
                            MetricType.EXECUTION_TIME, search_time
                        )
                        result.add_metric(
                            MetricType.THROUGHPUT, 1.0 / (search_time / 1000)
                        )
                ...
                result.metadata["error_rate"] = (
                    failed_searches / total_searches
                    if total_searches > 0
                    else 0
                )
        ...
        for benchmark_type, benchmark_class in self.benchmarks.items():
            ...
            try:
                ...
                if not benchmark.setup():
                    self.logger.error(
                        f"Failed to setup {benchmark_type.value}"
                    )
                    continue
                ...
                self.logger.error(
                    f"Benchmark {benchmark_type.value} error: {e}"
                )
                ...
                "results": [
                    {
                        ...
                        "metrics": {
                            metric_type.value: result.get_metric_stats(
                                metric_type
                            )
        ...
        with open(output_file, "w") as f:
            ...
            for result in self.results:
                ...
                if result.success:
                    ...
                    f.write(
                        f"  CPU Usage: {result.cpu_utilization_percent:.1f}%\n"
                    )
                else:
