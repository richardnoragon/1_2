...
class FilterEvaluator(ABC):
    ...
    @abstractmethod
    def evaluate(
        self, criterion: FilterCriterion, file_info: Dict[str, Any]
    ) -> bool:
        """
...
class DateParser:
    ...
    def __init__(self):
        ...
        self.relative_patterns = {
            "today": lambda: datetime.now().replace(
                hour=0, minute=0, second=0, microsecond=0
            ),
            "yesterday": lambda: (datetime.now() - timedelta(days=1)).replace(
                hour=0, minute=0, second=0, microsecond=0
            ),
            "this_week": lambda: datetime.now()
            - timedelta(days=datetime.now().weekday()),
            "last_week": lambda: datetime.now()
            - timedelta(days=datetime.now().weekday() + 7),
            "this_month": lambda: datetime.now().replace(
                day=1, hour=0, minute=0, second=0, microsecond=0
            ),
            "last_month": lambda: (
                datetime.now().replace(day=1) - timedelta(days=1)
            ).replace(day=1),
            "this_year": lambda: datetime.now().replace(
                month=1, day=1, hour=0, minute=0, second=0, microsecond=0
            ),
            "last_year": lambda: datetime.now().replace(
    ...
    def parse_date(self, date_str: str) -> datetime:
        ...
        offset_match = re.match(
            r"^(\d+)\s+(days?|weeks?|months?|years?)\s+ago$", date_str
        )
        if offset_match:
    ...
    def parse_date_range(self, range_str: str) -> Tuple[datetime, datetime]:
        """
...
class AdvancedFilterEngine:
    ...
    def delete_filter_chain(self, name: str) -> bool:
        ...
        if file_path.exists():
            try:
                ...
                self.logger.error(
                    f"Failed to delete filter chain file: {str(e)}"
                )

        return False

    def apply_filter_chain(
        self, chain: FilterChain, file_infos: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _evaluate_groups(
        self, groups: List[FilterGroup], file_info: Dict[str, Any]
    ) -> bool:
    ...
    def _evaluate_group(
        self, group: FilterGroup, file_info: Dict[str, Any]
    ) -> bool:
    ...
    def create_size_filter(
        ...
        return FilterCriterion(
            ...
            or f"File size {operator.value} {size_spec}",
        )
    ...
    def create_date_filter(
        ...
        return FilterCriterion(
            ...
            value=value,
            description=description
            or f"{date_type.value} {operator.value} {date_spec}",
        )
    ...
    def validate_filter_chain(self, chain: FilterChain) -> Dict[str, Any]:
        ...
        for i, group in enumerate(chain.groups):
            ...
            if group.operator == LogicalOperator.AND:
                for j, criterion in enumerate(group.criteria):
                    for k, other_criterion in enumerate(
                        group.criteria[j + 1 :], j + 1
                    ):
                        if self._check_criterion_conflict(
        ...
        regex_count = sum(
            1
            for criterion in chain.get_all_criteria()
    ...
    def _check_criterion_conflict(
        self, criterion1: FilterCriterion, criterion2: FilterCriterion
    ) -> bool:
        """Check if two criteria conflict with each other."""
        # Same field with conflicting operators
        if (
            criterion1.field_name == criterion2.field_name
            and criterion1.filter_type == criterion2.filter_type
        ):

            # Check for impossible combinations
            if (
                criterion1.operator == ComparisonOperator.EQUALS
                and criterion2.operator == ComparisonOperator.NOT_EQUALS
                and criterion1.value == criterion2.value
            ):
                return True
            ...
            if criterion1.filter_type == FilterType.FILESIZE:
                if (
                    criterion1.operator == ComparisonOperator.GREATER_THAN
                    and criterion2.operator == ComparisonOperator.LESS_THAN
                    and criterion1.value >= criterion2.value
                ):
                    return True
