...
class SearchCache:
    ...
    def _generate_cache_key(
        self, params: SearchParameters, directories: List[str]
    ) -> str:
...
class FileIndexer:
    ...
    def remove_file(self, file_path: Path):
        ...
        with self._index_lock:
            ...
            empty_words = [
                word
                for word, files in self._content_index.items()
                if not files
            ]
...
class SearchEngine:
    ...
    def __init__(self, enable_caching: bool = True):
        ...
        # Progress tracking
        self.progress_callback: Optional[Callable[[SearchProgress], None]] = (
            None
        )

    def set_progress_callback(
        self, callback: Callable[[SearchProgress], None]
    ):
    ...
    def _matches_size_filter(
        self, file_size: int, size_min: Optional[int], size_max: Optional[int]
    ) -> bool:
    ...
    def _matches_date_filter(
        self, file_stat: os.stat_result, params: SearchParameters
    ) -> bool:
    ...
    def _matches_type_filter(
        self, file_path: Path, params: SearchParameters
    ) -> bool:
        ...
        if (
            ...
            and file_ext not in params.include_extensions
        ):
            return False
        ...
        try:
            ...
            for root, dirs, files in os.walk(directory):
                ...
                for filename in files:
                    ...
                    if (
                        not params.include_hidden_files
                    ...
                    try:
                        ...
                        if params.content_search:
                            if not self._search_file_content(
                                file_path, params
                            ):
                                continue
            ...
            try:
                for directory_path in config.directory_paths:
                    ...
                    for result in self._search_directory(
                        ...
                        if (
                            len(results)
                            >= config.search_parameters.max_results
                        ):
                            break
    ...
    def index_directories(self, directories: List[str]):
        ...
        """
        self.logger.info(
            f"Starting indexing of {len(directories)} directories"
        )
