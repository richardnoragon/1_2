...
class FolderConfiguration:
    ...
    def _validate_configuration(self):
        ...
        try:
            ...
            raise ValidationException(
                f"Configuration validation failed: {str(e)}", cause=e
            )
    ...
    def add_tag(self, tag: str) -> bool:
        ...
        if not tag or not tag.strip():
            raise ValidationException(
                "Tag cannot be empty", field_name="tag", field_value=tag
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            # Configuration components
            "target_directories": [
                target.to_dict() for target in self.target_directories
            ],
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "FolderConfiguration":
        ...
        try:
            ...
            if "security_settings" in data:
                config.security_settings = SecuritySettings.from_dict(
            ...
            if "indexing_strategy" in data:
                config.indexing_strategy = IndexingStrategy(
                    data["indexing_strategy"]
                )

            config.auto_refresh_enabled = data.get(
                "auto_refresh_enabled", True
            )
            config.auto_refresh_interval_minutes = data.get(
            ...
            if data.get("last_scan_time"):
                config.last_scan_time = datetime.fromisoformat(
                    data["last_scan_time"]
                )
    ...
    def to_json(self, indent: Optional[int] = 2) -> str:
        ...
        try:
            ...
            raise ConfigurationException(
                f"Failed to serialize configuration to JSON: {str(e)}", cause=e
            )
    ...
    def from_json(cls, json_str: str) -> "FolderConfiguration":
        ...
        try:
            ...
            raise ConfigurationException(
                f"Invalid JSON format: {str(e)}", cause=e
            )
        except Exception as e:
            raise ConfigurationException(
                f"Failed to create configuration from JSON: {str(e)}", cause=e
            )
    ...
    def validate_integrity(self) -> bool:
        ...
        try:
            ...
            raise ValidationException(
                f"Integrity validation failed: {str(e)}", cause=e
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "target_paths": [
                target.path for target in self.target_directories
            ],
            "tags": list(self.tags),
            "is_active": self.is_active,
            "last_scan_count": self.last_scan_count,
