...
class UIDataBridge(QObject):
    ...
    def _setup_default_transformers(self):
        ...
        self.folder_transformers["table_row"] = (
            self._transform_folder_for_table
        )

        # Search result transformers
        self.search_transformers["ui_display"] = self._transform_search_for_ui
        self.search_transformers["table_model"] = (
            self._transform_search_for_table
        )
    ...
    def get_folder_data(
        self, folder_id: str, format_type: str = "ui_display"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Transform data
            transformer = self.folder_transformers.get(
                format_type, self.folder_transformers["ui_display"]
            )
            ...
            self.logger.error(
                f"Failed to get folder data for {folder_id}: {e}"
            )
    ...
    def update_folder_data(self, folder_id: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not validation_result.is_valid:
                self.validationResultChanged.emit(
                    folder_id,
            ...
            # Update backend
            operation_id = self.backend_manager.update_folder_configuration(
                folder_id, data
            )
            ...
            self.logger.error(
                f"Failed to update folder data for {folder_id}: {e}"
            )
    ...
    def create_folder_data(self, data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            # Create in backend
            operation_id = self.backend_manager.create_folder_configuration(
                data
            )
    ...
    def get_search_data(
        self, query: str, folder_id: str, format_type: str = "ui_display"
    ) -> Dict[str, Any]:
    ...
    def _transform_folder_for_tree(
        self, folder_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _transform_folder_for_table(
        self, folder_data: Dict[str, Any]
    ) -> List[Any]:
    ...
    def _transform_search_for_table(
        self, search_data: Dict[str, Any]
    ) -> List[List[Any]]:
    ...
    def _transform_search_for_export(
        self, search_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _clear_folder_cache(self, folder_id: str):
        ...
        keys_to_remove = [
            key
            for key in self.folder_data_cache.keys()
    ...
    def _handle_folder_configuration_changed(
    ...
    def _handle_search_results_changed(
        self, query: str, results: List[Any], metadata: Dict[str, Any]
    ):
