...
project_root = Path(
    __file__
).parent.parent.parent.parent.parent  # Go to project root (1_2)
...
def test_imports():
    ...
    try:
        from tools.advanced_folders.gui.constants import Colors, Fonts, Layout
    ...
    try:
        from tools.advanced_folders.gui.configuration_dialog import (
    ...
    try:
        from tools.advanced_folders.gui.config_tabs import GeneralConfigTab
    ...
    try:
        from tools.advanced_folders.gui.directory_browser import (
...
def test_constants_validation():
    ...
    try:
        from tools.advanced_folders.gui.constants import (
            Colors,
            FileTypes,
            Fonts,
            Layout,
        ...
        for color in required_colors:
            if hasattr(Colors, color):
                ...
                if color_value.startswith("#") and len(color_value) == 7:
                    ...
                    errors.append(
                        f"Invalid color format for {color}: {color_value}"
                    )
            else:
        ...
        # Test file types
        if hasattr(FileTypes, "DOCUMENTS") and isinstance(
            FileTypes.DOCUMENTS, list
        ):
