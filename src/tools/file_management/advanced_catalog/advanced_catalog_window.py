...
class AdvancedCatalogWindow(StandardWindow):
    ...
    def _create_directory_group(self) -> QGroupBox:
        ...
        self.dir_label.setStyleSheet("QLabel { border: 1px solid gray; padding: 5px; background-color: #f0f0f0; }")
    ...
    def _apply_sort(self):
        ...
        self.status_label.setText(f"Applied {criteria_text} sort to {len(self.catalog_data.entries)} files")
    ...
    def _update_color_legend(self):
        ...
        for category_name, color_items in self.catalog_data.color_legend.items():
            # Category header
            ...
            for color_info in color_items:
                ...
                # Label
                text_label = QLabel(f"{color_info.accessibility_label} - {color_info.category_name}")
...
"""
    ...
    def _export_catalog(self):
        ...
        if not self.catalog_data.entries:
            QMessageBox.warning(self, "No Data", "No files to export. Please scan a directory first.")
            return
        ...
        output_path, _ = QFileDialog.getSaveFileName(
            self, "Export Catalog", default_name,
