    ...
    def _check_duplicate(self, file_path: str) -> bool:
        ...
        try:
            ...
            for other_path in base_path.rglob("*"):
                if (
                    other_path.is_file()
                    and other_path != file_path
                    and other_path.stat().st_size == len(content)
                ):
                    try:
...
"""
    ...
    def _get_files_to_process(self) -> List[Tuple[str, str, str]]:
        ...
        if self.recursiveCheckBox.isChecked():
            ...
            for path in base_path.iterdir():
                if path.is_file():
                    files.append((str(base_path), path.name, path.name))

        return sorted(files)

    def _write_file_entry(
        self, html_file: TextIO, file_info: Tuple[str, str, str]
    ) -> None:
        ...
        try:
            ...
            is_dup = self.identdupCheckBox.isChecked() and self._check_duplicate(
                file_path
            )
    ...
    def _open_last_catalog(self) -> None:
        ...
        if self._last_catalog and os.path.exists(self._last_catalog):
            ...
            show_error_dialog("No catalog file available", title="Error", parent=self)
