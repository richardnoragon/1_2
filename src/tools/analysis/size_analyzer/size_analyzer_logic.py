...
class SizeAnalyzer(QObject):
    ...
    def analyze_directory(
        ...
        name_range: Optional[Tuple[Optional[str], Optional[str]]] = None,
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            name_range: Optional tuple defining inclusive name range filter
        ...
        if self._hub_connector:
            self._hub_connector.report_status_to_hub(
                "analyzing",
                {
                    ...
                    "start_time": self._performance_metrics["start_time"].isoformat(),
                },
            )
        ...
        try:
            ...
            total_items = self._count_items(
                directory_path, include_extensions, name_range
            )
            ...
            analysis["directory_tree"] = self._generate_directory_tree(directory_path)
        ...
        try:
            for root, dirs, files in os.walk(directory_path):
                for file_name in files:
                    if self._passes_filters(file_name, include_extensions, name_range):
                        total += 1
                if self._should_cancel:
                    break
        except OSError:
    ...
    def _scan_directory_contents(
        ...
        name_range: Optional[Tuple[Optional[str], Optional[str]]],
        total_items: int,
        progress_callback: Optional[Callable[[int], None]],
    ) -> None:
        """Traverse directory tree while honoring active filters."""
        items_processed = 0
        ...
        for root, dirs, files in os.walk(directory_path):
            ...
            for file_name in files:
                ...
                if not self._passes_filters(file_name, include_extensions, name_range):
                    continue
                ...
                try:
                    ...
                    self._update_file_type_stats(file_info, analysis["file_types"])
                except OSError:
                    continue

                items_processed += 1
                self._update_scan_progress(
                    items_processed, total_items, progress_callback
                )
    ...
    def _update_scan_progress(
        ...
        """Emit progress updates during directory traversal."""
        if total_items > 0:
        ...
        if items_processed and items_processed % 100 == 0:
    ...
    def _name_in_range(
        self,
        file_name: str,
        name_range: Tuple[Optional[str], Optional[str]],
    ) -> bool:
        """Check if file_name falls within the specified inclusive range."""
        start_token, end_token = name_range
        if not (start_token or end_token):
            return True
        ...
        if start_token and normalized < start_token:
            return False
        if end_token and normalized > end_token:
            return False
        return True
    ...
    def _passes_filters(
        ...
        """Return True when the file satisfies all active filters."""
        if include_extensions:
            file_ext = os.path.splitext(file_name)[1].lower()
            if file_ext not in include_extensions:
                return False

        if name_range and not self._name_in_range(file_name, name_range):
            return False
    ...
    def _prepare_for_export(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        exportable["export_metadata"] = {
            ...
            "formatted_total_size": self.format_size(exportable.get("total_size", 0)),
        }
