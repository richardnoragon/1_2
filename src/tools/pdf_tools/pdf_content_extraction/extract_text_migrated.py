...
class ExtractTextWindow(BaseWindow):
    ...
    def _load_settings(self):
        ...
        try:
            ...
            # Load last directory
            self.last_directory = self.config.get_setting('general', 
    ...
    def _save_settings(self):
        ...
        try:
            ...
            if hasattr(self, 'formatComboBox'):
                settings['default_format'] = self.formatComboBox.currentText().lower()
            
            # Save page numbers preference
            if hasattr(self, 'includePageNumbersCheckBox'):
                settings['include_page_numbers'] = self.includePageNumbersCheckBox.isChecked()
            
            # Save layout preference
            if hasattr(self, 'preserveLayoutCheckBox'):
                settings['preserve_layout'] = self.preserveLayoutCheckBox.isChecked()
            ...
            if hasattr(self, 'last_directory'):
                self.config.set_setting('general', 'last_opened_directory', 
                                       self.last_directory)
    ...
    def browse_file(self):
        ...
        try:
            ...
            if filename:
                ...
                # Update status
                self.set_status_message(f"File selected: {Path(filename).name}")
    ...
    def extract_text(self):
        ...
        try:
            ...
            pages_text = self.pagesEdit.text().strip() if hasattr(self, 'pagesEdit') else ""
            include_page_numbers = (self.includePageNumbersCheckBox.isChecked() 
                                  if hasattr(self, 'includePageNumbersCheckBox') else True)
            preserve_layout = (self.preserveLayoutCheckBox.isChecked() 
                             if hasattr(self, 'preserveLayoutCheckBox') else False)
            ...
            if result:
                ...
                self.set_status_message("Text extraction completed successfully")
            ...
            self.logger.error(f"Error in extract operation: {e}", exc_info=True)
            QMessageBox.critical(self, "Extraction Error", 
                               f"Error during text extraction: {e}")
            self.set_status_message("Error during extraction")
        finally:
            self.progress_bar.hide()
    
    def _extract_text_from_pdf(self, input_file: str, pages_text: str = "", 
                              include_page_numbers: bool = True,
                              preserve_layout: bool = False) -> Optional[str]:
        """
        ...
        try:
            ...
            with pdfplumber.open(input_file) as pdf:
                self.logger.debug(f"PDF opened successfully, pages: {len(pdf.pages)}")
                ...
                for idx, page_num in enumerate(page_list):
                    try:
                        if 0 <= page_num < len(pdf.pages):
                            ...
                            self.logger.warning(f"Page {page_num + 1} out of range")
                        ...
                        QMessageBox.warning(self, "Page Extraction Warning", 
    ...
    def _parse_page_range(self, pages_text: str, total_pages: int) -> Optional[list]:
        """
        ...
        try:
            ...
            for part in pages_text.split(','):
                ...
                if '-' in part:
                    ...
                    page_list.extend(range(start - 1, end))  # Convert to 0-based
                else:
                    page_list.append(int(part) - 1)  # Convert to 0-based
            
            # Validate page numbers
            invalid_pages = [p + 1 for p in page_list if p < 0 or p >= total_pages]
            if invalid_pages:
            ...
            self.logger.debug(f"Processing pages: {[p + 1 for p in page_list]}")
            return page_list
    ...
    def save_text(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Save Operation Error", 
                               f"Error in save operation: {e}")
