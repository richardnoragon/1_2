...
class PDFToDocumentConverter:
    ...
    def pdf_to_docx(
        self, input_path: str, output_path: str, settings: ConversionSettings
    ) -> ConversionResult:
        ...
        try:
            if not HAS_DOCX or not HAS_PYMUPDF:
                return ConversionResult(
                    False,
                    ConversionOperation.PDF_TO_DOCX,
            ...
            # Process pages
            pages_to_process = settings.extract_pages or list(
            ...
            for page_num in pages_to_process:
                ...
                if settings.preserve_formatting:
                    ...
                    self._process_text_blocks_to_docx(
                        text_blocks, docx_doc, settings
                    )
                else:
                    text = page.get_text()
    ...
    def _process_text_blocks_to_docx(
        self, text_dict: Dict, docx_doc, settings: ConversionSettings
    ):
    ...
    def _extract_images_to_docx(
        self, page, docx_doc, settings: ConversionSettings
    ):
    ...
    def pdf_to_xlsx(
        ...
        try:
            if not HAS_OPENPYXL or not HAS_PYMUPDF:
                return ConversionResult(
                    False,
                    ConversionOperation.PDF_TO_XLSX,
                ...
                if settings.preserve_tables:
                    ...
                    if tables:
                        ...
                        for row, line in enumerate(lines, 1):
                            if line.strip():
                                worksheet.cell(
                                    row=row, column=1, value=line.strip()
                                )
                else:
                    # Extract as text
                    text = page.get_text()
...
class PDFToImageConverter:
    ...
    def pdf_to_images(
        self, input_path: str, output_dir: str, settings: ConversionSettings
    ) -> ConversionResult:
        ...
        try:
            if not HAS_PYMUPDF:
                return ConversionResult(
                    False,
                    ConversionOperation.PDF_TO_IMAGES,
            ...
            for page_num in pages_to_process:
                ...
                mat = fitz.Matrix(
                    settings.image_dpi / 72, settings.image_dpi / 72
                )
                ...
                # Save image
                image_filename = (
                    f"page_{page_num + 1:03d}.{settings.image_format.lower()}"
                )
                ...
                if settings.image_format.upper() == "PNG":
                    ...
                    if HAS_PIL:
                        ...
                        pil_image.save(
                            image_path, format=settings.image_format.upper()
                        )
                    else:
                        pix.save(
                            image_path.replace(
                                f".{settings.image_format.lower()}", ".png"
                            )
                        )
    ...
    def images_to_pdf(
        ...
        try:
            if not image_paths:
                return ConversionResult(
                    False,
                    ConversionOperation.IMAGES_TO_PDF,
            ...
            if HAS_PYMUPDF:
                return self._images_to_pdf_pymupdf(
                    image_paths, output_path, settings, start_time
                )
            elif HAS_PIL:
                return self._images_to_pdf_pil(
                    image_paths, output_path, settings, start_time
                )
            else:
                return ConversionResult(
    ...
    def _images_to_pdf_pymupdf(
        ...
        try:
            ...
            for image_path in image_paths:
                ...
                page = doc.new_page(
                    width=img[0].rect.width, height=img[0].rect.height
                )
...
class PDFConversionEngine:
    ...
    def convert_pdf(
        ...
        if not valid:
            return ConversionResult(
                False, ConversionOperation.AUTO_DETECT, msg
            )

        valid, msg = PDFConversionValidator.validate_conversion_settings(
            settings
        )
        if not valid:
            return ConversionResult(
        ...
        if target_format == ConversionFormat.DOCX:
            return self.document_converter.pdf_to_docx(
                input_path, output_path, settings
            )
        elif target_format == ConversionFormat.XLSX:
            return self.document_converter.pdf_to_xlsx(
                input_path, output_path, settings
            )
            ...
            return self.image_converter.pdf_to_images(
                input_path, output_dir, settings
            )
        else:
            return ConversionResult(
                False,
                ConversionOperation.AUTO_DETECT,
    ...
    def convert_images_to_pdf(
        ...
        return self.image_to_pdf_converter.images_to_pdf(
            image_paths, output_path, settings
        )
        ...
        try:
            ...
            if input_format and input_format.value == "pdf":
                return self.convert_pdf(
                    input_path, output_path, output_format, settings
                )
            elif output_format.value == "pdf":
                # Convert to PDF (support images for now)
                if input_format and input_format.value in [
                    "png",
                    "jpg",
                    "jpeg",
                    "tiff",
    ...
    def batch_convert(
        ...
        for input_path, output_path in file_pairs:
            try:
                result = self.auto_detect_and_convert(
                    input_path, output_path, settings
                )
...
def create_conversion_settings(
    quality: ConversionQuality = ConversionQuality.STANDARD, **kwargs
) -> ConversionSettings:
