...
class PDFExtractionValidator:
    ...
    def validate_extraction_file(
        file_path: str, extraction_type: ExtractionType
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def _check_extraction_compatibility(
        doc, extraction_type: ExtractionType
    ) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Compatibility check failed for {extraction_type.value}: {e}"
            )
...
class PDFExtractionFileManager:
    ...
    def create_extraction_directory(
        self, base_path: str, extraction_type: ExtractionType
    ) -> str:
    ...
    def generate_output_filename(
        ...
        if suffix:
            ...
            filename = (
                f"{base_name}_{extraction_type.value}_{timestamp}{extension}"
            )
    ...
    def save_extraction_data(
        self, data: Any, output_path: str, data_format: str = "auto"
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to save extraction data to {output_path}: {e}"
            )
            return False
...
class PDFTextExtraction:
    ...
    def execute(
        ...
        try:
            ...
            # Validate input file
            validation = PDFExtractionValidator.validate_extraction_file(
            ...
            # Save extracted text
            if self.file_manager.save_extraction_data(
                extracted_text, output_path, "text"
            ):
                processing_time = (datetime.now() - start_time).total_seconds()

                self.logger.info(
                    f"Text extraction completed: {output_path} ({processing_time:.2f}s)"
                )
        ...
        with pdfplumber.open(input_file) as pdf:
            ...
            if page_range:
                ...
                pages_to_process = range(
                    max(0, start_page), min(end_page + 1, total_pages)
                )
            else:
...
class PDFImageExtraction:
    ...
    def execute(
        ...
        try:
            ...
            self.logger.info(
                f"Image extraction completed: {len(extracted_images)} images ({processing_time:.2f}s)"
            )
                    ...
                    try:
                        ...
                        if PIL_AVAILABLE:
                            ...
                            if (
                                ...
                                # Save image
                                image.save(
                                    output_file, format=image_format.upper()
                                )
                                extracted_images.append(output_file)

                                self.logger.debug(
                                    f"Extracted image: {output_file}"
                                )
                            else:
                                self.logger.debug(
                                    f"Skipped small image: {image.width}x{image.height}"
                                )
                        else:
...
class PDFMetadataExtraction:
    ...
    def execute(
        ...
        try:
            ...
            # Validate input file
            validation = PDFExtractionValidator.validate_extraction_file(
                input_file, ExtractionType.METADATA
            )
            if not validation["valid_pdf"]:
                error_msg = f"Invalid PDF file: {validation.get('error', 'Unknown error')}"
                return ExtractionResult(
            ...
            metadata = self._extract_metadata(
                input_file, include_extended, progress_tracker
            )
            ...
            if self.file_manager.save_extraction_data(
                ...
                self.logger.info(
                    f"Metadata extraction completed: {output_path} ({processing_time:.2f}s)"
                )
    ...
    def _extract_metadata(
        self, input_file: str, include_extended: bool, progress_tracker
    ) -> Dict[str, Any]:
        ...
        if PIKEPDF_AVAILABLE:
            try:
                with pikepdf.Pdf.open(input_file) as pdf:
                    ...
                    if pdf.docinfo:
                        ...
                        for key, value in pdf.docinfo.items():
                            ...
                            if (
                                ...
                                try:
                                    doc_info[key_str] = (
                                        self._transform_pdf_date(value)
                                    )
                                except:
                    ...
                    if progress_tracker:
                        progress_tracker.update_progress(
                            75, "Extracting page information"
                        )
        ...
        if PYMUPDF_AVAILABLE and "document_info" not in metadata:
            try:
                ...
                metadata["page_info"] = {"page_count": len(doc), "pages": []}
        ...
        if progress_tracker:
            progress_tracker.update_progress(
                100, "Metadata extraction complete"
            )
...
class PDFTableExtraction:
    ...
    def execute(
        ...
        try:
            ...
            self.logger.info(
                f"Table extraction completed: {len(extracted_tables)} tables ({processing_time:.2f}s)"
            )
    ...
    def _extract_tables_camelot(
        ...
        if progress_tracker:
            progress_tracker.update_progress(
                0, "Initializing table extraction"
            )

        # Default Camelot options
        default_options = {"flavor": "lattice", "pages": pages}
        ...
        try:
            ...
            if progress_tracker:
                progress_tracker.update_progress(
                    50, f"Processing {len(tables)} tables"
                )
            ...
            for i, table in enumerate(tables):
                ...
                output_file = os.path.join(
                    output_dir, f"{base_name}_table_{i + 1}.csv"
                )
...
class PDFLinkExtraction:
    ...
    def execute(
        ...
        try:
            ...
            # Validate input file
            validation = PDFExtractionValidator.validate_extraction_file(
                input_file, ExtractionType.LINKS
            )
            if not validation["valid_pdf"]:
                error_msg = f"Invalid PDF file: {validation.get('error', 'Unknown error')}"
                return ExtractionResult(
            ...
            # Extract links
            page_range = options.get("page_range")
            include_internal_links = options.get(
            ...
            # Save links
            if self.file_manager.save_extraction_data(
                links, output_path, "json"
            ):
                processing_time = (datetime.now() - start_time).total_seconds()

                self.logger.info(
                    f"Link extraction completed: {len(links['links'])} links ({processing_time:.2f}s)"
                )
            ...
            for i, page_num in enumerate(pages_to_process):
                ...
                for link in page_links:
                    ...
                    if link["kind"] == fitz.LINK_URI:
                        ...
                        link_info["destination"] = link.get(
...
class PDFExtractionEngine:
    ...
    def extract_text(
        ...
        try:
            return self.text_extraction.execute(
                input_file, output_path, options, progress_tracker
            )
        finally:
    ...
    def extract_images(
        ...
        try:
            return self.image_extraction.execute(
    ...
    def extract_metadata(
        ...
        try:
            return self.metadata_extraction.execute(
                input_file, output_path, options, progress_tracker
            )
        finally:
            self.file_manager.cleanup_temp_files()
    ...
    def extract_tables(
        ...
        try:
            return self.table_extraction.execute(
                input_file, output_dir, options, progress_tracker
            )
        finally:
            self.file_manager.cleanup_temp_files()
    ...
    def extract_links(
        ...
        try:
            return self.link_extraction.execute(
                input_file, output_path, options, progress_tracker
            )
        finally:
            self.file_manager.cleanup_temp_files()
