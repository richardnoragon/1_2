...
class PDFViewer(QMainWindow):
    def __init__(self):
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to initialize viewer: {str(e)}"
            )
            self.close()
    ...
    def load_settings(self):
        ...
        try:
            # Load last directory
            last_dir = self.config.get_setting(
                "general", "last_opened_dir", ""
            )
            self.last_directory = (
            ...
            QMessageBox.warning(
                self, "Warning", f"Error loading settings: {str(e)}"
            )
            # Use defaults
    ...
    def save_settings(self):
        ...
        try:
            self.config.set_setting(
                "general", "last_opened_dir", self.last_directory
            )
        except Exception as e:
            logger.warning("Error saving settings: %s", str(e))
            QMessageBox.warning(
                self, "Warning", f"Error saving settings: {str(e)}"
            )

    def open_file(self):
        try:
            file_name, _ = QFileDialog.getOpenFileName(
                self, "Open PDF file", self.last_directory, "PDF Files (*.pdf)"
            )
            ...
            if file_name:
                ...
                try:
                    ...
                    if self.total_pages > 0:
                        ...
                        QMessageBox.warning(
                            self, "Warning", "The PDF file is empty"
                        )
                    ...
                    QMessageBox.critical(
                        self, "Error", "Invalid or corrupted PDF file"
                    )
                except Exception as e:
                    logger.error("Could not open PDF file: %s", str(e))
                    QMessageBox.critical(
                        self, "Error", f"Could not open PDF file: {str(e)}"
                    )

        except Exception as e:
            logger.error("Error in file open operation: %s", str(e))
            QMessageBox.critical(
                self, "Error", f"Error in file open operation: {str(e)}"
            )
        ...
        try:
            if self.doc and 0 <= self.current_page < self.total_pages:
                ...
                try:
                    ...
                    self.pdfView.fitInView(
                        self.pdfView.scene().itemsBoundingRect(),
                        Qt.KeepAspectRatio,
                    )
                    ...
                    QMessageBox.warning(
                        self,
                        "Warning",
                        f"Error rendering page {self.current_page + 1}: {str(e)}",
                    )

        except Exception as e:
            logger.error("Error displaying page: %s", str(e))
            QMessageBox.critical(
                self, "Error", f"Error displaying page: {str(e)}"
            )
    ...
    def previous_page(self):
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Error navigating to previous page: {str(e)}"
            )
    ...
    def next_page(self):
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Error navigating to next page: {str(e)}"
            )
