...
def split_pdf(
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                "PDF opened successfully, total pages: %d", total_pages
            )
        ...
        try:
            if page_ranges:
                    ...
                    try:
                        ...
                        if 1 <= start <= end <= total_pages:
                            ...
                            output_file = os.path.join(
                                output_dir, f"split_{i}.pdf"
                            )
                            new_pdf.save(output_file)
                            logger.debug(
                                "Created split file %d (pages %d-%d): %s",
                                i,
                                start,
                                end,
                                output_file,
                            ...
                            logger.warning(
                                "Invalid page range: %s", page_range
                            )
                            QMessageBox.warning(
                                None,
                                "Warning",
                        ...
                        logger.error(
                            "Invalid page range format: %s", page_range
                        )
                        QMessageBox.warning(
                            None,
                            "Warning",
                ...
                total_files = (
                    total_pages + pages_per_file - 1
                ) // pages_per_file
                logger.debug(
                    "Splitting into %d files with %d pages each",
                    total_files,
                    pages_per_file,
                )
                ...
                for i in range(total_pages):
                    ...
                    new_pdf.save(output_file)
                    logger.debug(
                        "Created page file %d: %s", i + 1, output_file
                    )
            ...
            QMessageBox.critical(
                None, "Error", f"Error during split operation: {str(e)}"
            )
            return False

    except Exception as e:
        logger.error(
            "Unexpected error during split operation: %s",
            str(e),
            exc_info=True,
        )
        QMessageBox.critical(
            None, "Error", f"Unexpected error during split operation: {str(e)}"
        )
        return False
...
class SplitUI(QtWidgets.QMainWindow):
    ...
    def browse_output_dir(self):
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Error selecting output directory: {str(e)}"
            )

    def update_split_button(self):
        """Enable split button only if both input and output are selected"""
        self.splitButton.setEnabled(
            bool(self.current_file and self.current_output_dir)
        )
    ...
    def split_pdf(self):
        try:
            ...
            if self.pagesPerFileRadio.isChecked():
                try:
                    ...
                    QMessageBox.critical(
                        self,
                        "Error",
                        "Please enter a valid positive number for pages per file!",
                    )
                    return
                ...
                if ranges_text:
                    ...
                    QMessageBox.warning(
                        self, "Warning", "Please enter page ranges!"
                    )
                    return
            ...
            try:
                ...
                progress_per_split = (
                    90 - base_progress
                ) / total_splits  # Leave 10% for finalization
                ...
                if split_pdf(
                    ...
                    pages_per_file,
                    page_ranges,
                    progress_callback=lambda x: self.update_progress(
                        x, base_progress
                    ),
                ):
                    ...
                    QMessageBox.warning(
                        self, "Warning", "Failed to split PDF!"
                    )
                    self.statusBar().showMessage("Split failed", 3000)

            except Exception as e:
                logger.error(
                    "Error during split operation: %s", str(e), exc_info=True
                )
                QMessageBox.critical(
                    self, "Error", f"Error during split operation: {str(e)}"
                )
            ...
            QMessageBox.critical(
                self, "Error", f"Error in split operation: {str(e)}"
            )
