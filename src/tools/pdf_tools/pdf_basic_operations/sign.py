...
def sign_file(
    ...
    if os.path.exists(sign_filename):
        ...
        page_numbers = (
            [int(p) for p in pages] if pages else range(len(pdf.pages))
        )
...
def apply_signature(
    ...
    try:
        ...
        logger.debug(
            "Parameters - Input: %s, Signature: %s, Pages: %s, Position: %s",
            input_file,
            signature_file,
            pages,
            position,
        ...
        for page_num in page_list:
            try:
                ...
                logger.debug(
                    "Inserting signature on page %d at position %s",
                    page_num + 1,
                    position,
                )
                page.insert_image(
...
class SignUI(QtWidgets.QMainWindow):
    ...
    def browse_signature(self):
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Error selecting signature: {str(e)}"
            )
    ...
    def sign_document(self):
        try:
            ...
            if not signature_file:
                ...
                QMessageBox.warning(
                    self, "Error", "Please select a signature image!"
                )
                return
            ...
            try:
                ...
                if success:
                    ...
                    QMessageBox.information(
                        self, "Success", "Document signed successfully!"
                    )
                else:
                    logger.warning("Failed to sign document")
                    QMessageBox.warning(
                        self, "Warning", "Failed to sign document"
                    )

            except Exception as e:
                logger.error("Error during signing: %s", str(e))
                QMessageBox.critical(
                    self, "Error", f"Error during signing: {str(e)}"
                )

        except Exception as e:
            logger.error("Error in sign operation: %s", str(e))
            QMessageBox.critical(
                self, "Error", f"Error in sign operation: {str(e)}"
            )
