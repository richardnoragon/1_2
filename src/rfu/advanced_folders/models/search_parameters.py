...
class ContentSearchOptions:
    ...
    max_file_size_mb: int = 100  # Don't search content of files larger than this
...
class SearchParameters:
    ...
    def add_file_type_extension(self, extension: str, include: bool = True) -> None:
        """
    ...
    def set_size_filter(
        ...
        if min_size or max_size:
            self.size_filter = SizeFilter.from_human_readable(min_size, max_size)
        else:
    ...
    def add_date_filter(
        ...
        self.date_filters = [
            f for f in self.date_filters if f.field != field
        ]
    ...
    def set_pagination(self, offset: int = 0, max_results: int = 10000) -> None:
        """
    ...
    def matches_file(self, file_info: Dict[str, Any]) -> bool:
        ...
        if "extension" in file_info:
            if not self.file_type_filter.matches_extension(file_info["extension"]):
                return False
        ...
        if "path" in file_info:
            ...
            if self.include_paths:
                matches_include = any(
                    Path(file_path).match(pattern) for pattern in self.include_paths
                )
            ...
            if self.exclude_patterns:
                matches_exclude = any(
                    Path(file_path).match(pattern) for pattern in self.exclude_patterns
                )
                if matches_exclude:
                    return False
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "size_filter": self.size_filter.to_dict() if self.size_filter else None,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'SearchParameters':
        ...
        if "file_type_filter" in data and data["file_type_filter"]:
            params.file_type_filter = FileTypeFilter.from_dict(data["file_type_filter"])
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_file_type_filter": bool(
                self.file_type_filter.include_extensions or
                self.file_type_filter.include_documents or
                self.file_type_filter.include_images or
                self.file_type_filter.include_videos or
                self.file_type_filter.include_audio
            ),
            "has_size_filter": self.size_filter is not None,
            "date_filters_count": len(self.date_filters),
            "path_patterns_count": len(self.include_paths) + len(self.exclude_patterns),
            "max_results": self.max_results,
            "sort_field": self.sort_field.value,
            "timeout_seconds": self.timeout_seconds
        }
