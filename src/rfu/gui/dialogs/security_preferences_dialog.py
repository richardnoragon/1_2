...
try:
    ...
    class SecurityPreferencesDialog(QDialog):
        ...
        def create_theme_security_tab(self) -> QWidget:
            ...
            self.enable_corruption_detection = QCheckBox("Enable automatic corruption detection")
            ...
            corruption_layout.addRow("Check Frequency:", self.corruption_check_frequency)
            ...
            corruption_layout.addRow("Recovery Strategy:", self.corruption_recovery_strategy)
        ...
        def create_directory_security_tab(self) -> QWidget:
            ...
            self.protected_directories_list.setHeaderLabels(["Directory", "Access Level", "Status"])
            ...
            self.remove_protected_dir_btn.clicked.connect(self.remove_protected_directory)
            dir_buttons.addWidget(self.remove_protected_dir_btn)
            
            self.edit_dir_permissions_btn = QPushButton("✏️ Edit Permissions")
            self.edit_dir_permissions_btn.clicked.connect(self.edit_directory_permissions)
            ...
            # Enable monitoring
            self.enable_directory_monitoring = QCheckBox("Enable directory change monitoring")
            ...
            # Alert settings
            self.alert_on_unauthorized_access = QCheckBox("Alert on unauthorized access attempts")
        ...
        def create_audit_logging_tab(self) -> QWidget:
            ...
            categories = [
                "Database Operations", "Theme Security", "Directory Access",
                "User Authentication", "Configuration Changes", "System Events"
            ]
            ...
            self.audit_filter_level.addItems(["All Levels", "DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"])
        ...
        def create_status_monitoring_tab(self) -> QWidget:
            ...
            self.directory_security_status_widget = self.create_status_widget("Directory Security", "📁")
            dashboard_layout.addWidget(self.directory_security_status_widget, 1, 0)
            
            self.audit_logging_status_widget = self.create_status_widget("Audit Logging", "📋")
            ...
            metrics_layout.addRow("Protected Directories:", self.protected_directories_label)
            ...
            self.security_alerts_list.setHeaderLabels(["Severity", "Category", "Message", "Timestamp"])
        ...
        def create_advanced_settings_tab(self) -> QWidget:
            ...
            self.reset_security_btn.setStyleSheet("background-color: #fd7e14; color: white;")
        ...
        def load_current_settings(self):
            ...
            try:
                ...
                self.validate_migration.setChecked(
                    self.config_manager.get_setting('security_migration', 'validate_migration', True)
                )
                self.auto_rollback.setChecked(
                    self.config_manager.get_setting('security_migration', 'auto_rollback', True)
                )
                ...
                self.encryption_algorithm_combo.setCurrentText(
                    self.config_manager.get_setting('security_theme', 'encryption_algorithm', 'AES-256-GCM (Recommended)')
                )
                self.key_derivation_combo.setCurrentText(
                    self.config_manager.get_setting('security_theme', 'key_derivation', 'PBKDF2-SHA256 (Recommended)')
                )
                self.kdf_iterations.setValue(
                    self.config_manager.get_setting('security_theme', 'kdf_iterations', 100000)
                )
                ...
                self.enable_directory_monitoring.setChecked(
                    self.config_manager.get_setting('security_directory', 'enable_monitoring', False)
                )
                ...
                self.audit_log_level.setCurrentText(
                    self.config_manager.get_setting('security_audit', 'log_level', 'INFO')
                )
                ...
                QMessageBox.warning(self, "Load Error", f"Failed to load security settings: {e}")
        ...
        def apply_settings(self):
            ...
            try:
                ...
                self.config_manager.set_setting('security_migration', 'validate_migration', 
                                               self.validate_migration.isChecked())
                self.config_manager.set_setting('security_migration', 'auto_rollback', 
                ...
                self.config_manager.set_setting('security_theme', 'encryption_algorithm', 
                                               self.encryption_algorithm_combo.currentText())
                self.config_manager.set_setting('security_theme', 'key_derivation', 
                                               self.key_derivation_combo.currentText())
                self.config_manager.set_setting('security_theme', 'kdf_iterations', 
                ...
                self.config_manager.set_setting('security_directory', 'enable_monitoring', 
                ...
                self.config_manager.set_setting('security_audit', 'log_level', 
                                               self.audit_log_level.currentText())
                
                # Save advanced settings
                self.config_manager.set_setting('security_advanced', 'security_profile', 
                                               self.security_profile.currentText())
                self.config_manager.set_setting('security_advanced', 'session_timeout', 
                ...
                QMessageBox.information(self, "Settings Applied", "Security settings have been applied successfully.")
                
            except Exception as e:
                self.logger.error(f"Failed to apply security settings: {e}")
                QMessageBox.critical(self, "Apply Error", f"Failed to apply security settings: {e}")
        ...
        def check_migration_status(self) -> bool:
            ...
            try:
                ...
                if self._migration_manager is None:
                    from src.rfu.core.migrations.migration_manager import DatabaseMigrationManager
        ...
        def check_theme_security_status(self) -> bool:
            ...
            try:
                ...
                if self._theme_encryption is None:
                    from src.rfu.core.theme_security.theme_encryption import ThemeDataEncryption
        ...
        def check_directory_security_status(self) -> bool:
            ...
            try:
                ...
                enabled = self.config_manager.get_setting('security_directory', 'enable_access_control', False)
        ...
        def check_audit_logging_status(self) -> bool:
            ...
            try:
                ...
                enabled = self.config_manager.get_setting('security_audit', 'enable_logging', True)
                return enabled
        ...
        def execute_migration(self):
            ...
            QMessageBox.information(self, "Migration", "Database migration functionality will be implemented.")
        
        def rollback_migration(self):
            """Rollback database migration."""
            QMessageBox.information(self, "Rollback", "Migration rollback functionality will be implemented.")
        
        def validate_schema(self):
            """Validate database schema."""
            QMessageBox.information(self, "Validation", "Schema validation functionality will be implemented.")
        
        def encrypt_all_themes(self):
            """Encrypt all theme data."""
            QMessageBox.information(self, "Encryption", "Theme encryption functionality will be implemented.")
        
        def decrypt_all_themes(self):
            """Decrypt all theme data."""
            QMessageBox.information(self, "Decryption", "Theme decryption functionality will be implemented.")
        
        def test_theme_encryption(self):
            """Test theme encryption functionality."""
            QMessageBox.information(self, "Test", "Theme encryption test functionality will be implemented.")
        
        def scan_theme_corruption(self):
            """Scan for theme corruption."""
            QMessageBox.information(self, "Scan", "Theme corruption scanning functionality will be implemented.")
        
        def repair_theme_corruption(self):
            """Repair theme corruption."""
            QMessageBox.information(self, "Repair", "Theme corruption repair functionality will be implemented.")
        
        def add_protected_directory(self):
            """Add a protected directory."""
            dir_path = QFileDialog.getExistingDirectory(self, "Select Directory to Protect")
            if dir_path:
                QMessageBox.information(self, "Directory Added", f"Added protection for: {dir_path}")
        
        def remove_protected_directory(self):
            """Remove a protected directory."""
            QMessageBox.information(self, "Remove", "Remove protected directory functionality will be implemented.")
        
        def edit_directory_permissions(self):
            """Edit directory permissions."""
            QMessageBox.information(self, "Edit", "Directory permissions editing functionality will be implemented.")
        ...
        def export_directory_log(self):
            ...
            QMessageBox.information(self, "Export", "Directory log export functionality will be implemented.")
        
        def apply_audit_filter(self):
            """Apply audit log filter."""
            QMessageBox.information(self, "Filter", "Audit log filtering functionality will be implemented.")
        
        def refresh_audit_log(self):
            """Refresh audit log display."""
            QMessageBox.information(self, "Refresh", "Audit log refresh functionality will be implemented.")
        
        def export_audit_log(self):
            """Export audit log."""
            QMessageBox.information(self, "Export", "Audit log export functionality will be implemented.")
        
        def clear_audit_log(self):
            """Clear audit log."""
            QMessageBox.information(self, "Clear", "Audit log clearing functionality will be implemented.")
        
        def dismiss_security_alert(self):
            """Dismiss selected security alert."""
            QMessageBox.information(self, "Dismiss", "Alert dismissal functionality will be implemented.")
        
        def dismiss_all_alerts(self):
            """Dismiss all security alerts."""
            QMessageBox.information(self, "Dismiss All", "All alerts dismissal functionality will be implemented.")
        ...
        def security_lockdown(self):
            ...
            if reply == QMessageBox.Yes:
                QMessageBox.information(self, "Lockdown", "Security lockdown initiated.")
        
        def force_security_backup(self):
            """Force security backup."""
            QMessageBox.information(self, "Backup", "Security backup functionality will be implemented.")
        ...
        def reset_security_settings(self):
            ...
            if reply == QMessageBox.Yes:
                QMessageBox.information(self, "Reset", "Security settings reset to defaults.")
        
        def run_security_audit(self):
            """Run comprehensive security audit."""
            QMessageBox.information(self, "Audit", "Security audit functionality will be implemented.")
        
        def test_security_features(self):
            """Test all security features."""
            QMessageBox.information(self, "Test", "Security feature testing functionality will be implemented.")
        ...
        def export_security_config(self):
            ...
            if file_path:
                QMessageBox.information(self, "Export", f"Security configuration exported to: {file_path}")
        ...
        def import_security_config(self):
            ...
            if file_path:
                QMessageBox.information(self, "Import", f"Security configuration imported from: {file_path}")
        ...
        def emergency_disable_security(self):
            ...
            if reply == QMessageBox.Yes:
                QMessageBox.information(self, "Emergency", "All security features have been disabled.")
