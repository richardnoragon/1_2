...
class PDFValidator:
    ...
    @staticmethod
    def validate_multiple_files(file_paths: List[str]) -> Dict[str, Dict[str, Any]]:
...
class PDFFileManager:
    ...
    def generate_output_filename(self, base_path: str, operation: str, 
                                suffix: str = "", extension: str = ".pdf") -> str:
        """Generate unique output filename"""
        base_name = os.path.splitext(os.path.basename(base_path))[0]
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        if suffix:
            filename = f"{base_name}_{operation}_{suffix}_{timestamp}{extension}"
        else:
        ...
        while os.path.exists(output_path):
            if suffix:
                ...
                filename = f"{base_name}_{operation}_{timestamp}_{counter}{extension}"
            output_path = os.path.join(output_dir, filename)
            counter += 1
...
class PDFMergeOperation:
    ...
    def execute(self, input_files: List[str], output_file: str, 
                options: Dict[str, Any], progress_tracker: ProgressTracker) -> OperationResult:
        ...
        try:
            ...
            self.logger.info(f"Starting merge operation: {len(input_files)} files -> {output_file}")
            
            # Validate input files
            validation_results = PDFValidator.validate_multiple_files(input_files)
            invalid_files = [f for f, v in validation_results.items() if not v['valid_pdf']]
            ...
            # Save result
            progress_tracker.update_progress(total_files, "Saving merged PDF...")
            ...
            self.logger.info(f"Merge completed successfully: {output_file} ({processing_time:.2f}s)")
            ...
            return OperationResult(
                ...
                details={
                    'total_pages': len(result_doc) if 'result_doc' in locals() else 0,
...
class PDFSplitOperation:
    ...
    def execute(self, input_file: str, output_dir: str, 
                options: Dict[str, Any], progress_tracker: ProgressTracker) -> OperationResult:
        ...
        try:
            self.logger.info(f"Starting split operation: {input_file} -> {output_dir}")
            ...
            if split_method == 'pages':
                output_files = self._split_by_pages(
                    source_doc, input_file, output_dir, options, progress_tracker
                )
            ...
            self.logger.info(f"Split completed: {len(output_files)} files created ({processing_time:.2f}s)")
    ...
    def _split_by_pages(self, source_doc, input_file: str, output_dir: str, 
                       options: Dict[str, Any], progress_tracker: ProgressTracker) -> List[str]:
        ...
        for i in range(0, total_pages, pages_per_file):
            ...
            output_file = os.path.join(
                output_dir, 
                naming_pattern.format(index=len(output_files) + 1)
            )
            new_doc.save(output_file)
            new_doc.close()
            output_files.append(output_file)
            
            self.logger.debug(f"Created split file: {output_file} (pages {i+1}-{end_page+1})")
        
        return output_files
    
    def _split_by_ranges(self, source_doc, input_file: str, output_dir: str, 
                        options: Dict[str, Any], progress_tracker: ProgressTracker) -> List[str]:
        ...
        for i, page_range in enumerate(custom_ranges):
            ...
            output_file = os.path.join(
                output_dir, 
                f"range_{start_page+1}-{end_page+1}.pdf"
            )
            new_doc.save(output_file)
            new_doc.close()
            output_files.append(output_file)
            
            self.logger.debug(f"Created range file: {output_file} (pages {start_page+1}-{end_page+1})")
        
        return output_files
    
    def _split_by_bookmarks(self, source_doc, input_file: str, output_dir: str, 
                           options: Dict[str, Any], progress_tracker: ProgressTracker) -> List[str]:
    ...
    def _split_by_size(self, source_doc, input_file: str, output_dir: str, 
                      options: Dict[str, Any], progress_tracker: ProgressTracker) -> List[str]:
...
class PDFSignOperation:
    ...
    def execute(self, input_file: str, signature_file: str, output_file: str,
                options: Dict[str, Any], progress_tracker: ProgressTracker) -> OperationResult:
        """Execute PDF sign operation"""
        start_time = datetime.now()
        
        try:
            self.logger.info(f"Starting sign operation: {input_file} -> {output_file}")
            ...
            if pages_option == 'all':
                ...
                pages_to_sign = [p for p in pages_option if 0 <= p < total_pages]
            else:
            ...
            # Save signed PDF
            progress_tracker.update_progress(len(pages_to_sign), "Saving signed PDF...")
            pdf_doc.save(output_file)
            pdf_doc.close()
            
            # Add digital signature if requested
            if options.get('digital_signature', False) and options.get('certificate_file'):
                self._add_digital_signature(output_file, options['certificate_file'])
            
            processing_time = (datetime.now() - start_time).total_seconds()
            
            self.logger.info(f"Sign completed successfully: {output_file} ({processing_time:.2f}s)")
            ...
            return OperationResult(
                ...
                details={
                    ...
                    'digital_signature': options.get('digital_signature', False)
                }
            )
    ...
    def _calculate_signature_position(self, page_rect, position, size) -> fitz.Rect:
    ...
    def _add_digital_signature(self, pdf_file: str, certificate_file: str):
        ...
        self.logger.warning("Digital signature functionality not yet implemented")
...
class PDFOperationEngine:
    ...
    def merge_pdfs(self, input_files: List[str], output_file: str, 
                   options: Dict[str, Any] = None, 
                   progress_callback: Callable[[float, str], None] = None) -> OperationResult:
        """Execute PDF merge operation"""
        options = options or {}
        ...
        try:
            return self.merge_operation.execute(input_files, output_file, options, progress_tracker)
        finally:
    ...
    def split_pdf(self, input_file: str, output_dir: str, 
        ...
        try:
            return self.split_operation.execute(input_file, output_dir, options, progress_tracker)
        finally:
            self.file_manager.cleanup_temp_files()
    
    def sign_pdf(self, input_file: str, signature_file: str, output_file: str,
                 options: Dict[str, Any] = None,
                 progress_callback: Callable[[float, str], None] = None) -> OperationResult:
        """Execute PDF sign operation"""
        options = options or {}
        ...
        try:
            return self.sign_operation.execute(input_file, signature_file, output_file, options, progress_tracker)
        finally:
            self.file_manager.cleanup_temp_files()
