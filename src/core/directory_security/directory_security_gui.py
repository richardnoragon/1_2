...
class DirectorySecurityGUI:
    ...
    def __init__(
        ...
        user_id: str,
        user_role: DirectoryRole = DirectoryRole.USER,
    ):
        """
    ...
    def _create_widgets(self):
        ...
        self.security_status_label = ttk.Label(
            self.title_frame, text="🔒 Security: Active", foreground="green"
        )
        ...
        ttk.Label(self.filter_frame, text="Filter:").pack(
            side="left", padx=(0, 5)
        )
        ...
        self.directory_tree = ttk.Treeview(
            self.list_frame, columns=columns, show="tree headings", height=12
        )
        ...
        self.details_frame = ttk.LabelFrame(
            self.main_frame, text="Directory Details", padding="10"
        )
        ...
        self.progress_bar = ttk.Progressbar(
            self.status_frame, variable=self.progress_var, mode="determinate"
        )
    ...
    def _bind_events(self):
        ...
        self.directory_tree.bind(
            "<<TreeviewSelect>>", self._on_directory_select
        )
    ...
    def _load_directory_entries(self):
        ...
        try:
            ...
            def load_thread():
                try:
                    directories = self.security_manager.list_user_directories(
                        self.user_id
                    )
                    ...
                    for i, dir_data in enumerate(directories):
                        ...
                        entry = DirectoryEntry(
                            ...
                            display_path=(
                                pii_result.anonymized_path
                                if pii_result.contains_pii
                                else dir_data.get("display_path", "[Path]")
                            ),
                            is_pii_sensitive=dir_data.get(
                                "is_pii_sensitive", False
                            ),
                            sensitivity_level=dir_data.get(
                                "sensitivity_level", 1
                            ),
                            last_accessed=dir_data.get("last_accessed_at", ""),
                            encrypted=dir_data.get("encryption_metadata")
                            is not None,
                            pii_warnings=self.pii_detector.get_pii_warnings(
                                dir_data.get("display_path", "")
                            ),
                        )
                    ...
                    self.update_queue.put(
                        (
                            "status",
                            "Loaded {} directories".format(len(entries)),
                        )
                    )

                except Exception as e:
                    self.update_queue.put(
                        ("error", f"Failed to load directories: {str(e)}")
                    )
    ...
    def _on_directory_select(self, event):
        ...
        # Find the entry
        entry = next(
            (e for e in self.directory_entries if e.path_hash == path_hash),
            None,
        )
    ...
    def _update_details_panel(self, entry: Optional[DirectoryEntry]):
        ...
        if entry.pii_warnings:
            warning_text = "\n".join(
                f"⚠️ {warning}" for warning in entry.pii_warnings
            )
    ...
    def _add_directory(self):
        ...
        dialog = DirectoryAddDialog(
            self.parent_frame, self.security_manager, self.user_id
        )
        result = dialog.show()
    ...
    def _remove_directory(self):
        ...
        if not self.selected_entry:
            messagebox.showwarning(
                "Warning", "Please select a directory to remove."
            )
            return
        ...
        try:
            ...
            if success:
                ...
                messagebox.showerror(
                    "Error", "Failed to remove directory preference."
                )
    ...
    def _open_selected_directory(self):
        ...
        try:
            ...
            if result.success and os.path.exists(result.directory_path):
                ...
                messagebox.showwarning(
                    "Warning",
                    "Directory path no longer exists or cannot be accessed.",
                )
    ...
    def _toggle_encryption(self):
        ...
        messagebox.showinfo(
            "Info",
            "Encryption toggle functionality would be implemented here.",
        )
...
class DirectoryAddDialog:
    ...
    def __init__(
        self,
        parent: tk.Widget,
        security_manager: DirectorySecurityManager,
        user_id: str,
    ):
        self.parent = parent
    ...
    def _create_widgets(self):
        ...
        ttk.Label(
            path_frame, text="Directory Path:", font=("Segoe UI", 9, "bold")
        ).pack(anchor="w")

        path_entry_frame = ttk.Frame(path_frame)
        self.path_var = tk.StringVar()
        self.path_entry = ttk.Entry(
            path_entry_frame, textvariable=self.path_var, width=50
        )
        browse_button = ttk.Button(
            path_entry_frame, text="Browse...", command=self._browse_directory
        )
        ...
        ttk.Label(
            tool_frame, text="Tool Name:", font=("Segoe UI", 9, "bold")
        ).pack(anchor="w")
        self.tool_var = tk.StringVar()
        self.tool_entry = ttk.Entry(
            tool_frame, textvariable=self.tool_var, width=50
        )
        ...
        ttk.Label(
            type_frame, text="Directory Type:", font=("Segoe UI", 9, "bold")
        ).pack(anchor="w")
        ...
        analysis_frame = ttk.LabelFrame(
            main_frame, text="Security Analysis", padding="10"
        )
        self.analysis_text = tk.Text(
            analysis_frame, height=6, wrap="word", state="disabled"
        )
        analysis_scroll = ttk.Scrollbar(
            analysis_frame, orient="vertical", command=self.analysis_text.yview
        )
        ...
        ok_button = ttk.Button(
            button_frame, text="Add Directory", command=self._ok_clicked
        )
    ...
    def _browse_directory(self):
        ...
        directory = filedialog.askdirectory(
            title="Select Directory", initialdir=os.path.expanduser("~")
        )
        if directory:
    ...
    def _on_path_change(self, *args):
        ...
        if not path:
            self._update_analysis(
                "Enter a directory path to see security analysis."
            )
            return
        ...
        try:
            ...
            if analysis.contains_pii:
                ...
                analysis_lines.append(
                    f"📊 Sensitivity Level: {analysis.sensitivity_level}/5"
                )
                analysis_lines.append(
                    f"🔐 Encryption Recommended: {'Yes' if analysis.requires_encryption else 'No'}"
                )
                analysis_lines.append("")
                ...
                analysis_lines.append("")
                analysis_lines.append(
                    "Anonymized Path: " + analysis.anonymized_path
                )
            else:
    ...
    def _ok_clicked(self):
        ...
        if not os.path.exists(path):
            if not messagebox.askyesno(
                "Warning",
                f"The directory '{path}' does not exist. Add anyway?",
            ):
                return
...
class PropertiesDialog:
    ...
    def _setup_security_tab(self, frame):
        ...
        # Security warnings
        ttk.Label(
            frame, text="Security Warnings:", font=("Segoe UI", 9, "bold")
        ).pack(anchor="w", pady=(0, 5))

        warnings_text = tk.Text(frame, height=6, wrap="word", state="disabled")
        warnings_scroll = ttk.Scrollbar(
            frame, orient="vertical", command=warnings_text.yview
        )
    ...
    def _setup_audit_tab(self, frame):
        """Setup audit log tab"""
        ttk.Label(
            frame, text="Recent Audit Events:", font=("Segoe UI", 9, "bold")
        ).pack(anchor="w", pady=(0, 5))
        ...
        audit_scroll = ttk.Scrollbar(
            frame, orient="vertical", command=audit_tree.yview
        )
...
class SecuritySettingsDialog:
    ...
    def _create_widgets(self):
        ...
        ttk.Label(
            main_frame, text="Security Settings", font=("Segoe UI", 12, "bold")
...
def create_directory_security_widget(
    parent_frame: tk.Frame,
    db_manager: DatabaseManager,
    user_id: str,
    user_role: DirectoryRole = DirectoryRole.USER,
