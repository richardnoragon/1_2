...
class DirectoryPathValidator:
    ...
    def validate_directory_path(
        ...
        try:
            self.logger.debug(
                f"Validating directory path for user {user_id[:8]}..."
            )
            ...
            user_result = self._validate_user_access_rights(
                directory_path, user_id
            )
            ...
            return PathValidationResult(
                valid=False, reason=f"Validation system error: {e}"
            )
    ...
    def _validate_path_format(
        ...
        if len(directory_path) > self.max_path_length:
            return PathValidationResult(
                valid=False,
                reason=(
                    f"Path exceeds maximum length of "
        ...
        if "\x00" in directory_path:
            return PathValidationResult(
                valid=False, reason="Path contains null bytes"
            )
        ...
        try:
            ...
            if path_depth > self.max_depth:
                return PathValidationResult(
                    valid=False,
                    reason=(
                        f"Path depth exceeds maximum of "
                        f"{self.max_depth} levels"
                    ),
                )
        except Exception:
            return PathValidationResult(
                valid=False, reason="Invalid path format"
            )
    ...
    def _validate_path_traversal(
        ...
        try:
            ...
            return PathValidationResult(
                valid=False, reason="Path normalization failed"
            )

        # Check for malicious patterns using centralized logic
        has_malicious, pattern_type = self._contains_malicious_patterns(
            directory_path
        )
        if has_malicious:
    ...
    def _validate_symbolic_links(
        ...
        try:
            ...
            return PathValidationResult(
                valid=False, reason=f"Symbolic link validation failed: {e}"
            )
    ...
    def _validate_user_access_rights(
        self, directory_path: str, user_id: str
    ) -> PathValidationResult:
        ...
        try:
            ...
            if not os.access(directory_path, os.R_OK):
                return PathValidationResult(
                    valid=False, reason="User lacks read access to directory"
                )
            ...
            return PathValidationResult(
                valid=False, reason=f"Access rights validation failed: {e}"
            )
    ...
    def _validate_against_patterns(
        ...
        if self._matches_any_pattern(
            directory_path, self.blocked_path_patterns
        ):
            return PathValidationResult(
                valid=False, reason="Path matches blocked pattern"
            )

        # Check against allowed patterns
        if self._matches_any_pattern(
            directory_path, self.allowed_path_patterns
        ):
            return PathValidationResult(valid=True)
