...
class PIIDetector:
    ...
    def get_pii_warnings(self, directory_path: str) -> List[str]:
        ...
        try:
            ...
            indicator_warnings = {
                "username_directory": "Path contains user directory information",
                "personal_documents": "Path may contain personal documents",
                "desktop_directory": "Path accesses user desktop directory",
                "downloads_directory": "Path accesses user downloads directory",
                "pictures_directory": "Path may contain personal photos/images",
                "videos_directory": "Path may contain personal videos",
                "music_directory": "Path may contain personal music files",
                "email_directory": "Path may contain email data (HIGH SENSITIVITY)",
                "browser_data": "Path may contain browser data and history",
                "social_security": "Path may contain social security information (CRITICAL)",
                "financial_data": "Path may contain financial information (CRITICAL)",
                "medical_records": "Path may contain medical records (CRITICAL)",
                "legal_documents": "Path may contain legal documents",
                "private_keys": "Path may contain private keys/certificates (CRITICAL)",
                "backup_data": "Path may contain backup/archive data",
                "cloud_sync": "Path may contain cloud synchronized data",
            }
    ...
    def _calculate_sensitivity_level(
        self, pii_indicators: List[str], path: str
    ) -> int:
    ...
    def _calculate_base_sensitivity(self, path: str) -> int:
        ...
        if any(
            ...
            for keyword in ["private", "confidential", "secret", "personal"]
        ):
            return 3
    ...
    def _create_anonymized_path(
        self, directory_path: str, pii_indicators: List[str]
    ) -> str:
        ...
        anonymized = re.sub(
            r"\\users\\[^\\]+\\", "\\[USER]\\", anonymized, flags=re.IGNORECASE
        )
