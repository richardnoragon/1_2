...
class EnhancedConfigManager:
    ...
    def _setup_config(self):
        ...
        if self.use_database:
            try:
                ...
                self.logger.info(
                    "Enhanced ConfigManager with database support initialized"
                )
    ...
    def _finalize_migration(self, migration_count):
        ...
        self._migrated_to_db = True
        self.logger.info(
            f"Successfully migrated {migration_count} settings "
    ...
    def _increment_recursion_depth(self) -> bool:
        ...
        with self._recursion_lock:
            if self._recursion_depth >= self._max_recursion_depth:
                self.logger.warning(
                    f"Maximum recursion depth ({self._max_recursion_depth}) reached"
                )
                return False
        ...
        try:
            ...
            if self.use_database:
                ...
                result = self.db_manager.execute_query(
                    query, ("general", "auto_save_config")
                )
                if result:
                ...
                if (
                    ...
                    and "auto_save_config" in self.config["general"]
                ):
    ...
    def _get_setting_from_database(
        self, section: str, key: Optional[str], default: Any
    ) -> Any:
            ...
            info.update(
                {
                    ...
                    "total_settings": db_info.get("table_counts", {}).get(
                        "app_settings", 0
                    ),
                }
            )
