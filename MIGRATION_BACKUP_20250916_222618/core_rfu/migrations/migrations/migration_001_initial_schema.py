...
class Migration001InitialSchema(MigrationBase):
    ...
    def down(self, connection: sqlite3.Connection) -> None:
        ...
        # Drop indexes first
        connection.execute("DROP INDEX IF EXISTS idx_preference_audit_log_user")
        connection.execute("DROP INDEX IF EXISTS idx_preference_audit_log_timestamp")
        connection.execute("DROP INDEX IF EXISTS idx_user_preferences_secure_category")
        connection.execute("DROP INDEX IF EXISTS idx_user_preferences_secure_user")
    ...
    def validate_postconditions(self, connection: sqlite3.Connection) -> ValidationResult:
        ...
        if len(indexes) < 2:  # Should have at least 2 indexes for user_preferences_secure
            return ValidationResult(
                success=False,
