...
class Repository(ABC, Generic[T]):
    ...
    def _cache_key(self, operation: str, *args) -> str:
        ...
        return f"{self.__class__.__name__}:{operation}:{':'.join(map(str, args))}"
...
class FolderConfigurationRepository(Repository[FolderConfiguration]):
    ...
    def _get_entity_by_id(self, entity_id: int) -> Optional[FolderConfiguration]:
        """Get folder configuration by ID."""
        return self.db_manager.get_folder_configuration(entity_id)
    
    def _update_entity(self, entity_id: int, entity: FolderConfiguration) -> bool:
    ...
    def _list_entities(self, options: QueryOptions) -> List[FolderConfiguration]:
    ...
    def get_by_type(self, folder_type: str) -> List[FolderConfiguration]:
        ...
        try:
            configs = self.db_manager.get_folder_configurations_by_type(folder_type)
            self._set_cache(cache_key, configs)
            return configs
        except Exception as e:
            self.logger.error(f"Error getting folders by type {folder_type}: {e}")
            return []
...
class SearchParameterRepository(Repository[SearchParameter]):
    ...
    def get_by_folder(self, folder_config_id: int) -> List[SearchParameter]:
        ...
        try:
            params = self.db_manager.get_search_parameters_by_folder(folder_config_id)
            self._set_cache(cache_key, params)
            return params
        except Exception as e:
            self.logger.error(f"Error getting search params for folder {folder_config_id}: {e}")
            return []
...
class FileMetadataRepository(Repository[FileMetadata]):
    ...
    def get_by_folder(self, folder_config_id: int) -> List[FileMetadata]:
        ...
        try:
            metadata = self.db_manager.get_file_metadata_by_folder(folder_config_id)
            self._set_cache(cache_key, metadata)
            return metadata
        except Exception as e:
            self.logger.error(f"Error getting file metadata for folder {folder_config_id}: {e}")
            return []
    ...
    def search_by_pattern(self, pattern: str) -> List[FileMetadata]:
        ...
        try:
            ...
            self._set_cache(cache_key, results, ttl=60)  # Shorter TTL for search results
            return results
        except Exception as e:
            self.logger.error(f"Error searching files by pattern {pattern}: {e}")
            return []
    
    def search_by_size_range(self, min_size: int, max_size: int) -> List[FileMetadata]:
        ...
        try:
            results = self.db_manager.search_files_by_size_range(min_size, max_size)
            self._set_cache(cache_key, results, ttl=60)
            return results
        except Exception as e:
            self.logger.error(f"Error searching files by size range {min_size}-{max_size}: {e}")
            return []
        ...
        if self._transaction:
            try:
                if exc_type is None:
                    ...
                    self.logger.warning(f"Transaction rolled back due to: {exc_val}")
            except Exception as e:
...
class RepositoryManager:
    ...
    def __init__(self, db_manager: AdvancedFoldersDBManager,
        ...
        self.folder_configs = FolderConfigurationRepository(db_manager, cache_strategy)
        self.search_params = SearchParameterRepository(db_manager, cache_strategy)
